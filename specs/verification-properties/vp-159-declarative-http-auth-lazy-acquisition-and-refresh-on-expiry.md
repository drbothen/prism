...
> `relative_seconds` path uses plain u64 (ADR-054 §D4 step 4 → `"relative_seconds"`: u64 seconds, default 1799, minus `ttl_buffer_secs`; no lenient-parse qualifier);
...
- **AC-7d (P2 — `oauth2_client_credentials` string-typed `$.expires_in`, RU-Q2):** For
  ...
  plain u64 (ADR-054 §D4 step 4 → `"relative_seconds"`: u64 seconds, default 1799, minus `ttl_buffer_secs`) — no lenient parse applies there.
...
|---------|-------|------|--------|-------|
| 1.27 | wave-a-spec-evolution-fix-burst-68a | 2026-07-27 | spec-steward | F-WASE-P65-MED-001. Two live-body bare-L volatile cites to `ADR-054 §D4 step 4` replaced with stable anchor form per TD-VSDD-091. Sites: §Property Statement scope note (F-WASE-P55-OBS-001 auth-type harness split) and §Acceptance Criteria AC-7d scope boundary note. Old form: `ADR-054 §D4 step 4 L308`. New form: `ADR-054 §D4 step 4 → "relative_seconds": u64 seconds, default 1799, minus ttl_buffer_secs`. No behavioral content changed. |
