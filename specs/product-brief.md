...
1. **Unified sensor access** — Full API coverage for CrowdStrike Falcon, Cyberint Argos, Claroty xDome, and Armis. All data access is through a single `query` tool using the PrismQL query language -- there are no per-sensor read tools. Write operations (containment, blocking, alert status updates) are per-sensor MCP tools gated behind a two-tier feature flag system (compile-time cargo features + runtime per-client TOML config).
...
4. **AI-opaque credential management** — Per-client, per-sensor credential management where secret values NEVER transit through the AI's context window. Three ingestion paths: Prism CLI (interactive hidden prompt), environment variables with `_FILE` pattern (Kubernetes/Docker), and external vault references (HashiCorp Vault, AWS SM, Azure KV, GCP SM — feature-gated). The AI orchestrates credential configuration (where to find secrets) via `configure_credential_source` but never handles raw values. Storage backends: OS keyring (macOS Keychain, Windows Credential Vault, Linux libsecret) primary, AES-256-GCM encrypted file fallback. Namespaced as `(client_id, sensor_id, credential_name)`.
...
8. **Extensible sensor adapter architecture** — Trait-based `DataSource` pattern making it straightforward to add new sensors in the future without modifying core infrastructure. Config-driven sensor adapters (CAP-029) enable adding new REST API sensors by dropping a TOML spec file -- no Rust code changes required for approximately 80% of sensors. Spec files declare authentication type, base URL, tables with typed columns and OCSF mappings, multi-step fetch pipelines with variable interpolation (`${step_name.field}`), pagination config, and rate limit hints. A Rust escape hatch (`CustomAdapter` trait) handles the remaining approximately 20% requiring exotic auth or binary protocols. The `prism-spec-engine` crate provides TOML spec parsing, multi-step pipeline execution, and arc-swap config management.

9. **Hot configuration reload** — `reload_config` MCP tool (CAP-030) re-reads prism.toml, sensor spec files, and aliases.toml without restart. Uses arc-swap for lock-free reads on the query hot path. Hash-based change detection skips reload if unchanged. Validates before applying; failed validation leaves running config unchanged. Sends `notifications/tools/list_changed` if sensor table schemas change.
...
| tally | Rust MCP reference | rmcp patterns, tool registration, dual CLI/MCP |
