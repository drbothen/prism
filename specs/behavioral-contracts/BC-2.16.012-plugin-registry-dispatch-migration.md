...
| `E-PLUGIN-021` | `register_write_tool` acquires the `DYNAMIC_WRITE_TOOLS` RwLock write guard and the lock is poisoned (a prior writer panicked while holding the guard) | `register_write_tool` returns `Err(SpecEngineError::WriteToolRegistryPoisoned)`. The lock stays poisoned; no further registrations are possible in this process. Operators must restart the process. Construction site: `crates/prism-query/src/invalidation.rs` `register_write_tool()` — `map_err` on the `RwLock::write()` `PoisonError`. Recurrence policy: SINGLE_PER_PROCESS_LIFETIME (the poisoned lock persists until process restart). Severity: ERROR (unrecoverable). Error code: E-PLUGIN-021. |
...
|---------|-------|------|--------|--------|
| 1.33 | FB-IMPL-6 (PLUGIN-MIGRATION-001-E) | 2026-05-23 | implementer | POL-30 Fork B catalog bullet cite-pin sweep: `(v1.24)` → `(v1.25)` at 3 live-narrative sites — §Postconditions line 84 (2 cites) + EC-016-012-005 line 110 (1 cite). BC-2.16.002 catalog bullet label advanced from `(v1.24)` to `(v1.25)` in FB-IMPL-6 same burst (addition of `plugin.auth_token_parse_error` row 37; F-LP7-MED-001 closure). |
| 1.32 | FB-IMPL-3 (PLUGIN-MIGRATION-001-E) | 2026-05-22 | implementer | POL-30 Fork B catalog bullet cite-pin sweep: `(v1.23)` → `(v1.24)` at 3 live-narrative sites — §Postconditions line 84 (2 cites) + EC-016-012-005 line 110 (1 cite). BC-2.16.002 catalog bullet label advanced from `(v1.23)` to `(v1.24)` in FB-IMPL-3 same burst (addition of `plugin_auth_provider_constructed` row 36; F-LP3-LOW-001 closure). |
...
| 1.25 | FB69 | 2026-05-17 | product-owner | F-LP81-HIGH-002 closure (PO scope): ADR-026 D7 pin v1.21→v1.22 propagation at lines 84, 109, 124, 138 (4 sites; §D7 form). Recurrence #22+ of POL-29 step 3a class (b). Sibling files story v1.44 + BC-2.16.002 v1.29 + error-taxonomy v1.37 + VP-156 v0.17 + HS-003 v1.14 swept in same burst. |
