...
- `IIN` requires at least one value in the membership list. An empty `IIN ()` list is a parse error (`E-QUERY-001`). Additionally, every value in the `IIN (...)` list MUST be a quoted string literal; integer, float, or boolean values are rejected at parse time with `E-QUERY-001` (mirrors the IEQ/INE non-string RHS rejection, symmetric parse-time behavior). Quoted date-like strings (e.g., `'2026-06-01'`) are accepted as string literals — the IIN parser normalizes `RawTemporalLiteral` and `Timestamp` variants to `Literal::String` so the `lower()` lowering path is always reached (ADR-052 §D4, pass-14 refinement).
...
| `E-QUERY-001` | `IIN` with one or more non-string list elements (e.g., `severity IIN (42, 43)` or `status IIN (true, false)`) — integer, float, or boolean values in the membership list | Parse-time rejection (symmetric with IEQ/INE non-string RHS): `"E-QUERY-001: IIN operator requires quoted string literals in the membership list; got a non-string value (integer, float, or boolean). Example: status IIN ('new', 'open'). If you need date-like strings, wrap them in quotes: created_at IIN ('2026-06-01', '2026-06-02')."` MCP mapping: `-32602 INVALID_PARAMS` (caller-resolvable). Quoted date-like strings ARE accepted — the IIN parser normalizes them to string type. |
| `E-QUERY-001` | `IIN` with an empty membership list: `severity IIN ()` | Parse-time rejection: `"E-QUERY-001: IIN operator requires at least one value in the membership list; empty () is not valid. Use at least one quoted string: e.g. \`status IIN ('new', 'open')\`."` MCP mapping: `-32602 INVALID_PARAMS`. |
...
| EC-11-024-012 | `severity IIN (42, 43)` — IIN list contains integer literals instead of quoted strings | **Parse-time `E-QUERY-001` rejection**: `"E-QUERY-001: IIN operator requires quoted string literals in the membership list; got a non-string value (integer, float, or boolean). Example: status IIN ('new', 'open'). If you need date-like strings, wrap them in quotes: created_at IIN ('2026-06-01', '2026-06-02')."` — symmetric with IEQ/INE non-string RHS rejection; PLAN time is not reached. `status IIN (true, false)` (booleans) and `score IIN (1.5, 2.5)` (floats) behave identically. Quoted `'2026-06-01'` IS accepted (date-like string normalized to `Literal::String` by the IIN parser). |
...
| `severity IIN ()` | `Err(E-QUERY-001)` — parse-time rejection: `"E-QUERY-001: IIN operator requires at least one value in the membership list; empty () is not valid. Use at least one quoted string: e.g. \`status IIN ('new', 'open')\`."` | error |
| `severity IIN (42, 43)` | `Err(E-QUERY-001)` — parse-time rejection: integer literals rejected; `"E-QUERY-001: IIN operator requires quoted string literals in the membership list; got a non-string value (integer, float, or boolean). Example: status IIN ('new', 'open')..."` PLAN time NOT reached; symmetric with `IEQ 42` rejection. | error (non-string IIN elements) |
| `status IIN ('2026-06-01', '2026-06-02')` | Row returned if `status` field matches `'2026-06-01'` or `'2026-06-02'` — quoted date-like strings accepted, normalized to `Literal::String` by IIN parser (ADR-052 §D4) | happy-path (date-like strings as labels) |
