...
input-hash: "f0b3278"
# input-hash: updated 2026-09-02 — OBS-001 accuracy fix applied (§Postconditions §1 sort-by postcondition + EC-016-015-009 accepted-residual language); hash refreshed to f0b3278. Prior: 2e91a4e (updated 2026-08-31 — claroty.sensor.toml modified by S-CLAROTY-VULNS-001, PR #245 squash-merged D-2387).
traces_to: ["CAP-029"]
...
arbitrary API-ordered subset. The `name asc` secondary sort (mapping to `finding_info.title` — vulnerability title,
`options=["REQUIRED"]` for presence, NOT a uniqueness constraint) is the **best-available
sortable tiebreaker** from `Vulnerability__sortable_fields_enum`; the opaque record PK `id`
is NOT a member of that enum and cannot serve as a sortable tiebreaker (OBS-001).
**Accepted residual non-determinism (symmetric with BC-2.16.013 §Sort-by postcondition):**
where two rows share BOTH the same `adjusted_vulnerability_score` AND the same `name` (title),
page-boundary ordering between them is not guaranteed. This residual is bounded — ties require
identical score AND identical title — and is further mitigated by the risk-primary ordering
keeping the highest-risk rows below the DI-019 10K cap. Accepted, not a defect. Anchor:
`DEFECT-CLAROTY-SORTBY-DETERMINISM-001` RG-001 (`test_rg_vulnerabilities_sort_by_in_request_body`)
+ RG-008 (`test_rg_vulnerabilities_sort_by_tiebreaker_is_best_available_field`). ORDER BY push-down to the API
...
| EC-016-015-009 | Offset pagination determinism: two sequential pages of `claroty_vulnerabilities` must not duplicate or skip records | The `body_template` MUST contain `"sort_by": [{"field":"adjusted_vulnerability_score","order":"desc"},{"field":"name","order":"asc"}]`. The `name` column (`finding_info.title`, `options=["REQUIRED"]`) is the **best-available sortable tiebreaker** from `Vulnerability__sortable_fields_enum`; it is NOT a provably-unique key — the opaque record PK `id` is NOT in the sortable fields enum and cannot serve as a tiebreaker (OBS-001). **Accepted residual non-determinism:** where two rows share BOTH the same `adjusted_vulnerability_score` AND the same `name` (title), page-boundary ordering between them is not guaranteed; this residual is bounded (ties require identical score AND identical title) and is further mitigated by the risk-primary ordering + the DI-019 10K cap retaining highest-risk rows. Accepted, not a defect. Without a `sort_by` override the xDome API default (`published_date desc`) is non-unique and causes page-boundary instability. Anchor: `DEFECT-CLAROTY-SORTBY-DETERMINISM-001` RG-001 (`test_rg_vulnerabilities_sort_by_in_request_body`) + RG-008 (`test_rg_vulnerabilities_sort_by_tiebreaker_is_best_available_field`). |
...
|---------|-------|------|--------|--------|
| 2.2 | defect-claroty-sortby-determinism-rg008-rename | 2026-09-02 | product-owner | MED-001 + OBS-001 dim-2 downstream-copy sweep (TD-VSDD-097): renamed stale RG-008 test anchor `test_rg_vulnerabilities_sort_by_tiebreaker_is_unique_field` → `test_rg_vulnerabilities_sort_by_tiebreaker_is_best_available_field` at all four citation sites in this BC (§Postconditions §1 sort-by postcondition, EC-016-015-009, v2.1 changelog row, v2.0 changelog row). The old name was self-contradictory against the OBS-001 correction: §Postconditions §1 explicitly states `name` is NOT provably unique (it is the best-available sortable tiebreaker); a test named `_is_unique_field` misstated the property it verified. TD-VSDD-097: (1) Sibling pair — no BC sibling from the same split requires a mirror correction; CLEAR. (2) Downstream copy target — the story test file (`crates/prism-sensors/tests/`) carried the renamed test from a prior burst; this BC was the missed dim-2 copy-source target. (3) Mandate anchor — RG-008 anchor symbol updated in place; no new unanchored MUSTs introduced. |
| 2.1 | defect-claroty-sortby-determinism-obs001-fix | 2026-09-02 | product-owner | OBS-001 accuracy fix: §Postconditions §1 sort-by postcondition and EC-016-015-009 — corrected overclaim that `name` (`finding_info.title`) is "provably unique CVE ID" making the sort order "total." Per OBS-001: `name` maps to `ocsf_field = "finding_info.title"` with `options=["REQUIRED"]` (presence, NOT uniqueness); the opaque record PK `id` is NOT a member of `Vulnerability__sortable_fields_enum` (`ValidatingSortClause__6` in xDome OpenAPI) and cannot serve as a sortable tiebreaker. Amended language: `name` is the best-available sortable tiebreaker; accepted residual non-determinism documented for ties on identical score AND identical title (symmetric with BC-2.16.013 §Sort-by postcondition treatment for audit_logs). sort_by array is UNCHANGED. DI-019 truncation rationale retained. TD-VSDD-097: (1) Sibling pair — `claroty_vulnerabilities` has no BC sibling from the same split requiring a mirror correction; CLEAR. (2) Downstream copy target — §Postconditions §1 sort-by postcondition and EC-016-015-009 are not verbatim copy-source sections in any downstream artifact; CLEAR. (3) Mandate anchor — RG-001 (`test_rg_vulnerabilities_sort_by_in_request_body`) and RG-008 (`test_rg_vulnerabilities_sort_by_tiebreaker_is_best_available_field`) remain as anchor tests; the accepted-residual documentation does not introduce new unanchored MUSTs. |
