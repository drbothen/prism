...
2. The query references a column name in a position where column resolution is possible. The gate covers fourteen clause positions across all three AST modes — **this is an exhaustive closed list**:
   ...
   **DERIVED-COLUMN BINDING RULE (pipe-mode positions 8–14):**
   ...
   - **Enrich stage (`PipeStage::Enrich`):** Position 13 (input column) is checked against `available` BEFORE this stage updates the binding context. After the position-13 check: if the infusion is registered in `InfusionRegistry` AND the registry provides a static output column list for it, those output column names are **unioned** into `available` (the infusion output columns become bindable for downstream stages). If the infusion output schema is NOT statically resolvable at plan time: `suspended := true` — all E-QUERY-038 checking for subsequent stages is suspended (fail-open). Rationale: downstream references to infusion output columns (e.g., `cvss_base_score` from the canonical NVD pivot per BC-2.06.019 v1.13 §PC-4) are valid at execution time but absent from the plan-time schema; a false positive on a correct query is forbidden by invariant FP-001.
...
| EC-11-054 | Canonical NVD pivot `FROM armis_devices \| where has device_cves_first \| enrich cvss_base_score(device_cves_first) \| where cvss_base_score >= 7.0 \| sort cvss_base_score desc` | No E-QUERY-038 on `cvss_base_score` — after the Enrich stage, either `cvss_base_score` is added to the binding set (if infusion output is statically resolvable) or `suspended = true`; either way the downstream `\| where` and `\| sort` stages do not fire E-QUERY-038 (ADV-FIX-P2-CRIT-001 regression closed; canonical form per BC-2.06.019 v1.13 §PC-4) |
| EC-11-055 | Post-enrich typo `FROM armis_devices \| enrich cvss_base_score(device_cves_first) \| where cvvs_base_score >= 7.0` where `cvvs_base_score` is a typo | Behavior depends on infusion output resolution: (a) if infusion output NOT statically resolvable — `suspended = true` after Enrich → no E-QUERY-038 (fail-open; false negative tolerated per FP-001); (b) if infusion output IS statically resolvable and `cvvs_base_score` not in output columns → E-QUERY-038 fires on `cvvs_base_score`. Either outcome is correct — the gate MUST NOT false-positive on the exact infusion output column name |
...
| `query("FROM armis_devices \| where has device_cves_first \| enrich cvss_base_score(device_cves_first) \| where cvss_base_score >= 7.0 \| sort cvss_base_score desc", clients=["acme"])` | No E-QUERY-038 on `cvss_base_score` — gate fails open or resolves it from infusion output (EC-11-054; canonical NVD pivot per BC-2.06.019 v1.13 §PC-4) | enrich-output (position 13 derived) |
