...
| LEFT JOIN with empty right table; SELECT `_sensor` from right side | Right-side `_sensor` column is NULL (not absent); schema includes all **four** virtual fields via `append_virtual_fields_to_schema`; no schema-mismatch error | edge-case (DEC-022 / F-CSD-P14-001) — `test_BC_2_11_012_F_CSD_P14_001_T32_left_join_empty_side_virtual_fields_return_null_not_error` |
| SELECT * from both populated and zero-batch table in same query; check schema parity | Both table schemas include `_sensor`, `_client`, `_source_table`, `_source_type`; empty path uses nullable=true, populated path uses nullable=false | edge-case (DEC-022 / F-CSD-P14-001) — `test_BC_2_11_012_F_CSD_P14_001_T33_select_star_empty_side_schema_includes_virtual_fields_parity` |
...
|---------|-------|------|--------|--------|
| 1.13 | DEFECT-PQL-FNCALL-LHS-001 / F-PQLFN-P19-MED-001 fix-burst 14 mechanism anchoring | 2026-07-13 | product-owner | **Closes F-PQLFN-P19-MED-001 (MED) — `pre_register_source_table` spec-invisible (grep = 0 hits in .factory/specs before this change).** Fix-burst 14 (F-PQLFN-P18-MED-001) added `pre_register_source_table(session_ctx, table_name, table_registry)` to `crates/prism-query/src/materialization.rs` — called in the `Ast::Pipe` arm (registers `datafusion_table_name(&pipe.source.raw)`) and source-qualified `Ast::Filter` arm (registers `datafusion_table_name(&filter.source.raw)`) — but the mechanism was not reflected in any spec. **(1) §Postconditions DEC-022 bullet opening sentence:** changed from `(pre_register_empty_tables)` with `FROM`/`JOIN`/`WHERE`/subquery positions to name the mechanism PAIR: `pre_register_empty_tables` (SQL/SqlPipe positions) and `pre_register_source_table` (`Ast::Pipe.source` and source-qualified `Ast::Filter.source`; Priority-1/2/3: live `TableRegistry` → bundled TOML spec schemas via `BUNDLED_SPEC_SCHEMAS: OnceLock` → `Schema::empty()` — no JOIN-equality inference, Pipe/Filter context has no JOIN ON peers; `virtual_fields::append_virtual_fields_to_schema` and double-registration guard apply on all three priority paths). **(2) §Edge Cases DEC-022 row:** updated "Pre-registration covers" sentence to enumerate both function/position groups. No change to Preconditions, Invariants, Error Cases, Canonical Test Vectors, Verification Properties, or Traceability. |
| 1.12 | DEFECT-CSDEVICES-EMPTY-PIPELINE-001 / F-CSD-P23-001 POL-29 within-burst sibling-sweep gap closure | 2026-07-10 | product-owner | **Closes F-CSD-P23-001 (MED) — POL-29 within-burst sibling-sweep gap: v1.11 corrected §Postconditions DEC-022 bullet and the DEC-022 §Edge Cases row but missed two sibling rows in the SAME §Canonical Test Vectors section.** (1) **T32 row (was "three"):** "schema includes all **three** virtual fields" → "schema includes all **four** virtual fields via `append_virtual_fields_to_schema`". (2) **T33 row (missing `_source_type`):** "Both table schemas include `_sensor`, `_client`, `_source_table`" → "Both table schemas include `_sensor`, `_client`, `_source_table`, `_source_type`". Canonical set remains four sensor-table virtual fields per BC-2.11.012 v1.11; code-verified against `virtual_fields::append_virtual_fields_to_schema`. No change to Preconditions, Postconditions, Invariants, Error Cases, Verification Properties, or Traceability. |
