...
- **Empty-table pre-registration (DEC-022 — DEFECT-CSDEVICES-EMPTY-PIPELINE-001 Sub-defect 2 expansion):** When a referenced sensor table returns 0 batches, a schema-only empty `MemTable` is pre-registered for that table before DataFusion query planning (`pre_register_empty_tables`). Pre-registration is **position-invariant** — it covers every query position where a `prism_*` table can appear: `FROM`, `JOIN`, `WHERE Predicate::InSubquery`, `SELECT Expr::InSubquery`, and nested subqueries at arbitrary depth (via `walk_sql_query` recursive walk). Schema is resolved via a **3-priority ladder**: (P1) live `TableRegistry` schema if available; (P2) bundled TOML spec schemas (compile-time `include_str!` embedding of `prism-sensors/specs/` TOMLs, keyed `{sensor_id}_{table_name}`, backed by `BUNDLED_SPEC_SCHEMAS: OnceLock`); (P3) JOIN ON equality inference from peer table Arrow columns for any fields not covered by P1/P2. Spec-declared `datetime` columns are mapped to Arrow `Timestamp(Microsecond, UTC)` to preserve type fidelity. This guarantees DataFusion can plan queries that JOIN or subquery against empty sensor tables, returning 0 rows gracefully instead of `DataFusionError::Plan("table not found")` mapped to `PrismError::QueryExecutionFailed`.
...
|----|-------------|-------------------|
| DEC-022 | All sensor API calls return empty — position-invariant schema-only MemTable registration | Schema-only empty `MemTable` pre-registered for every referenced table that returned 0 batches. Registration is position-invariant: FROM, JOIN, WHERE `Predicate::InSubquery`, SELECT `Expr::InSubquery`, nested subqueries at arbitrary depth. Schema resolved via 3-priority ladder: (P1) live `TableRegistry`; (P2) bundled TOML spec schemas; (P3) JOIN-equality inference fallback. Spec-declared `datetime` columns mapped to Arrow `Timestamp(Microsecond, UTC)`. Result: graceful 0-row DataFusion execution; no `DataFusionError::Plan("table not found")`. 11 locking tests in `crates/prism-query/src/tests/defect_csdevices_empty_memtable_tests.rs`. |
...
| SELECT (col IN (SELECT col FROM zero-batch table)) AS alias | All-false boolean column; no plan error; Expr::InSubquery table pre-registered | edge-case (DEC-022) — `test_BC_2_11_005_F_CSD_P3_001_T2_expr_insubquery_projection_empty_table_returns_false_col_not_error` |
