...
| `E-QUERY-038` | Column referenced in a pipe-mode stage position not found in the `TableRegistry` schema for the table and client. Covered positions: (8) `\| where` stage predicate (every `PipeStage::Where`, `FilterExpr` tree walked recursively), (10) `\| sort by` field key references (`PipeStage::Sort` sort-key list), (11) `\| stats ... by` grouping field references (`PipeStage::Stats { by }` grouping list), (12) `\| fields` column refs (`PipeStage::Fields` inclusion/exclusion list). For SqlPipe (`Ast::SqlPipe`), head SQL positions 1–6 are also gated (see BC-2.11.003); `\| where` stages are position 9 (same walking as position 8). | `E-QUERY-038` with `column`, `available_columns` (always present, org-scoped), `did_you_mean` (Levenshtein ≤ 3); MCP `-32602 INVALID_PARAMS`. Full spec: BC-2.11.016. |
