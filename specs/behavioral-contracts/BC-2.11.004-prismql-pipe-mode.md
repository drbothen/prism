...
| `E-QUERY-038` | Column referenced in a pipe-mode stage position not found in the `TableRegistry` schema for the table and client (or not in the running derived-column binding context). Covered positions: (8) `\| where` stage predicate (every `PipeStage::Where`, `FilterExpr` tree walked recursively), (10) `\| sort` field key references (`PipeStage::Sort` sort-key list), (11) `\| stats ... by` grouping field references AND aggregate function argument field paths (`PipeStage::Stats { by }` grouping list AND aggregate func args such as `sum(col)`, `count(col)` — checked before Stats replaces binding context), (12) `\| fields` column refs (`PipeStage::Fields` inclusion/exclusion list), (13) `\| enrich` input column (`PipeStage::Enrich { input_col }` — checked before Enrich updates binding context), (14) `\| dedup` field key references (`PipeStage::Dedup { fields }` dedup-key list). For SqlPipe (`Ast::SqlPipe`), head SQL positions 1–6 are also gated (see BC-2.11.003); `\| where` stages are position 9 (same walking as position 8). **Derived-column binding rule:** the gate maintains a running `{ available, suspended }` context — Enrich stages add infusion output columns when resolvable (fail-open otherwise with `suspended = true`); Stats stages replace `available` with `{explicit_aliases ∪ by_fields}`; once suspended all remaining stages skip E-QUERY-038. False-positive prohibition (FP-001): when binding provenance is uncertain the gate fails open. Full spec: BC-2.11.016 v1.17 §Preconditions DERIVED-COLUMN BINDING RULE. | `E-QUERY-038` with `column`, `available_columns` (always present, org-scoped), `did_you_mean` (Levenshtein ≤ 3); MCP `-32602 INVALID_PARAMS`. Full spec: BC-2.11.016 v1.17. |
