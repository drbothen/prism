...
| `FROM crowdstrike_detections \| stats count by '2026-06-24'` (temporal literal as GROUP BY literal in `stats` — non-comparison position) | Valid — `check_temporal_literals` arm (4): non-comparison position → COERCE to `Literal::String("2026-06-24")`; no E-QUERY-041 | edge-case |
