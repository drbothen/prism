...
- **AllTargetsFailed Per-Target Logging:** When `SensorError::AllTargetsFailed` is about to be returned from `fanout()` in `crates/prism-sensors/src/fanout.rs`, each `FanOutError` in the `errors` vec MUST be logged at WARN level with `event_type = "fan_out_target_failed"` (per BC-2.16.002 Canonical Structured Event Catalog row 91 — see that BC for the full field schema including `org_id`, `sensor_id`, `attempts`, `is_transient`, `error`) before the error propagates. The E-SENSOR-030 Display remains count-only per BC-2.10.007 Rule 1 (E-SENSOR-* errors are MCP-redacted). Per-target HTTP diagnostic detail is observable via TWO channels: (1) the WARN `fan_out_target_failed` events logged before `AllTargetsFailed` propagates (always; see BC-2.16.002 Canonical Structured Event Catalog for the full field schema), and (2) the `sensor_errors` entries in the `query` tool response, where each failing target contributes a `"<table>: HTTP <status>: <body-snippet>"` (non-empty body) or `"<table>: HTTP <status>"` (empty body) entry per BC-2.11.001 §Postconditions per-target HTTP detail bullet + EC-11-088/089/090 (D-2151, human-approved 2026-08-14). Story: DEFECT-ADAPTER-TLS-XDOME-LIVE-001 AC-ERR-004 + AC-SAP1-001 + RG-004 (`test_fanout_all_failed_emits_fan_out_target_failed_warn`); AC-QERR-001 + RG-016 for the `sensor_errors` wire shape (story-writer will add AC and RG).
...
|---------|-------|------|--------|--------|
| 1.7 | DEFECT-ADAPTER-TLS-XDOME-LIVE-001-hs002-spec | 2026-08-14 | product-owner | **D-2151 sibling-pair sweep (TD-VSDD-097 dim-1) — AllTargetsFailed Per-Target Logging postcondition updated to dual-channel surfacing.** The previous postcondition stated per-target HTTP diagnostic detail was observable ONLY via the WARN `fan_out_target_failed` events before `AllTargetsFailed` reached the MCP surface. D-2151 (human-approved 2026-08-14) extends surfacing to the MCP response layer: per-target HTTP status + body snippet now ALSO appear in the `sensor_errors` entries of the `query` tool response (BC-2.11.001 §Postconditions per-target HTTP detail bullet + EC-11-088/089/090). Change: the AllTargetsFailed Per-Target Logging bullet sentence "per-target diagnostic detail is observable via the WARN events before `AllTargetsFailed` reaches the MCP surface" updated to dual-channel language (WARN events always; PLUS `sensor_errors` entries per BC-2.11.001 D-2151). The E-SENSOR-030 Display remains count-only (BC-2.10.007 Rule 1 unchanged). No new MUSTs introduced — the per-target HTTP surfacing contract and its MUSTs live in BC-2.11.001; this amendment corrects the previously-incorrect claim that the WARN log was the ONLY observable channel. **TD-VSDD-097:** dim-1 sibling-pair sweep from BC-2.11.001 v1.23 burst; dim-2 no downstream copy-target restates this bullet verbatim; dim-3 no new MUSTs. |
