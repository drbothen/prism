...
- **Post-DEFECT-PQL-FNCALL-LHS-001 predicate fn-call positions:** A `FuncCall::Scalar { func: ScalarFunc::Unknown(name), args }` node appearing as the LHS of a `Predicate::Compare` in any of the following positions (all share `build_predicate_parser`): (i) pipe `| where` stage predicate (`PipeStage::Where(Predicate)` — `Predicate::Compare { lhs: Expr::FuncCall(FuncCall::Scalar), ... }`), (ii) filter-mode root predicate (`| lower(col) = 'val'` style), or (iii) SqlPipe pipe-stage `| where` predicate (`Ast::SqlPipe` tail-stage `| where`) — where ALL THREE of conditions (a/b/c) above hold. **Pre-grammar-extension:** fn-call LHS in these positions was grammar-rejected (E-QUERY-001), so the walk was never reached. **Post-extension (DEFECT-PQL-FNCALL-LHS-001 Option A):** the grammar accepts them; the gate MUST walk fn-call LHS nodes in these predicate positions for `ScalarFunc::Unknown(name)`. **Enumeration note — 3 new vs. 5 total (ADR-048 §D.7.1):** Positions (i)–(iii) above are the positions where the walk MECHANISM is new — prior to the grammar extension, fn-call LHS inputs never reached the walker in these positions (grammar-rejected at parse time with E-QUERY-001); the grammar extension (DEFECT-PQL-FNCALL-LHS-001 Option A) makes them newly reachable. SQL WHERE (position iv) and SqlPipe-head `WHERE` were pre-existing walk targets via `collect_unknown_scalars_from_sql_query` but now additionally receive fn-call LHS inputs post-grammar-extension (no new walk code required — the mechanism already existed; only the inputs are new post-extension). SQL HAVING (position v) is also walked via the SQL-mode predicate walk. All **five** positions enumerated in ADR-048 §D.7.1 — (i) pipe `\| where`, (ii) filter-mode root predicate, (iii) SqlPipe pipe-stage `\| where`, (iv) SQL WHERE, (v) SQL HAVING — are therefore in the gate's effective coverage. Readers who see only positions (i)–(iii) listed in the bullet above should note that (iv)–(v) are covered by the SQL-mode arm ("in `SELECT` projection or `WHERE` clause") without requiring new walk code. For the ADR-048 §D.7.1 aggregate-gate walker (predicate_fncall_names), positions (iv) and (v) are ALSO NEW walker calls added in ADR-048 v1.2 — see ADR-048 §D.7.1 (now v1.4) for the aggregate-gate walker scope. **Implementation note:** the `collect_unknown_scalar_from_predicate` walk function must descend into `Predicate::Compare { lhs: Expr::FuncCall(FuncCall::Scalar { func: ScalarFunc::Unknown(name), .. }), .. }` in pipe `| where` and filter-mode predicate contexts. This is NEW walk coverage added by the grammar extension; prior to the fix, these inputs never reached the walker.
