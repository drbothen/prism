...
- **Post-DEFECT-PQL-FNCALL-LHS-001 predicate fn-call positions:** A `FuncCall::Scalar { func: ScalarFunc::Unknown(name), args }` node appearing as the LHS of a `Predicate::Compare` in any of the following positions (all share `build_predicate_parser`): (i) pipe `| where` stage predicate (`PipeStage::Where(Predicate)` — `Predicate::Compare { lhs: Expr::FuncCall(FuncCall::Scalar), ... }`), (ii) filter-mode root predicate (`| lower(col) = 'val'` style), or (iii) SqlPipe pipe-stage `| where` predicate (`Ast::SqlPipe` tail-stage `| where`) — where ALL THREE of conditions (a/b/c) above hold. **Pre-grammar-extension:** fn-call LHS in these positions was grammar-rejected (E-QUERY-001), so the walk was never reached. **Post-extension (DEFECT-PQL-FNCALL-LHS-001 Option A):** the grammar accepts them; the gate MUST walk fn-call LHS nodes in these predicate positions for `ScalarFunc::Unknown(name)`. SQL WHERE and SQL HAVING are already covered by the SQL-mode arm above ("in `SELECT` projection or `WHERE` clause" — the WHERE arm covers SQL WHERE; HAVING is also walked via the SQL-mode predicate walk). **Implementation note:** the `collect_unknown_scalar_from_predicate` walk function must descend into `Predicate::Compare { lhs: Expr::FuncCall(FuncCall::Scalar { func: ScalarFunc::Unknown(name), .. }), .. }` in pipe `| where` and filter-mode predicate contexts. This is NEW walk coverage added by the grammar extension; prior to the fix, these inputs never reached the walker.
