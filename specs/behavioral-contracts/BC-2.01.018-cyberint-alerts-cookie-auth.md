...
`iso8601 / unix_epoch_seconds / unix_epoch_millis`). The parser maintains a
`(Timestamp, AlertID)` 2-tuple cursor. Timestamps that cannot be parsed through any of
...
| TV-BC-2.01.018-002 | Timestamp as Unix epoch seconds integer — e.g., `whois_created_date` field value `1705708800` | Unix-epoch-seconds branch of CyberintTime parser succeeds; OCSF `time` field set to correct RFC-3339 timestamp; cursor timestamp extracted |
...
## Related BCs
- BC-2.01.006: Cyberint Assets Cookie-Based Authentication and Multi-Format Timestamp Parsing (sibling BC; same auth mechanism, different surface and cursor key)
