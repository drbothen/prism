...
## Related BCs
- BC-2.01.006: Cyberint Assets Cookie-Based Authentication and Multi-Format Timestamp Parsing (sibling BC; same auth mechanism, different surface — Assets surface retains `(Timestamp, AssetID)` cursor pagination; Alerts surface uses `PageNumber` pagination per ADR-056 §D3)
