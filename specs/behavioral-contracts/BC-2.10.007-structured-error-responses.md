...
- **`_` non-exhaustive catch-all arm** — `suggestion` = `"See audit log for details."`: **From v1.18 onward, ZERO currently-known `PrismError` variants fall to this arm.** All 28 formerly-catch-all variants received explicit arms in F-MCPRS-PRL10-OBS-003 (fix-burst 22, 2026-07-14). The `_ =>` catch-all is reserved exclusively for FUTURE/unknown variants added to `PrismError` after this BC version. Catch-all behavior: `category: "upstream_error"`, `suggestion: "See audit log for details."`, `retryable: false`, `original_params_valid: true`, `upstream_message: null`. **Any new `PrismError` variant MUST be added to BOTH: (1) `crates/prism-core/src/tests/error_category_coverage.rs` sentinel AND (2) a dedicated arm in `prism_error_to_structured_call_result`'s VariantMeta match.** The 28 variants moved out of the catch-all (four groups per F-MCPRS-PRL10-OBS-003 comment in `error_mapping.rs`): **Group 1 — 12 internal Prism framework failures:** `Infusion`, `Plugin`, `OcsfProtobufEncode`, `OcsfProtobufDecode`, `OcsfDescriptorNotFound`, `CredentialStoreError`, `CredentialEncryptionError`, `ScheduleNotFound`, `DetectionRuleParseFailed`, `DetectionRuleNotFound`, `DetectionStateCorrupt`, `CaseNotFound`; **Group 2 — 3 configuration errors:** `InvalidCredentialName`, `CredentialNotFound`, `EncryptionKeyMissing`; **Group 3 — 3 validation errors:** `ScheduleConflict`, `ScheduleCronInvalid`, `CaseStateTransitionInvalid`; **Group 4 — 10 upstream_error explicit:** `OcsfFieldMissing`, `OcsfFieldTypeMismatch`, `OcsfUnknownClassUid`, `OcsfUnknownEventClass`, `OcsfUnknownRecordType`, `OcsfTimestampParseError`, `OcsfNormalizationFailed`, `WritePartialFailure`, `IocFeedParseFailed`, `IocLookupFailed`.
...
| `suggestion` | **CLASS-DIFFERENTIATED:** infrastructure arm → `"Prism infrastructure failure. Contact Prism operator; see audit log for details."`; `_` catch-all arm → `"See audit log for details."` **(applies to future/unknown variants only from v1.18 — ZERO currently-known variants fall to the catch-all; see §Rule 2)**; dedicated-arm variants → category-appropriate variant-specific suggestion per §Category rule (e.g., `AuthTokenExpired` → `"The auth token has expired. Re-authenticate and obtain a fresh token."`). |
...
| `"validation"` | Caller-supplied parameters structurally or semantically invalid | Fix the tool call parameters | `QueryParseFailed`, `McpParameterInvalid`, `QueryLimitExceeded`, `QuerySecurityLimitExceeded`, `UnknownSourceTable`, `AliasNotFound`; **`ScheduleConflict`, `ScheduleCronInvalid`, `CaseStateTransitionInvalid`** (from v1.18; previously catch-all) |
...
Rationale: BC §Complete field specification for `structuredContent.error` defines `retryable` as "`true` for transient errors (rate limit, timeout, network); `false` for permanent (invalid params, auth invalid)." HTTP 503 Service Unavailable is an explicitly transient HTTP status — the upstream service is temporarily unavailable and the caller may retry after some delay. The transient-only set is: **408** (Request Timeout), **425** (Too Early), **429** (Too Many Requests / rate limit), **500** (Internal Server Error), **502** (Bad Gateway), **503** (Service Unavailable), **504** (Gateway Timeout). All other status codes are permanent or require non-retry remediation: 400 (Bad Request), 401/403 (auth failures), 404 (Not Found), 422 (Unprocessable Entity), 501 (Not Implemented), and similar. Marking 400/404/422 as retryable would waste LLM-agent cycles — retrying a malformed request or a genuinely absent resource will never succeed. Note: the explicit transient list excludes 401/403 (permanent credential failures requiring re-auth action), but also excludes 400/404/422 and other 4xx codes that indicate a permanent client-side problem.

**Code change required in `error_mapping.rs` `SensorHttpError` arm:**
```rust
// BC-2.10.007 v1.17 §RETRYABLE-503: only explicitly transient HTTP status codes are retryable.
// Transient set: 408 (Request Timeout), 425 (Too Early), 429 (Rate Limit),
// 500 (Internal Server Error), 502 (Bad Gateway), 503 (Service Unavailable),
// 504 (Gateway Timeout). All other codes — including 400/404/422 (permanent client errors)
// and 401/403 (auth failures requiring re-auth, not retry) — are non-retryable.
// Pre-existing gap: prior arm set retryable: false unconditionally. Spec correction per
// RETRYABLE-503 adjudication v1.16 (coordinator-raised overbroad-rule finding).
...
- Update any existing test that asserts `SensorHttpError` with a transient status code (408/425/429/500/502/503/504) → `retryable: false` to assert `retryable: true`. Tests with permanent status codes (400/404/422/501/etc.) that assert `retryable: false` are CORRECT under the v1.16 rule and MUST NOT be changed.
