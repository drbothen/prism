...
| EC-016-014-009 | `CredentialResolver::resolve()` returns `NotFound` for any required credential ref | `acquire_token()` returns `AuthAcquisitionFailed` with E-AUTH-005 detail. Zero HTTP POSTs are issued. |
...
| `E-AUTH-005` (detail within `E-AUTH-001`) | `CredentialResolver::resolve()` returns `CredentialResolutionError::NotFound` for a required credential ref | `acquire_token()` returns `AuthAcquisitionFailed` with E-AUTH-005 detail message. No HTTP POST issued. |
...
| **TV-9** Credential not found | Either | `MockCredentialResolver` returns `CredentialResolutionError::NotFound` | `acquire_token()` returns `AuthAcquisitionFailed` with E-AUTH-005 detail; zero HTTP POSTs issued |
