...
|---------|-------|------|--------|--------|
| 1.15 | FIX-IEQ-ERRPATH-001-ADV-PR-P1-OBS-001 | 2026-07-09 | product-owner | **ADV-PR-P1-OBS-001 POL-25 companion to BC-2.11.016 v1.22 — pin-only update; no content change.** BC-2.11.016 v1.22 adds `### Injection-safety of \`column\` (MCP-facing payload)` — a payload postcondition documenting `sanitize_for_log` control-char sanitization (CWE-116/CWE-117; ADV-PR-P1-OBS-001) applied to `ColumnNotFoundDetails.column` and its Display rendering. This is orthogonal to the SQLPIPE HEAD-PROJECTION BINDING RULE and HEAD-JOIN SUSPENSION RULE restated in this BC's §Error Cases E-QUERY-038 Condition column; no restatement of raw-echo semantics is present here. **Amendments (pin-only):** (1) **SQLPIPE HEAD-PROJECTION BINDING RULE BC anchor:** `(BC-2.11.016 v1.21)` → `(BC-2.11.016 v1.22)`; (2) **HEAD-JOIN SUSPENSION RULE clause:** `(BC-2.11.016 v1.21):` → `(BC-2.11.016 v1.22):`; (3) **Behavior column BC anchor:** `Full spec: BC-2.11.016 v1.21 (…)` → `Full spec: BC-2.11.016 v1.22 (… column MCP-payload injection-safety)`. Origin anchors `(BC-2.11.016 v1.15)`, `(BC-2.11.016 v1.17)`, `(BC-2.11.016 v1.18)`, `(BC-2.11.016 v1.19)` within the same row UNCHANGED. `modified: 2026-07-09`. |
