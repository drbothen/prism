...
|---------|-------|------|--------|--------|
| 1.1 (trace-pin hygiene) | F-P3-MED-001-adr-pin-correction | 2026-06-26 | product-owner | **F-P3-MED-001 closure — stale ADR trace-pin correction (hygiene only, no semantic change).** §Traceability `ADR traces` cell corrected from `ADR-043 v1.1` to `ADR-043 v1.2`, reflecting the ADR-043 version bump that occurred in the D-1357 burst. No preconditions, postconditions, invariants, edge cases, or error codes changed. BC semantic version is NOT bumped; this is a cross-reference label correction only. |
