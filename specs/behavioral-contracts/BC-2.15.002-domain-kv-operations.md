...
| VP ID | Description |
|-------|-------------|
| VP-055 | StorageEngine put_batch atomicity and domain isolation — (1) for any `put_batch` where the underlying write fails partway through, zero entries from that batch are readable via `get`; (2) for any pair of distinct domains (A, B), a write to domain A at key K produces `get(domain_B, K) == None`; the MockStorageEngine upholds both invariants across all domain-pair and batch-size combinations. Method: Proptest. Priority: P1. |
