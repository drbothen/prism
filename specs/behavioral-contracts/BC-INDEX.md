...
| BC-2.16.013 | Bundled Sensor Spec Authoring and DTU-Parity Verification — 4 Initial Sensors | 16 - Spec Engine | CAP-029 | P0 | draft (D-731 PLUGIN-MIGRATION-001-D PO authoring 2026-05-20; v1.3 FB-IMPL-P3-PO 2026-05-20 — all sensor URL paths corrected to CODE-GROUNDED patterns [CrowdStrike: /queries/{r}, /entities/{r}/GET; Claroty: /api/v1/{r}s no /xdome; Cyberint: /api/{r}s no /v1; Armis: single /api/v1/search no trailing slash]; parse_spec_file phantom replaced with SpecLoader::parse(toml_input: &str) in §Postconditions §2 + §Canonical Test Vectors) — v1.3 |
