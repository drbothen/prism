...
- `claroty.sensor.toml` — `sensor_id: "claroty"`, `auth_type: "bearer_static"`,
  ...
  - `audit_logs` — POST `/api/v1/audit_log/get`; DTU route registered by
    ...
    **Bounded push-down via `filter_by` JSON-object injection (S-CLAROTY-AUDITLOG-TIMEBOX-001):** The
    ...
    `'{"filter_by": ${query.filter._claroty_audit_filter_by}, "sort_by": [{"field":"timestamp","order":"asc"}]}'`
    (see **Sort-by postcondition** below for live-validation rationale and accepted residual non-determinism) and wires dynamic filter injection
    ...
    **Sort-by postcondition (DEFECT-CLAROTY-SORTBY-DETERMINISM-001, canonical fallback adopted 2026-09-02):** The
    `body_template` MUST include a `sort_by` key alongside `filter_by`. The canonical sort form
    for `audit_logs` is `[{"field":"timestamp","order":"asc"}]` (timestamp-only).

    **Live-validation outcome (2026-09-02):** Both compound sort variants were tested against the
    live xDome audit_log endpoint: `[{"field":"timestamp","order":"asc"},{"field":"id","order":"asc"}]`
    and `[{"field":"timestamp","order":"asc"},{"field":"id","order":"desc"}]` both returned 0 rows
    (HTTP 200, empty result set). `[{"field":"timestamp","order":"asc"}]` alone returned live rows.
    Conclusion: `id` is NOT a valid sort field for `audit_logs`. The `GetAuditLogParameters`
    documented sortable fields (`category`, `action`, `user_display_name`, `note`, `timestamp`,
    `details`) do not include `id`. Including `id` in `sort_by` causes xDome to silently return
    0 rows (HTTP 200) rather than a 4xx response. The compound form
    `[{"field":"timestamp","order":"asc"},{"field":"id","order":"asc"}]` is RETIRED for `audit_logs`
    — it is live-proven-broken.

    **Accepted residual non-determinism:** `audit_logs` exposes no valid unique sortable tiebreaker
    in the `SortClause`/`GetAuditLogParameters` documented field set (`category`, `action`,
    `user_display_name`, `note`, `timestamp`, `details`). Equal-timestamp ties resolve in
    API-dependent (non-guaranteed-stable) order. The 7-day time-window filter bounds the practical
    blast radius. This residual is accepted, not a defect.

    **Filter-by coexistence:** `sort_by` MUST NOT displace `filter_by` from `body_template`.
    The 7-day-window `filter_by` injection requirement (§Bounded push-down via `filter_by`
    above) is unchanged. Both keys MUST be present in every request body.
    (Mandate anchor: S-CLAROTY-AUDITLOG-TIMEBOX-001 AC and its Red Gate test.)

    **RG-002 and RG-009 coupling:** RG-002 (`test_rg_audit_logs_sort_by_in_request_body`)
    continues to assert that `sort_by` is present in the request body alongside `filter_by`;
    its sort_by-presence + filter_by coexistence logic is unchanged. RG-009
    (`test_rg_audit_logs_sort_by_id_tiebreaker_or_fallback`) was HARDENED when the
    timestamp-only canonical was adopted: its assertion was tightened from accept-either-form
    to strictly enforce timestamp-only — it asserts `sort_by` len == 1, the sole element is
    `timestamp asc`, and `id` is ABSENT. The retired compound form now fails RG-009, making
    it a regression guard against reintroduction of the live-proven-broken compound sort.
...
| EC-016-013-011 | Offset pagination determinism for `audit_logs`: two sequential pages must not duplicate or skip records | The `body_template` MUST contain `"sort_by": [{"field":"timestamp","order":"asc"}]` (timestamp-only canonical form). **Live finding (2026-09-02, DEFECT-CLAROTY-SORTBY-DETERMINISM-001):** Including `id` in `sort_by` — both `[{"field":"timestamp","order":"asc"},{"field":"id","order":"asc"}]` and `[{"field":"timestamp","order":"asc"},{"field":"id","order":"desc"}]` — returned 0 rows (HTTP 200, empty result set) on the xDome audit_log endpoint. The compound form is RETIRED (live-proven-broken). `id` is not in the documented sortable field set (`category`, `action`, `user_display_name`, `note`, `timestamp`, `details`). Without `sort_by`, the xDome default is undefined, making page boundaries non-deterministic under offset pagination. **Accepted residual non-determinism:** no valid unique sortable tiebreaker exists in the documented `SortClause`/`GetAuditLogParameters` field set; equal-timestamp ties resolve in API-dependent (non-guaranteed-stable) order; bounded by the 7-day time-window filter; accepted, not a defect. **Filter-by coexistence:** `sort_by` MUST coexist with `filter_by` in `body_template` — the 7-day-window injection is unchanged. **Test coupling:** RG-002 (`test_rg_audit_logs_sort_by_in_request_body`) asserts `sort_by` present alongside `filter_by`; its logic is unchanged. RG-009 (`test_rg_audit_logs_sort_by_id_tiebreaker_or_fallback`) was HARDENED to strictly enforce the timestamp-only canonical — asserts `sort_by` len == 1, sole element `timestamp asc`, and `id` ABSENT; the retired compound form now fails it as a regression guard. Anchor: `DEFECT-CLAROTY-SORTBY-DETERMINISM-001` RG-002 (`test_rg_audit_logs_sort_by_in_request_body`) + RG-009 (`test_rg_audit_logs_sort_by_id_tiebreaker_or_fallback`). |
