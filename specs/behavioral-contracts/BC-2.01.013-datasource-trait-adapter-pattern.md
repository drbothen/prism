...
introduced: cycle-1
modified: "2026-06-05"  # v1.13 S-DEMO-QUERY-PUSHDOWN-001-v2-respec — per-sensor push-down translation table corrected per pushdown-redesign.md §6 + §1; ADR-033 referenced
amendment_burst: S-DEMO-QUERY-PUSHDOWN-001-v2-bc-respec
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
- **SpecDrivenSensorAdapter Pagination / Push-Down Scope Clause (D-924 initial scope-out; superseded by S-DEMO-QUERY-PUSHDOWN-001 approved story v1.1 — F-PUSHDOWN-008):**
  ...
  **Per-sensor push-down translation (corrected in v1.13 per pushdown-redesign.md §6 + §1, ADR-033):**
  ...
  | Armis | AQL verbatim passthrough only (BC-2.11.007 Mechanism B). The `aql` value from `FetchContext.query_filters["aql"]` is interpolated via `${query.filter.aql}` into the TOML path_template as a URL query param: `GET /api/v1/search?aql=<value>`. No additional push-down translation is performed or correct. `offset`/`limit` pagination is handled by the existing OffsetLimit pipeline. | AQL passthrough (existing; no new push-down dimensions) | Time-window: NONE — `SearchQueryParams` has no `timeFrame`, `from_date`, or equivalent field. `maxResults`: NONE — `SearchQueryParams` has no such field. The v1.12 `maxResults`+`timeFrame` claim was WRONG. |
...
| Priority | P0 |
| Related ADRs | ADR-023 (SensorAuth open trait, runtime auth-composition prevention), ADR-033 (push-down time-window extraction strategy — pre-fan-out heuristic T1; motivates the `run_materialization_pipeline` wiring required by TV-BC-2.01.013-006 v1.13) |
