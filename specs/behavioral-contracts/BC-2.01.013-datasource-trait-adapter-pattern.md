...
introduced: cycle-1
modified: "2026-06-05"  # v1.12 F-PUSHDOWN-008 — push-down scope clause updated per S-DEMO-QUERY-PUSHDOWN-001 (approved story supersedes scope-out)
amendment_burst: F-PUSHDOWN-008-po-fix-burst
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
| TV-BC-2.01.013-006 | `SpecDrivenSensorAdapter::fetch()` called on the first / query-plan step with `FetchContext { limit: Some(50), start_time: Some(t), ..Default::default() }` for a CrowdStrike sensor | The CrowdStrike HTTP request carries `limit=50` as a query parameter AND the `filter` FQL string includes `created_timestamp:>'<t as ISO8601>'`. All pages up to `MAX_PAGES_PER_STEP` concatenated into returned `Vec<RecordBatch>`. DataFusion applies LIMIT post-materialization as correctness backstop (BC-2.11.007 result-equivalence invariant). (Supersedes prior TV-BC-2.01.013-006 v1.8 which asserted push-down absence as positive conformance; re-cast to assert push-down presence on first/query step per S-DEMO-QUERY-PUSHDOWN-001 approved story.) |
