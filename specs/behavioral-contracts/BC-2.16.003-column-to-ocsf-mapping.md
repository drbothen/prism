...
- `crates/prism-ocsf/src/class_selector.rs` — `select_by_class_name` and `select`; must map `"audit_activity"` to `CLASS_UID_ENTITY_MANAGEMENT = 3004` (not `CLASS_UID_ACCOUNT_CHANGE = 3001`; KF-01 code obligation per ADR-058 §I5/§K5 Divergence 3); Armis `("armis", "audit_log")` arm requires the same correction (TD-VSDD-097 sibling sweep)
