...
- A fail-closed collision check is enforced at `pipeline_result_to_record_batch` execution time per ADR-058 §J2 (both halves): (1) no flattened `ocsf_field` name may equal the `col.name` of a different column in the same table (inter-column shadow prevention); (2) no flattened `ocsf_field` name may equal any of the four synthesized envelope/blob reserved names — the three unconditionally-emitted envelope columns (`class_uid`, `category_uid`, `_sensor`) and the conditionally-emitted blob column `raw_extensions` (emitted when `ocsf_column_naming == true` AND ≥1 column has `ocsf_field == None`) — (synthesized-name reservation per ADR-058 §J2); violation on either condition returns `Err(ArrowError::SchemaError(...))`, blocking schema construction; mandate anchor: S-ADR058-OCSF-ROUTING-001 RG-027.
...
| Integer | Bool/Null/Array | any | Rule 3 | Unchanged | OCSF field (no coercion) |
| Integer | Object | any | **Rule 3 (current — KNOWN GAP, EC-016-013-030)**; contracted: Path A `None` + `column_coercion_failure` warn; Path B `Err(CoercionWarning)` | Path A: currently `Unchanged` (WRONG — object placed in Arrow Int64 column); contracted: `None` (null cell); Path B: currently `Unchanged` (WRONG); contracted: `Err(CoercionWarning)` | OCSF field — KNOWN GAP (EC-016-013-030); Fix: S-ADR058-OCSF-COERCION-001 (ADR-058 §H item 4) |
...
- EC-016-013-030: `column_type = "integer"` + Object input — **both Path A and Path B**: currently Rule 3 pass-through (object placed in integer-typed OCSF field — WRONG). Contracted: Path A `None` (null cell) + `column_coercion_failure` warn; Path B `Err(CoercionWarning)` → demotion to `raw_extensions`. Symmetric with String+Object (EC-016-013-008). Fix: S-ADR058-OCSF-COERCION-001 (ADR-058 §H item 4)
...
- Under Interpretation A (`ocsf_column_naming = true`): no flattened `ocsf_field` name in a table may (1) equal the `col.name` of a different column in the same table (inter-column shadow prevention), nor (2) equal any of the four synthesized envelope/blob reserved names — the three unconditionally-emitted envelope columns (`class_uid`, `category_uid`, `_sensor`) and the conditionally-emitted blob column `raw_extensions` (emitted when `ocsf_column_naming == true` AND ≥1 column has `ocsf_field == None`) — (synthesized-name reservation per ADR-058 §J2); violation on either condition fails closed with `Err(ArrowError::SchemaError(...))` at schema construction time (ADR-058 §J2 both halves; S-ADR058-OCSF-ROUTING-001 RG-027)
...
| KNOWN GAP | `column_type = "integer"` + **Object** input (both paths) | Both Path A (`build_column_array`) and Path B (`coerce_value`) currently pass Object through via Rule 3 (WRONG — object placed in integer-typed Arrow Int64 column). Contracted: Path A `None` (null cell) + `column_coercion_failure` warn (`actual_json_kind = "object"`); Path B `Err(CoercionWarning)` → demotion to `raw_extensions`. EC-016-013-030. Fix: S-ADR058-OCSF-COERCION-001 (ADR-058 §H item 4). |
| Schema error (fail-closed) | Under `ocsf_column_naming = true`: (1) flattened `ocsf_field` name equals `col.name` of a different column in the same table (inter-column shadow); OR (2) flattened `ocsf_field` name equals any of the four synthesized envelope/blob reserved names — the three unconditionally-emitted envelope columns (`class_uid`, `category_uid`, `_sensor`) or the conditionally-emitted blob column `raw_extensions` (emitted when `ocsf_column_naming == true` AND ≥1 column has `ocsf_field == None`) — (synthesized-name reservation per ADR-058 §J2 both halves; see EC-016-013-029) | `pipeline_result_to_record_batch` returns `Err(ArrowError::SchemaError(...))` — flag cannot activate until the collision is resolved; S-ADR058-OCSF-ROUTING-001 RG-027 |
...
| EC-016-013-028 | ENRICH-1 wildcard source_path columns (`ocsf_field == None` with `source_path = "$.X[*]"`) in `raw_extensions` value representation — Claroty `devices` columns `ip_list`, `mac_list`, `network_list`, `vlan_list` | The value stored under `col.name` in the `raw_extensions` JSON object MUST be the **ENRICH-1-normalized value**: the compact JSON-list string (e.g., `["192.168.1.1","10.0.0.1"]` serialized as the string `"[\"192.168.1.1\",\"10.0.0.1\"]"`) — not a raw JSON array value from the API response and not the top-level JSON structure before source_path extraction. **Mechanism (ADR-058 §I2 / story AC-007a):** `pipeline_result_to_record_batch` owns the raw_extensions aggregation+serialization and MUST apply the same source_path extraction and ENRICH-1 `Value::Array` normalization to `ocsf_field == None` columns that it applies to first-class columns (ADR-058 §I2). EC-016-013-026 documents the compact JSON-list string FORMAT produced by the `Value::Array` arm at serialization time; the structural OWNER of that normalization for `raw_extensions` aggregation is `pipeline_result_to_record_batch` per ADR-058 §I2, not `build_column_array` independently. Wire-shape assertion: `SELECT raw_extensions FROM claroty_devices` for a row with non-empty `ip_list` MUST yield a `raw_extensions` JSON object containing an `"ip_list"` key whose value is a JSON-encoded compact array string — not a nested JSON array, not null. **POL-38 obligation: story-writer leg of S-ADR058-OCSF-ROUTING-001 MUST add AC-007a and wire-shape Red Gate test RG-026 asserting this representation (parse `raw_extensions` JSON for a Claroty devices row, access the `"ip_list"` key, assert its Rust-level type is a string whose content parses as a JSON array — not a raw array type).** |

| EC-016-013-029 | Under `ocsf_column_naming = true`, a TOML spec column declares `ocsf_field` equal to any of the four synthesized envelope/blob reserved names: the three unconditionally-emitted envelope columns (`"class_uid"`, `"category_uid"`, `"_sensor"`) or the conditionally-emitted blob column `"raw_extensions"` (emitted when `ocsf_column_naming == true` AND ≥1 column has `ocsf_field == None`) | `pipeline_result_to_record_batch` returns `Err(ArrowError::SchemaError(...))` — fail-closed; the sensor spec cannot activate until the reserved-name collision is resolved; symmetric with the inter-column shadow rule (§Invariants §J2 both halves; ADR-058 §J2); mandate anchor: S-ADR058-OCSF-ROUTING-001 RG-027. **POL-38 obligation: story-writer leg of S-ADR058-OCSF-ROUTING-001 MUST add Red Gate test RG-027 asserting that a sensor spec whose `ocsf_field` resolves to `"raw_extensions"` (or `"class_uid"`, `"category_uid"`, `"_sensor"`) causes `pipeline_result_to_record_batch` to return `Err(ArrowError::SchemaError(...))` and blocks schema construction.** |
| EC-016-013-030 | `column_type = "integer"`, `Value::Object({...})` input | **Contracted behavior** (symmetric with String+Object, EC-016-013-008, and Integer+non-parseable-String Path A, EC-016-013-025): **Path A (`build_column_array`)**: contracted `None` (null cell) + `tracing::warn!(event_type = "column_coercion_failure", column = %col.name, column_type = "integer", actual_json_kind = "object")`. Current behavior (pre-fix): Rule 3 pass-through — object placed in integer-typed Arrow Int64 column (WRONG). **Path B (`coerce_value`)**: contracted `Err(CoercionWarning)` — caller (`map_record`) diverts to `raw_extensions` and emits `column_coercion_failure` warn. Current behavior (pre-fix): Rule 3 pass-through (WRONG). Closes the no-silent-data-loss/observability gap symmetric to String+Object (EC-016-013-008) and Integer+non-parseable-String Path A (EC-016-013-025). Integer+Array is a separate concern (Rule 3 pass-through; not a parallel gap for this EC — see §KNOWN GAPs). Mandate anchor: S-ADR058-OCSF-COERCION-001 (ADR-058 §H item 4). |
...
| Priority | P0 |
| Known-Gap Story Needed | ANCHORED — S-ADR058-OCSF-COERCION-001 (Stage 1): CoercionWarning tracing emission (AC-004), EC-016-013-007/008 structured-type demotion fix (AC-001/AC-002/AC-005), EC-016-013-009 Path B integer-column coerce_value fix (AC-003), EC-016-013-025 Path A build_column_array integer-column String parse fix (AC-007 / RG-008 / RG-009 — added per F-P16-MED-003 adjudication). S-ADR058-OCSF-ROUTING-001 (Stage 2): ocsf_column_naming flag, Claroty TOML/code corrections KF-01..KF-12, EC-016-013-027 `prism_describe` Tier-2 raw_extensions descriptor (three MUST assertions in §Postconditions §Interpretation A: phantom-descriptor prohibition, col_type=Json/nullable=true ColumnDescriptor shape, description source-key enumeration — story-writer leg adds AC + Red Gate RG-025, POL-38 obligation). EC-016-013-028 ENRICH-1 source_path columns in `raw_extensions` value representation (ADR-058 §I2 / AC-007a / RG-026): story-writer leg of S-ADR058-OCSF-ROUTING-001 MUST ensure AC-007a body and RG-026 test description attribute the normalization to `pipeline_result_to_record_batch` (not `build_column_array`); wire-shape Red Gate test asserts compact JSON-list string in `raw_extensions["ip_list"]` (POL-38 obligation). EC-016-013-029 synthesized-name reservation (ADR-058 §J2 synthesized-name half): story-writer leg MUST add Red Gate test RG-027 asserting `pipeline_result_to_record_batch` returns `Err(ArrowError::SchemaError(...))` when a spec column declares `ocsf_field` equal to `"raw_extensions"`, `"class_uid"`, `"category_uid"`, or `"_sensor"` (POL-38 obligation). |

## Changelog

| Version | Burst | Date | Author | Change |
|---------|-------|------|--------|--------|
| 1.20 | f-coerce-adv-obs-003-integer-object | 2026-08-20 | product-owner | F-COERCE-ADV-OBS-003 closure (ADR-058 §H item 4): added EC-016-013-030 for `column_type = "integer"` + `Value::Object` input — contracted behavior on both paths: Path A (`build_column_array`) returns `None` (null cell) + emits `column_coercion_failure` (column_type="integer", actual_json_kind="object"); Path B (`coerce_value`) returns `Err(CoercionWarning)` → demotion to `raw_extensions`. Current behavior: Rule 3 pass-through (WRONG). Closes the no-silent-data-loss/observability gap symmetric to String+Object (EC-016-013-008) and Integer+non-parseable-String Path A (EC-016-013-025). Also: (A) updated §Full Coercion Matrix to split the `Integer + Bool/Null/Array/Object` row into `Bool/Null/Array` (unchanged, Rule 3) and `Object` (KNOWN GAP, EC-016-013-030); (B) added EC-016-013-030 bullet to §KNOWN GAPs; (C) added KNOWN GAP row to §Error Conditions for Integer+Object. Mandate anchor: S-ADR058-OCSF-COERCION-001 (ADR-058 §H item 4). TD-VSDD-097: (1) sibling pair — BC-2.16.003 ↔ BC-2.16.002 both edited this burst; BC-2.16.002 receives corresponding trigger-conditions extension and F-COERCE-ADV-MED-001 stale-clause removal in same burst; CLEAR. (2) downstream copy target — EC-016-013-030 is new content, not a verbatim copy-source in downstream artifacts; story-writer must add AC+RG to S-ADR058-OCSF-COERCION-001 per mandate anchor (story-writer obligation noted). (3) mandate anchor — EC-016-013-030 anchored to S-ADR058-OCSF-COERCION-001 (ADR-058 §H item 4); no unanchored MUSTs introduced. |
| 1.19 | pol39-terminal-normalization-leg1-product-owner | 2026-08-19 | product-owner | POL-39/TD-VSDD-091 terminal normalization: stripped `v2.23` version stamp from `ADR-058 §G` section cite in §Postconditions §Interpretation A intro clause (`in two tiers per ADR-058 §G v2.23:` → `in two tiers per ADR-058 §G:`). No behavioral change. TD-VSDD-097: (1) sibling pair — BC-2.16.003 has no named split-event twin; CLEAR. (2) downstream copy target — this §G intro clause is not a verbatim copy-source in downstream artifacts; CLEAR. (3) mandate anchor — no new MUSTs; CLEAR. |
| 1.18 | fb-52-53-54-leg2-product-owner | 2026-08-19 | product-owner | F-P52-LOW-1 (≡ F-P54-LOW-001) closure: corrected four "emits unconditionally" phrasing errors — §Postconditions Tier-2 collision-check bullet, §Invariants Interpretation-A shadow entry, §Error Conditions fail-closed SchemaError row, and EC-016-013-029 description — distinguishing the three unconditionally-emitted envelope columns (`class_uid`, `category_uid`, `_sensor`) from the conditionally-emitted blob column `raw_extensions` (emitted when `ocsf_column_naming == true` AND ≥1 `ocsf_field == None` column exists); reservation rule unchanged — all four names remain reserved fail-closed against `ocsf_field` collision per ADR-058 §J2. Also re-pinned §Postconditions §Interpretation A intro clause from ADR-058 §G v2.22 → v2.23. TD-VSDD-097: (1) sibling pair — BC-2.16.003 has no named split-event twin; CLEAR. (2) downstream copy target — S-ADR058-OCSF-ROUTING-001 story text may carry pre-correction "emits unconditionally" phrasing for all four synthesized names; story-writer must sweep S-ADR058-OCSF-ROUTING-001 for "emits unconditionally" and RG-027 test description. (3) mandate anchor — no new MUSTs introduced; existing RG-027 mandate anchors to S-ADR058-OCSF-ROUTING-001 unchanged; CLEAR. |
| 1.17 | fb-49-51-product-owner-leg1 | 2026-08-18 | product-owner | F-P49-MED-001: rewrote EC-016-013-028 to remove false mechanistic claims — the compact-JSON-list serialization for `ocsf_field == None` source_path columns in `raw_extensions` is attributed to `pipeline_result_to_record_batch` (ADR-058 §I2 / story AC-007a / RG-026), not to `build_column_array`; removed "ENRICH-1 pipeline runs before `pipeline_result_to_record_batch`" and "produced by `build_column_array`'s String+Array arm" attributions; EC-016-013-026 is now cited as FORMAT precedent only, not the mechanism owner; POL-38 obligation updated to anchor to AC-007a / RG-026 explicitly. F-P49-MED-002 / F-P51-MED-002 (two-pass corroboration): extended §Postconditions §Interpretation A Tier-2 collision-check sentence, §Invariants Interpretation-A shadow entry, and §Error Conditions fail-closed SchemaError row to cover ADR-058 §J2 both halves — synthesized-name reservation (`class_uid`, `category_uid`, `_sensor`, `raw_extensions`) added alongside the existing inter-column shadow rule; both conditions fail-closed with `Err(ArrowError::SchemaError(...))` at schema construction time; added new EC-016-013-029 for the synthesized-name reservation test obligation (POL-38: RG-027 anchor for story-writer leg); §Traceability Known-Gap updated; removed stale "v2.22" version pin from Known-Gap prose per TD-VSDD-091. TD-VSDD-097: (1) sibling pair — BC-2.16.003 has no named split-event twin; CLEAR. (2) downstream copy targets — EC-016-013-028 reword: story-writer must update ROUTING-001 §AC-007a body (remove `build_column_array` attribution) and RG-026 test description to attribute normalization to `pipeline_result_to_record_batch`; §J2 synthesized-name extension: story-writer must add RG-027 Red Gate test to ROUTING-001; §Error Conditions + §Invariants §J2 extensions are not verbatim copy-sources in downstream artifacts; CLEAR. (3) mandate anchor — EC-016-013-029 synthesized-name MUST anchored to S-ADR058-OCSF-ROUTING-001 RG-027 (POL-38 obligation); EC-016-013-028 MUST anchored to AC-007a / RG-026; no unanchored MUSTs introduced; CLEAR. |
