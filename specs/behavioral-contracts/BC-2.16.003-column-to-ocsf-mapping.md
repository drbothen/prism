...
- A fail-closed collision check is enforced at `pipeline_result_to_record_batch` execution time per ADR-058 §J2 (both halves): (1) no flattened `ocsf_field` name may equal the `col.name` of a different column in the same table (inter-column shadow prevention); (2) no flattened `ocsf_field` name may equal any of the four synthesized envelope/blob names — `class_uid`, `category_uid`, `_sensor`, `raw_extensions` — that `pipeline_result_to_record_batch` emits unconditionally (synthesized-name reservation); violation on either condition returns `Err(ArrowError::SchemaError(...))`, blocking schema construction; mandate anchor: S-ADR058-OCSF-ROUTING-001 RG-027.
...
| Schema error (fail-closed) | Under `ocsf_column_naming = true`: (1) flattened `ocsf_field` name equals `col.name` of a different column in the same table (inter-column shadow); OR (2) flattened `ocsf_field` name equals any of the four synthesized envelope/blob names — `class_uid`, `category_uid`, `_sensor`, `raw_extensions` — that `pipeline_result_to_record_batch` emits unconditionally (synthesized-name reservation; ADR-058 §J2 both halves; see EC-016-013-029) | `pipeline_result_to_record_batch` returns `Err(ArrowError::SchemaError(...))` — flag cannot activate until the collision is resolved; S-ADR058-OCSF-ROUTING-001 RG-027 |
