...
| `primary_device`, `lateral_devices` | prism-dtu-crowdstrike | `routes/hosts.rs` | `GET /devices/queries/devices/v1` and `GET /devices/entities/devices/v2` (existing `get_host_details`) + `POST /devices/entities/devices/v2` (new `post_host_details`; mirrors `get_host_details` StageMask/scenario/session-registry logic exactly per DEFECT-CSDEVICES-EMPTY-PIPELINE-001 ratification 2026-07-10) | `stage_idx > 0` for primary; `mask.lateral_devices` for lateral | ACTIVE (GET pre-bc0f36c5, B-P1-01; POST pending DEFECT-CSDEVICES-EMPTY-PIPELINE-001 fix lane) |
...
| `prism-dtu-crowdstrike/src/routes/hosts.rs` | hosts list + entities | `GET /devices/queries/devices/v1`, `GET /devices/entities/devices/v2` (existing), and `POST /devices/entities/devices/v2` (new `post_host_details`; same StageMask guards as `get_host_details` per DEFECT-CSDEVICES-EMPTY-PIPELINE-001 ratification 2026-07-10) | Row 4 (pre-bc0f36c5); POST handler pending DEFECT-CSDEVICES-EMPTY-PIPELINE-001 fix lane |
