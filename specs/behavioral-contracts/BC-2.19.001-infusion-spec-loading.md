...
|---------|-------|------|--------|--------|
| 2.4 | DEFECT-ADAPTER-TLS-XDOME-LIVE-001 | 2026-08-13 | product-owner | **F-P-MED-001 (records correction): E-INFUSE-015 §Error Conditions firing-path description extended to include `load_spec`.** Prior Condition column text described firing as occurring "during the RUNTIME PHASE ... in `load_spec_with_runtime` or `hot_reload`" — omitting `load_spec`, which has a live `HttpLookup` construction path (`infusion/mod.rs §load_spec`). Code has three call sites (one each in `load_spec §load_spec`, `load_spec_with_runtime §load_spec_with_runtime`, `hot_reload §hot_reload`); error-taxonomy v2.74 already enumerates all three. Updated Condition column to: fires in `load_spec`, `load_spec_with_runtime`, and `hot_reload` (one call site each). No mechanism change — records-only correction per TD-VSDD-096. Matches error-taxonomy v2.74 and code. |
