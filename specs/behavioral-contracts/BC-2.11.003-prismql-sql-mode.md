...
- `Predicate::InSubquery` (WHERE/HAVING IN-subquery, e.g. `WHERE field IN (SELECT ...)`) is supported — DataFusion's `decorrelate_predicate_subquery` optimizer handles these natively with standard SQL three-valued semantics (TRUE when `field ∈ subquery`; FALSE when `field ∉ subquery` AND subquery has no NULLs; NULL when `field IS NULL` OR when `field ∉ subquery` AND subquery contains a NULL).
- `Expr::InSubquery` (SELECT projection/GROUP BY/ORDER BY IN-subquery, e.g. `SELECT (field IN (SELECT ...)) AS alias FROM t`) is not currently supported — returns E-QUERY-043 with a message directing the analyst to the WHERE-clause form or a JOIN equivalent. Correctness rationale: a COUNT-rewrite that executed this form was REJECTED (architect adjudication F-CSD-P4-001, D-1650, 2026-07-10) because it collapses NULL semantics (always TRUE/FALSE, never NULL), which is a correctness defect for nullable OCSF columns under the agent-harness SQL semantics goal.
- The parser accepts both `Predicate::InSubquery` and `Expr::InSubquery` forms (parse succeeds for both); rejection of the projection form is plan-time only, after the temporal-literal gate (`check_temporal_literals`) passes.
...
| ~~`E-QUERY-001`~~ | ~~Subquery detected~~ **(SUPERSEDED 2026-07-10, F-CSD-P4-001 adjudication D-1650):** blanket subquery rejection retired — WHERE/HAVING `Predicate::InSubquery` is SUPPORTED (DataFusion three-valued semantics); `Expr::InSubquery` in projection/GROUP BY/ORDER BY position now returns E-QUERY-043 (plan-time structured rejection) rather than parse-time E-QUERY-001. Scalar subquery comparisons (e.g., `WHERE (SELECT count(*) FROM t) > N`) remain unaffected by this ruling and continue to fail at parse or DataFusion planning time. | ~~Error: "Subqueries are not supported. Use pipe mode for multi-stage operations."~~ |
| `E-QUERY-043` | `field IN (SELECT ...)` in SELECT projection, GROUP BY, or ORDER BY position (DataFusion 53.1.0 physical planner cannot execute `InSubquery` in scalar expression positions). WHERE/HAVING `Predicate::InSubquery` (`WHERE field IN (SELECT ...)`) is unaffected — fully supported with standard three-valued semantics. | `"E-QUERY-043: IN subquery in projection position is not supported. Use a WHERE clause subquery instead: \`WHERE field IN (SELECT ...)\`. Alternatively, a JOIN achieves the same result: \`SELECT * FROM t JOIN (SELECT col FROM src) s ON t.field = s.col\`."` |
...
| `E-QUERY-042` | A temporal literal in a structural/positional context where `check_temporal_literals` rejects. **GroupBy and OrderBy arms fire for BOTH `Literal::RawTemporalLiteral` (date-only/offset-less form, e.g., `'2026-06-24'`) AND `Literal::Timestamp` (RFC-3339 fast-path constant, e.g., `'2026-07-01T00:00:00Z'`)** — grouping or ordering by any literal constant is a degenerate no-op regardless of form (DEFECT-EQUERY042-GROUPBY-DEADARM-001). **NonColumnLhsComparison arm fires for `Literal::RawTemporalLiteral` only** (`Literal::Timestamp` in a non-`Field` LHS comparison is already well-typed and passes through `Ok`): (a) GROUP BY position (`GROUP BY '2026-06-24'` or `GROUP BY '2026-07-01T00:00:00Z'` — grouping by any literal constant is a degenerate no-op); (b) ORDER BY position (`ORDER BY '2026-06-24'` or `ORDER BY '2026-07-01T00:00:00Z'` — ordering by any literal constant is a degenerate no-op); (c) non-`Field` LHS comparison, `Literal::RawTemporalLiteral` only (`WHERE lower(hostname) = '2026-06-24'` — LHS type unresolvable at plan time). | GroupBy: `"E-QUERY-042: GROUP BY expects a column reference, not a literal constant. '<first_50_chars>' is a date-shaped literal — grouping by a constant has no effect and is almost certainly a query mistake. Did you mean to reference a column name, or to add a WHERE filter before grouping?"` OrderBy: `"E-QUERY-042: ORDER BY expects a column reference, not a literal constant. '<first_50_chars>' is a date-shaped literal — ordering by a constant has no effect. Did you mean to reference a column name that contains this value?"` NonColumnLhsComparison: `"E-QUERY-042: A date-like literal compared against a computed expression cannot be type-checked at plan time. Compare against a bare datetime column using RFC-3339 (e.g., '2026-07-03T00:00:00Z'), against a string column using a non-date-shaped value, or wrap the expression in an explicit CAST."` |
...
| EC-11-003-008 | `SELECT severity, count(*) FROM events GROUP BY '2026-07-01T00:00:00Z'` (RFC-3339 string literal → `Literal::Timestamp` fast-path in GROUP BY position — DEFECT-EQUERY042-GROUPBY-DEADARM-001) | `Err(E-QUERY-042)` GroupBy — PrismQL parser emits `Literal::Timestamp` for the full RFC-3339 UTC form `'2026-07-01T00:00:00Z'` (fast-path parse, not `RawTemporalLiteral`); `check_temporal_literals` walker detects GROUP BY position → GroupBy arm REJECT. E-QUERY-042 GroupBy fires for ANY literal constant — date-only (`RawTemporalLiteral`) or RFC-3339 (`Literal::Timestamp`) — since grouping by a constant is always a degenerate no-op regardless of literal form. |
| EC-11-003-009 | `SELECT * FROM events ORDER BY '2026-07-01T00:00:00Z'` (RFC-3339 string literal → `Literal::Timestamp` fast-path in ORDER BY position — DEFECT-EQUERY042-GROUPBY-DEADARM-001) | `Err(E-QUERY-042)` OrderBy — PrismQL parser emits `Literal::Timestamp` for the full RFC-3339 UTC form `'2026-07-01T00:00:00Z'` (fast-path parse, not `RawTemporalLiteral`); `check_temporal_literals` walker detects ORDER BY position → OrderBy arm REJECT. E-QUERY-042 OrderBy fires for ANY literal constant — date-only (`RawTemporalLiteral`) or RFC-3339 (`Literal::Timestamp`) — since ordering by a constant is always a degenerate no-op regardless of literal form. |
...
| `SELECT * FROM events WHERE (SELECT count(*) FROM events) > 5` | `Err(E-QUERY-001)` scalar subquery in WHERE comparison rejected (not a `Predicate::InSubquery` — scalar subquery comparisons remain unsupported) | error |
| `SELECT (hostname IN (SELECT hostname FROM crowdstrike_detections)) AS is_known FROM crowdstrike_devices` | `Err(E-QUERY-043)` — `Expr::InSubquery` in SELECT projection position; plan-time gate `check_expr_insubquery_projection` fires after `check_temporal_literals` returns `Ok`; use `WHERE hostname IN (SELECT hostname FROM crowdstrike_detections)` instead | error |
| `SELECT * FROM crowdstrike_devices WHERE hostname IN (SELECT hostname FROM crowdstrike_detections)` | Valid; WHERE-position `Predicate::InSubquery` is SUPPORTED — DataFusion `decorrelate_predicate_subquery` handles natively with standard three-valued semantics | happy-path |
...
| `SELECT severity, count(*) FROM events GROUP BY '2026-07-01T00:00:00Z'` (RFC-3339 `Literal::Timestamp` in GROUP BY — EC-11-003-008, DEFECT-EQUERY042-GROUPBY-DEADARM-001) | `Err(E-QUERY-042)` GroupBy — GroupBy arm fires for `Literal::Timestamp` (RFC-3339 fast-path constant); grouping by any literal constant is a degenerate no-op regardless of form | error |
| `SELECT * FROM events ORDER BY '2026-07-01T00:00:00Z'` (RFC-3339 `Literal::Timestamp` in ORDER BY — EC-11-003-009, DEFECT-EQUERY042-GROUPBY-DEADARM-001) | `Err(E-QUERY-042)` OrderBy — OrderBy arm fires for `Literal::Timestamp` (RFC-3339 fast-path constant); ordering by any literal constant is a degenerate no-op regardless of form | error |
...
|---------|-------|------|--------|--------|
| 1.13 | F-CSD-P4-001-adjudication-2026-07-10 | 2026-07-10 | product-owner | **F-CSD-P4-001 architect adjudication (D-1650, 2026-07-10): update subquery support surface per IN-subquery execution ruling.** (1) **§Invariants:** stale "Subqueries are not supported in v1; nested SELECT in WHERE or FROM returns a parse error with explanation" claim REMOVED; replaced with accurate three-point split: (a) WHERE/HAVING `Predicate::InSubquery` is SUPPORTED (DataFusion three-valued semantics — TRUE/FALSE/NULL for nullable fields); (b) `Expr::InSubquery` in SELECT projection/GROUP BY/ORDER BY is NOT SUPPORTED → E-QUERY-043 plan-time rejection; (c) parser accepts both forms (rejection is plan-time only, after temporal gate). Correctness rationale for rejection cited inline (COUNT-rewrite collapsed NULL semantics for nullable OCSF columns — a correctness defect under the agent-harness SQL semantics goal). (2) **§Error Cases:** stale `E-QUERY-001 \| Subquery detected` row (blanket rejection, retired by PR #220 WHERE IN-subquery support) MARKED SUPERSEDED with forward reference to E-QUERY-043; clarification that scalar subquery comparisons (non-IN forms) remain rejected; `E-QUERY-043` row ADDED with message template and supported alternatives. (3) **§Canonical Test Vectors:** three new vectors added — scalar subquery comparison (still E-QUERY-001), projection-position IN-subquery (E-QUERY-043), and WHERE-position IN-subquery (valid, DataFusion three-valued semantics). **Companion:** error-taxonomy.md v2.38. |
| 1.12 | DEFECT-EQUERY042-GROUPBY-DEADARM-001 | 2026-07-10 | product-owner | **F-EQ42-P1-001 closure POL-25 sweep companion (D-1650): E-QUERY-042 row updated to note dual trigger (RawTemporalLiteral + Literal::Timestamp) for GroupBy/OrderBy arms; EC-11-003-008 and EC-11-003-009 added.** Prior E-QUERY-042 error-case description said "A date-like string literal in a structural/positional context" — the term "date-like string literal" implicitly scoped to `Literal::RawTemporalLiteral` forms only; after DEFECT-EQUERY042-GROUPBY-DEADARM-001 `Literal::Timestamp` (RFC-3339 fast-path constant, e.g., `'2026-07-01T00:00:00Z'`) ALSO triggers GroupBy and OrderBy. NonColumnLhsComparison arm remains `Literal::RawTemporalLiteral`-only — `Literal::Timestamp` passes through `Ok` in that position. **§Error Cases:** E-QUERY-042 row Condition column updated to note dual trigger; GROUP BY and ORDER BY sub-cases extended with RFC-3339 examples; NonColumnLhsComparison sub-case explicitly noted as `RawTemporalLiteral`-only. **§Edge Cases:** EC-11-003-008 ADDED (RFC-3339 `Literal::Timestamp` in GROUP BY → E-QUERY-042 GroupBy); EC-11-003-009 ADDED (RFC-3339 `Literal::Timestamp` in ORDER BY → E-QUERY-042 OrderBy). **§Canonical Test Vectors:** two corresponding vectors added. Frontmatter v1.11→v1.12; modified: 2026-07-10. |
