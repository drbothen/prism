...
| EC-11-003-004 | `SELECT '2026-06-24', * FROM events` (temporal literal in bare SELECT projection — non-comparison position, no column type in scope) | Valid — PrismQL parser emits `Literal::RawTemporalLiteral("2026-06-24")`; `check_temporal_literals` encounters the literal with no surrounding column type context → arm (4) COERCE to `Literal::String("2026-06-24")`. Processed as constant string in projection. No E-QUERY-041 emitted. |
...
| `SELECT '2026-06-24', * FROM events` (temporal literal in bare SELECT projection — non-comparison position) | Valid — `check_temporal_literals` arm (4): non-comparison position → COERCE to `Literal::String("2026-06-24")`; no E-QUERY-041 | edge-case |
