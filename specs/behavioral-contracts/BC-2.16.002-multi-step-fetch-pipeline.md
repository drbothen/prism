...
- **Structured Event Catalog (v1.8)** — `PipelineExecutor` and its private helper functions emit the following 14 structured `tracing` events. Each event sets `event_type` as a structured field. All events are emitted via `tracing` macros; no event includes the token value. New `event_type` sites added to `pipeline.rs` or its helpers MUST be enumerated here as a BC amendment before merge.
