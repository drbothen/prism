...
| `pipeline_max_requests_exceeded` | error | `PipelineExecutor` executor loop | `plugin_id`, `total_requests`, `max: MAX_REQUESTS_PER_PIPELINE` | Cumulative HTTP request count across all pipeline steps reaches `MAX_REQUESTS_PER_PIPELINE` (10,000); pipeline aborts; no further steps attempted |
...
| `plugin_load_failed_compilation` | error | `PluginRuntime::load_all_plugins` | `plugin_path`, `error: E-PLUGIN-008` | WASM Component Model compilation of a `.prx` file failed (corrupt binary, bad magic, component wrapping failed); plugin is skipped; remaining plugins continue loading (n-1 survivor) |
