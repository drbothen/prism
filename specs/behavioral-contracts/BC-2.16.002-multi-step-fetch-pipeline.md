...
- **Auth initial acquisition audit signal (v1.5)** — When `PipelineExecutor::execute` (and its mirror entry point `PipelineExecutor::execute_step`) invokes `AuthProvider::acquire_token` eagerly at pipeline start, the executor emits exactly one of THREE `tracing` events depending on the outcome: (1) **Non-empty token, Ok path** — `tracing::info!` with `event_type = "auth_initial_acquired"` (fields: `sensor_id`, `client_id`); normal production path. (2) **Empty token, Ok path** — `tracing::debug!` with `event_type = "auth_initial_acquired_empty"` (fields: `sensor_id`, `client_id`); provider returned `Ok("")` — typically only `NullAuthProvider` (test-only) or a buggy production provider returning an empty token without signalling an error; the pipeline continues with an empty credential and downstream HTTP requests may subsequently 401. (3) **Err path** — `tracing::error!` with `event_type = "auth_initial_failed"` (fields: `sensor_id`, `client_id`, `error`); token acquisition failed and the pipeline aborts immediately (no fetch steps are attempted). The token value itself is NEVER included in any of the three events. Note: the `auth_refresh_*` event family (refresh-on-401 mid-pipeline path) is governed by the separate postcondition row below and must not be conflated with these initial-acquisition events.
