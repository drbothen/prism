...
- `"cookie:<name>"` — inject `Cookie: <name>={token}`, where `<name>` is the non-empty cookie name substring after `cookie:` (`<name>` must be non-empty and must consist entirely of RFC 6265 `cookie-name` / `token` characters (tchar per RFC 9110 §5.6.2: letters, digits, and ``! # $ % & ' * + - . ^ _ ` | ~``))
...
**Error message (syntactic):** ``"sensor '{sensor_id}' has invalid header_scheme = '{value}'. Valid values: bearer, raw, cookie:<name> (non-empty name required, RFC 6265 token characters only: A-Z a-z 0-9 ! # $ % & ' * + - . ^ _ ` | ~)"``
...
**Entry points and function coverage (normative):** Two distinct functions participate in spec validation, and they cover different rule sets:

- **`validate_sensor_spec()`** — covers Rules 1–7 only. VP-059 is explicitly scoped to `validate_sensor_spec()` and does NOT cover Rule 9 (per S-WAVE-A-ENGINE-001 §Tasks Phase B T-B02 Q2 note: "VP-059 governs `validate_sensor_spec` (Rules 1–7) and is NOT affected by Rule 9 in `parse()`").
- **`SpecLoader::parse()`** — the top-level entry point that calls `validate_sensor_spec()` (Rules 1–7) and then executes Rules 8, 9, and 10 in sequence before returning `Ok(spec)`.

**Security requirement (Rule 9):** security-triage-rule9-cookie-name-charset.md §1.3 identifies exactly one active injection vector: a client holding `sensor_spec.write` calling the `add_sensor_spec` MCP tool with crafted `toml_content`. For Rule 9 to close this vector, `add_sensor_spec` MUST reach `SpecLoader::parse()` — an implementation that calls only `validate_sensor_spec()` in the `add_sensor_spec` handler bypasses Rules 8/9/10 and leaves the injection vector open. BC-2.16.008 states that `add_sensor_spec` uses "the same validation pipeline as startup loading (BC-2.16.009)"; this BC asserts that "the same validation pipeline" means `SpecLoader::parse()` (which includes Rule 9), not `validate_sensor_spec()` alone. The S-WAVE-A-ENGINE-001 implementation MUST verify that the `add_sensor_spec` handler path reaches `SpecLoader::parse()` so Rule 9 applies on the sole exploitable surface.
...
| `E-SPEC-027` | Three templates: (a) `header_scheme` value is syntactically invalid (not `"bearer"`, `"raw"`, or `"cookie:<name>"` with non-empty name consisting entirely of RFC 6265 tchar characters — any non-tchar character including `;`, `=`, SP, CTL, high bytes, and RFC 9110 §5.6.2 delimiters triggers this template; message: ``"sensor '{sensor_id}' has invalid header_scheme = '{value}'. Valid values: bearer, raw, cookie:<name> (non-empty name required, RFC 6265 token characters only: A-Z a-z 0-9 ! # $ % & ' * + - . ^ _ ` | ~)"``); (b) `header_scheme` value is syntactically valid but incoherent with the declared `auth_type` (e.g., `auth_type = "cookie_roundtrip"` with `header_scheme = "bearer"`); (c) `header_scheme` is absent (`None`) AND `auth_type = "cookie_roundtrip"` (absence path B — no sensible cookie-name default; message: `"sensor '{sensor_id}': auth_type = 'cookie_roundtrip' requires an explicit header_scheme = 'cookie:<name>' value; absent header_scheme is not valid for cookie_roundtrip auth (cookie name unknown)"`). Absent `header_scheme` IS an error for `cookie_roundtrip` (template (c)); absent `header_scheme` is NOT an error for all other auth_types (absence path A — silent `"bearer"` default at runtime in `build_request()`) | [PLANNED — Wave-A engine story per ADR-053 D2 / ADR-054 D10] Spec rejected; syntactic error (template a) includes `sensor_id`, `header_scheme` value, and corrective guidance citing the RFC 6265 tchar constraint (SEC-001 / CWE-20 fix per security-triage-rule9-cookie-name-charset.md); coherence error (template b) includes `sensor_id`, `auth_type`, `header_scheme` value, and allowed set for that auth_type; absence error (template c) includes `sensor_id`; ADR-053 v0.30 §D2 |
...
| Field | Value |
|-------|-------|
| Stories | S-1.11, S-1.13, PLUGIN-MIGRATION-001-F, S-SPEC-ENV-VAR-001, S-DEMO-CROWDSTRIKE-MULTIREGION-001, S-SPEC-HTTP-METHOD-VALIDATION-001, S-5.04, S-WAVE-A-ENGINE-001 |
