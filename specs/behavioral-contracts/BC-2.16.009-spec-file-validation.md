...
**(a) `token_exchange` requires `[auth_acquisition]`:** If `auth_type = "token_exchange"` and no `[auth_acquisition]` block is present → `E-SPEC-028`. The token-exchange flow requires endpoint, credential body field, and token response path configuration.

**(b) `auth_plugin` + `[auth_acquisition]` coexist (UNCONDITIONAL):** If both `auth_plugin` and `[auth_acquisition]` are declared in the same sensor spec, regardless of `auth_type` → `E-SPEC-028`. The declarative `DeclarativeHttpAuthProvider` path and the WASM plugin `auth_plugin` path are mutually exclusive; declaring both is a misconfiguration.
...
**(d) `token_exchange` missing required `[auth_acquisition]` fields:** If `auth_type = "token_exchange"` and `[auth_acquisition]` is present, both `token_path` and `ttl_buffer_secs` MUST be present. If either is absent → `E-SPEC-028` citing the missing field name.
...
**(f) `oauth2_client_credentials` missing credential refs:** If `auth_type = "oauth2_client_credentials"` and no `[[credential_refs]]` blocks are declared → `E-SPEC-028`. OAuth2 client credentials require at minimum `client_id` and `client_secret` credential ref declarations.
...
**(h) `token_exchange`-only fields on non-`token_exchange` auth_type:** If `[auth_acquisition]` is present and any of the `token_exchange`-specific fields (`credential_body_field`, `token_response_path`, `expiry_field`, `expiry_mode`) are set, but `auth_type != "token_exchange"` → `E-SPEC-028` citing the mismatched field name.
...
| `E-SPEC-028` | `[auth_acquisition]` block coherence violation — any of 8 sub-conditions: (a) `token_exchange` missing block; (b) `auth_plugin` + block coexist; (c) invalid `expiry_mode`; (d) `token_exchange` missing required fields; (e) `credential_body_field` not in `[[credential_refs]]`; (f) `oauth2_client_credentials` missing credential refs; (g) block on non-declarative auth_type; (h) `token_exchange`-only fields on non-`token_exchange` auth_type | Spec rejected; error message includes `sensor_id`, specific sub-condition identifier, field name(s), and corrective guidance; all sub-condition errors collected in single multi-error pass |
...
| EC-009-035 | `auth_type = "token_exchange"` with `[auth_acquisition]` present but `token_path` absent | Rule 10(d) fires `E-SPEC-028` citing missing `token_path`; spec rejected. |
| EC-009-036 | `auth_plugin = "crowdstrike-oauth2"` AND `[auth_acquisition]` both present | Rule 10(b) fires `E-SPEC-028` (UNCONDITIONAL — any auth_type); spec rejected. |
