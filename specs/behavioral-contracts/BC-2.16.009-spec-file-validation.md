...
| EC-009-036 | `auth_type = "oauth2_client_credentials"`, `auth_plugin = "crowdstrike-oauth2"` present, with or without `[auth_acquisition]` (Definition 1 — ADR-054 v0.35 §D10(b)) | **Branch A — WITH `[auth_acquisition]` (and `token_path` present):** Rule 10(b) fires only — one `E-SPEC-028`; spec rejected. Message: `"sensor '<id>': auth_type = 'oauth2_client_credentials' uses native declarative provider and does not accept auth_plugin..."` **Branch B — WITHOUT `[auth_acquisition]`:** Rule 10(a) AND Rule 10(b) both fire — two `E-SPEC-028` errors collected in the same multi-error pass: (a) fires for missing `[auth_acquisition]` block on declarative auth_type; (b) fires for `auth_plugin` coexisting with declarative auth_type. This is a (a)∩(b) co-fire. If `auth_type` were `custom_via_plugin` instead, (b) would NOT fire (non-declarative type), and in Branch B only (a) would fire for the missing block. |
