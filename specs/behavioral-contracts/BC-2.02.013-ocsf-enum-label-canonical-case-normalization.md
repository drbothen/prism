...
- **In-Scope Field Enumeration (F-HIGH-002 closure).** Normalization coverage is determined by which `{field_name}_id` entries exist in `OcsfEnumMap` (`enum_map.rs`); adding new `_id` entries automatically extends coverage to the corresponding string label field. The demo-critical fields (`severity`, `status`) are the OD-1 guaranteed minimum per D-1398.
  ...
  | String label field | Integer companion field | Canonical OCSF Title-case captions | Caption source | `enum_map.rs` coverage |
  |--------------------|------------------------|-------------------------------------|----------------|-------------------------------|
  ...
  | `status` | `status_id` | Unknown, Success, Failure, Other | OCSF v1.7.0 `schema.json` `dictionary_attributes.status_id.enum` {0→Unknown, 1→Success, 2→Failure, 99→Other} | **Present — 10 entries (generic 0/1/2/99 + finding-class synthetic keys 1001-1006)** |
  | `activity_name` | `activity_id` | Unknown, Create, Read, Update, Delete, Other | `enum_map.rs` `activity_id` entries {1→Create, 2→Read, 3→Update, 4→Delete}; OCSF v1.7.0 base adds {0→Unknown, 99→Other} | **Present — 6 entries (0,1,2,3,4,99)** — **OCSF sibling exception:** `activity_id.sibling = "activity_name"` (OCSF v1.7.0 schema.json, `suppress_checks: ["sibling_convention"]`); the string label field is `activity_name`, NOT `activity`. The general `{F}_id → {F}` sibling naming rule does NOT hold for activity; the implementer MUST map `activity_id` caption lookups to the `"activity_name"` field key. |
  | `disposition` | `disposition_id` | Unknown, Allowed, Blocked, Quarantined, Isolated, Deleted, Dropped, Custom Action, Approved, Restored, Exonerated, Corrected, Partially Corrected, Uncorrected, Delayed, Detected, No Action, Logged, Tagged, Alert, Count, Reset, Captcha, Challenge, Access Revoked, Rejected, Unauthorized, Error, Other | OCSF v1.7.0 `schema.json` `dictionary_attributes.disposition_id.enum` (29 values: integers 0–27, 99) | **Present — 29 entries (0-27, 99)** |
...
- Values NOT found in the case-insensitive caption set for the field (e.g., vendor-specific extension values, wholly vendor-proprietary strings) are **left as-received** in the `DynamicMessage` (RG-021, pass-through). A `tracing::warn!` is emitted with the following field schema (registered in BC-2.16.002 §Postconditions Canonical Structured Event Catalog as row 91):
  ...
  - `sensor_type: %display` — the sensor identifier string (e.g., `"armis"`, `"crowdstrike"`), **capped at 50 codepoints** before emission (SEC-002 defense; symmetric with `value` field)
...
- **Null and empty-string passthrough (silent, no warn):** Values that are `null` or empty string `""` bypass normalization entirely and pass through unchanged; NO `ocsf.enum_label_unrecognized` warn is emitted. An empty string represents an unset or missing sensor field, not a vendor enum value — emitting an unrecognized-value warning for it would produce misleading audit noise. Contrast: non-empty unrecognized values (e.g., `"UNHANDLED"`, `"Unresolved"`) are left as-received AND trigger the warn. `null` cannot be a recognized caption and requires no lookup; it passes through on both PRIMARY (null cell in Arrow `StringArray`) and SECONDARY (`DynamicMessage` field unset) paths. See EC-02-025 (null) and EC-02-028 (empty string) for concrete examples.
...
| Record where field value is empty string `""` (e.g., `severity=''`) | `""` passes through unchanged; no normalization attempted; no `ocsf.enum_label_unrecognized` warn emitted | empty-string passthrough |
...
| Architecture Module | **PRIMARY:** SS-22 (Process Lifecycle) — `crates/prism-bin/src/spec_driven_adapter.rs` `build_column_array` (adapter materialization boundary; production spec-driven query path); **SECONDARY:** SS-02 (OCSF Normalization) — `prism-ocsf` normalization pipeline, `enum_map.rs` (OcsfEnumMap canonical casing authority; DynamicMessage/protobuf path) |
| Priority | P1 |
...
| 1.5 | S-PRISMQL-CASE-INSENSITIVE-001-adversary-pass-14-obs002-dual-subsystem | 2026-07-07 | product-owner | **LOCAL adversary pass-14 OBS-002 closure: §Traceability Architecture Module row extended to dual-subsystem ownership.** PRIMARY insertion point `build_column_array` in `crates/prism-bin/src/spec_driven_adapter.rs` is owned by SS-22 (Process Lifecycle); SECONDARY site `prism-ocsf` normalization pipeline + `OcsfEnumMap` canonical casing authority is owned by SS-02 (OCSF Normalization). Prior v1.4 row cited only SS-02 — omitting the PRIMARY production insertion point subsystem ownership was OBS-002. |
