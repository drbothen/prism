...
| EC-11-021-015 | `SELECT '2026-06-24' FROM t` (date-like literal in non-comparison projection position, no column type context — OBS-2 human-ratified behavior change, ADR-052 §D4 v1.8) | COERCE: `check_temporal_literals` rewrites `RawTemporalLiteral("2026-06-24")` in-place to `Literal::String("2026-06-24")` — SUCCESS, no error emitted. Standard SQL `SELECT '2026-06-24'` returns the string constant `2026-06-24`. The same arm applies to date-like literals in GROUP BY and ORDER BY positions with no column type context. |
...
| ADR traces | ADR-044 v1.0, ADR-033 (push-down benefits automatically), ADR-052 §D4 v1.4 (§D3 `arrow_cast` explicit emission — `TIMESTAMP '...'` produces `Nanosecond/None` in DF 53.1.0; §D7 ADR-044 §D4 supersession; §D4 v1.4 E-QUERY-041 detection: lenient-parse-then-AST-walk with 7-form `is_date_like` acceptance set — date-only, T-sep full/fractional/no-seconds, space-sep full/fractional/no-seconds; over-match forms unpadded/big/signed years ACCEPTED BENIGN; plan-time walker `check_temporal_literals` performs four-way dispatch: Datetime col → E-QUERY-041; String/Utf8 col → COERCE to `Literal::String(s)` (SUCCESS, no error); Integer/Float/Bool col comparison → E-QUERY-002 (QueryTypeMismatch); non-comparison position (projection/GROUP BY/ORDER BY, no column type) → COERCE to `Literal::String(s)` (SUCCESS, OBS-2 human-ratified, ADR-052 §D4 v1.8)) |
