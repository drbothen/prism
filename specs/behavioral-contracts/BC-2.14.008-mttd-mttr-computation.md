...
| VP ID | Description |
|-------|-------------|
| VP-054 | TTR uses first resolution timestamp across reopen cycles — for any `CaseRecord` with N reopen cycles (N >= 1), `compute_ttr(case)` equals `case.resolved_at - case.created_at` using the FIRST `resolved_at` timestamp; `compute_mttd_avg([])`, `compute_mtti_avg([])`, `compute_mttr_avg([])` all return `None` (not zero) when given empty input; all metrics are non-negative (floored at Duration::ZERO). Method: Proptest. Priority: P1. |
