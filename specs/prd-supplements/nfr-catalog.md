...
|-----------|-------|
| Category | Performance |
| Requirement | Prism's resident memory usage should remain under 256MB during normal operation (single analyst session, up to 50 configured clients). Sensor API responses are processed page-by-page, not buffered entirely in memory. |
| Memory Budget Breakdown | Base process + runtime: ~30MB. Cache (50 clients x 4 sensors x 50 entries x ~10KB): ~100MB. Confirmation tokens + cursors: ~5MB. Query materialization: ~50MB per query (10K records in Arrow columnar format with two-tier storage). Concurrent queries each get their own 10K record budget but share the 256MB process limit. Under typical single-analyst usage, at most 1-2 queries are materialized concurrently (~100MB for materialization). Headroom: ~21MB for transient allocations, HTTP buffers, and OCSF normalization intermediates. |
| Concurrent Query Note | The 10K record limit (DI-019) is per-query. Concurrent queries compete for the shared 256MB process memory. If concurrent materialization would exceed the memory budget, the later query receives `E-QUERY-005` (retryable: true) with a suggestion to wait for the active query to complete or narrow the query scope. |
| Traces to | R-010 |
