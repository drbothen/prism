...
| E-QUERY-013 | broken | validation | "E-QUERY-013: page_size must be greater than 0" | No | Pagination request specified page_size=0; reject as malformed input (BC-2.07.002 §CursorPageSizeInvalid). |
...
| 1.14 | S-3.05-fix-pass-16-sub-burst | 2026-05-07 | implementer | Added E-QUERY-012 (CursorExpired — pagination cursor TTL >60s elapsed; re-execute query; BC-2.07.002 §Cursor TTL Expiry; distinct from E-QUERY-004 query timeout). Added E-QUERY-013 (CursorPageSizeInvalid — page_size=0 rejected as malformed input; BC-2.07.002 §CursorPageSizeInvalid). Added E-QUERY-014 (CursorTokenUnknown — UUID not in registry: garbage UUID, already-released after exhaustion, or cross-instance; distinct from E-QUERY-012 which is for tokens that DID exist). Codes correspond to PrismError variants renumbered by S-3.05 fix-pass-16 (commit d36ecf22) from incorrect 006/007/009 → spec-correct 012/013/014 (F-PASS9-CRIT-001/002/003). D-272. |
