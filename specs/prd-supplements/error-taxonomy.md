...
| E-PLUGIN-022 | broken | runtime | "plugin '{plugin_id}' acquire-token dispatch completed but no token was cached in KV store (guest AuthError::ResponseParse or missing kv_set call): {message}" | No | `PluginError::AuthTokenNotCached { plugin_id, message }` — the WASM Component Model binary compiled and was instantiated successfully; the guest's `acquire-token` function was dispatched without trap or timeout; but after dispatch `kv_store.get(plugin_id, "token")` returned `None`. This is a **runtime-behavioral failure** — the guest either returned `AuthError::ResponseParse` (invalid JSON body [EC-002], missing `access_token` field [EC-003]), called `kv_set` with the wrong key, or returned without calling `kv_set` at all. DISTINCT from E-PLUGIN-008 (`CompilationFailed` — WASM binary compilation failure): operators searching for token-parse failures should grep for E-PLUGIN-022, NOT E-PLUGIN-008. The structured tracing event `plugin.auth_token_parse_error` (BC-2.16.002 row 37) is emitted by `emit_acquire_token_parse_error_and_fail` BEFORE this error is returned. Construction site: `crates/prism-spec-engine/src/plugin/mod.rs` `emit_acquire_token_parse_error_and_fail`. Recurrence: one per failed token acquisition where `func.call` succeeds but KV has no "token" key. Operator-investigable: check OAuth2 token endpoint response format; may indicate misconfigured `token_endpoint`, proxy/WAF response rewrite, or temporarily degraded upstream service. F-LP8-MED-002 closure (PLUGIN-MIGRATION-001-E pass-8 FB-IMPL-7 2026-05-23). | BC-2.16.002 row 37; PLUGIN-MIGRATION-001-E EC-002, EC-003; `PluginError::AuthTokenNotCached` |
