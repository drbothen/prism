...
|---------|-------|------|--------|--------|
| 2.14 | DEFECT-MCP-ROWSHAPE-NULLS-001 pass-17 (F-MCPRS-PRL17-HIGH-002) | 2026-07-14 | product-owner | **F-MCPRS-PRL17-HIGH-002 closure — EC-11-081 Float64 non-finite serialization acknowledged in §1.9 rows description.** The §1.3 query output schema `rows.items.description` at the EC-11-079 citation site lacked any mention of EC-11-081: callers reading interface-definitions alone could not know that Float64 NaN/±Infinity values serialize as JSON null (key present, indistinguishable from Arrow null at the wire boundary). Added companion sentence: "Non-finite Float64 values (NaN, ±Infinity) serialize as JSON null per EC-11-081 (BC-2.11.001 v1.22) — key present, indistinguishable from Arrow null at the wire boundary; callers MUST NOT rely on distinguishing NaN from missing data." TD-VSDD-060 sweep: only one value-serialization description site found in the file (line ~488, rows description); no other sites enumerate serialization semantics for Float64 or other types. Combined citation block updated to include F-MCPRS-PRL17-HIGH-002 2026-07-14. |
...
| 2.9 | DEFECT-MCP-ROWSHAPE-NULLS-001-events-to-rows | 2026-07-13 | product-owner | **POL-23 sibling sweep — `query` tool response schema key renamed `events` → `rows`.** §1.3 query tool JSON output schema: `"events"` array key updated to `"rows"` per human adjudication 2026-07-13 (DEFECT-MCP-ROWSHAPE-NULLS-001 F-MCPNULL-P1-MED-001). Key name shipped in S-5.01-FOLLOWUP-MCP-BOOT (PR #163); spec sibling brought into alignment with BC-2.11.001 v1.17 and shipped behavior. Description updated: "OCSF-normalized result records" (not "event records") to match the tabular-projection rationale. NOTE: v2.8 changelog row was absent at v2.9 authoring time (pre-existing POL-32 gap); backfilled from git history (commit b39e21e9, D-1203) at v2.11 per F-MCPNULL-P15-OBS-002. |
| 2.8 | D-1203 | 2026-06-16 | product-owner | **D-1203 (commit b39e21e9) — `available_tables` field added to E-QUERY-037 error payload schema.** `available_tables`: org-scoped list of registered table names returned in the E-QUERY-037 error context to help agents reformulate queries. In multi-tenant overlay deployments (ADR-029 per-org overlay map), list is filtered to the requesting org's registered tables only — other orgs' tables are NEVER enumerated (ADR-039 v1.1 SEC-003/CWE-200 invariant). Single-tenant deployments (or `clients` null/omitted) list all globally registered tables. Empty array when no sensors configured. Changelog row was not authored at original commit time (pre-existing POL-32 gap); backfilled from git history at v2.11 per F-MCPNULL-P15-OBS-002. |
