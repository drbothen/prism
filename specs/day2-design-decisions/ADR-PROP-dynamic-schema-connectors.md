...
| **Drift detection + re-pin workflow is Prism-built** | DataFusion provides runtime `register_table`/`SchemaRef`. The detection, classification, degraded-state, and re-pin lifecycle have no out-of-box DataFusion support. This is the C4 analog of C3's "the cost-based join-guard has no DataFusion hook" (D-C3-1: the join guard is cost-based degrade, not hard-reject; similarly, the C4 drift guard is Prism-built, not a DataFusion config knob). |
