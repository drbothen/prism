...
| **OQ-C7-7** | **Central ML score caching under Option 3 (ADS conformance 2026-06-27; P-ADS-04; ripple-audit P3 correction):** With Option 3 (Tenant-Keyed-Central-Persistence) locked globally, derived ML scores (ANOMALY_SCORE results, behavioral trend aggregates, peer-outlier scores) WILL be cached at Central under the tenant CMEK (PAT-ADS-02 Tenant-Keyed-Central-Cache). These are DERIVED results (computed at satellite/edge per D-C7-1), NOT raw model state or raw ModelState CF data (which remain at the edge per P-ADS-03). **Critical distinction:** the Phase 2 `ModelState` CF (D-C7-1 edge-ML) lives at satellite/edge and is NOT replicated to Central. Only the SCORED OUTPUT that transits to Central as a query result is eligible for Central CMEK caching. Story-writers implementing C7 at morph must NOT conflate the edge-local ModelState CF (where the model lives) with the Central-cached anomaly-score results (what Option 3 caches). Morph-time ADR authorship must specify: the Central cache schema for ML-derived scores (retention policy, per-tenant DEK, staleness TTL), and the boundary between edge model state and Central result cache. | Open — morph-time design decision | Before E-ML-ONLINE-001 or E-ML-PRIMITIVES-001 Central result caching stories |
