...
|---------|----------|-----------------------|------------|-----|-------------------|---------------------------|---------------------------|-----------|--------|
| **GRAMMAR-001** Mode disjunction: SQL mode has no `\|`; pipe mode has no SQL clauses | BLOCKER | Two entirely separate Chumsky grammars chosen by first-token heuristic; no diagnostic at mode boundary | grammar/code + error-messages | ADR-043 (SQL→Pipe composition) + ADR-046 (mode-bridge error) | prism-query | BC for `Ast::SqlPipe` variant + E-QUERY-001 `normalized_pql` in ParseErrorDetails | `SELECT … FROM t \| limit 5` parses as `SqlPipe`; or when it fails, the error says "use `FROM t \| limit 5` instead" with `normalized_pql` rewrite | architect (done: ADR-043, ADR-046) → product-owner (BC) → implementer | [NEW-STORY] |
...
| **MAJOR-001** `list_capabilities` returns `client_registered: False` for all orgs | MAJOR | `client_exists()` in `feature_flag.rs` line 263 checks `self.client_capabilities` map; this map is populated from `prism.toml` `[clients.*.capabilities]` write-capability config entries, NOT from the `OrgRegistry` populated by spec overlays (ADR-029). Demo provisioning uses spec overlays, not `prism.toml` client entries — so `client_capabilities` is always empty | code | n/a | prism-security, prism-mcp | No new BC — existing BC for `list_capabilities` must return correct data; implementer chooses fix path (a) `demo-setup.sh` adds `[clients.org-*]` entries to `prism.toml`, OR (b) `list_capabilities` consults `OrgRegistry` for basic org existence | `list_capabilities(org-c)` returns `client_registered: True` for all demo-provisioned orgs | implementer (fix path choice; both are valid; path a is faster for demo, path b is architecturally correct per ADR-006) | [NEW-STORY] |
...
| **GRAMMAR-016** `normalized_pql` field documented but absent in practice | MINOR | `ParseErrorDetails` does not have a `normalized_pql` field; ADR-041 `OPD-1` planned it but it was not implemented | code | ADR-046 D3 | prism-query | `ParseErrorDetails` carries `normalized_pql: Option<String>`; mode-bridge errors populate it with a best-effort rewrite | product-owner (error-taxonomy E-QUERY-001 update) → implementer | [NEW-STORY] (part of three-mode correctness story) |
...
| GRAMMAR-005 / GRAMMAR-015 / GRAMMAR-016 | Parse-time enrich error improvements + `normalized_pql` implementation | E-QUERY-001 `ParseErrorDetails` struct change; error-taxonomy row update |
...
|-----|-------|--------|---------------|
| ADR-043 | True SQL→Pipe Composition | PROPOSED v1.0 | New `Ast::SqlPipe(SqlPipeQuery)` variant; `QueryMode` tristate enum; SQL head then pipe stages execution; LIMIT interaction semantics |
...
| ADR-045 | Auto-Generated PrismQL Reference | PROPOSED v1.0 | Replace static `pql_reference.md` with `build_reference_content(infusion_registry: Option<&InfusionRegistry>)`; CI parse-round-trip gate; amends ADR-041 §L3 |
| ADR-046 | Three-Mode Correctness | PROPOSED v1.0 | Mode-bridge E-QUERY-001 heuristic on `\|` in SQL mode; `ParseErrorDetails.normalized_pql: Option<String>`; Filter mode execution test mandate; BLOCKER-003 investigation protocol; amends ADR-041 §L4 |
...
| Gate | Decision | Default if not overridden |
|------|----------|--------------------------|
| **HRG-1 (ADR-043)** | LIMIT semantics: when `SELECT … LIMIT 5 FROM t \| limit 3` — which limit wins? Architect recommendation: SQL head LIMIT sets intermediate fanout cap; pipe `\| limit` sets final result cap; if both present, pipe `\| limit` wins as the terminal stage | ADR-043 default: pipe `\| limit` wins |
| **HRG-2 (ADR-043)** | Confirm Option 2 (full SQL→Pipe composition) over Option 3 (mode-bridge error only, no grammar extension) | Human already confirmed at session start; recorded here for traceability |
| **HRG-3 (ADR-045)** | Reference generation strategy: static Rust string constants for grammar sections vs build-time codegen from `ast.rs`. Architect recommendation: static constants maintained alongside the grammar (simpler, no proc-macro), with CI gate enforcing parity | ADR-045 default: static constants + CI gate |
| **HRG-4 (MAJOR-001)** | Fix path: Path A (demo-setup.sh adds prism.toml entries, fast) vs Path B (`list_capabilities` consults OrgRegistry, architecturally correct). Story-writer must know which path is in scope | No default — human decision required before story authoring |
