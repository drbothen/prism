...
| `prism://config/clients/{client_id}/sensors` | application/json | Resource template. Sensor inventory for a specific client: sensor ID, API host (redacted), enabled status, configured data sources. OrgSlug-validated (formerly TenantId, renamed per ADR-006). | On reload |
