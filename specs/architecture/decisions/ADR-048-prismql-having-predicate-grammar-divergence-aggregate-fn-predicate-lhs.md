...
ACCEPTED v1.14 (2026-07-14). F-PQLFN-P34-MED-001 (DEFECT-PQL-FNCALL-LHS-001 pass 34). §D.7.2 and related §D.3 body prose sweep: "all six" → "all seven" gated-position references. Three live-prose sites corrected: (1) §D.3 self-contradiction sentence "enumerates six predicate positions" → "enumerates seven predicate positions" — §D.7.1 now enumerates seven positions (Position 7 INSERT source_select WHERE added in v1.13); (2) §D.7.2 "All six predicate positions receive an identical, helpful message" → "All seven predicate positions"; (3) §D.7.2 "Two canonical forms apply to all six predicate positions" → "all seven predicate positions". Exempt sites: §Changelog rows for v1.6/v1.8/v1.13 quoting "six" are historical records (accurate at those versions); "sixth gated position" (line 382 §D.6) remains correct (Position 6 is still the sixth); "non-six-name aggregates" / "six named functions" (§D.7.3, §D.2, Related Architecture Nodes) refer to the six named aggregate functions in build_agg_call_parser — UNCHANGED. "Six callers" of build_predicate_parser is UNCHANGED per v1.13 OD-7 note. Version bump 1.13 → 1.14; no locked-decision changes; no new ODs.
...
at parse time) but is self-contradicted by §D.7.1, which enumerates seven predicate
positions — including SQL WHERE, SQL DML WHERE, and INSERT source_select WHERE — that now feed FuncCall names to the
...
|---|---|---|---|
| Pipe `| where` | `collect_unknown_scalar_offsets_from_predicate` on `PipeStage::Where` | YES | YES |
| Filter root | `collect_unknown_scalar_offsets_from_predicate` on `Ast::Filter` | YES | YES |
| SqlPipe `\| where` | `collect_unknown_scalar_offsets_from_predicate` on `Ast::SqlPipe` pipe stages | YES | YES |
| SQL WHERE | `collect_unknown_scalar_offsets_from_predicate` on `sq.where_` in `Ast::Sql` arm | NO (was in sql_unknown_names; DFBIAFN filter bypassed gate) | YES (v1.2) |
| SqlPipe-head WHERE | `collect_unknown_scalar_offsets_from_predicate` on `spq.head.where_` in `Ast::SqlPipe` arm | NO (was in sql_unknown_names via collect_unknown_scalars_from_sql_query) | YES (v1.2) |
| SQL DML WHERE | `collect_unknown_scalar_offsets_from_predicate` on `dml.filter` in `Ast::Sql(SqlStatement::Dml(dml))` arm | NO (fell to `_ => {}`; pre-branch: parser E-QUERY-001) | YES (v1.6, OD-6) |
| INSERT source_select WHERE | `collect_unknown_scalar_offsets_from_predicate` on `dml.source_select.where_` in `Ast::Sql(SqlStatement::Dml(dml))` arm | NO (INSERT has no filter; source_select not walked; silent Ok(vec[])) | YES (v1.13, OD-7) |

**Two-variant walker relationship (v1.15 — F-PQLFN-P36-LOW-003):** The predicate walker exists in two variants:

- **`collect_unknown_scalar_offsets_from_predicate`** (runtime gate walker for all seven positions above): accumulates `(name, span.start)` pairs into `predicate_fncall_names: Vec<(String, usize)>` so E-QUERY-001 reports a truthful source offset pointing to the aggregate fn name in the original query string (F-PQLFN-P21-OBS-003). Its Expr-level companion is `collect_unknown_scalar_offsets_from_expr`.

- **`collect_unknown_scalar_from_predicate`** (non-offset variant, retained in production): accumulates bare `String` names into `out: &mut Vec<String>`. Used by `collect_unknown_scalars_from_sql_query` (via its Expr-level companion `collect_unknown_scalar_from_expr`) to walk the six SQL scalar positions (SELECT, WHERE, JOIN ON, GROUP BY, ORDER BY, HAVING) into `sql_unknown_names: Vec<String>` for the E-QUERY-039 gate. Also exercised in unit tests that build programmatic ASTs (no Chumsky `span` populated → `span.start == 0`; offset-carrying variant still works but tests use the non-offset variant for simplicity). NOT the runtime walker for the seven `predicate_fncall_names` positions in this table.
...
- The plan-time gate in `check_enrich_udf_availability` catches the name and emits
  ...
  filters, ADR-048 D.3)"
- All seven predicate positions receive an identical, helpful message — single message
...
prefix in `detail`. Two canonical forms apply to all seven predicate positions:
...
**Historical note (v1.15 — F-PQLFN-P36-LOW-003):** The v1.6 original specified `collect_unknown_scalar_from_predicate` (non-offset variant, `out: &mut Vec<String>`). At v1.6, `predicate_fncall_names` was `Vec<String>`. In the same fix-burst as F-PQLFN-P21-OBS-003, `predicate_fncall_names` was promoted to `Vec<(String, usize)>` for offset truthfulness, and the gate walker was updated to `collect_unknown_scalar_offsets_from_predicate` across all seven positions. The shipped implementation uses the offset variant throughout (confirmed in engine.rs `Ast::Sql(SqlStatement::Dml(dml))` arm). The §D.7.6 block was authored after this change and correctly shows the offset variant.
