...
For the SEC-001 threat examples driving the tchar constraint (cookie-name header injection via semicolons/spaces, CTL characters), see BC-2.16.009 EC-009-043 and EC-009-046, which are the canonical edge-case carriers for this rejection surface.
...
Registered in `error-taxonomy.md` with all three templates as part of the standalone Wave-A engine story (same story as `SensorSpec::header_scheme` addition); registration completed at error-taxonomy.md §Changelog v2.57 / v2.66 / v2.67.
...
| `error-taxonomy.md` | Register E-SPEC-027 with three message templates (a)/(b)/(c): (a) unknown/malformed `header_scheme` value; (b) well-formed value incoherent with `auth_type` (generalized form — `sensor '{sensor_id}': auth_type = '{auth_type}' does not permit header_scheme = '{value}'; allowed for this auth_type: {allowed_set}`); (c) absent `header_scheme` with `auth_type = "cookie_roundtrip"` — `sensor '{sensor_id}': auth_type = 'cookie_roundtrip' requires an explicit header_scheme = 'cookie:<name>' value; absent header_scheme is not valid for cookie_roundtrip auth (cookie name unknown)` (F-WASE-P48-MED-003). Registration completed at error-taxonomy.md §Changelog v2.57 / v2.66 / v2.67. **[COMPLETED — error-taxonomy.md §Changelog v2.67]** | D2 |
