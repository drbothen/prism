...
ACCEPTED v1.15 (2026-07-13). F-PQLFN-P9-OBS-002 changelog ordering fix (DEFECT-PQL-FNCALL-LHS-001 pass 9). The "1.1 (ratified)" row was misplaced between v1.4 and v1.3 in an otherwise-descending changelog table — neither ascending nor descending, and it duplicated the "1.1" version key. Fix: renamed to "1.1-r" (eliminates duplicate version key ambiguity with the v1.1 authored row) and relocated to the chronologically correct descending position between v1.2 and v1.1. Rationale: the ratification event (2026-07-03, D-1520) occurred after v1.1 authoring on the same day and before v1.2/v1.3 on 2026-07-04; in descending reading order it therefore sits immediately below the v1.2 row. No content changes to any row.

ACCEPTED v1.13 (2026-07-13). Human ratification of core decisions recorded 2026-07-03
...
**v1.14 (DEFECT-PQL-FNCALL-LHS-001 adversary pass-8 DML WHERE caller enumeration, 2026-07-13):** F-PQLFN-P8-HIGH-002 finding closure. ADR-048 v1.6 (OD-6) added SQL DML WHERE as the sixth gated predicate position in `check_enrich_udf_availability`; `build_delete_parser` and `build_update_parser` both bind `build_predicate_parser`, so `fn_call_comparison` is grammar-reachable in six (not five) `build_predicate_parser` call sites post-fix. The E-QUERY-042 `NonColumnLhsComparison` arm is also reachable from DML WHERE — `check_temporal_literals` walks `dml.filter` via the `Ast::Sql(SqlStatement::Dml(dml))` arm in `materialization.rs`. Four §D4 body sites updated: (1) dispatch table NonColumnLhsComparison row — five → six caller count + DML WHERE added to enumeration; (2) "Pipe `| where` grammar reachability" subsection — five callers/positions → six throughout, DML WHERE added; (3) E-QUERY-039 gate companion — "all five call-site positions" → "all six"; (4) blast-radius row 14 — five → six caller/position counts + DML WHERE added. Historical v1.13 status/changelog entries unchanged (five was accurate at pass-1; OD-6 landed at pass-7).

**v1.13 (DEFECT-PQL-FNCALL-LHS-001 adversary pass-1 accuracy corrections, 2026-07-13):** Two corrections to the §D4 "Pipe `| where` grammar reachability" block added in v1.12 (F-PQLFN-P1-LOW-003 + F-PQLFN-P1-MED-001 architect half). **(LOW-003 accuracy fix)** v1.12 stated "pipe predicates could not contain a non-`Field` LHS before the grammar extension, so only SQL-mode queries could reach it" — both halves of that claim are inaccurate. `build_predicate_parser` is shared by five callers (filter mode, pipe `| where`, SQL WHERE, SQL HAVING base fallthrough, SqlPipe pipe stages); none could produce a function-call LHS before the fix because `fn_call_comparison` was absent from `build_predicate_parser`. SQL WHERE routes through `build_predicate_parser` and was equally unable to reach the NonColumnLhsComparison arm before the fix. The SOLE grammar-reachable pre-fix path to this arm was SQL HAVING via `agg_call_parser` (ADR-048), which produces `Predicate::Compare { lhs: Expr::FuncCall(FuncCall::Aggregate) }` — a function-call LHS that bypasses `build_predicate_parser` entirely. **(MED-001 scope-widening)** Post-fix, `fn_call_comparison` is grammar-reachable in ALL five `build_predicate_parser` call sites (filter mode, pipe `| where`, SQL WHERE, SQL HAVING base, SqlPipe pipe stages); `check_temporal_literals` arm (4) fires uniformly across all five positions for date-like RHS. E-QUERY-039 gate-extension companion noted: `build_predicate_parser` column-existence gate (E-QUERY-039) is extended to cover `fn_call_comparison` args across all five positions; tracked in BC-2.11.019 amendment (F-PQLFN-P1-MED-004, product-owner). Dispatch table NonColumnLhsComparison row and blast-radius row 14 updated for full five-caller scope and accurate pre-fix description.
...
| Comparison where LHS is a function/compound expression (non-`Field`), date-like RHS | LHS type unresolvable at plan time | E-QUERY-042 `TemporalLiteralInvalidPosition` (NonColumnLhsComparison) — REJECT with analyst-facing INVALID_PARAMS; silent coercion would reintroduce RISK-1 for datetime-valued LHS expressions (v1.10 NEW). Grammar-reachable in all six `build_predicate_parser` call sites (filter mode, pipe `| where`, SQL WHERE, SQL HAVING base, SqlPipe pipe stages, SQL DML WHERE via `build_delete_parser`/`build_update_parser`) post-fix via `fn_call_comparison` production (`FuncCall::Scalar` only), DEFECT-PQL-FNCALL-LHS-001 (2026-07-13 [H5c]); pre-fix, ONLY SQL HAVING via `agg_call_parser` (ADR-048) was grammar-reachable (v1.12 claim "only SQL-mode queries could reach it" corrected v1.13). Reachable from DML WHERE: `DELETE FROM t WHERE lower(col) = '2026-06-24'` hits this arm — `check_temporal_literals` walks `dml.filter` via the `Ast::Sql(SqlStatement::Dml(dml))` arm in `materialization.rs` (v1.14). Non-date-like RHS (`Literal::String`) → valid, passes to DataFusion without interception; fn-call args participate in E-QUERY-038 column walk. |
...
**Pipe `| where` grammar reachability (DEFECT-PQL-FNCALL-LHS-001, 2026-07-13 [H5c]; corrected v1.13):**

As of DEFECT-PQL-FNCALL-LHS-001, the `fn_call_comparison` production (`FuncCall::Scalar`
only) is added to `build_predicate_parser`. Because `build_predicate_parser` is shared by
six callers — filter mode (`| where`), pipe `| where`, SQL WHERE, SQL HAVING base
fallthrough, SqlPipe pipe stages, and SQL DML WHERE via `build_delete_parser`/`build_update_parser`
(ADR-048 v1.6 OD-6) — `fn_call_comparison` is now grammar-reachable in
ALL six positions post-fix.

Previously this arm was defense-in-depth-only for the following reason: `build_predicate_parser`
lacked the `fn_call_comparison` production, so none of the six positions that route through
it could produce a function-call LHS comparison. The SOLE grammar-reachable pre-fix path to
the NonColumnLhsComparison arm was SQL HAVING, which routes through `agg_call_parser`
(ADR-048) rather than `build_predicate_parser` — `agg_call_parser` produces
`Predicate::Compare { lhs: Expr::FuncCall(FuncCall::Aggregate) }`, a function-call LHS that
bypasses `build_predicate_parser` entirely. SQL WHERE, which also routes through
`build_predicate_parser`, was equally unable to reach this arm before the fix. The v1.12
claim "only SQL-mode queries could reach it" was inaccurate on both counts: SQL WHERE could
not reach the arm before the fix, and SQL HAVING via `agg_call_parser` was the only
grammar-reachable pre-fix path.

Post-fix, `check_temporal_literals` arm (4) fires uniformly across all six
`build_predicate_parser` call sites (plus the unchanged HAVING path via `agg_call_parser`)
when a date-like RHS (`Literal::RawTemporalLiteral`) is encountered against a function-call
LHS. The arm behavior is unchanged: date-like RHS → E-QUERY-042
...
**E-QUERY-039 gate companion (BC-2.11.019 amendment, F-PQLFN-P1-MED-004):** The
column-existence gate in `build_predicate_parser` (E-QUERY-039) is extended by the same
DEFECT-PQL-FNCALL-LHS-001 fix to cover function-call LHS arguments across all six
call-site positions. This gate extension is tracked in BC-2.11.019 (amended by the
product-owner as F-PQLFN-P1-MED-004). The ADR's NonColumnLhsComparison arm (arm 4,
`check_temporal_literals`) fires for date-like RHS independently of whether the function-call
LHS arguments pass or fail the E-QUERY-039 gate; the two gates are independent error
surfaces.
...
| 14 | `crates/prism-query/src/sql_parser.rs` (+ `filter_parser.rs` if separate) | [CHANGE] | Modify timestamp literal production: lenient `is_date_like` fallback → `RawTemporalLiteral` instead of parse error. `filter_parser.rs::build_predicate_parser` adds the `fn_call_comparison` production (`FuncCall::Scalar` only) as of DEFECT-PQL-FNCALL-LHS-001 (2026-07-13 [H5c]). Because `build_predicate_parser` is shared by six callers (filter mode, pipe `| where`, SQL WHERE, SQL HAVING base fallthrough, SqlPipe pipe stages, SQL DML WHERE via `build_delete_parser`/`build_update_parser` — ADR-048 v1.6 OD-6), `fn_call_comparison` is grammar-reachable in ALL six positions post-fix, making `check_temporal_literals` arm (4) fire uniformly across all six. Pre-fix, the NonColumnLhsComparison arm was grammar-reachable ONLY via SQL HAVING through `agg_call_parser` (ADR-048); all `build_predicate_parser` positions were defense-in-depth-only (v1.13 accuracy correction — v1.12 incorrectly stated "only SQL-mode queries could reach it"). |
...
| 1.1-r | 2026-07-03 | state-manager | Human ratification recorded 2026-07-03 (D-1520). Status: PROPOSED → ACCEPTED. No decision content changes; v1.1 content ratified as authored. (Renamed from "1.1 (ratified)" and relocated to correct descending position by v1.15 changelog fix — F-PQLFN-P9-OBS-002.) |
