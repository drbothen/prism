...
| Comparison where LHS is a function/compound expression (non-`Field`), date-like RHS | LHS type unresolvable at plan time | E-QUERY-042 `TemporalLiteralInvalidPosition` (NonColumnLhsComparison) — REJECT with analyst-facing INVALID_PARAMS; silent coercion would reintroduce RISK-1 for datetime-valued LHS expressions (v1.10 NEW). Grammar-reachable in all five `build_predicate_parser` call sites (filter mode, pipe `| where`, SQL WHERE, SQL HAVING base, SqlPipe pipe stages) post-fix via `fn_call_comparison` production (`FuncCall::Scalar` only), DEFECT-PQL-FNCALL-LHS-001 (2026-07-13 [H5c]); pre-fix, ONLY SQL HAVING via `agg_call_parser` (ADR-048) was grammar-reachable (v1.12 claim "only SQL-mode queries could reach it" corrected v1.13). Non-date-like RHS (`Literal::String`) → valid, passes to DataFusion without interception; fn-call args participate in E-QUERY-038 column walk. |
...
five callers — filter mode (`| where`), pipe `| where`, SQL WHERE, SQL HAVING base
fallthrough, and SqlPipe pipe stages — `fn_call_comparison` is now grammar-reachable in
ALL five positions post-fix.

Previously this arm was defense-in-depth-only for the following reason: `build_predicate_parser`
lacked the `fn_call_comparison` production, so none of the five positions that route through
...
Post-fix, `check_temporal_literals` arm (4) fires uniformly across all five
...
DEFECT-PQL-FNCALL-LHS-001 fix to cover function-call LHS arguments across all five
...
| 14 | `crates/prism-query/src/sql_parser.rs` (+ `filter_parser.rs` if separate) | [CHANGE] | Modify timestamp literal production: lenient `is_date_like` fallback → `RawTemporalLiteral` instead of parse error. `filter_parser.rs::build_predicate_parser` adds the `fn_call_comparison` production (`FuncCall::Scalar` only) as of DEFECT-PQL-FNCALL-LHS-001 (2026-07-13 [H5c]). Because `build_predicate_parser` is shared by five callers (filter mode, pipe `| where`, SQL WHERE, SQL HAVING base fallthrough, SqlPipe pipe stages), `fn_call_comparison` is grammar-reachable in ALL five positions post-fix, making `check_temporal_literals` arm (4) fire uniformly across all five. Pre-fix, the NonColumnLhsComparison arm was grammar-reachable ONLY via SQL HAVING through `agg_call_parser` (ADR-048); all `build_predicate_parser` positions were defense-in-depth-only (v1.13 accuracy correction — v1.12 incorrectly stated "only SQL-mode queries could reach it"). |
