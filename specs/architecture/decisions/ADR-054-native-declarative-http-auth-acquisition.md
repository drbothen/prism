...
> **Rust field encoding (ratified FB52a, F-WASE-P64-MED-012):** The four token_exchange-only fields are declared `Option<String>` (`credential_body_field`, `token_response_path`, `expiry_field`) and `Option<ExpiryMode>` (`expiry_mode`) in `AuthAcquisitionConfig`. Using `Option` rather than `String` with an empty-string default is required: Rule 10(d) detects absence via `field.is_none()` and Rule 10(h) detects wrong-position presence via `field.is_some()`. Both checks are unambiguous only when the absent state is representable as a distinct value in the type. A `String` default of `""` collapses the absent case (TOML key omitted) and the present-but-empty case (`credential_body_field = ""`) into the same deserialized value, making Rule 10(d) undetectable as a separate state and making Rule 10(h) trivially-always-true for `oauth2_client_credentials` blocks (all fields would be `""`-present at every deserialization). `token_path: String` and `ttl_buffer_secs: u64` are not `Option` because they use `#[serde(default)]` and are not subject to per-field absence detection by any Rule 10 sub-condition.
...
> **Absence predicate (ratified FB52a, F-WASE-P64-MED-012):** A field is "absent" if and only if its Rust `Option` value is `None` — i.e., the TOML key is omitted from the `[auth_acquisition]` block. The implementing check is `field.is_none()` for each of the four required fields. An explicitly-provided empty string (`credential_body_field = ""`) resolves to `Some("")`, passes sub-condition (d) (the key is present), but fails sub-condition (e) (the value `""` is not found in any `[[credential_refs]]` name). The `Option<String>` / `Option<ExpiryMode>` Rust encoding (specified in story `S-ADR054-WAVE-A-001` AC-002 and ratified at §D3) makes the absent vs present distinction mechanically representable. A `String` field with an empty-string default would make omission and explicit `""` indistinguishable, rendering the "absent" state undetectable without a non-specified `is_empty()` predicate.
