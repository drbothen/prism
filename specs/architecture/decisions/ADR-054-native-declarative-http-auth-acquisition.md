...
| `VP-153` §Feasibility Assessment | **[EXECUTED — VP-153 v0.21, Wave-A spec evolution burst 3, 2026-07-22]** "5 auth_type variants × 5 credential structural shapes = 25 pairs" → "6 auth_type variants × 5 credential structural shapes = 30 pairs"; all harness amendments for VALID_AUTH_TYPES, arb_valid_auth_type(), arb_matching_auth_type(), and arb_mismatched_auth_type_pair() range bounds executed in VP-153 v0.21. `token_exchange` reuses the ApiKey single-string shape (structural shapes stay at 5); 6 identifier strings × 5 mismatched shapes = 30 ordered pairs. Rule B `allowed_count = if valid_type == "oauth2_client_credentials" { 2 } else { 1 }` auto-routes `token_exchange` to `allowed_count = 1` via `else` branch — no separate change. | D1 |
