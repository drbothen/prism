...
| `VP-153` §Proof Harness Skeleton `arb_auth_type()` + inline E-SPEC-012 message comment | Add `AuthType::TokenExchange` arm to the `proptest::sample::select()` call in `arb_auth_type()`; update the inline E-SPEC-012 message comment in the `prop_compose!` block body to include `token_exchange` in the "Valid values:" list | D1 |
