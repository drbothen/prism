...
> erroneously list `absolute_epoch_secs, ttl_secs` — these are PO authoring errors from burst 3 that
> require correction; see D11 PO follow-up rows.
...
| `error-taxonomy.md` E-SPEC-028(c) message template | **[PO FOLLOW-UP REQUIRED — Wave-A burst 3 adjudication]** The `valid values:` clause currently reads `absolute_epoch_secs, ttl_secs`; must be corrected to `absolute_utc_string, relative_seconds` to match the D3 field table and D10(c) message template. This was a PO authoring error in burst 3. BC-2.01.008's use of `expiry_mode = "absolute_utc_string"` is CORRECT per D3 and must NOT be changed. Behavioral anchor: E-SPEC-028(c) `valid values:` clause in the message_template column (grep-recoverable by `E-SPEC-028` id in error-taxonomy.md; TD-VSDD-091). Route to product-owner. | D3/D10(c) |
| `BC-2.16.009` Rule 10(c) description and EC-009-038 expected-error row | **[PO FOLLOW-UP REQUIRED — Wave-A burst 3 adjudication]** Rule 10(c) and its companion EC-009-038 row currently reference `{absolute_epoch_secs, ttl_secs}` as the valid-value set; must be corrected to `{absolute_utc_string, relative_seconds}` to match the D3 field table and D10(c) ratified value set. Behavioral anchor: Rule 10(c) body text and EC-009-038 expected-error string in BC-2.16.009 (grep-recoverable by `EC-009-038` in BC-2.16.009; TD-VSDD-091). Route to product-owner. | D3/D10(c) |
