...
| `VP-153` §Feasibility Assessment | Update "5 auth_type variants × 5 credential structural shapes = 25 pairs; all enumerable" → "6 auth_type variants × 5 credential structural shapes = 30 pairs; all enumerable". **Shape decision:** `token_exchange`'s credential is a single secret string consumed via `credential_body_field` → `[[credential_refs]] name="secret_key"` (ADR-054 D3 / ADR-053 D2) — structurally matching `MockCredentialType::ApiKey("key")` (a static single-string credential, not a directly-used access token). `token_exchange` reuses the existing `ApiKey` structural shape; no new `MockCredentialType` variant is required; credential space stays at 5 structural shapes; 6×5=30 arithmetic is correct. **Harness amendments implied:** (1) `is_coherent_pair` must recognize `(AuthType::TokenExchange, MockCredentialType::ApiKey)` as a valid pair alongside the existing `(AuthType::ApiKey, MockCredentialType::ApiKey)`; (2) `matching_credential_for(&auth_type)` must gain a `TokenExchange → MockCredentialType::ApiKey` arm — without this, `valid_auth_type_credential_pairs_accepted` fails for the new variant (load-bearing omission); (3) `mismatched_auth_type_credential_rejected` needs no separate change — it uses `prop_assume!(!is_coherent_pair(&auth_type, &credential_type))`, so once (1) lands the pair auto-excludes; no `prop_filter` exclusion mechanism exists or is needed in that test. **Implementer note:** validate helper function names against as-built test files (`crates/prism-spec-engine/tests/vp153_sensorauth_cross_composition.rs` for Rules A+B; `crates/prism-bin/tests/vp153_rule_c_shaped_probe.rs` for Rule C) at execution time — names are as-built per VP-153 v0.18. | D1 |
...
| `VP-153` §Proof Harness Skeleton `arb_auth_type()` + inline E-SPEC-012 message comment | Add `Just(AuthType::TokenExchange)` to the `prop_oneof!` in `arb_auth_type()`; update the inline E-SPEC-012 message comment inside the Rule A `proptest!` test body to include `token_exchange` in the "Valid values:" list; add `"token_exchange"` to the `prop_filter` exclusion list in the out-of-set arm (prevents generated valid `token_exchange` values from being incorrectly treated as out-of-set by the filter) | D1 |
| `error-taxonomy.md` E-SPEC-012 message template (Rule A — auth_type closed-enum validation) | Add `token_exchange` to the "Valid values: …" enumeration in the E-SPEC-012 `message_template` column: current text ends "…oauth2_client_credentials, bearer_static, cookie_roundtrip, api_key, custom_via_plugin"; add `token_exchange` as 6th value. **POL-24 atomicity:** `error-taxonomy.md` E-SPEC-012 (source) + VP-153 §Property Statement Rule A + VP-153 `arb_auth_type()` harness inline comment (copies) MUST be updated in the SAME commit — byte-verbatim invariant breaks if source and copies diverge across any commit boundary. | D1 |
