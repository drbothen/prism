...
| TD-W2-FIX-H-001 | W2-FIX-H PR delivery | `lefthook.yml` pre-commit runs `cargo fmt --check {staged_files}` with per-file path arguments, which stable `cargo fmt` does not support (no positional file path args). Implementer and pr-manager both bypassed with `LEFTHOOK=0` after manual `cargo fmt --all --check` verification. Fix: replace `cargo fmt --check {staged_files}` with `cargo fmt --all --check` in `lefthook.yml` pre-commit `fmt` command. Low urgency (workaround is safe; CI runs fmt check independently). | P3 | W2-FIX-H | — | lefthook.yml | Before Wave 3 start |
