...
| 43 | CODIFICATION-CANDIDATE-STRONGLY-WARRANTED | POL-29 fix-burst-commit-checklist — **6TH RECURRENCE evidence base now DECISIVE.** After pass-15 BLOCKED (FB12's BC-2.16.002 bullet-label not synced to match bumped frontmatter), POL-29 is HIGHEST-PRIORITY cycle-close governance policy. Proposed text: "Before closing any fix-burst that bumps a versioned source artifact (ADR, BC, VP), MUST grep all VP + BC live-narrative AND internal §-label headings for prior version pins and update all in SAME commit. When bumping downstream consumer only, do NOT re-bump the source (single-bump-per-source-artifact). EXPLICIT sweep targets must be enumerated in dispatch instructions." | 6 occurrences of same defect class (FB5/FB6/FB7→BLOCKED; FB8 BROKE via single-bump→pass-9 CLEAN★; FB12→pass-14 BLOCKED; FB13 applied discipline→pass-15 BLOCKED-again-different-axis; pattern = STRUCTURAL; codification STRONGLY WARRANTED cycle-close) |
...
| 48 | BLOCKED | PREREQ-E fix-burst-14 (3 findings: F-LP15-HIGH-001 BC-2.16.002 bullet label sync + F-LP15-HIGH-002 error-taxonomy BC anchor correction + F-LP15-MED-001 BC-2.16.012 changelog duplicate v1.2 renumber) | Awaiting FB14 dispatch — PO (2 HIGH) + state-manager (1 MED + closure) |
| 49 | PENDING | PREREQ-E pass-16 (fresh-context; post-FB14; if CLEAN streak 0/3 → 1/3) | Blocked on Task #48 FB14 CLOSED |
