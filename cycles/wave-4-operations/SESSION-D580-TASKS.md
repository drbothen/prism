...
| 43 | CODIFICATION-CANDIDATE-URGENCY-CRITICAL | POL-29 fix-burst-commit-checklist — **7TH OCCURRENCE evidence base now COMPLETE (pass-16 D-612 2026-05-16). URGENCY NOW CRITICAL — 7 of 14 fix-bursts produced this class.** POL-29 is HIGHEST-PRIORITY cycle-close governance policy. Proposed text: "Before closing any fix-burst that bumps a versioned source artifact (ADR, BC, VP), the sibling-sweep MUST grep ALL phrasing variants of the pin: parenthesized form, no-parens form, bare-version form, alternate-prefix forms. Single-form grep is INSUFFICIENT and is the documented 7-occurrence defect class. EXPLICIT variant enumeration required in dispatch instructions." | 7 occurrences of same defect class (FB5/FB6/FB7→BLOCKED; FB8 BROKE via explicit instruction→pass-9 CLEAN★; FB12→pass-14 BLOCKED; FB13 canonical-form-only→pass-15 BLOCKED; FB14 canonical-form-only→pass-16 BLOCKED; pattern = STRUCTURAL; codification URGENCY CRITICAL at cycle-close — NOT YET CODIFIED; task remains OPEN until cycle-close session-reviewer codifies POL-29) |
...
| 50 | BLOCKED | PREREQ-E fix-burst-15 (1 HIGH F-LP16-HIGH-001: 3 story sites + 1 ADR-026 site v1.19→v1.20 pin sweep with EXPLICIT POL-25 variant-phrasing grep mandate) | Blocked on dispatch with explicit POL-25 mandate (PO + architect + state-manager); must enumerate ALL variant phrasings before declaring closed |
| 51 | PENDING | PREREQ-E pass-17 (fresh-context; if CLEAN streak 0/3 → 1/3) | Blocked on Task #50 FB15 closure |
