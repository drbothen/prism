...
pass_6_head: 9e987c3f
timestamp: 2026-05-26T10:00:00Z
---

# PR #155 -- pr-reviewer pass-7

Fresh-context pass-7 review of PR #155 (S-CONFIG-MULTI-TENANT-OVERRIDE-001 -- per-org
sensor endpoint overlay loading per ADR-029). Diff baseline `develop@f19575ff`; feature
HEAD `f66287df`. This pass follows the pass-6 fix-burst (commit `f66287df`) which
closed SEC-PASS6-001 by sanitizing `dir_display` at derivation in the E-SPEC-022 path.
CI GREEN (36/36). Pass-5 and pass-6 were both CLEAN(strict). Target: verify fix-burst
did not regress, achieve 3rd consecutive CLEAN(strict) for convergence.
...
+ ~290 LOC in fanout unit tests + ~98 LOC in sanitize_for_log unit tests. 23 commits
...
| 5 | Commit Quality | PASS -- conventional commits with story ID. 23 commits reflecting TDD progression: stubs, red gate, implementation, 13 LOCAL adversarial fix-bursts, 6 PR-level fix-bursts. Each commit message is clear and descriptive. |
...
## Pass-6 Fix-Burst Verification (f66287df)

The pass-6 fix-burst commit `f66287df` made exactly one change to `overlay.rs`
(9 insertions, 1 deletion):

### SEC-PASS6-001: `dir_display` sanitization at derivation in E-SPEC-022 path

**Before:** `let dir_display = format!("customers/{slug_str}/");`

**After:** `let dir_display = format!("customers/{}/", sanitize_for_log(&slug_str));`

This closes the final gap in the CWE-117 sanitization sweep. `slug_str` comes from
`readdir` (via `entry.file_name().to_string_lossy()`) and can contain control characters
on POSIX systems. The `dir_display` string flows into `make_e_spec_022_unknown_org_slug`
as the `customers_dir_name` argument, which is embedded verbatim in the error message
body and `file_path` field. Without sanitization, an attacker-controlled directory name
could inject newlines or other control characters into log output (CWE-117).

**Defense-in-depth analysis:** `make_e_spec_022_unknown_org_slug` ALSO sanitizes the
`slug` parameter internally (via `sanitize_for_log(slug)`). However, the
`customers_dir_name` argument was NOT sanitized within the function -- it was used
verbatim in the error message. The fix at the call site (derivation point) mirrors the
SEC-PASS5-002 pattern for `overlay_file_path` and ensures ALL readdir-sourced values
are sanitized before they enter any error message path.

**Regression check:** The `dir_display` string is used ONLY for error messages and the
`file_path` metadata field -- never as a filesystem path for I/O operations. The real
I/O uses `entry.path()`. Sanitizing the display path has zero impact on functionality.
CI GREEN confirms no regression.

**Doc comment update:** The `sanitize_for_log` doc comment was updated to enumerate
this new call site (SEC-PASS6-001), completing the TD-VSDD-060 sibling-sweep.

**Verdict: Fix-burst is correct, minimal, and introduces no regression.**

---

## Prior-Pass Finding Disposition (Carried from Pass-5/6)
...
## Pass-7 Fresh-Context Verification
...
#### Production code -- overlay.rs (1034 LOC including SEC-PASS6-001 fix)
...
- `sanitize_for_log` applied at ALL user-controlled embedding sites (10 total after SEC-PASS6-001):
  ...
  7. `slug` in `make_e_spec_022_unknown_org_slug` (internal sanitization)
  8. `field_name` in `make_e_spec_023_unrecognized_field`
  9. `extends_value` in `make_e_spec_019_unknown_extends` (all 3 occurrences)
  10. `dir_display` at derivation point via `sanitize_for_log(&slug_str)` (SEC-PASS6-001)
...
The pass-6 fix-burst (SEC-PASS6-001) correctly closes the `dir_display` sanitization
gap identified in pass-6. The fix is minimal (9 lines added, 1 line replaced),
well-documented with CWE-117 references and TD-VSDD-060 compliance notes, and introduces
no behavioral regression (CI GREEN 36/36).
...
All 5 have inline comments documenting audit role and recurrence policy. No new emissions
added in the pass-6 fix-burst (f66287df only modifies sanitization, not tracing).
...
| Pass-6 (pr-reviewer) | CLEAN(strict) |
| Pass-7 (pr-reviewer, this pass) | CLEAN(strict) |
| Consecutive CLEAN(strict) streak | 3/3 -- CONVERGED |
...
Zero findings of any severity (CRIT, HIGH, MED, LOW, OBS, PROCESS-GAP). The pass-6
fix-burst at `f66287df` correctly closed SEC-PASS6-001 without regression. All
51 changed files reviewed against the 8-item checklist and CLAUDE.md conventions. This
is the 3rd consecutive CLEAN(strict) pass.

**BC-5.39.001 3-CLEAN(strict) convergence achieved at pass-7.**

PR #155 is approved for merge from the pr-reviewer perspective.
