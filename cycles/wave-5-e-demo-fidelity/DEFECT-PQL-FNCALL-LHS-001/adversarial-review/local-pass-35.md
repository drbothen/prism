...
- **`Dml` arm code confirmed:** `check_enrich_udf_availability`'s `Ast::Sql(SqlStatement::Dml(dml))` arm confirmed in `engine.rs` at HEAD bd044a2e; `dml.source_select.where_` walk confirmed via `collect_unknown_scalar_from_predicate` call; load-bearing test `test_f_pqlfn_p32_obs_001_insert_source_select_where_aggregate_fires_e_query_001` confirmed present and GREEN.
