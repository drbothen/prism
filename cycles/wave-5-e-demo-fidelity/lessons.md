...
## Lesson 114 — Three independent instances of POL-29 dimension-9a (named-twin sibling-pair sweep) failure in a single pass; TD-VSDD-097 3-recurrence threshold met [recurrence-flag]

**Category:** POL-29 enforcement, sibling-pair sweep discipline, codification threshold

**Context:** Pass-72b surfaced three independent, structurally identical instances of the same defect class in a single adversary pass: a burst fixed the dispatched artifact correctly and stopped exactly at the dispatch perimeter boundary, leaving the structural twin in the same state as before. The three instances:

1. **VP-160/VP-161 propagation split (F-WASE-P72B-HIGH-001):** A single FB62 burst created both VPs as an explicit twin pair from one split, with identical anchor story, subsystem, tool, and priority. VP-160 was fully propagated to `S-WAVE-A-ENGINE-001` (6 occurrences: frontmatter, §Token Budget, §Merge gate). VP-161 has zero occurrences in the story. Every individual artifact is internally correct; only the reverse cross-artifact direction is broken.

2. **BC-2.02.004/BC-2.02.006 SAP-2 treatment vs. untouched BC-2.02.003/BC-2.02.005 (F-WASE-P72B-HIGH-002):** FB68 closed SAP-2 findings against BC-2.02.004 (cyberint) and BC-2.02.006 (armis) in one burst. BC-2.02.003 (crowdstrike) and BC-2.02.005 (claroty) are structurally identical files in the same `BC-2.02.00x` sensor field-mapping family, same subsystem SS-02, same capability CAP-003. The FB68 burst did not sweep them. Both untouched siblings carry undocumented DTU field exclusions that SAP-2 requires be documented.

3. **ADR-050..056 `anchor_stories:` sweep vs. untouched remaining 15 true ADRs (F-WASE-P72B-HIGH-003):** FB62 populated `anchor_stories:` on ADR-050 through ADR-056 (7 files). The corpus contains 54 true ADRs (`document_type: adr`). The remaining 47 were not swept, and 15 of them are missing the key entirely.

In all three cases, the dispatched burst's work is correct. The gap is that the dispatch perimeter was narrower than the blast radius of the rule being applied.

**Why dimension-9a is uniquely resistant to string-grep discharge:** POL-29 dimension 9a states "absence of the string is the failure mode, not evidence of cleanliness." All three instances share exactly this property: a string-grep for the changed content (e.g., grep for `anchor_stories` in the decisions directory, or for VP-161 in the story) PASSES for the dispatched artifact and PASSES for the twin — because the twin LACKS the string, and the rule says that absence is the failure, not the pass. A correct dimension-9a discharge requires naming the twin and asserting the twin carries the expected content, not merely verifying that the twin "doesn't have the problem string."

**Meeting the TD-VSDD-097 codification threshold:** TD-VSDD-097 defines the 3-recurrence threshold as the trigger for structural intervention. Three independent instances of the same `POL-29 dimension 9a` failure pattern in a single pass clearly meets this threshold. The pattern name: "dispatch-perimeter-terminates-at-the-perimeter-boundary." The dispatch contained no explicit twin-sweep instruction; the fixing agent completed the stated scope correctly; the gap was in scope-definition, not in execution quality.

**Going-forward rule (proposed — codification candidate):** When a fix-burst dispatch involves a `document_type: adr` file, a `BC-2.02.00x` sensor-field-mapping BC, or a VP created as part of an explicit named-twin pair, the orchestrator dispatch MUST name the structural twins and include an explicit twin-sweep clause. "Fix the dispatched artifact AND confirm that its structural twin carries the analogous content (or document why the twin intentionally diverges)." This is the mechanical operationalization of dimension-9a for these three artifact families.
