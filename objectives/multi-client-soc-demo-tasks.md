...
**Step 1: `dclaude:remove-uncertainty` on S-DEMO-MULTI-TENANT-DTU-001** (story file: `.factory/stories/S-DEMO-MULTI-TENANT-DTU-001-dtu-per-instance-multi-address-binding.md`). Mandatory re-run per D-1110 extension. Story was last re-uncertainty'd at D-1076 2026-06-09; re-run now immediately before TDD.

**Step 2: `vsdd-factory:deliver-story S-DEMO-MULTI-TENANT-DTU-001`** — full 12-gate per-story TDD delivery. See §Task Ledger T6 row for done-when.
...
| T6 | **in-progress** | orchestrator-driven per-story delivery | T3 (SATISFIED) + T4-A+T5 (SATISFIED) | S-DEMO-MULTI-TENANT-DTU-001 | Multi-instance bind implemented + merged (remove-uncertainty RE-RUN → test-writer → implementer → LOCAL 3-CLEAN strict → demo → PR → PR-LEVEL 3-CLEAN strict → merge). NEXT: `dclaude:remove-uncertainty` re-run, then `vsdd-factory:deliver-story S-DEMO-MULTI-TENANT-DTU-001`. |
