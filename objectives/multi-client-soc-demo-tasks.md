...
Foundations: COMPLETE (reused). Build: 3/15 tasks done; T4 **RECONCILED+COMPLETE** (D-1079: ADR-036 v2.0 substrate reconciliation; BC-2.06.018/019/020 v1.1; E-DEMO-004/005 registered; story split materialized — Story A 001-A ready + Story B 001-B draft). **CURRENT TASK: T4→Story A delivery (per-story 12-gate sequence NEXT).**

## CURRENT POINTER

**T4 RECONCILED+COMPLETE (D-1079).** ADR-036 v2.0 substrate reconciliation complete (architect). BC-2.06.018/019/020 v1.1 corrections complete (PO). E-DEMO-004/005 in error-taxonomy v1.64. Story split complete (story-writer): original S-DEMO-DTU-LIVE-SCENARIO-001 SUPERSEDED → Story A (S-DEMO-DTU-LIVE-SCENARIO-001-A; 8pt; ready; BC-2.06.018) + Story B (S-DEMO-DTU-LIVE-SCENARIO-001-B; 7pt; draft; depends_on A). **NEXT: deliver Story A via 12-gate per-story sequence. Story B after A merges.**

## NEXT ACTION (verbatim, for cold resume)

Run dclaude:remove-uncertainty on S-DEMO-DTU-LIVE-SCENARIO-001-A (pre-delivery validation of the reconciled ADR-036 v2.0 design), then deliver Story A via the 12-gate per-story sequence (worktree → test-writer Red Gate → implementer TDD → LOCAL adversary 3-CLEAN → demo → PR → PR-LEVEL 3-CLEAN → merge). Story B follows after Story A merges.
...
| T4-A | not-started | per-story delivery | T4 | S-DEMO-DTU-LIVE-SCENARIO-001-A | Story A (baseline seeding retrofit; 8pt; BC-2.06.018) implemented + merged via 12-gate per-story sequence. **NEXT UNBLOCKED DELIVERY — run dclaude:remove-uncertainty first (standing directive D-1061).** |
