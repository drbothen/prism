...
bc_index_version: "6.48"
vp_index_version: "1.79"
story_index_version: "v2.376"
...
# ── WAVE-5 PHASE STATUS ──
current_step: "D-1149 — T6 LOCAL adversary Pass-5 DONE (3 MED spec-accuracy, all CLOSED; BC v1.5/story v1.8); streak 0/3; consistency-audit + Pass-6 NEXT. F-P5-MED-001 BC-version-citation drift (8 sites citing stale v1.1..v1.4) → story-writer swept §Behavioral Contracts pin to v1.5; amendment-log historical entries preserved. F-P5-MED-002 H1 'v1.4' vs frontmatter v1.7 desync → story-writer made H1 version-agnostic. F-P5-MED-003 SUBSTANTIVE overlay-format spec gap: BC/story described base_url-only but OverlayLoader requires 3 REQUIRED fields (extends, instance_id, base_url) per INV-SCALAR-003 → product-owner BC v1.4→v1.5 (Postcondition 2 + TV-017-009) + story-writer (AC-005/§File-Structure/Task-6). SAP-1 clean; gate exact 60; INV-PERIMETER-001 clean. Code UNCHANGED since Pass-1. BC-INDEX v6.47→v6.48. STORY-INDEX v2.375→v2.376. STATE v7.791→v7.792."
...
**Prism** | Rust | brownfield | per-analyst stdio (MCP) | Started 2026-04-13 | Last Updated 2026-06-13 (D-1149 T6 LOCAL adversary Pass-5 DONE; 3 MED spec-accuracy all CLOSED incl overlay-format gap; BC v1.5/story v1.8; streak 0/3; consistency-audit + Pass-6 NEXT; STATE v7.792)

## Active Objective (North Star)

**NORTH STAR: Multi-client SOC-analyst live demo — multiple DTU clients, per-client data, prism MCP wired into Claude (stdio), deterministic scenario progression, ThreatIntel+NVD enrichment.** Full detail: SESSION-HANDOFF.md §ACTIVE OBJECTIVE + `.factory/objectives/DEMO-SCOPE.md`. Task ledger: `.factory/objectives/multi-client-soc-demo-tasks.md` CURRENT POINTER = **T6 IN PROGRESS** (S-DEMO-MULTI-TENANT-DTU-001 v1.8; D-1149 LOCAL Pass-5 DONE; streak 0/3; consistency-audit + Pass-6 NEXT). T1–T5+T4-A DONE.
...
_No open PRs. Last merges: PR #185 develop@7fd35b77 (T5 DONE), PR #186 develop@f7400f83 (D-1134 bypass remediation RESOLVED). D-1149: T6 LOCAL Pass-5 + spec-accuracy sweep DONE — 3 findings (3 MED) all CLOSED incl F-P5-MED-003 overlay-format spec gap; BC v1.4→v1.5; story v1.7→v1.8; streak 0/3; consistency-audit + Pass-6 NEXT._

## Session Resume Checkpoint (D-1149 — 2026-06-13; STATE v7.792)

**STATE v7.792. CURRENT POSITION: T6 — S-DEMO-MULTI-TENANT-DTU-001 v1.8 in progress; LOCAL adversary Passes 1-5 DONE + all fix-bursts CLOSED (D-1146 + D-1147 + D-1148 + D-1149). BC-5.39.001 streak: 0/3. Consistency-audit + Pass-6 NEXT. VERIFIED STATE: `MultiInstanceServers` (#[non_exhaustive]) is the `start_instances` return type; `servers.socket_map()` returns `&HashMap<(String,String),SocketAddr>` (String keys per U-004/D-1075 — NOT OrgSlug/SensorId newtypes); `servers.shutdown()` + Drop graceful drain wired; server-side request counter in ArmisClone is AtomicUsize (does NOT mirror ClarotyState AtomicU64); watcher-task comments describe shutdown_tx broadcast + with_graceful_shutdown (not "watcher loop"); iter() (not iter_mut()) in ALL doc/sketch usage examples (AC-004/AC-007/Task-5/§Locked API sketch); 20 tests total (18 multi_instance/harness + 2 bind-failure); EXPECTED=60 at merge; ci.yml failure-branch reads "60 types (including MultiInstanceServers)"; struct_violations.rs enumeration extends through v61/MultiInstanceServers with D-1145 citation; test files grep-clean of all scaffold comment patterns. BC-2.06.017 v1.5: Postcondition 2 = (String,String) keys + 3 REQUIRED overlay fields (extends, instance_id, base_url) per INV-SCALAR-003; Postcondition 1 = Ok(MultiInstanceServers); EC-017-002 = Err(BindFailure); TV-017-009 added (3-field overlay format). H1 heading is version-agnostic (frontmatter version: "1.8" is sole authority). develop HEAD: f7400f83 (UNCHANGED — story not yet merged; MID-DELIVERY). BC-INDEX v6.48 (active 234 / draft 3 / retired 6). STORY-INDEX v2.376 (200 stories). VP-INDEX v1.79 (158). policies v1.33. error-taxonomy v1.78. demo_scope_doc: .factory/objectives/DEMO-SCOPE.md. No open PRs.**

**RESUME PROTOCOL (run on fresh session start):**
0. Read SESSION-HANDOFF.md §ACTIVE OBJECTIVE (North Star) + §RESUME SNAPSHOT (latest D-1149). Read `.factory/objectives/DEMO-SCOPE.md` (authoritative full demo scope).
...
3. Verify `grep "^version:" .factory/STATE.md` shows `"7.792"`.
...
6. **NEXT ACTION: T6 — run consistency-audit on S-DEMO-MULTI-TENANT-DTU-001 v1.8 + BC-2.06.017 v1.5 cross-document citation sweep, then dispatch LOCAL adversary Pass-6. BC-5.39.001 streak 0/3. Target: CLEAN(strict)=YES (zero findings of any severity). KEY INVARIANTS TO CHECK: socket_map() returns (String,String) keys (not OrgSlug/SensorId); watcher comments describe shutdown_tx broadcast; ArmisClone counter is AtomicUsize (NOT AtomicU64); iter() not iter_mut() in ALL usage examples; overlay format = 3 REQUIRED fields (extends, instance_id, base_url) per INV-SCALAR-003; BC-2.06.017 v1.5 §Behavioral Contracts table pin in story; H1 heading version-agnostic; ci.yml failure-branch "60 types (including MultiInstanceServers)"; struct_violations.rs enumeration through v61; test files scaffold-clean; EXPECTED=60 at merge.**
