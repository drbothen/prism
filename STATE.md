...
# ── WAVE-5 PHASE STATUS ──
current_step: "D-1147 — T6 LOCAL adversary passes 2-3 DONE (all findings CLOSED, severity decaying to doc-sweeps); story v1.6/BC v1.4; just check GREEN; streak 0/3; Pass-4 NEXT. Pass-2: 3 findings (2M+1L) — success-path watcher comments (4 sites); ArmisClone false mirrors-ClarotyState claim; stale Arc comment. Pass-3: 3 findings (1M+2L) — BC-2.06.017 PC-2 newtype (String,String) BC v1.3→v1.4; Ok(HashMap::new()) test comment; iter_mut()→iter() doc (harness+story). BC-INDEX v6.46→v6.47. STORY-INDEX v2.373→v2.374. STATE v7.790."
...
**Prism** | Rust | brownfield | per-analyst stdio (MCP) | Started 2026-04-13 | Last Updated 2026-06-13 (D-1147 T6 LOCAL adversary Pass-2+3 doc/spec fix-bursts DONE; 3+3 findings all CLOSED; story v1.6, BC-2.06.017 v1.4; just check GREEN; streak 0/3; Pass-4 NEXT; STATE v7.790)

## Active Objective (North Star)

**NORTH STAR: Multi-client SOC-analyst live demo — multiple DTU clients, per-client data, prism MCP wired into Claude (stdio), deterministic scenario progression, ThreatIntel+NVD enrichment.** Full detail: SESSION-HANDOFF.md §ACTIVE OBJECTIVE + `.factory/objectives/DEMO-SCOPE.md`. Task ledger: `.factory/objectives/multi-client-soc-demo-tasks.md` CURRENT POINTER = **T6 IN PROGRESS** (S-DEMO-MULTI-TENANT-DTU-001 v1.6; D-1147 LOCAL Pass-2+3 fix-bursts DONE; streak 0/3; Pass-4 NEXT). T1–T5+T4-A DONE.
...
|------|-------|--------|--------|
_D-735 through D-1133 archived to cycles/wave-5-e-demo-fidelity/burst-log.md and decisions-archive files._
...
_No open PRs. Last merges: PR #185 develop@7fd35b77 (T5 DONE), PR #186 develop@f7400f83 (D-1134 bypass remediation RESOLVED). D-1147: T6 LOCAL Passes 2-3 + doc/spec fix-bursts DONE — 6 findings all CLOSED; story v1.6; BC-2.06.017 v1.4; streak 0/3; Pass-4 NEXT._

## Session Resume Checkpoint (D-1147 — 2026-06-13; STATE v7.790)

**STATE v7.790. CURRENT POSITION: T6 — S-DEMO-MULTI-TENANT-DTU-001 v1.6 in progress; LOCAL adversary Passes 1-3 DONE + all fix-bursts CLOSED (D-1146 + D-1147). BC-5.39.001 streak: 0/3. Pass-4 NEXT. VERIFIED STATE: `MultiInstanceServers` (#[non_exhaustive]) is the `start_instances` return type; `servers.socket_map()` returns `&HashMap<(String,String),SocketAddr>` (String keys per U-004/D-1075 — NOT OrgSlug/SensorId newtypes); `servers.shutdown()` + Drop graceful drain wired; server-side AtomicU64 isolation counter in ArmisClone (AtomicUsize; does NOT mirror ClarotyState AtomicU64); watcher-task comments describe shutdown_tx broadcast + with_graceful_shutdown (not "watcher loop"); 20 tests total (18 multi_instance/harness + 2 bind-failure); EXPECTED=60 at merge. BC-2.06.017 v1.4: Postcondition 2 (String,String) keys; Postcondition 1 = Ok(MultiInstanceServers); EC-017-002 = Err(BindFailure). develop HEAD: f7400f83 (UNCHANGED — story not yet merged; MID-DELIVERY). BC-INDEX v6.47 (active 234 / draft 3 / retired 6). STORY-INDEX v2.374 (200 stories). VP-INDEX v1.79 (158). policies v1.33. error-taxonomy v1.78. demo_scope_doc: .factory/objectives/DEMO-SCOPE.md. No open PRs.**

**RESUME PROTOCOL (run on fresh session start):**
0. Read SESSION-HANDOFF.md §ACTIVE OBJECTIVE (North Star) + §RESUME SNAPSHOT (latest D-1147). Read `.factory/objectives/DEMO-SCOPE.md` (authoritative full demo scope).
...
3. Verify `grep "^version:" .factory/STATE.md` shows `"7.790"`.
...
6. **NEXT ACTION: T6 — LOCAL adversary Pass-4 on S-DEMO-MULTI-TENANT-DTU-001 v1.6. BC-5.39.001 streak 0/3. Adversary reads post-fix code + BC-2.06.017 v1.4 + story v1.6. Target: CLEAN(strict)=YES (zero findings of any severity). KEY INVARIANTS TO CHECK: socket_map() returns (String,String) keys (not OrgSlug/SensorId); watcher comments describe shutdown_tx broadcast (not "watcher loop"); server-side AtomicU64 is AtomicUsize (NOT AtomicU64 like ClarotyState); iter() not iter_mut() in usage examples; EXPECTED=60 at merge.**
