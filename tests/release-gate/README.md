...
| test_AC-010_linux-setup.sh | AC-010 | Linux apt dependencies installed (musl-tools, libdbus-1-dev, etc.); protoc toolchain pinned (arduino/setup-protoc SHA+version comment, no if: gate — DEFECT-REL001-PROTOC-MISSING-001); §15 cargo-zigbuild design: pip3 --require-hashes + cargo-zigbuild@0.23.0 + composed zigbuild line + requirements-musl-ci.txt (ziglang==0.16.0 pin + sha256 hash) + CXX_x86_64_unknown_linux_musl=clang++ ABSENT (DEFECT-REL001-MUSL-LIBSTDCXX-001, SID-2 composed assertions, 7 assertions); §14 Option B linux-gnu persistence invariant: target-cfg header present + keyring linux-native-sync-persistent in block (awk block-scoped) + NOT in default features (negative guard) — F-REL001-P14-001, crates/prism-credentials/Cargo.toml (3 assertions) |
...
After implementation, all 74 assertions should pass. actionlint must be installed
...
| `EXPECTED_ASSERTIONS` | 74 | Total TAP assertions across all test files |
