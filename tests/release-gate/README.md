...
| test_AC-006_matrix-targets.sh | AC-006 | All 4 platform targets present and correctly spelled; exactly 4 matrix entries (ADR-065 D1) |
...
| test_AC-012_install-scripts.sh | S-REL-003 AC-001..AC-010 | install.sh and install.ps1 exist with required structure (set -euo pipefail, dual-path SHA-256, all 4 platform targets (3 in install.sh + Windows in install.ps1), composite musl detection, /releases?per_page=1 not /releases/latest, PATH guidance, #Requires -Version 5.1, Get-FileHash, checksum-mismatch abort); ci.yml has shellcheck-install-scripts + psscriptanalyzer-install-ps1 jobs; release.yml publish-release uploads both scripts (ADJ-002) |
...
After implementation, all 121 assertions should pass. actionlint must be installed
...
| `EXPECTED_ASSERTIONS` | 121 | Total TAP assertions across all test files |
