...
| test_AC-010_linux-setup.sh | AC-010 | Linux apt dependencies installed (musl-tools, libdbus-1-dev, etc.); protoc toolchain pinned (arduino/setup-protoc SHA+version comment, no if: gate — DEFECT-REL001-PROTOC-MISSING-001); musl C++ toolchain wiring present (clang in apt-get install + CXX_x86_64_unknown_linux_musl=clang++ env assignment — DEFECT-REL001-MUSL-CXX-001, SID-2 composed assertions) |
...
After implementation, all 65 assertions should pass. actionlint must be installed
...
| `EXPECTED_ASSERTIONS` | 65 | Total TAP assertions across all test files |
