...
if ! command -v actionlint >/dev/null 2>&1; then
  tap_fail "AC-011: actionlint not installed — gate fails closed" \
    "actionlint is required but not found in PATH. macOS: brew install actionlint  Linux: see ci.yml release-gate job for the pinned+SHA-verified install pattern  (NOT cargo install — actionlint is a Go tool, not a Rust crate)"
  tap_done
  exit 1
fi
