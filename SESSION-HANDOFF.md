...
successor_focus: "## NEXT ACTION (D-432 — PREREQ-C LOCAL PASS-1 BLOCKED-SOFT, FIX-BURST-1 READY)\n\nDispatch implementer for S-PLUGIN-PREREQ-C fix-burst-1 to close 3 CRIT + 8 HIGH findings from adversary pass-1.\n\n**Adversary report:** /Users/jmagady/Dev/prism/.factory/code-delivery/S-PLUGIN-PREREQ-C/adversary-pass-1.md (~280 lines, full evidence + recommended fixes).\n\n**Fix scope (prioritized):**\n\nCRITICAL (must close all 3):\n1. F-LP1-CRIT-001: Wire AC-5 compile-fail CI invocation in .github/workflows/ci.yml (modeled on perimeter-violation job). Positive-coverage log line required (POL-11). Add tests/external/non-exhaustive-violation to workspace exclude in Cargo.toml.\n2. F-LP1-CRIT-002: Add `impl Default` to all 8 non_exhaustive types (CredentialRef, SensorSpec, SensorTableDescriptor, FetchStep, ColumnSpec, TableSpec, PaginationConfig variant fields, AuthType) OR replace positional ::new with builder pattern. Document `..Default::default()` as canonical external construction. Update compile-fail test to also exercise Default-based construction succeeds while struct-literal fails.\n3. F-LP1-CRIT-003: Rewrite test_BC_2_16_002_cursor_pagination_page_size_none_omitted to actually invoke build_paged_url_for_test on both first-call AND continuation with page_size=None. Assert neither output contains page_size=.\n\nHIGH (must close all 8):\n4. F-LP1-HIGH-001: Add `detail: String` field to SpecEngineError::JsonPathExtractionFailed AND emit tracing::warn!(event_type = \"jsonpath_extraction_failed\", path, detail) at call sites. Amend BC-2.16.002 v1.9→v1.10 with new catalog row (this triggers PG-LP11-001 SOP — workspace commit must be paired with state-manager BC amendment).\n5. F-LP1-HIGH-002: Add any::<serde_json::Value>() body strategy with depth limit to extract_at_path totality proptest. Expand path regex to include ~ escape.\n6. F-LP1-HIGH-003: Decide escape grammar: context-free $$ OR anchored $${ . Implement consistently, fix doc-comment, add test_AC4_escape_only_before_brace.\n7. F-LP1-HIGH-004: Expand AC-5 audit to RateLimitHints + types.rs duplicates + infusion::CredentialRef + prism_core::ColumnType + ColumnOptions. Or document scope limitation in story + file TDs for missed types. Investigate types.rs SensorTableDescriptor duplicate (dead code candidate).\n8. F-LP1-HIGH-005: Convert new_unchecked_audit.rs allowlist from file-suffix-keyed to symbol-keyed. Add parser to extract type name from impl context.\n9. F-LP1-HIGH-006: Decide AC-6 production caller fix: (a) use validated OrgSlug::new() with error handling (preferred — micro-cost), (b) add 1000-iter property test, OR (c) use hardcoded sentinel.\n10. F-LP1-HIGH-007: Add MAX_JSONPATH_RESULT_SIZE (100_000) + MAX_JSONPATH_DEPTH (32) constants. Thread current_size accumulator through extract_with_tokens. Add nested-wildcard amplification test.\n11. F-LP1-HIGH-008: Rewrite AC-7 doctest to construct + match on SensorIdValidationError variant.\n\nOBS (implementer discretion — recommended close):\n- OBS-001 (PREREQ-B-inherited volatile pins): defer to maintenance story (out of PREREQ-C scope).\n- OBS-002 (page_size=0): document in CursorToken doc-comment OR add validation (low-cost; recommend close).\n- OBS-003 (template proptest narrow): add malformed-template strategy (medium-cost; recommend close).\n- OBS-004/005/006/007: implementer judgment.\n\n**Discipline reminders:**\n- TD-VSDD-091 anti-volatile-pin: no file:line citations.\n- TD-VSDD-053 single-commit-per-burst: each AC fix is ONE commit on feature branch. Multiple fixes across ACs may aggregate into one or multiple commits — but NO chain of consecutive same-theme commits.\n- PG-LP11-001: HIGH-001 introduces a new event_type — must amend BC-2.16.002 v1.9→v1.10 + BC-INDEX v4.60→v4.61. BC amendment goes in factory-artifacts via state-manager AFTER workspace commit.\n- Paper-fix detection: for each fix, ask: would my test PASS if I revert the production change? If yes, the test is paper-fix. F-LP1-CRIT-003 and F-LP1-HIGH-008 are exactly this class — be rigorous.\n- Sibling-sweep BROAD (TD-VSDD-060): F-LP1-HIGH-004 already broadens the sibling sweep; verify the new audit covers ALL siblings.\n\n**After fix-burst-1:**\n- Run `just check` — must be clean\n- Update demo evidence directory if any (docs/demo-evidence/S-PLUGIN-PREREQ-C/) - per-AC closure evidence\n- Dispatch state-manager to log D-433 (fix-burst-1 complete, BC v1.10 amendment if applicable)\n- Dispatch adversary for LOCAL pass-2 with paper-fix-detection rigor + sibling-sweep verification\n\n**Pin SHAs at D-432:** develop@ae7e26c8 | factory-artifacts: run `git -C .factory log -1 --format='%H'` | feature/S-PLUGIN-PREREQ-C@0e7d8bca (unchanged — adversary read-only) | worktree adversary-pass-1-blocked-soft."

**CURRENT STATE (2026-05-12, D-432 — S-PLUGIN-PREREQ-C LOCAL ADVERSARY PASS-1 BLOCKED-SOFT)**

**Pin SHAs:**
- develop HEAD: ae7e26c8 (PR #143 PREREQ-B merged 2026-05-12T06:58:48Z)
- feature/S-PLUGIN-PREREQ-C HEAD: 0e7d8bca (adversary read-only — unchanged from D-431)
- worktree: /Users/jmagady/Dev/prism/.worktrees/S-PLUGIN-PREREQ-C/ (adversary-pass-1-blocked-soft)
- factory-artifacts HEAD: run `git -C .factory log -1 --format='%H'` (per TD-VSDD-053)
- STATE.md / SESSION-HANDOFF: v7.165 → v7.166

**D-432 — LOCAL adversary pass-1 BLOCKED-soft. 18 findings. Streak 0/3.**
- Adversary report: /Users/jmagady/Dev/prism/.factory/code-delivery/S-PLUGIN-PREREQ-C/adversary-pass-1.md
- 3 CRITICAL: F-LP1-CRIT-001 (AC-5 CI-inert), F-LP1-CRIT-002 (positional ::new defeats non_exhaustive), F-LP1-CRIT-003 (AC-1(c) paper-fix)
- 8 HIGH: observability silence, proptest partial totality, escape grammar drift, sibling-sweep miss, allowlist over-broad, security-precondition fragility, nested-wildcard OOM, doctest tautology
- 7 OBS/LOW: 3 MEDIUM + 4 LOW (see adversary-pass-1.md for details)
- Trajectory: 18 at pass-1 (vs PREREQ-A 14, PREREQ-B 20)
- NEXT: implementer fix-burst-1 (3 CRIT + 8 HIGH mandatory; selected OBS at discretion)

**Standing rules ACTIVE post-compact:**
- Edit/Write tools ONLY for all .factory/ mutations
- Single-commit-per-burst (orchestrator MUST avoid "backfill" in commit subjects)
...
- BC-5.39.001 3-CLEAN protocol satisfied for S-PLUGIN-PREREQ-A; same protocol applies to PREREQ-B/C/D/E when their cascades run

**Open methodology TDs (continue tracking):**
- TD-FACTORY-HOOK-BYPASS-001 P0 (Python/sed file-write bypass — pattern stabilized after fix-burst-17 of prior ADR-023 cycle)
- TD-VSDD-054..063 (cycle methodology improvements from ADR-023 cycle reflection)
- New process-gaps from S-PLUGIN-PREREQ-A cascade:
  - PG-LP7-001 Red Gate test materialization audit
  - PG-LP7-002 Sibling-rename axis layer-enumeration
  - OBS-LP9-001 Adversary evidence-or-not-happened protocol (absolute paths required)

**Branch state at compact:**
- feature/S-PLUGIN-PREREQ-A: HEAD 8b949bba pushed to origin (durability secured)
- develop: c6dd6602 (PR #141 S-3.02-FOLLOWUP-RUNTIME merge)
- factory-artifacts: D-393 PRE-COMPACT CHECKPOINT commit (this burst)
- Worktrees: .worktrees/S-PLUGIN-PREREQ-A retained for PR cycle; .worktrees/S-3.09 FROZEN per D-298; .worktrees/W3-FIX-S307-001 retained at fcab8717 per D-333

**Bundle B Phase B-2 status:**
- 5 stories remain BLOCKED on plugin migration (S-5.01-FOLLOWUP-MCP-BOOT, S-1.12-FOLLOWUP, S-1.14-REDO, W3-FIX-S307-001, W3-FIX-S307-002)
- Will re-scope after PREREQ-A merge + PREREQ-B/C/D/E ship (Wave 0 complete)
---
