...
- STATE.md / SESSION-HANDOFF: v7.122
...
**NEXT ACTION:**
S-PLUGIN-PREREQ-A LOCAL pass-9 BLOCKED-soft (D-389). Fix-burst-9 required: (1) `crates/prism-query/src/invalidation.rs:125,129` — replace `sensor_type` with `sensor_id` in doc-comments (F-LP9-MED-001); (2) `crates/prism-query/src/materialization.rs:775` — replace `sensor_type` with `sensor_id` in doc-comment (F-LP9-LOW-001). 3-line total edit. Story v1.4 already committed (F-LP9-MED-002 OPTION B). F-LP9-LOW-002 (registry.rs:56) is intentional — no fix needed. After fix-burst-9 commits, dispatch adversary pass-10 fresh-context (streak 0/3 → 1/3 attempt).
---
