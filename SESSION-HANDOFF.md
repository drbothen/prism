...
> develop HEAD origin/develop `903c8fcb` (PR #202 S-5.04 sensor-health merged D-1319). LOCAL develop `e4506a00` (LOCAL-ONLY: chore(skills): add /wrap session-durability skill — DIVERGED at 5504c152; HUMAN DECISION PENDING reconciliation). factory-artifacts HEAD: run `git -C .factory log -1 --format='%h %s'` (do not hard-code). STATE v7.949.
