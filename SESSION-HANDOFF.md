...
>
> **PRIORITY READ ORDER:** Read §ACTIVE OBJECTIVE (North Star) FIRST, then **D-1618 pass-7 fix-burst closure** (above; current state). STATE.md frontmatter (`develop_head`, `current_step`) is authoritative.
> **SOURCE-OF-TRUTH FOR CURRENT PIPELINE POSITION:** D-1618 bullet above + STATE.md frontmatter. §RESUME SNAPSHOT D-1615 below is the durable resume snapshot (latest SESSION WRAP; still valid). §RESUME SNAPSHOT D-1600 below that is historical. `.factory/objectives/DEMO-SCOPE.md` is the demo SCOPE/NARRATIVE reference — not the live pipeline tracker.
> develop HEAD origin/develop `f935edb6` (PR #217 S-PRISMQL-CASE-INSENSITIVE-001 squash-merged 2026-07-08T21:18:56Z). factory-artifacts HEAD: run `git -C .factory log -1 --format='%h %s'` (do not hard-code). STATE v8.218. D-1618 (pass-7 fix-burst closure; FIX-IEQ-ERRPATH-001 streak 0/3; BC-2.11.016 v1.15 SIBLING-GATE CONSISTENCY provenance; VERY NEXT ACTION: freeze eafe10c2 → LOCAL adversary pass 8).
...
**STATE v8.218. develop f935edb6 (PUSHED, origin==local). BC-INDEX v7.67. STORY-INDEX v2.639. ARCH-INDEX v2.174. error-taxonomy v2.29. active_contracts 257. draft_contracts 0. non-exhaustive EXPECTED=89. total_stories 228. workspace_test_count 5360 (fix-branch eafe10c2 LOCAL-ONLY; develop baseline 5319 @f935edb6). bc_count_corrected 266.**
...
**HEADS:** develop f935edb6 (PUSHED, origin==local); factory-artifacts = this burst commit (PUSHED); fix/IEQ-nonexistent-column-errpath @ eafe10c2 (LOCAL-ONLY, ~22 commits; 5360/5360 GREEN; cannot push until LOCAL 3-CLEAN converged). Parked: S-3.09 @43c41389 (local-only, backup/S-3.09-preresume-43c41389) + W3-FIX-S307-001 @fcab8717 (local-only, 1 unpushed commit + ~1070 lines uncommitted write_dispatch_tests.rs — do NOT touch).

**OPEN FIX-CASCADE:** FIX-IEQ-ERRPATH-001 — fix-branch eafe10c2 (LOCAL-ONLY). CLOSES: DRIFT-IEQ-NONEXISTENT-COL-ERRPATH-001 + audit-script 62→70 extension (DRIFT-AUDIT-SCRIPT-UNCOMMITTED-001 at merge). 7 passes done; streak 0/3. 5360/5360 GREEN. RESUME NEXT-ACTION: freeze eafe10c2 → LOCAL adversary pass 8 (fresh context, strict; no commits between passes per DRIFT-ORCH-PRLEVEL-PUSH-001). Pass files: .factory/cycles/wave-5-e-demo-fidelity/FIX-IEQ-ERRPATH-001/adversarial-review/.

**WORKTREES:**
- `.worktrees/FIX-IEQ-ERRPATH-001` ACTIVE on `fix/IEQ-nonexistent-column-errpath` @ d983613b (LOCAL-ONLY; do NOT push until LOCAL 3-CLEAN converged)
...
**DECISION DELTA (since last wrap D-1600):** D-1607..D-1618 this session: PR #217 merged; RUSTSEC fixed; demo audit rerun; FIX-IEQ-ERRPATH-001 LOCAL cascade 7 passes; BC-2.11.016 v1.12→v1.15 (head-projection rule + MIXED-STAR branch (c) + SIBLING-GATE CONSISTENCY provenance + FROM-ALIAS + FIELDS TRANSITION); BC-2.11.017 v1.3→v1.4 E-QUERY-002 cross-ref; L21+L22 codified.
