...
- BC-INDEX v4.53, VP-INDEX v1.29, HOLDOUT-INDEX v1.3, invariants.md v1.5, L2-INDEX v1.13, STORY-INDEX v2.33 (149 stories), ARCH-INDEX v2.37, module-decomposition v1.16
...
## PASS-2-RESIDUAL CHECKPOINT (2026-05-10 — D-335, ADR-023 pass-2)

State is durable. ADR-023 adversary pass-2 complete. 16 findings (2C/4H/5M/3L/2O). 2 residuals + 14 new defects. Streak resets to 0/3. 2 process-gap TDs registered. Bundle B Phase B-2 remains BLOCKED pending Wave 0+1 plugin foundation.

**Top 3 findings (pass-2):**

1. **F-CRIT-NEW-001-PASS2-RESIDUAL** — spec_parser.rs has zero CustomAdapter references. ADR-023 v1.1 §C Rule 3 body (L220-222) and §F Implementation Note (L475-476) both assert spec_parser.rs uses CustomAdapter. The file has zero occurrences. The v1.1 fix-burst adopted pass-1 proposed-fix language verbatim; that language itself contained the same factual error. Root cause: architect did not read spec_parser.rs before authoring the fix. Verification would have taken 30 seconds.

2. **F-CRIT-NEW-002** — Sandbox URL-allowlist claim contradicts implementation. ADR-023 v1.1 §D closes the sandbox-escape finding by asserting "host_http_request validates the target URL against the sensor spec's allowed_urls list." Implementation at `crates/prism-spec-engine/src/plugin/mod.rs:165`: `allowed_urls: None, // TODO: populate from sensor spec`. The ADR now actively asserts a security control is implemented when the implementation explicitly marks it as a TODO.

3. **F-HIGH-NEW-003** — ADR-022 v1.2 amendment is scheduled simultaneously as a Wave 1 exit gate (L50-54) AND a Wave 2/G story (L734-738, L775-778). Mutually exclusive scheduling decisions. An implementer reading §A will treat it as a Wave 1 prerequisite; an implementer reading §F will treat it as Wave 2 concurrent work.

**Meta-process-gap insight (root cause of both residuals):**

Both residuals (F-CRIT-NEW-001-PASS2-RESIDUAL + F-MED-NEW-001-PASS2-RESIDUAL) share one structural root cause: the fix-burst-1 architect adopted adversary proposed-fix language verbatim without verifying the underlying factual claims against source-of-truth. The adversary writes from information-asymmetric context (read-only, no code execution) — proposed fixes are directionally correct but may be factually imprecise. Verbatim adoption without verification propagates adversary errors directly into the specification.

**Verification discipline TD (new — must land before next fix-burst):**
- TD-FIX-BURST-VERIFY-001 (P2, methodology): "Before adopting any adversary proposed-fix language verbatim into a spec body, the architect MUST verify the underlying factual claim against current source-of-truth (BC, code, audit). PR review checklist must include: 'I verified each adopted proposed-fix claim against source-of-truth.'"

**Process-gap TDs (all registered, not blocking ADR-023 convergence):**
- TD-ADR-AMEND-001 (P2, methodology): AUGMENTED — now includes bidirectional BC lifecycle consistency requirement
- TD-FIX-BURST-VERIFY-001 (P2, methodology): NEW — architect verify-before-adopt discipline
- TD-AUDIT-ADR-001, TD-USER-DECISION-001, TD-SIGNING-PREREQ-001, TD-ADR-OPEN-Q-001 — unchanged from pass-1

**Fix-burst-2 sequence:**
1. architect dispatch → ADR-023 v1.2 amendment (closes all 16 pass-2 findings; must read source files before authoring any fix language)
2. adversary dispatch → ADR-023 pass-3 (target CLEAN, streak 1/3)

**Predecessor session:** D-334 (ADR-023 pass-1 + fix-burst plan) → D-335 (ADR-023 pass-2 NOT_CLEAN)

**User mandate verbatim (2026-05-10):** "we arent suppose to have anything built in, everything uses the plugin system. We need to do a full audit"

**Audit summary:** 21 violations (8 CRIT / 5 HIGH / 3 MED / 3 LOW / 2 OBS). Full catalog at `.factory/cycles/wave-4-operations/audits/plugin-only-violations-2026-05-10.md`. Three root-cause clusters: (1) closed `SensorType` enum in prism-core is the architectural keystone locking all downstream dispatch; (2) four named Rust auth modules in prism-sensors with sealed `SensorAuth` trait; (3) five sensor-name `match` dispatch sites in prism-query.

**5 user decisions (2026-05-10):**
1. OCSF mapping: hybrid TOML column `ocsf_field` (80%) + in-repo `.prx` WASM transformers (20%); drop 4 in-tree Rust mappers
2. SensorAuth sealing: un-seal entirely; remove `private::Sealed` marker
3. TOML authorship: reverse-engineer from existing Rust adapters with DTU-parity tests
4. CrowdStrike OAuth2 refresh-on-401: in-repo `.prx` WASM plugin signed by us, loaded by `PluginRuntime` at boot
5. `CustomAdapter` Rust trait RETIRED — `.prx` WASM is sole escape hatch (eat own dog food)
