...
## PASS-4-RESIDUAL-AND-CASCADE CHECKPOINT (2026-05-10 — D-337, ADR-023 pass-4)

State is durable. ADR-023 adversary pass-4 complete. 14 findings (3C/5H/4M/3L/2OBS). Trajectory REVERSED for first time: 26→16→12→14. Streak 0/3 unchanged. 2 pass-3 residuals remain deferred-not-closed. 12 new cascade defects from v1.3 Wave 1/E rescope. 2 new process-gap TDs registered. Bundle B Phase B-2 remains BLOCKED pending Wave 0+1 plugin foundation.

**Trajectory reversal root cause:** Fix-burst-3 removed PLUGIN-MIGRATION-001-E from Wave 1 scope but only updated the Wave 1 table body — not the 5+ sites citing story counts and SP arithmetic. Additionally, architect used Python open/write (non-Edit/Write) to mutate files, bypassing factory-dispatcher hooks that would have caught the inconsistencies at write-time.

**Top 3 findings (pass-4):**

1. **F-PASS4-CRIT-001** — Story count 12 vs 13 contradicts at 5+ sites simultaneously (§C Status, §D Negative Consequences, §D Migration Plan heading, §E Wave 1 header row, §F Source/Origin). Wave 1 rescoped from 5→4 stories but only the table body was updated.

2. **F-PASS4-CRIT-002** — Wave 1 SP arithmetic 30-47 claimed vs actual 32-52 for 4 remaining stories (A+B+C+D at 8-13 SP each). Three-wave total 95-138 claimed vs actual 95-145. Error propagated from incorrect subtraction after removing PLUGIN-MIGRATION-001-E.

3. **F-PASS4-CRIT-003 [process-gap]** — Fix-burst-3 architect used Python open/write (non-Edit/Write tool path) to mutate ADR-023 files, bypassing factory-dispatcher validate-changelog-monotonicity hook. Bypass causally connected to CRIT-001/002. Left stale "COMMITTED v1.2" stamp in v1.3 changelog row and v1.0+1 leftovers at body sites.

**New process-gap TDs registered (D-337):**
- TD-FACTORY-HOOK-BYPASS-001 (P1): Python open/write or any non-Edit/Write file mutation in `.factory/` is policy-forbidden. Required actions: (1) CLAUDE.md update; (2) fix validate-changelog-monotonicity to permit two-Edit atomic pattern; (3) post-commit hook for bypass detection; (4) architect agent prompt explicit prohibition.
- TD-FIX-BURST-VERIFY-002 scope extension: arithmetic claims (story counts, SP totals) added to citation-integrity validator scope (was VP/BC/policy citations only).

**Fix-burst-4 prerequisites (MUST complete before architect dispatch):**
1. Codify TD-FACTORY-HOOK-BYPASS-001 P1 — add prohibition to architect agent prompt + CLAUDE.md
2. Extend TD-FIX-BURST-VERIFY-002 scope to arithmetic claims in architect checklist

**Fix-burst-4 sequence:**
1. architect dispatch → ADR-023 v1.4 amendment (closes all 14 pass-4 findings; Edit/Write tool path ONLY)
2. adversary dispatch → ADR-023 pass-5 (target CLEAN, streak 1/3)

**User mandate (2026-05-10):** continue full 3-CLEAN cycle, no shortcuts.

**Predecessor session:** D-336 (ADR-023 pass-3 NOT_CLEAN) → D-337 (ADR-023 pass-4 NOT_CLEAN)
