...
successor_focus: "## POST-COMPACT FIRST ACTION (D-428 PRE-COMPACT CHECKPOINT)\n\nDispatch story-writer for S-PLUGIN-PREREQ-C with the user-authorized scope from D-428:\n\n**Story title (already in STORY-INDEX line 391):** \"prism-spec-engine: TOML Grammar Extensions — batch, retry, two-step-fetch, ocsf_field, cache_ttl, table_name [PLUGIN-MIGRATION Wave 0]\"\n\n**Explicit ACs (story-writer must write each as a verifiable AC with Red Gate test pointer):**\n1. AC for TD-S-PLUGIN-PREREQ-B-001 P2: `PaginationConfig::CursorToken` adds `page_size: Option<u32>` field; `build_paged_url()` appends `page_size` query parameter to BOTH first-call and cursor-continuation URLs when set. Real-world test vehicle: CrowdStrike GraphQL `first: N` semantic.\n2. AC for TD-S-PLUGIN-PREREQ-B-003 P3: `extract_at_path` supports bracket notation `$.x[0]` (array indexing) AND wildcards `$.x[*]` (array enumeration). Implementation choice: extend in-tree JSON Pointer OR add dependency on `jsonpath-rust`. Story-writer should defer the choice to the implementer/architect but require the test surface.\n3. AC for TD-S-PLUGIN-PREREQ-B-006 P2: proptest fixtures for `fan_out_batches`, `extract_at_path`, `Interpolator::interpolate`, `Interpolator::extract_references`. Properties: totality, batch-size bounds, adversarial-input round-trip.\n4. AC for TD-S-PLUGIN-PREREQ-B-008 P3: `${...}` literal escape mechanism (e.g., `$${...}` -> literal `${...}`); Interpolator regex + replacement updated; spec authors can embed template syntax in documentation strings.\n5. AC for TD-S-PLUGIN-PREREQ-B-016 P2: `#[non_exhaustive]` applied to ALL pub TOML-deserialized types in prism-spec-engine: `CredentialRef`, `SensorSpec`, `SensorTableDescriptor`, `FetchStep`, `ColumnSpec`, `TableSpec`, `PaginationConfig`, `AuthType` (per pass-10 F-LP10-LOW-001 enumeration). Verify external struct-literal construction requires `..Default::default()`.\n6. AC for TD-S-PLUGIN-PREREQ-A-006 P3: cross-newtype audit of pub-API validation-bypass entry points in prism-core (e.g., `OrgSlug::new_unchecked` and any sibling `*::new_unchecked`); enumerate, justify each existence, optionally feature-gate behind `cfg(test, feature = \"test-helpers\")`.\n7. AC for TD-S-PLUGIN-PREREQ-A-008 P3: re-export `SensorIdValidationError` at `prism_core` crate root for ergonomic consistency with PR-A pub-API surface (currently buried at `prism_core::sensor_id::SensorIdValidationError`).\n\n**Optional 8th AC (story-writer's discretion if scope tolerates):**\n- AC for TD-S-PLUGIN-PREREQ-A-007 P3: Reorder `validate_sensor_id_string` to length-check before charset-check (micro-performance — bundles cheaply with #[non_exhaustive] audit).\n\n**Dependencies (verify in story frontmatter):**\n- depends_on: [S-PLUGIN-PREREQ-A (merged develop@ae7e26c8), S-PLUGIN-PREREQ-B (merged via PR #143 in develop@ae7e26c8)]\n- subsystems: [SS-16 prism-spec-engine, SS-01 prism-core]\n- capabilities: [CAP-029 (plugin sensor architecture), CAP-001 (sensor identity)]\n- estimated_passes: 8-12 LOCAL adversary (per scope size; recall PREREQ-B took 16 from 20->0)\n\n**Story-writer must:**\n- Read /Users/jmagady/Dev/prism/.factory/specs/architecture/decisions/ADR-023-plugin-only-sensor-architecture.md (TOML grammar baseline)\n- Read /Users/jmagady/Dev/prism/.factory/specs/behavioral-contracts/BC-2.16.002-multi-step-fetch-pipeline.md (v1.9 active — PipelineExecutor contract)\n- Read /Users/jmagady/Dev/prism/.factory/tech-debt-register.md (TD-001/003/006/008/016 + TD-A-006/008 for AC text source-of-truth)\n- Decompose into 7-8 ACs as enumerated above\n- File path: /Users/jmagady/Dev/prism/.factory/stories/S-PLUGIN-PREREQ-C-toml-grammar-extensions-plus-pub-api-hardening.md\n- Update STORY-INDEX row at line 391 from \"planned\" to \"ready\" with new line count + AC count + Red Gate count placeholder\n- Per /Users/jmagady/Dev/prism/.factory/cycles/wave-4-operations/lessons.md Lesson 1: any new tracing event_type sites introduced in PREREQ-C MUST amend BC-2.16.002 catalog (currently 14 rows at v1.9 active).\n\nAfter story-writer completes, dispatch test-writer for Red Gate stubs (per per-story-delivery step b), then implementer for Green TDD (step c). Estimated LOCAL adversary cycle 8-12 passes; PREREQ-C will then merge enabling PLUGIN-MIGRATION-001-B (prism-query dispatch site conversion) and PLUGIN-MIGRATION-001-D (production TOML sensor specs).\n\n**Pin SHAs at D-428:** develop@ae7e26c8 | factory-artifacts HEAD: run `git -C .factory log -1 --format='%H'` (per TD-VSDD-053) | worktree S-PLUGIN-PREREQ-B retained at b75f317e (archival; future cleanup).\n\n**Spec versions at D-428:** Story S-PLUGIN-PREREQ-B v1.23 (status: merged), BC-2.16.002 v1.9 (active), STORY-INDEX v2.61, BC-INDEX v4.60 (active_contracts 228), VP-INDEX v1.32, tech-debt-register v2.14, BC-2.01.013 v1.6 (active, merged PR #142). STATE v7.162 SESSION-HANDOFF v7.162 (current)."

**CURRENT STATE (2026-05-12, D-428 PRE-COMPACT CHECKPOINT)**

**Pin SHAs:**
- develop HEAD: ae7e26c8 (PR #143 PREREQ-B merged 2026-05-12T06:58:48Z)
- origin/feature/S-PLUGIN-PREREQ-B: b75f317e (retained for archival; worktree can be cleaned up)
- factory-artifacts HEAD: run `git -C .factory log -1 --format='%H'` (per TD-VSDD-053)
- STATE.md / SESSION-HANDOFF: v7.161 → v7.162

**D-428 — PRE-COMPACT CHECKPOINT. PREREQ-C scope authorized. TD Routing Matrix codified.**
- PREREQ-C must-close TDs (7): TD-B-001/003/006/008/016 + TD-A-006/008 (all in PREREQ-C ACs)
- Routing matrix covers 25 entries total (24 active + 1 closed TD-B-010)
- See STATE.md "Carry-Forward TD Routing Matrix" section for full table
- POST-COMPACT FIRST ACTION: dispatch story-writer for S-PLUGIN-PREREQ-C
