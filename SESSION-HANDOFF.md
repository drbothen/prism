...
## PASS-3-RESIDUAL CHECKPOINT (2026-05-10 — D-336, ADR-023 pass-3)

State is durable. ADR-023 adversary pass-3 complete. 12 findings (1C/4H/4M/1L/2OBS). 14/14 pass-2 spec defects substantively closed (2 partial-close caveats). 10 new defects. Streak resets to 0/3. 2 process-gap TDs registered. Bundle B Phase B-2 remains BLOCKED pending Wave 0+1 plugin foundation.

**Top 3 findings (pass-3):**

1. **F-PASS3-CRIT-001** — ADR-023 v1.2 frontmatter contains `amends_bcs_pending_full_amendment_in_wave_2_g:`, a wave-specific non-generic field not in any ADR template schema and not validated by any hook. Adopted verbatim from pass-2 adversary proposed-fix without verifying reusability — a recurrence of the verbatim-adoption pattern TD-FIX-BURST-VERIFY-001 was intended to eliminate, but whose scope was limited to body prose rather than frontmatter schema choices. Fix: rename to generic `amends_bcs_pending:` list form. Register TD-ADR-AMEND-002.

2. **F-PASS3-HIGH-001** — VP-PLUGIN-006 referenced in ADR-023 v1.2 §E but absent from VP-INDEX.md. The entire VP-PLUGIN-* series does not exist in VP-INDEX. Broken reference introduced in fix-burst-2 by citing from memory without reading VP-INDEX. Fix: author VP-PLUGIN-006 or replace citation with existing VP.

3. **F-PASS3-HIGH-002** — POL-11 miscited as `ci_positive_coverage_assertion` in ADR-023 v1.2 §E. Actual POL-11 in policies.yaml is `index_bump_required_for_index_mutations`. TD-FIX-BURST-VERIFY-001 scope was too narrow (body prose only); TD-FIX-BURST-VERIFY-002 escalated to P1 to cover ALL inline citations.

**Process-gap TDs (newly registered, not blocking ADR-023 convergence):**
- TD-ADR-AMEND-002 (P2, methodology): NEW — generic amends_bcs_pending schema for ADR template; wave-specific field names are not reusable
- TD-FIX-BURST-VERIFY-002 (P1, methodology): NEW — citation-integrity validator; P1 escalated from P2 because pass-3 shows TD-FIX-BURST-VERIFY-001 alone didn't prevent recurrence across ALL inline citations

**Fix-burst-3 sequence:**
1. architect dispatch → ADR-023 v1.3 amendment (closes all 10 pass-3 new defects; architect MUST grep-verify every POL/VP/BC/SS citation against source-of-truth — extending TD-FIX-BURST-VERIFY-001 discipline to ALL inline citations, not just body prose)
2. adversary dispatch → ADR-023 pass-4 (target CLEAN, streak 1/3)

**User mandate (2026-05-10):** continue full 3-CLEAN cycle; close process-gap TDs (TD-ADR-AMEND-002 + TD-FIX-BURST-VERIFY-002) BEFORE Wave 0 dispatch.

**Predecessor session:** D-335 (ADR-023 pass-2 NOT_CLEAN) → D-336 (ADR-023 pass-3 NOT_CLEAN)
