...
successor_focus: "## NEXT ACTION (D-430 — PREREQ-C RED GATE ESTABLISHED, GREEN TDD READY)\n\nDispatch implementer for S-PLUGIN-PREREQ-C Green TDD per per-story-delivery step c. Worktree at /Users/jmagady/Dev/prism/.worktrees/S-PLUGIN-PREREQ-C/ on feature/S-PLUGIN-PREREQ-C@f97e91d1.\n\n**Implementation order (test-writer recommended sequence — minimizes blast radius):**\n\n1. **AC-5 first (~30min)** — pub-API stability gate. Add #[non_exhaustive] to 8 types in prism-spec-engine: CredentialRef, SensorSpec, SensorTableDescriptor, FetchStep, ColumnSpec, TableSpec, PaginationConfig, AuthType. Fix any internal blast-radius breakage (struct-literal constructions need ..Default::default() or struct-update syntax). Verify tests/external/non-exhaustive-violation/ now FAILS to compile (gate passes).\n\n2. **AC-7 (~5min)** — one-line change: add `pub use sensor_id::SensorIdValidationError;` to crates/prism-core/src/lib.rs. Verify test_AC_7 doctest/test passes.\n\n3. **AC-6 (~30min)** — feature-gate OrgSlug::new_unchecked behind cfg(any(test, feature = \"test-helpers\")) OR document allowlist marker. Verify no production callers exist. Update new_unchecked_audit.rs allowlist if applicable.\n\n4. **AC-1 (~30min)** — update build_paged_url in pipeline.rs to append page_size query param when PaginationConfig::CursorToken::page_size is Some. Both first-call and cursor-continuation paths. Verify all 4 failing AC-1 tests turn green.\n\n5. **AC-4 (~45min)** — update Interpolator regex in interpolation.rs to recognize $${...} as literal escape. Round-trip property: escape(interpolate(s)) ≡ s for adversarial input. Verify all 2 failing AC-4 tests turn green.\n\n6. **AC-2 (~2-3h)** — JSONPath bracket [N] indexing + wildcard [*] enumeration in extract_at_path. Decision: extend in-tree pointer parser OR add jsonpath-rust dep. Add bounds-check structured error variant (may introduce new tracing::*!(event_type = \"...\") site — if so, MUST amend BC-2.16.002 Structured Event Catalog in SAME atomic commit per PG-LP11-001). Verify 2 failing AC-2 tests turn green.\n\n7. **AC-3 sentinel cleanup (~15min)** — expose extract_at_path under cfg(test) so the proptest in tests/proptest_AC_3.rs can call it OR move proptest into pipeline.rs as in-process module. Verify proptest_extract_at_path_totality_sentinel turns green.\n\n**Critical hooks/discipline:**\n- TD-VSDD-091 anti-volatile-pin: NO file:line citations in commit messages or code comments.\n- TD-VSDD-053 single-commit-per-burst: each AC's implementation should be ONE commit on feature/S-PLUGIN-PREREQ-C. Do NOT chain multiple consecutive fix-up commits with the same theme.\n- PG-LP11-001 (cycles/wave-4-operations/lessons.md): BC-2.16.002 catalog amendments must land in the same atomic commit as new event_type emission sites.\n- POL-7 H1 source-of-truth: AC text in story file is the contract — match it.\n\n**After all 7 ACs green:**\n- Run `just check` (full workspace gate) — MUST be clean\n- Run `just iter prism-spec-engine` and `just iter prism-core` — all relevant tests green\n- Dispatch state-manager to log D-431 (Green TDD complete)\n- Dispatch adversary for first LOCAL pass (per-story-delivery step 4 — 3-CLEAN cascade begins)\n\n**Pin SHAs at D-430:** develop@ae7e26c8 | factory-artifacts: run `git -C .factory log -1 --format='%H'` | feature/S-PLUGIN-PREREQ-C@f97e91d1 | worktree red-gate-established."

**CURRENT STATE (2026-05-12, D-430 — S-PLUGIN-PREREQ-C RED GATE ESTABLISHED)**

**Pin SHAs:**
- develop HEAD: ae7e26c8 (PR #143 PREREQ-B merged 2026-05-12T06:58:48Z)
- feature/S-PLUGIN-PREREQ-C HEAD: f97e91d1 (red-gate-established)
...
- STATE.md / SESSION-HANDOFF: v7.163 → v7.164

**D-430 — S-PLUGIN-PREREQ-C Red Gate established. 11 failing tests across 7 ACs.**
- Worktree at: /Users/jmagady/Dev/prism/.worktrees/S-PLUGIN-PREREQ-C/ on feature/S-PLUGIN-PREREQ-C@f97e91d1
- AC-1 (4 fail/2 pass), AC-2 (2 fail/2 pass), AC-3 (1 fail/4 pass), AC-4 (2 fail/1 pass), AC-5 (1 fail compile-fail crate), AC-6 (1 fail), AC-7 (1 fail)
- 543 pre-existing tests still pass (no regressions)
- Minimal stub: page_size: Option<u32> added to PaginationConfig::CursorToken with #[serde(default)]
- AC-5 uses tests/external/non-exhaustive-violation/ pattern (no trybuild dep)
- NEXT: implementer Green TDD (per-story-delivery step c)
