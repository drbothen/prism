...
- S-PLUGIN-PREREQ-A story: v1.3 (input-hash 6954524; OPTION B &SensorId canonical API; task 7 + AC-4 amended D-387)
...
**NEXT ACTION:**
S-PLUGIN-PREREQ-A LOCAL pass-7 BLOCKED-soft (D-387). Dispatch implementer fix-burst-7 against feature/S-PLUGIN-PREREQ-A (post-fix-burst-6 SHA bc57c80d): Add `test_sensor_id_borrow_str_lookup` to `crates/prism-core/src/sensor_id.rs` tests module (~8 lines). Test body: construct `HashMap<SensorId, u32>`, insert via `SensorId::from("armis")`, look up via `map.get("armis" as &str)`, assert `Some`. Verify `just iter prism-core test_sensor_id_borrow_str_lookup` passes. After fix-burst-7 commits, dispatch adversary pass-8 fresh-context (expected CLEAN; streak 0/3 → 1/3)."
---
