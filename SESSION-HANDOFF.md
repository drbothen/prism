...
**Step 1: test-writer Red Gate stubs**
- Create worktree `.worktrees/S-PLUGIN-PREREQ-D/` (devops-engineer if missing)
- 25 named tests: 7 in prism-bin (`test_BC_2_22_001_boot_step_plugin_load_placement`, `test_BC_2_22_001_plugin_load_failure_exits_code_4`, `test_BC_2_22_001_plugin_load_disabled_env`, `test_BC_2_22_001_disable_env_takes_precedence_over_plugin_dir_config`, plus 3 more from §Red Gate Tests prism-bin block) + 18 in prism-spec-engine (see story §Red Gate Tests)
- All stubs COMPILE and ALL FAIL with `todo!()` or `#[should_panic(expected = "not yet implemented")]`
- Red Gate verification: `just iter prism-bin` + `just iter prism-spec-engine` show all 25 FAIL correctly

**Step 2: Red Gate confirmation gate**
- ALL 25 stubs compile (workspace builds)
- ALL 25 tests fail (zero implementation logic present)
- BC-5.38.001 Red Gate discipline confirmed before proceeding

**Step 3: implementer TDD green**
