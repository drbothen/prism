...
>
> **PRIORITY READ ORDER:** Read §ACTIVE OBJECTIVE (North Star) FIRST, then **D-1621 pass-10 fix-burst closure** (above; current state). STATE.md frontmatter (`develop_head`, `current_step`) is authoritative.
> **SOURCE-OF-TRUTH FOR CURRENT PIPELINE POSITION:** D-1621 bullet above + STATE.md frontmatter. §RESUME SNAPSHOT D-1615 below is the durable resume snapshot (latest SESSION WRAP; still valid). §RESUME SNAPSHOT D-1600 below that is historical. `.factory/objectives/DEMO-SCOPE.md` is the demo SCOPE/NARRATIVE reference — not the live pipeline tracker.
> develop HEAD origin/develop `f935edb6` (PR #217 S-PRISMQL-CASE-INSENSITIVE-001 squash-merged 2026-07-08T21:18:56Z). factory-artifacts HEAD: run `git -C .factory log -1 --format='%h %s'` (do not hard-code). STATE v8.221. D-1621 (pass-10 fix-burst closure; FIX-IEQ-ERRPATH-001 streak 0/3; LAST-SEGMENT OUTPUT-NAME RULE EC-11-069 — GREEN @e5170899; VERY NEXT ACTION: freeze e5170899 → LOCAL adversary pass 11).
...
**OPEN FIX-CASCADE:** FIX-IEQ-ERRPATH-001 — fix-branch e5170899 (LOCAL-ONLY). CLOSES: DRIFT-IEQ-NONEXISTENT-COL-ERRPATH-001 + audit-script 62→70 extension (DRIFT-AUDIT-SCRIPT-UNCOMMITTED-001 at merge). 10 passes done; streak 0/3. 5369/5369 GREEN. BC-2.11.016 v1.17 (LAST-SEGMENT OUTPUT-NAME RULE). RESUME NEXT-ACTION: freeze e5170899 → LOCAL adversary pass 11 (fresh context, strict; no commits between passes per DRIFT-ORCH-PRLEVEL-PUSH-001); if CLEAN(strict), passes 12 and 13 on unchanged HEAD complete BC-5.39.001 3-CLEAN → push branch → open fix-PR. Pass files: .factory/cycles/wave-5-e-demo-fidelity/FIX-IEQ-ERRPATH-001/adversarial-review/.
...
**DECISION DELTA (since last wrap D-1600):** D-1607..D-1621 this session: PR #217 merged; RUSTSEC fixed; demo audit rerun; FIX-IEQ-ERRPATH-001 LOCAL cascade 10 passes; BC-2.11.016 v1.12→v1.17 (head-projection rule + MIXED-STAR branch (c) + SIBLING-GATE CONSISTENCY provenance + FROM-ALIAS + FIELDS TRANSITION + Enrich-bullet gate-ordering note fix + LAST-SEGMENT OUTPUT-NAME RULE EC-11-069); BC-2.11.017 v1.3→v1.5; L21+L22 codified; 5 alias-qualified regression locks positions 10-14 @7a2a0f73; EC-11-041 single-tenant zero-column gate @656bc7a0; EC-11-069 join-qualified last-segment seeding @e5170899.
