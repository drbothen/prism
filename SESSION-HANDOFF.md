...
>
> **PRIORITY READ ORDER:** Read §ACTIVE OBJECTIVE (North Star) FIRST, then **D-1624 LOCAL pass-13 CLEAN(strict)=YES — streak 1/3; fix-branch 09ea9979** (above; current state). STATE.md frontmatter (`develop_head`, `current_step`) is authoritative.
> **SOURCE-OF-TRUTH FOR CURRENT PIPELINE POSITION:** D-1624 bullet above + STATE.md frontmatter. §RESUME SNAPSHOT D-1615 below is the durable resume snapshot (latest SESSION WRAP; still valid). §RESUME SNAPSHOT D-1600 below that is historical. `.factory/objectives/DEMO-SCOPE.md` is the demo SCOPE/NARRATIVE reference — not the live pipeline tracker.
> develop HEAD origin/develop `f935edb6` (PR #217 S-PRISMQL-CASE-INSENSITIVE-001 squash-merged 2026-07-08T21:18:56Z). factory-artifacts HEAD: run `git -C .factory log -1 --format='%h %s'` (do not hard-code). STATE v8.224. D-1624 (LOCAL pass-13 CLEAN(strict)=YES; streak 1/3; fix-branch 09ea9979; VERY NEXT ACTION: LOCAL adversary pass 14 on FROZEN HEAD 09ea9979).
...
**STATE v8.224. develop f935edb6 (PUSHED, origin==local). BC-INDEX v7.70. STORY-INDEX v2.642. ARCH-INDEX v2.174. error-taxonomy v2.31. active_contracts 257. draft_contracts 0. non-exhaustive EXPECTED=89. total_stories 228. workspace_test_count 5371 (fix-branch 09ea9979 LOCAL-ONLY; develop baseline 5319 @f935edb6). bc_count_corrected 266. [Updated D-1624: LOCAL pass-13 CLEAN(strict); streak 1/3; no code changes]**
...
**HEADS:** develop f935edb6 (PUSHED, origin==local); factory-artifacts = this burst commit (PUSHED); fix/IEQ-nonexistent-column-errpath @ 09ea9979 (LOCAL-ONLY, ~29 commits; 5371/5371 GREEN; BC-2.11.016 v1.18; cannot push until LOCAL 3-CLEAN converged). Parked: S-3.09 @43c41389 (local-only, backup/S-3.09-preresume-43c41389) + W3-FIX-S307-001 @fcab8717 (local-only, 1 unpushed commit + ~1070 lines uncommitted write_dispatch_tests.rs — do NOT touch).

**OPEN FIX-CASCADE:** FIX-IEQ-ERRPATH-001 — fix-branch 09ea9979 (LOCAL-ONLY). CLOSES: DRIFT-IEQ-NONEXISTENT-COL-ERRPATH-001 + audit-script 62→70 extension (DRIFT-AUDIT-SCRIPT-UNCOMMITTED-001 at merge). 13 passes done; streak 1/3 (pass-13 CLEAN(strict)=YES on frozen 09ea9979; no fix-burst). 5371/5371 GREEN. BC-2.11.016 v1.18 (STAR-WITH-JOIN SUSPENSION RULE + LAST-SEGMENT OUTPUT-NAME RULE). RESUME NEXT-ACTION: LOCAL adversary pass 14 on FROZEN HEAD 09ea9979 (fresh context, strict; NO commits to fix branch per DRIFT-ORCH-PRLEVEL-PUSH-001); if CLEAN, pass 15 completes 3-CLEAN → push branch → open fix-PR via pr-manager. Pass files: .factory/cycles/wave-5-e-demo-fidelity/FIX-IEQ-ERRPATH-001/adversarial-review/.

**WORKTREES:**
- `.worktrees/FIX-IEQ-ERRPATH-001` ACTIVE on `fix/IEQ-nonexistent-column-errpath` @ e5170899 (LOCAL-ONLY; do NOT push until LOCAL 3-CLEAN converged)
