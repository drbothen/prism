...
## FIX-BURST-PLAN CHECKPOINT (2026-05-10 — D-334, ADR-023 pass-1)

State is durable. ADR-023 adversary pass-1 complete. 26 findings (4C/9H/7M/4L/5O). 4 user decisions recorded verbatim. Fix-burst plan set. 5 process-gap TDs registered. STORY-INDEX v2.34 (150 stories). Bundle B Phase B-2 remains BLOCKED pending Wave 0+1 plugin foundation.

**User decisions verbatim (2026-05-10):**

1. **Rule 5 — confirmed:** "Retire CustomAdapter Rust trait — `.prx` WASM is the sole escape hatch." Eat own dog food. ADR Rule 5 stays. (Response to ADV-W4OPS-P01-MED-001.)

2. **Plugin signing — defer to v1.0+1:** v1.0 ships unsigned plugins with explicit security warning at boot and an audit log entry recording the absence of signature verification. TD-PLUGIN-SIGNING-001 (P0, v1.0+1, security). ADR-023 Negative Consequences must honestly document the v1.0 security exposure. (Response to ADV-W4OPS-P01-HIGH-002.)

3. **Wave 1 reorder — replacement before deletion:** Wave 1/E (CrowdStrike `.prx` plugin) and Wave 1/D (TOML specs with DTU parity tests) MUST land and pass parity tests BEFORE Wave 1/A (delete Rust adapters). New Wave 1 order: D → E → A → B → C. At every PR boundary in Wave 1, all four sensors must remain functional via either legacy Rust path OR new TOML+plugin path. Cutover commit (Wave 1/A) deletes Rust path only after DTU-parity tests pass for all four sensors. (Response to ADV-W4OPS-P01-HIGH-007.)

4. **Wave 0/F — add BC+DI amendments first:** New story S-PLUGIN-PREREQ-F (3-5 SP) lands FIRST in Wave 0 before any code changes: (a) deprecate the rust-escape-hatch behavioral contract; (b) amend the datasource-trait-adapter-pattern behavioral contract; (c) amend the sealed-auth-trait domain invariant; (d) sweep sensor-named behavioral contracts. All Wave 0 code stories (A-E) depend on PREREQ-F. (Response to ADV-W4OPS-P01-MED-007.)

**Fix-burst sequence:**
1. architect dispatch → ADR-023 v1.1 amendment (closes all 4 CRIT + 9 HIGH + 7 MED + 4 LOW)
2. product-owner dispatch → Wave 0/F BC+DI amendments (S-PLUGIN-PREREQ-F)
3. architect dispatch → production-runtime-wiring decision v1.2 amendment
4. adversary dispatch → ADR-023 pass-2 (3-CLEAN convergence target)

**Process-gap TDs registered (not blocking ADR-023 convergence):**
- TD-ADR-AMEND-001 (P2, methodology): ADR template needs amends_bcs/retires_bcs/amends_dis/amends_caps fields
- TD-AUDIT-ADR-001 (P2, methodology): audit-to-ADR transition needs coverage matrix
- TD-USER-DECISION-001 (P2, methodology): user-decision verbatim capture pattern
- TD-SIGNING-PREREQ-001 (P2, methodology): signing infra has dimension-rich threat model; cannot be one PREREQ bullet
- TD-ADR-OPEN-Q-001 (P3, methodology): ADR template needs "Open Questions" section

**Security TD registered (blocking at v1.0+1):**
- TD-PLUGIN-SIGNING-001 (P0, v1.0+1, security): plugin .prx signing infrastructure — key management, trust anchor, signature scheme, verification in loader, revocation

**Predecessor session:** D-332 (PRE-COMPACT) → D-333 (PLUGIN-AUDIT-001 architectural pivot)
