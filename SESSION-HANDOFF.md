...
>
> **PRIORITY READ ORDER:** Read §ACTIVE OBJECTIVE (North Star) FIRST, then **D-1620 pass-9 fix-burst closure** (above; current state). STATE.md frontmatter (`develop_head`, `current_step`) is authoritative.
> **SOURCE-OF-TRUTH FOR CURRENT PIPELINE POSITION:** D-1620 bullet above + STATE.md frontmatter. §RESUME SNAPSHOT D-1615 below is the durable resume snapshot (latest SESSION WRAP; still valid). §RESUME SNAPSHOT D-1600 below that is historical. `.factory/objectives/DEMO-SCOPE.md` is the demo SCOPE/NARRATIVE reference — not the live pipeline tracker.
> develop HEAD origin/develop `f935edb6` (PR #217 S-PRISMQL-CASE-INSENSITIVE-001 squash-merged 2026-07-08T21:18:56Z). factory-artifacts HEAD: run `git -C .factory log -1 --format='%h %s'` (do not hard-code). STATE v8.220. D-1620 (pass-9 fix-burst closure; FIX-IEQ-ERRPATH-001 streak 0/3; EC-11-041 single-tenant zero-column gate — GREEN @656bc7a0; VERY NEXT ACTION: freeze 656bc7a0 → LOCAL adversary pass 10).
...
**STATE v8.220. develop f935edb6 (PUSHED, origin==local). BC-INDEX v7.68. STORY-INDEX v2.640. ARCH-INDEX v2.174. error-taxonomy v2.29. active_contracts 257. draft_contracts 0. non-exhaustive EXPECTED=89. total_stories 228. workspace_test_count 5367 (fix-branch 656bc7a0 LOCAL-ONLY; develop baseline 5319 @f935edb6). bc_count_corrected 266.**
...
**HEADS:** develop f935edb6 (PUSHED, origin==local); factory-artifacts = this burst commit (PUSHED); fix/IEQ-nonexistent-column-errpath @ 656bc7a0 (LOCAL-ONLY, ~25 commits; 5367/5367 GREEN; cannot push until LOCAL 3-CLEAN converged). Parked: S-3.09 @43c41389 (local-only, backup/S-3.09-preresume-43c41389) + W3-FIX-S307-001 @fcab8717 (local-only, 1 unpushed commit + ~1070 lines uncommitted write_dispatch_tests.rs — do NOT touch).

**OPEN FIX-CASCADE:** FIX-IEQ-ERRPATH-001 — fix-branch 656bc7a0 (LOCAL-ONLY). CLOSES: DRIFT-IEQ-NONEXISTENT-COL-ERRPATH-001 + audit-script 62→70 extension (DRIFT-AUDIT-SCRIPT-UNCOMMITTED-001 at merge). 9 passes done; streak 0/3. 5367/5367 GREEN. RESUME NEXT-ACTION: freeze 656bc7a0 → LOCAL adversary pass 10 (fresh context, strict; no commits between passes per DRIFT-ORCH-PRLEVEL-PUSH-001); if CLEAN(strict), passes 11 and 12 on unchanged HEAD complete BC-5.39.001 3-CLEAN → push branch → open fix-PR. Pass files: .factory/cycles/wave-5-e-demo-fidelity/FIX-IEQ-ERRPATH-001/adversarial-review/.

**WORKTREES:**
- `.worktrees/FIX-IEQ-ERRPATH-001` ACTIVE on `fix/IEQ-nonexistent-column-errpath` @ 7a2a0f73 (LOCAL-ONLY; do NOT push until LOCAL 3-CLEAN converged)
...
**DECISION DELTA (since last wrap D-1600):** D-1607..D-1620 this session: PR #217 merged; RUSTSEC fixed; demo audit rerun; FIX-IEQ-ERRPATH-001 LOCAL cascade 9 passes; BC-2.11.016 v1.12→v1.16 (head-projection rule + MIXED-STAR branch (c) + SIBLING-GATE CONSISTENCY provenance + FROM-ALIAS + FIELDS TRANSITION + Enrich-bullet gate-ordering note fix); BC-2.11.017 v1.3→v1.4 E-QUERY-002 cross-ref; L21+L22 codified; 5 alias-qualified regression locks positions 10-14 @7a2a0f73; EC-11-041 single-tenant zero-column gate @656bc7a0.
