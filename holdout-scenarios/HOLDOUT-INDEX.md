...
Story-level holdout gate for S-REL-AGENT-VERSION-001 (migrate BOTH agent-facing version surfaces to PRISM_VERSION: Surface A = prism-mcp `PrismServer::get_info` via runtime wiring; Surface B = prism-spec-engine outbound HTTP User-Agent via per-crate build.rs + pipeline.rs migration; ADR-064 §D4 + ADR-050 §D6). HIDDEN from test-writer and implementer. SINGLE-USE. Evaluates against prism binary built from S-REL-AGENT-VERSION-001 story branch. Requires: MCP stdio session (Surface A) + Claroty DTU or HTTP echo server for User-Agent observation (Surface B). All 3 scenarios must_pass P1 (beta.1-BLOCKING per human-directed S-1).

| ID | Title | Surfaces Tested |
|----|-------|----------------|
| HS-032-001 | MCP `initialize` handshake serverInfo.version wire shape — NOT "0.1.0"; must equal PRISM_VERSION ("1.0.0-dev" local dev build) — catches hardcoded version in get_info or broken with_deps wiring at boot step 9 (ADR-064 D4 §Surface A; Surface A only) | prism-mcp (PrismServer), prism-bin (boot.rs) |
| HS-032-002 | prism-spec-engine outbound HTTP User-Agent header — NOT library crate version ("0.9.0"); must equal "prism/1.0.0-dev" — catches CARGO_PKG_VERSION not migrated in pipeline.rs or build.rs not created (ADR-064 §D4 §Surface B + ADR-050 §D6; Surface B only) | prism-spec-engine (pipeline.rs, build.rs) |
| HS-032-003 | Coherent product version — V_MCP (serverInfo.version) EQUALS V_UA (User-Agent version suffix) in same binary instance; both equal "1.0.0-dev" — catches split-wiring defect: one surface migrated, other not (ADR-064 D4 §Purpose "all four surfaces must emit the same PRISM_VERSION"; both surfaces) | prism-mcp + prism-spec-engine (combined session) |
