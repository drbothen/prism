...
**Phase:** 0 (Multi-Repo Synthesis -- Step 5) / Phase 4.B (Wave 4 Holdout Coverage) / Phase 3 Wave 0 Plugin Migration / Phase 3 DRIFT-CLAROTY-AUDITLOG-TIMEOUT-001 / Phase 3 S-ADR058-OCSF-ROUTING-001 A+W re-gate / Phase 3 Wave A xDome Expansion (S-CLAROTY-VULNS-001 + S-CLAROTY-OT-EVENTS-001) / Phase 3 Wave B xDome Expansion (S-CLAROTY-DEVVULNREL-001) / Phase 3 Wave C xDome Expansion (S-CLAROTY-SERVERS-001 + S-CLAROTY-ORGPOLICY-001) / Phase 3 S-ENGINE-LIMIT-EARLY-STOP-001 holdout backfill / Phase 3 DEFECT-CLAROTY-SORTBY-DETERMINISM-001 sort_by determinism holdout
**Total Scenarios:** 125 (122 prior + 3 new HS-032 for S-REL-AGENT-VERSION-001)
**Total Groups:** 26
...
| HS-032 | [S-REL-AGENT-VERSION-001-HS-032-001-mcp-serverinfo-version-wire-shape.md](S-REL-AGENT-VERSION-001-HS-032-001-mcp-serverinfo-version-wire-shape.md), [S-REL-AGENT-VERSION-001-HS-032-002-spec-engine-user-agent-wire-header.md](S-REL-AGENT-VERSION-001-HS-032-002-spec-engine-user-agent-wire-header.md), [S-REL-AGENT-VERSION-001-HS-032-003-coherent-product-version-both-surfaces.md](S-REL-AGENT-VERSION-001-HS-032-003-coherent-product-version-both-surfaces.md) | Agent-facing version surfaces migration — S-REL-AGENT-VERSION-001 (ADR-064 §D4 + ADR-050 §D6; beta.1-BLOCKING) — HIDDEN, SINGLE-USE — **override build: PRISM_BUILD_VERSION=1.0.0-beta.1** — **[HS-032-001: PASS/CONSUMED 2026-09-06 (sat 1.00; serverInfo.version="1.0.0-beta.1" on wire); HS-032-002/003: HARNESS-BLOCKED/NOT CONSUMED — human ACCEPT-ON-SUBSTANCE; Surface-B deferred to S-REL-HOLDOUT-HARNESS-001]** | 3 | P1 | HS-032-001 MCP initialize serverInfo.version wire shape (override build PRISM_BUILD_VERSION=1.0.0-beta.1): NOT "0.1.0"; must equal "1.0.0-beta.1"; Surface A wiring (PrismServer.product_version + with_deps + boot step 9 env!("PRISM_VERSION")); non-migrated get_info retains "0.1.0" regardless of override — discriminating. HS-032-002 prism-spec-engine outbound User-Agent header (same override build): NOT library crate version "prism/0.9.0"; must equal "prism/1.0.0-beta.1"; Surface B migration (pipeline.rs env!("PRISM_VERSION") via crates/prism-spec-engine/build.rs); non-migrated CARGO_PKG_VERSION gives "prism/0.9.0" — discriminating. HS-032-003 coherence (same override build): V_MCP == V_UA == "1.0.0-beta.1" in same binary instance; primary FAIL case V_UA="prism/0.9.0" ≠ V_MCP="1.0.0-beta.1" (Surface B stale); ADR-064 §D4 §Purpose "all surfaces emit same PRISM_VERSION". |
...
### HS-031: Claroty xDome sort_by Determinism — 7 Tables Fix (P0) — DEFECT-CLAROTY-SORTBY-DETERMINISM-001

Story-level holdout gate for DEFECT-CLAROTY-SORTBY-DETERMINISM-001 (7-table sort_by fix — vulnerabilities, audit_logs, server_interfaces, organization_zones, zone_policies, firewall_groups, firewall_policies; TOML-only change to body_template strings; BCs: BC-2.16.015 v2.2, BC-2.16.013 v1.44, BC-2.16.019 v1.3, BC-2.16.020 v1.3, BC-2.16.021 v1.3). HIDDEN from test-writer and implementer. SINGLE-USE. Evaluates against `crates/prism-sensors/specs/claroty.sensor.toml` (data artifact) + Claroty DTU MCP smoke queries (prism-dtu-claroty). SAP-2 N/A (request-body fields; no response-shape impact per story Task 9). All 4 scenarios must_pass P0.

| ID | Title | Crates Tested |
|----|-------|--------------|
| HS-SORTBY-001-001 | claroty_vulnerabilities body_template: sort_by present with adjusted_vulnerability_score DESC first + unique ASC tiebreaker second; DI-019 truncation retains highest-risk records (DESC before ASC required); MCP smoke non-error (BC-2.16.015 §Postconditions §1 sort-by + EC-016-015-009) | prism-sensors (claroty.sensor.toml), prism-spec-engine, prism-dtu-claroty |
| HS-SORTBY-001-002 | claroty_audit_logs body_template: filter_by PRESERVED (not replaced) AND sort_by ADDED — coexistence invariant; timestamp in sort_by; id tiebreaker fallback acceptable; 7-day window guard not regressed from S-CLAROTY-AUDITLOG-TIMEBOX-001 (BC-2.16.013 §Postconditions §1 audit_logs sort-by + EC-016-013-011) | prism-sensors (claroty.sensor.toml), prism-spec-engine, prism-dtu-claroty |
| HS-SORTBY-001-003 | Breadth coverage: grep count of "sort_by" in claroty.sensor.toml ≥ 7 (one per each of 7 affected fetch steps); all 7 table categories confirmed; sort_by arrays well-formed (not empty); MCP smoke for server_interfaces + org_zones non-error (BCs: BC-2.16.015, BC-2.16.013, BC-2.16.019, BC-2.16.020, BC-2.16.021) | prism-sensors (claroty.sensor.toml), prism-spec-engine, prism-dtu-claroty |
| HS-SORTBY-001-004 | claroty_server_interfaces body_template: sort_by has EXACTLY 2 ASC elements; element[0] is server-level field; element[1] is interface-level field; two distinct field names (not duplicates); composite PK (server + interface) guarantees total sort order for deterministic pagination; MCP smoke LIMIT 2 non-error (BC-2.16.019 §Postconditions §1 + EC-016-019-007 composite PK) | prism-sensors (claroty.sensor.toml), prism-spec-engine, prism-dtu-claroty |

### HS-032: Agent-Facing Version Surfaces Migration (P1) — S-REL-AGENT-VERSION-001

Story-level holdout gate for S-REL-AGENT-VERSION-001 (migrate BOTH agent-facing version surfaces to PRISM_VERSION: Surface A = prism-mcp `PrismServer::get_info` via runtime wiring; Surface B = prism-spec-engine outbound HTTP User-Agent via per-crate build.rs + pipeline.rs migration; ADR-064 §D4 + ADR-050 §D6). HIDDEN from test-writer and implementer. SINGLE-USE. Evaluates against prism binary built from S-REL-AGENT-VERSION-001 story branch with `PRISM_BUILD_VERSION=1.0.0-beta.1` override. Requires: MCP stdio session (Surface A) + two-server holdout setup for Surface B: sensor data server on port 19089 (Claroty device rows) + enrichment echo server on port 19090 (User-Agent observation via `ENRICHMENT-UA:` stdout line); `PRISM_DTU_MODE=true` bypasses SSRF for localhost enrichment endpoint. All 3 scenarios must_pass P1 (beta.1-BLOCKING per human-directed S-1).

| ID | Title | Surfaces Tested | Result |
|----|-------|----------------|--------|
| HS-032-001 | MCP `initialize` handshake serverInfo.version wire shape — NOT "0.1.0"; must equal PRISM_VERSION ("1.0.0-beta.1" override build) — catches hardcoded version in get_info or broken with_deps wiring at boot step 9 (ADR-064 §D4 §Surface A; Surface A only) | prism-mcp (PrismServer), prism-bin (boot.rs) | **PASS / CONSUMED** (sat 1.00; 2026-09-06; observed `"serverInfo":{"name":"prism","version":"1.0.0-beta.1"}` on wire; scenario v1.2 used=true) |
| HS-032-002 | prism-spec-engine outbound HTTP User-Agent header (enrichment echo server observation) — NOT library crate version ("prism/0.9.0"); must equal "prism/1.0.0-beta.1" override — observed via `ENRICHMENT-UA:` line on port-19090 echo server stdout when `SELECT device_uid FROM claroty_devices LIMIT 1 \| enrich ua_result(device_uid)` is executed (HttpLookup infusion path triggers `prism-spec-engine::pipeline::build_http_client_with_timeout` — the Surface B migration target; plain sensor query uses prism-bin's already-migrated client and is non-discriminating); catches `env!("CARGO_PKG_VERSION")` not replaced by `env!("PRISM_VERSION")` in pipeline.rs (ADR-064 §D4 §Surface B + ADR-050 §D6; Surface B only) | prism-spec-engine (pipeline.rs, build.rs), prism-sensors (claroty infusion pipe stage) | **HARNESS-BLOCKED / NOT CONSUMED** (2026-09-06; 0 adapters registered at boot — enrich stage never fired; Surface-B UA not observable; deferred to S-REL-HOLDOUT-HARNESS-001; scenario v1.3 used=false) |
| HS-032-003 | Coherent product version — V_MCP (serverInfo.version from MCP initialize) EQUALS V_UA (User-Agent version suffix from enrichment echo server `ENRICHMENT-UA:` line) in same binary instance; both equal "1.0.0-beta.1" override — catches split-wiring defect: one surface migrated, other not; primary FAIL case V_UA="prism/0.9.0" ≠ V_MCP="1.0.0-beta.1" (ADR-064 §D4 §Purpose "all four surfaces must emit the same PRISM_VERSION"; both surfaces) | prism-mcp + prism-spec-engine (combined session: MCP initialize + enrichment query) | **HARNESS-BLOCKED / NOT CONSUMED** (2026-09-06; 0 adapters registered at boot — enrich stage never fired; V_MCP="1.0.0-beta.1" confirmed from HS-032-001 but V_UA unobservable; deferred to S-REL-HOLDOUT-HARNESS-001; scenario v1.3 used=false) |
...
phase: 0_and_4b_and_plugin_migration_and_drift_claroty_and_ocsf_routing_regate_and_wave_a_xdome_and_wave_b_xdome_and_wave_c_xdome_and_engine_limit_backfill_and_defect_sortby_determinism_and_s_rel_agent_version_001
step: 5_and_wave4_and_prereq_and_drift_claroty_auditlog_and_hs023_and_hs024_hs025_and_hs026_and_hs027_and_hs028_and_hs029_and_hs030_and_hs031_and_hs032
status: complete
total_scenarios: 125
total_groups: 26
p0_scenarios: 106
...
defect_sortby_determinism_groups_added: 1
defect_sortby_determinism_scenarios_added: 4
s_rel_agent_version_001_groups_added: 1
s_rel_agent_version_001_scenarios_added: 3
...
hs031_defect_claroty_sortby_determinism_001: true
hs031_consumed: true
hs031_consumed_ref: D-2429
hs032_s_rel_agent_version_001: true
hs032_001_consumed: true
hs032_001_consumed_sat: 1.00
hs032_002_003_harness_blocked: true
hs032_human_accepted: true
hs032_human_accepted_disposition: ACCEPT-ON-SUBSTANCE
hs032_deferral_anchor: S-REL-HOLDOUT-HARNESS-001
hs032_consumed: false
timestamp: 2026-09-06T00:00:00Z
```
...
|---------|-------|------|--------|--------|
| 1.42 | hs-032-gate-disposition-2026-09-06 | 2026-09-06 | product-owner | HS-032 holdout gate disposition (human-adjudicated ACCEPT-ON-SUBSTANCE 2026-09-06). HS-032-001 PASS/CONSUMED (sat 1.00; observed serverInfo.version="1.0.0-beta.1" on wire, matching PRISM_BUILD_VERSION=1.0.0-beta.1 override; ADR-064 §D4 §Surface A gate closes). HS-032-002/003 HARNESS-BLOCKED/NOT CONSUMED (0 adapters registered at boot — claroty DTU echo not wired into harness config; enrich stage never fired; build_http_client_with_timeout never called; Surface-B UA not observable end-to-end; substance guaranteed by wiremock test test_infusion_http_client_sends_prism_user_agent + AC-007 source grep + all LOCAL adversary passes). Interface errors fixed in HS-032-002/003 §Setup: --mcp-stdio removed from step 8; sql→query arg key; claroty.devices→claroty_devices table name; holdout_ua→ua_result enrichment function name. HS-032-002/003 deferred to S-REL-HOLDOUT-HARNESS-001 per Canonical Principle Rule 3. HS-032 summary row and detail table updated with dispositions. State Checkpoint: hs032_001_consumed: true + hs032_human_accepted: true + hs032_deferral_anchor: S-REL-HOLDOUT-HARNESS-001. S-REL-HOLDOUT-HARNESS-001 REGISTERED in STORY-INDEX (draft v0.1; E-REL-IDENTITY; P2; POST-beta.1). total_scenarios 125 UNCHANGED. |
| 1.41 | f-srav-high-001-enrichment-path-redesign | 2026-09-06 | product-owner | F-SRAV-HIGH-001 + F-SRAV-MED-001 fix. (1) HS-032 detail rows corrected: "1.0.0-dev" expected-value claims removed; rows rewritten to reflect override-build model (1.0.0-beta.1) and enrichment-endpoint observation for HS-032-002/003. HS-032-001 row: "1.0.0-dev" → "1.0.0-beta.1". HS-032-002 row: "prism/1.0.0-dev" → "prism/1.0.0-beta.1"; observation via enrichment echo server GET (not DTU access log). HS-032-003 row: "1.0.0-dev" → "1.0.0-beta.1"; V_UA from enrichment echo server, not DTU. (2) HS-032 intro paragraph updated: removed "Claroty DTU" as primary observation path; added enrichment echo server and PRISM_DTU_MODE. (3) HS-032-002 bumped v1.1→v1.2 (enrichment-path redesign). (4) HS-032-003 bumped v1.1→v1.2 (Surface B observation path correction). total_scenarios 125 UNCHANGED. p0_scenarios/p1_scenarios UNCHANGED. |
| 1.40 | hs-032-override-build-model-fix | 2026-09-06 | product-owner | F-SRAV-HIGH-001 holdout-design defect fix. HS-032-001/002/003 all three scenarios rewritten to override-build model (PRISM_BUILD_VERSION=1.0.0-beta.1). Root cause: on plain local dev, BOTH migrated and un-migrated Surface B emit "prism/0.9.0" — plain local-dev assertion was non-discriminating. Fix: evaluator builds with PRISM_BUILD_VERSION=1.0.0-beta.1; correct wiring gives "1.0.0-beta.1" on both surfaces; non-migrated Surface B still emits "prism/0.9.0" ≠ "1.0.0-beta.1" (discriminating FAIL). HS-032-001 expected value: "1.0.0-dev" → "1.0.0-beta.1". HS-032-002 expected value: "prism/1.0.0-dev" → "prism/1.0.0-beta.1". HS-032-003 coherence value: V_MCP == V_UA == "1.0.0-beta.1"; FAIL case now V_UA="prism/0.9.0" (genuine mismatch, not ratified local-dev state). Summary row Key Risk updated. HS-032-001/002/003 each bumped 1.0→1.1. total_scenarios 125 UNCHANGED. |
| 1.39 | s-rel-agent-version-001-local-cascade-fixes | 2026-09-06 | state-manager | D-2468: SINGLE-COMMIT BURST (TD-VSDD-053) — F-AV-HIGH-001 (POL-39/TD-VSDD-091): removed ADR version pins from HS-032 group Summary row (main table line 59) — "ADR-064 v1.8 D4 + ADR-050 v2.4 D6" → "ADR-064 §D4 + ADR-050 §D6"; HS-032-001/002/003 detailed scenario rows already used section cites (no change needed). F-MED-001 (holdout-gate blocker): git mv 3 scenario files from -HS-001-/-HS-002-/-HS-003- prefix pattern to -HS-032-001-/-HS-032-002-/-HS-032-003- — filename↔ID mismatch reconciled; Summary and Detail tables were already using -HS-032-001/002/003- IDs. HOLDOUT-INDEX filenames now match HS-NNN-SSS canonical convention. total_scenarios 125 UNCHANGED. p0_scenarios/p1_scenarios UNCHANGED. |
| 1.38 | s-rel-agent-version-001-holdout-authoring | 2026-09-05 | state-manager | D-2463: Registered HS-032 (3 scenarios for S-REL-AGENT-VERSION-001 agent-facing version surfaces migration; ADR-064 v1.8 D4 + ADR-050 v2.4 D6; beta.1-BLOCKING per human-directed S-1 2026-09-05): HS-032-001 MCP initialize serverInfo.version wire shape [NOT "0.1.0"; must equal "1.0.0-dev" on local dev build; Surface A correctness — PrismServer::get_info via self.product_version + with_deps + boot step 9 env!("PRISM_VERSION"); ADR-064 D4 §Surface A; threshold ≥ 0.75], HS-032-002 prism-spec-engine outbound User-Agent header [NOT library version; must equal "prism/1.0.0-dev"; Surface B migration — pipeline.rs env!("PRISM_VERSION") via crates/prism-spec-engine/build.rs; ADR-050 D6; threshold ≥ 0.75], HS-032-003 product-version coherence [V_MCP == V_UA == "1.0.0-dev" in same binary instance; catches split-wiring defect; ADR-064 D4 §Purpose "all surfaces emit same PRISM_VERSION"; threshold ≥ 0.75]. All 3 P1 must_pass SINGLE-USE HIDDEN. total_scenarios 122→125; total_groups 25→26; p0_scenarios 106 UNCHANGED; p1_scenarios 16→19. |
| 1.37 | defect-claroty-sortby-holdout-consumed | 2026-09-02 | state-manager | D-2429 (TD-VSDD-053): HS-031 all 4 scenarios CONSUMED — DEFECT-CLAROTY-SORTBY-DETERMINISM-001 story-level holdout gate PASS (4/4 must-pass; BC-5.39.001 LOCAL 3-CLEAN passes 10/11/12 on frozen HEAD abd7eeb9f). HS-SORTBY-001-001 satisfaction 0.90 (STRUCTURAL dims PASS; MCP-non-error SETUP-FAILURE). HS-SORTBY-001-002 satisfaction 0.90 (STRUCTURAL dims PASS; MCP-non-error SETUP-FAILURE). HS-SORTBY-001-003 satisfaction 0.90 (STRUCTURAL dims PASS; MCP-non-error SETUP-FAILURE). HS-SORTBY-001-004 satisfaction 0.95 (STRUCTURAL dims PASS; MCP-non-error SETUP-FAILURE). SETUP-FAILURE rationale: prism-dtu-claroty ships no standalone server binary; DTU only instantiable inside integration-test harnesses; corroborated structurally (valid-JSON/valid-TOML + binary post-dates spec). [process-gap] standalone Claroty DTU server for headless holdout MCP dims — single occurrence, logged not codified. Scenario files: used→true / lifecycle_status→consumed / last_evaluated 2026-09-02 / last_eval_satisfaction set per-scenario. State Checkpoint: hs031_consumed: true / hs031_consumed_ref: D-2429. total_scenarios/total_groups/p0_scenarios UNCHANGED at 122/25/106. |
| 1.36 | defect-claroty-sortby-pass5-pin-sync | 2026-09-02 | state-manager | Records-only micro-burst (TD-VSDD-096) D-2426: HS-031 group description BC version pins synced to current-state — BC-2.16.015 v2.0→v2.2 + BC-2.16.013 v1.43→v1.44 (BCs amended in D-2422/D-2423/D-2424/D-2425; description was stale). No holdout scenario files modified. total_scenarios/total_groups/p0_scenarios UNCHANGED. |
| 1.35 | defect-claroty-sortby-holdout-authoring | 2026-09-02 | product-owner | Registered HS-031 (4 scenarios for DEFECT-CLAROTY-SORTBY-DETERMINISM-001: HS-SORTBY-001-001 vulnerabilities sort_by DESC+ASC tiebreaker DI-019 truncation-relevance [adjusted_vulnerability_score DESC first + unique ASC second; DESC before ASC required for truncation to retain highest-risk CVEs; BC-2.16.015 §PC1 sort-by + EC-016-015-009], HS-SORTBY-001-002 audit_logs filter_by+sort_by coexistence [filter_by NOT replaced; timestamp in sort_by; fallback timestamp-only accepted; BC-2.16.013 §PC1 audit_logs sort-by + EC-016-013-011], HS-SORTBY-001-003 breadth coverage gate [grep count "sort_by" ≥ 7 across all affected fetch steps; well-formed arrays; MCP smoke server_interfaces+org_zones; all 5 BCs], HS-SORTBY-001-004 server_interfaces composite PK [exactly 2 ASC elements; server field + interface field; distinct names; EC-016-019-007 composite PK total-order guarantee; BC-2.16.019 §PC1+§PC3]). All 4 P0 must_pass against crates/prism-sensors/specs/claroty.sensor.toml (data artifact read) + Claroty DTU MCP smoke queries. SAP-2 N/A (request-body fields; no response-shape impact). total_scenarios 118→122; total_groups 24→25; p0_scenarios 102→106. |
