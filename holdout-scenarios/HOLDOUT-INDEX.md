...
Story-level holdout gate for S-REL-AGENT-VERSION-001 (migrate BOTH agent-facing version surfaces to PRISM_VERSION: Surface A = prism-mcp `PrismServer::get_info` via runtime wiring; Surface B = prism-spec-engine outbound HTTP User-Agent via per-crate build.rs + pipeline.rs migration; ADR-064 v1.8 D4 + ADR-050 v2.4 D6). HIDDEN from test-writer and implementer. SINGLE-USE. Evaluates against prism binary built from S-REL-AGENT-VERSION-001 story branch. Requires: MCP stdio session (Surface A) + Claroty DTU or HTTP echo server for User-Agent observation (Surface B). All 3 scenarios must_pass P1 (beta.1-BLOCKING per human-directed S-1).
...
| HS-032-002 | prism-spec-engine outbound HTTP User-Agent header — NOT library crate version ("0.9.0"); must equal "prism/1.0.0-dev" — catches CARGO_PKG_VERSION not migrated in pipeline.rs or build.rs not created (ADR-064 D4 §Surface B + ADR-050 v2.4 D6; Surface B only) | prism-spec-engine (pipeline.rs, build.rs) |
