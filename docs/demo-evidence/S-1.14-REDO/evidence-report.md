...
| AC-9 | BC-2.19.001 inv | No `unimplemented!()`/`todo!()` in production infusion modules (POL-12) | [gif](AC-009-no-unimplemented-stubs.gif) / [webm](AC-009-no-unimplemented-stubs.webm) | `test_pol_12_no_untraced_todo_in_prism_bin_production_code` PASS; VP-048 N-fields proof PASS | — (absence of stubs proven by passing test suite) | recorded |
