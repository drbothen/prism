...
| AC-004 | RG-011 | AC-004-wire-shape-mock (.tape/.gif/.webm) + AC-004-005-009-live-queries.txt | VHS + live transcript (real data) | PASS |
...
| AC-006 | RG-006 | AC-006-009-edge-cases (.tape/.gif/.webm) | VHS (test suite) | PASS |
...
**Covers:** AC-004 wire shape (mock production-path companion), AC-009 EC-004-WIRE (RG-010)
...
- **AC-004 wire shape / RG-011:**
  ...
  PASS. Mock response with seeded device-vulnerability row. Serialized JSON wire output contains
  `class_uid=2002`, `finding_info_title` present, `time` present, `raw_extensions` present as
  JSON string with `device_uid` + `vulnerability_id` keys. No Tier-2 keys at row root.
  Production code path: `SpecDrivenSensorAdapter::fetch → pipeline_result_to_record_batch → arrow-json`.
  Traces to BC-2.16.017 §PC1 (class_uid=2002), §PC2 (Tier-1/Tier-2), §PC3 (composite PK).

- **AC-009 EC-004-WIRE / RG-010:**
  `test_BC_2_16_017_claroty_device_vulnerability_relations_wire_shape_native_json_array_and_null_passthrough`
  PASS. `device_vulnerability_relevance_reasons = []` serialized as native JSON array `[]` (NOT
  a JSON string). Null passthrough for nullable Tier-2 fields confirmed at wire level.
...
| AC-004-wire-shape-mock (.gif/.webm) | AC-004 mock (RG-011) | BC-2.16.017 §PC1, §PC2, §PC3 | — |
