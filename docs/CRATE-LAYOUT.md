...
## 5. 24-Crate Conformance Table

Generated by `scripts/check-crate-layout.sh --markdown` against the 24 `crates/*/`
subdirectories that carry a `Cargo.toml`. `crates/*/` expands to 25 entries;
`crates/plugins/` (a container directory for non-workspace WASM plugin crates, with
no `Cargo.toml` of its own) is skipped by the script's manifest guard. Separately,
`crowdstrike-oauth2` (the 25th workspace member) is nested at
`crates/prism-spec-engine/plugins/crowdstrike-oauth2/` and is outside `crates/*/`
entirely. Baseline from S-3.5.01 (22 crates); updated with `prism-bin` and
`prism-dtu-harness` added in later waves.

| Crate | src/lib.rs | No loose .rs | fixtures/ clean | Status |
|-------|-----------|--------------|-----------------|--------|
| ocsf-proto-gen | OK | OK | OK | PASS |
| prism-audit | OK | OK | OK | PASS |
| prism-bin | OK | OK | OK | PASS |
...
| prism-dtu-harness | OK | OK | OK | PASS |
...
The hook checks the 24 `crates/*/` subdirectories that carry a `Cargo.toml` once
per commit (not once per staged file). `crates/plugins/` (no `Cargo.toml` — a
container directory for non-workspace WASM plugin crates) is skipped by the manifest
guard; `crowdstrike-oauth2` (the 25th workspace member, nested at
`crates/prism-spec-engine/plugins/crowdstrike-oauth2/`) is outside `crates/*/`
entirely. Runtime is under 1 second per ADR-012 §5 analysis.
