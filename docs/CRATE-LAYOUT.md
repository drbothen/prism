...
Generated by `scripts/check-crate-layout.sh --markdown` against the 24 direct
workspace crates (script scans `crates/*/` subdirectories only; the 25th workspace
member `crowdstrike-oauth2` is nested under `prism-spec-engine/plugins/` and excluded
from the direct-subdirectory scan). Baseline from S-3.5.01 (22 crates); updated with
`prism-bin` and `prism-dtu-harness` added in later waves.
...
The hook scans the 24 direct `crates/*/` subdirectories once per commit (not once
per staged file). The 25th workspace member (`crowdstrike-oauth2` WASM plugin, nested
under `crates/prism-spec-engine/plugins/`) is excluded from the direct-subdirectory
scan. Runtime is under 1 second per ADR-012 §5 analysis.
