...
crates_touched: [prism-dtu-demo-server, prism-dtu-harness, prism-dtu-armis, prism-sensors]
...
risk_mitigations:
  ...
  - "reqwest::Client timeout: PRODUCTION clients use .timeout(Duration::from_secs(30)) per CLAUDE.md
    conventions; this story adds NO production HTTP client. Test harness clients use an explicit
    non-default timeout (Duration::from_secs(10), matching the established convention in
    review_2026_06_10_deny_unknown.rs and harness_clone_parity_test.rs) — the binding requirement
    is an explicit non-default timeout (never infinite/default), which is satisfied."
inputs:
...
1. `prism-dtu-demo-server` can start two named DTU clone instances of the same sensor type
   ...
   `127.0.0.1:0` for org B) and expose their distinct `SocketAddr`s to callers via
   `servers.socket_map()` on the returned `MultiInstanceServers` lifecycle handle.
...
3. At least one multi-tenant routing integration test (Red Gate level) proves distinct-listener
   isolation: requests addressed to S_A reach S_A exclusively; requests addressed to S_B reach
   S_B exclusively; zero cross-socket contamination (server-side GET /dtu/request-count counter).
   This is the in-harness DISTINCT-LISTENER ISOLATION proof (AC-006 / INV-ISOLATION-001). The
...
| BC | Title | Role |
|----|-------|------|
| BC-2.06.017 | Per-DTU-Instance Multi-Address Binding for Multi-Tenant Overlay Testing | Anchoring BC — governs the multi-address bind semantics, the `MultiInstanceServers` lifecycle handle, the per-instance `SocketAddr` registry (`socket_map()`), overlay TOML integration (3-field format: `extends` + `instance_id` + `base_url`), INV-ISOLATION-001 no-cross-tenant-leakage invariant, INV-COMPAT-001 backward-compat invariant, INV-ERR-003-COMPAT multi-error aggregation, and duplicate-key error semantics (Postconditions 1–7). Authoritative version lives in BC-INDEX + frontmatter `behavioral_contracts` array. |
...
is `127.0.0.1:0`) and the bound `SocketAddr` is exposed to callers via
`servers.socket_map()` on the returned `MultiInstanceServers` lifecycle handle.
...
instances start on distinct ephemeral ports. The returned `Ok(MultiInstanceServers)`
value has `servers.socket_map()` returning two entries with distinct addresses
(`map["armis-acme"] != map["armis-contoso"]`).
...
(traces to BC-2.06.017 Postcondition 2 + Postcondition 3 — `write_overlay_temp_dir`
writes 3-field overlays (extends + instance_id + base_url) per BC-2.06.017 Postcondition 2
...
HTTP requests are addressed to the acme instance socket (S_A), they reach S_A exclusively;
zero requests addressed to S_A reach S_B. Symmetrically, zero requests addressed to S_B
reach S_A. Verification uses the server-side per-instance request counter on each DTU
instance (GET /dtu/request-count) to confirm zero cross-instance contamination.

This is the DISTINCT-LISTENER ISOLATION proof — grounded in a server-side request counter
added to `prism-dtu-armis/src/` during the F-P1-HIGH-001 LOCAL adversary Pass-1 fix.
Specifically, `prism-dtu-armis/src/state.rs` gained a `pub request_counter: AtomicU64`
field (with `received_request_count()` accessor and reset in `reset_all()`), and
`prism-dtu-armis/src/clone.rs` gained a `count_request_middleware` axum layer (outermost
layer in `build_router`, increments `request_counter` on every request) + `pub fn request_count(&self)`
...
load-bearing proof. This production code addition to `prism-dtu-armis/src/` is the scope
expansion recorded by F-P8-HIGH-001 (see Changelog v1.10; crates_touched now includes prism-dtu-armis).

**Scope of this harness test:** The harness proves DISTINCT-LISTENER ISOLATION only — that
each DTU instance socket receives only the requests addressed to it. The end-to-end proof
...
(traces to BC-2.06.017 Postcondition 4 / INV-ISOLATION-001 — distinct-listener isolation:
...
The real (unchanged) signature per BC-2.06.017 Postcondition 5 and INV-COMPAT-001
...
real signature for the implementer (U-001 correction; BC-2.06.017 Amendment 1).
...
| `MultiInstanceServers` (new struct, demo-server) | `crates/prism-dtu-demo-server/src/multi_instance.rs` | Effectful (lifecycle handle: owns shutdown_tx + task_handles; `#[non_exhaustive]`; per architect D-1075-API-GAP-001) |
...
| End-to-end FanOutTarget→DTU-socket routing isolation test | `crates/prism-sensors/tests/multi_tenant_dtu_routing_integration.rs` (CREATE — drives the real `fan_out_with_overlay_map` production dispatch through the overlay base_url against two live ArmisClone DTU instances; PASSES; the genuine FanOutTarget→per-org-instance routing proof; F-PR3-HIGH-001) | Effectful (integration test; spawns live DTU sockets) |
| `prism-sensors/Cargo.toml` dev-dependencies | `crates/prism-sensors/Cargo.toml` (MODIFY — `[dev-dependencies]` += `prism-dtu-harness`, `prism-dtu-armis`, `prism-dtu-common`; permitted INV-PERIMETER-001 direction: sensors consuming dtu-harness in test-only context is allowed) | N/A (build config) |
...
// Returns Ok(MultiInstanceServers) or Err(MultiInstanceBindError).
...
impl MultiInstanceServers {
    pub fn socket_map(&self) -> &HashMap<String, SocketAddr>;  // per-name bound SocketAddr map
    pub fn shutdown(&self);  // send graceful-shutdown signal to all instances; idempotent
}
impl Drop for MultiInstanceServers {
    // sends shutdown_tx.send(()); drops task_handles WITHOUT abort — axum graceful drain
}

// Note: shutdown_tx is a SINGLE shared broadcast sender owned exclusively by
// MultiInstanceServers; each instance subscribes via shutdown_tx.subscribe() at bind time.
// Watcher tasks hold only the subscriber (Receiver), never an additional Sender.
// Dropping the handle or calling shutdown() wakes all axum with_graceful_shutdown futures
// → drain → port release. This mirrors MultiInstanceHarness (no zombie instances;
// success-path analogue of Postcondition 6).
...
// (BC-2.06.017 Postcondition 2 + TV-017-009 + OverlayLoader requirements):
...
| BC-2.06.017 (anchoring BC — full read) | ~4,000 |
...
6. **Create `crates/prism-dtu-harness/src/overlay_wiring.rs`** — implement:
   ...
   (per BC-2.06.017 Postcondition 2 + TV-017-009 + OverlayLoader requirements):
   ```toml
   extends = "{sensor_id}"
...
6a. **Wire non-exhaustive-violation compile-fail gate (U-006):** The
    ...
    - (b) Add 7 struct-literal violation arms in `struct_violations.rs`:
      `MultiInstanceConfig`, `InstanceEntry`, `DemoBindError`, `MultiInstanceServers` (from demo-server);
...
| New public structs require `#[non_exhaustive]` | CLAUDE.md + BC-2.06.017 INV-NONEXHAUSTIVE-001 | `ci.yml EXPECTED` bumped 52→60 (8 arms: 7 E0639 structs + 1 E0004 enum; +1 for MultiInstanceServers lifecycle handle per architect D-1075-API-GAP-001; base re-baselined 49→52 per D-1143 remove-uncertainty re-run — 001-A AC-014 + 001-B grew the gate to 52); `non-exhaustive-violation` crate Cargo.toml gains `prism-dtu-demo-server` + `prism-dtu-harness` deps; 7 E0639 struct arms + 1 E0004 enum arm added |
...
| reqwest::Client timeout: PRODUCTION clients use `.timeout(Duration::from_secs(30))` (CLAUDE.md); this story adds NO production HTTP client. Test harness clients use explicit non-default `.timeout(Duration::from_secs(10))` matching established sibling test convention. Binding requirement: explicit non-default timeout — never infinite/default. | CLAUDE.md conventions + OBS-2 clarification (v1.11) | No new production reqwest::Client in this story; test clients must never use default (infinite) timeout |
...
| EC-009 | Two `InstanceEntry` items with the same `name` string | `Err(MultiInstanceBindError::DuplicateName { name: String })` returned before any bind attempt (BC-2.06.017 EC-017-009 / Postcondition 7). Inner field is `name: String` matching the `DuplicateName { name: String }` variant shape. |
