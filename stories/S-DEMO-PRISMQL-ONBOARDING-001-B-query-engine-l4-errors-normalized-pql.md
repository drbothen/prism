...
|-------|-------|-------------|
| BC-2.11.016 v1.25 | E-QUERY-038 Column-Not-Found Plan-Time Gate (L4) | Gate at plan time after E-QUERY-037; available_columns always present (DI-008 org-scoped); did_you_mean when Levenshtein ≤ 3; -32602 MCP error code; FOURTEEN gate positions (pos 1–12 original; pos 13 = enrich input column, pos 14 = dedup field keys, both added at v1.8); DERIVED-COLUMN BINDING RULE: running {available, suspended} context maintained across pipe stages; SQLPIPE HEAD-PROJECTION BINDING RULE — three branches: (a) SELECT * → full raw schema; (b) fully-explicit SELECT (no Star/TableStar) → {aliases} ∪ {bare Field names} ∪ {GROUP BY fields}; (c) MIXED-STAR SELECT (≥1 Star/TableStar AND ≥1 explicit item) → schema_columns ∪ {aliases} ∪ {bare Field names} ∪ {GROUP BY fields}; anonymous non-Field items without alias → suspended:=true (FP-001 fail-open); SIBLING-GATE CONSISTENCY: E-QUERY-002 gate skips DERIVED-provenance names (stats alias/enrich output/SqlPipe head alias) — fail-open FP-001; FROM-ALIAS RESOLUTION: alias-qualified pipe-stage refs stripped and checked against binding context (EC-11-065); FIELDS TRANSITION: include \| fields → REPLACE available, exclude \| fields → subtract available (EC-11-066..068); LAST-SEGMENT OUTPUT-NAME RULE: join-qualified head SELECT items (t.col) use last segment ('col') as output name in binding context (EC-11-069); STAR-WITH-JOIN SUSPENSION RULE: in branches (a) and (c), when head JOIN list non-empty AND ≥1 Star/TableStar present → suspended:=true for pipe-stage walk, overriding partial seeding (EC-11-070/071; FP-001 fail-open for star-with-join heads); STAGE-JOIN SUSPENSION RULE: when pipe-stage walk encounters PipeStage::Join → suspended:=true for remainder of walk (EC-11-072/073; FP-001 fail-open for stage-join); HEAD-JOIN SUSPENSION RULE: when head query's JOIN list non-empty AND bare unqualified column ref at positions 1–6 absent from schema_columns(table, OrgId) → E-QUERY-038 MUST NOT fire (fail-open; skip reject); applies to Ast::Sql and Ast::SqlPipe head SQL positions 1–6; distinct from STAGE-JOIN (pipe-stage walk) — HEAD-JOIN operates on SQL-head positions directly; PER-REFERENCE SCOPING: suspension applies per individual column reference, not per column name — qualified refs (FROM-table- or FROM-alias-qualified) to a name that also appears bare elsewhere in positions 1–6 are NOT suspended (retain full E-QUERY-038 checking); EC-11-074/075/076; FP-001 fail-open for head-join |
| BC-2.11.017 v1.13 | E-QUERY Pedagogical Enrichments (L4 — Codes 001, 002, 003, 037) | E-QUERY-001: near_text ≤50 chars + reference_pointer; E-QUERY-002: valid_operators_for_type list; E-QUERY-003: how_to_fix string; E-QUERY-037: suggestion contains prism_describe reference; SIBLING-GATE CONSISTENCY invariant: E-QUERY-002 MUST skip names with DERIVED provenance (stats alias/enrich output/SqlPipe head alias) — fail-open per BC-2.11.016 FP-001; LAST-SEGMENT OUTPUT-NAME RULE: join-qualified head SELECT cols (t.col) — last segment ('col') treated as DERIVED output name, E-QUERY-002 skips per SIBLING-GATE CONSISTENCY (EC-11-069); STAR-WITH-JOIN SUSPENSION RULE extension (BC-2.11.016 v1.19): when initial binding context SUSPENDED via STAR-WITH-JOIN SUSPENSION RULE, E-QUERY-002 also suspended for ALL subsequent pipe stages; STAGE-JOIN SUSPENSION RULE extension (BC-2.11.016 v1.19): when PipeStage::Join encountered during pipe-stage walk, suspended:=true engages — E-QUERY-002 also suspended for all remaining stages (parallel to STAR-WITH-JOIN extension; same suspension-propagation principle, stage-level trigger) |
| BC-2.11.018 v1.5 | `normalized_pql` Field on Successful Query Responses (L4 Echo / OPD-1) | Present (non-empty) on every successful execution incl. zero-row; absent (not null, not present) on all errors; Chumsky-normalized form; excludes DataFusion plan internals |
...
| BC-2.11.016 v1.25 | ~1,200 |
| BC-2.11.017 v1.13 | ~1,000 |
| BC-2.11.018 v1.5 | ~800 |
...
|---------|-------|------|--------|--------|
| 2.21 | DEFECT-MCP-ROWSHAPE-NULLS-001-F-MCPNULL-P3-MED-001-bc-pin-propagation-2026-07-13 | 2026-07-13 | story-writer | **BC-2.11.018 v1.2→v1.5 pin propagation (DEFECT-MCP-ROWSHAPE-NULLS-001 F-MCPNULL-P3-MED-001 POL-23). PO bumped BC-2.11.018 v1.2→v1.5 during pass-3 spec burst @7d725c74. Two live version-pin cites updated: (1) §Behavioral Contracts table BC-2.11.018 row version cell; (2) §Token Budget table BC-2.11.018 row. Historical changelog rows contain prior version pins (1.8 row: v1.0→v1.1; 1.9 row: v1.1→v1.2) — left unchanged per TD-VSDD-091-exempt (past-tense changelog records). No BC-2.11.001 or BC-2.15.009 pins present in this file. AC semantics UNCHANGED. Frontmatter version 2.20→2.21; updated 2026-07-13 (POL-23).** |
| 2.20 | ADV-PR-P5-MED-003-closure-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **ADV-PR-P5-MED-003 closure + BC-2.11.016 v1.24→v1.25 / BC-2.11.017 v1.12→v1.13 pin-sync (§Preconditions.2 type-name fixes + OrderExpr::expr table fix, pass-5 PO micro-amendment). BC-2.11.016 v1.24→v1.25 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell; (2) §Token Budget table BC-2.11.016 row. BC-2.11.017 v1.12→v1.13 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell; (2) §Token Budget table BC-2.11.017 row. L22 adjacent-cell check: grep for old shapes (FilterExpr/PipeStage::Stats { by }/PipeStage::Enrich { input_col }/PipeStage::Dedup { fields }/OrderBy::expr/OrderExpr::expr) across Key Clauses and task cells: zero hits; no adjacent-cell extension needed. BC-2.11.020, BC-2.11.004 not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.19→2.20; updated 2026-07-09 (POL-23).** |
| 2.19 | ADV-PR-P4-MED-001-closure-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **ADV-PR-P4-MED-001 closure + BC-2.11.016 v1.23→v1.24 / BC-2.11.017 v1.11→v1.12 pin-sync (type-name corrections, ADV-PR-P4-MED-001). BC-2.11.016 v1.23→v1.24 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell; (2) §Token Budget table BC-2.11.016 row. Semantic-cell currency: Key Clauses cell describes gate positions/binding/suspension rules — v1.24 corrects AST type names (Where(Predicate) not Where(FilterExpr); Predicate::Logical/Not not And/Or/Not; SortExpr.field not SortEntry.field); L22 grep for stale AST type names across story: zero hits in Key Clauses or task cells; no adjacent-cell extension needed. BC-2.11.017 v1.11→v1.12 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell; (2) §Token Budget table BC-2.11.017 row. BC-2.11.020, BC-2.11.004, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.18→2.19; updated 2026-07-09 (POL-23).** |
| 2.18 | ADV-PR-P3-LOW-001-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync BC-2.11.016 v1.22→v1.23 / BC-2.11.017 v1.10→v1.11 (_with_bareness extractor rename, ADV-PR-P3-LOW-001) + semantic-cell currency (FIX-IEQ-ERRPATH-001 PR-LEVEL pass-3 story pin round). BC-2.11.016 v1.22→v1.23 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell; (2) §Token Budget table BC-2.11.016 row. Semantic-cell currency: Key Clauses cell describes gate positions/binding/suspension rules — v1.23 renames internal extractors for positions 1–6 to _with_bareness variants and positions 10–14 to extract_column_name_from_field_path; no Key Clauses cell cites old extractor function names (grep confirmed zero hits); no adjacent-cell extension needed. BC-2.11.017 v1.10→v1.11 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell; (2) §Token Budget table BC-2.11.017 row. BC-2.11.020, BC-2.11.004, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.17→2.18; updated 2026-07-09 (POL-23).** |
| 2.17 | ADV-PR-P1-MED-001-OBS-001-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync BC-2.11.016 v1.21→v1.22 / BC-2.11.017 v1.9→v1.10 (Injection-safety of `column` MCP-facing payload, CWE-116, ADV-PR-P1-MED-001/OBS-001) + semantic-cell currency (FIX-IEQ-ERRPATH-001 PR-LEVEL pass-1 story pin round). BC-2.11.016 v1.21→v1.22 (pin-only): three live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell; (2) §Token Budget table BC-2.11.016 row; (3) no Key Clauses extension (v1.22 adds injection-safety of `column` MCP payload; Key Clauses cell describes gate positions/binding/suspension rules, not payload field content — no "exact column name as written" language present). BC-2.11.017 v1.9→v1.10 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell; (2) §Token Budget table BC-2.11.017 row. Key Clauses origin anchors (BC-2.11.016 v1.19) for STAR-WITH-JOIN and STAGE-JOIN SUSPENSION RULE extensions UNCHANGED per POL-29. BC-2.11.020, BC-2.11.004, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.16→2.17; updated 2026-07-09 (POL-23).** |
| 2.16 | ADV-FIX-P16-MED-001-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync BC-2.11.016 v1.20→v1.21 / BC-2.11.017 v1.8→v1.9 (PER-REFERENCE SCOPING, ADV-FIX-P16-MED-001) + adjacent-cell Key Clauses extension + semantic-cell currency (FIX-IEQ-ERRPATH-001 pass-16 story pin round). BC-2.11.016 v1.20→v1.21: three live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell; (2) §Behavioral Contracts table BC-2.11.016 row Key Clauses cell — PER-REFERENCE SCOPING sentence appended to HEAD-JOIN SUSPENSION RULE clause (suspension applies per individual column reference, not per column name; qualified refs NOT suspended; retain full E-QUERY-038 checking) + EC-11-074/075 updated to EC-11-074/075/076; (3) §Token Budget table BC-2.11.016 row. BC-2.11.017 v1.8→v1.9 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell; (2) §Token Budget table BC-2.11.017 row. Key Clauses origin anchors (BC-2.11.016 v1.19) for STAR-WITH-JOIN and STAGE-JOIN SUSPENSION RULE extensions UNCHANGED per POL-29 task instructions. BC-2.11.020, BC-2.11.004, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.15→2.16; updated 2026-07-09 (POL-23).** |
| 2.15 | ADV-FIX-P15-MED-001-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync BC-2.11.016 v1.19→v1.20 / BC-2.11.017 v1.7→v1.8 (HEAD-JOIN SUSPENSION RULE, ADV-FIX-P15-MED-001) + semantic-cell currency (FIX-IEQ-ERRPATH-001 pass-15 story pin round). BC-2.11.016 v1.19→v1.20: three live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell + Key Clauses cell extended with HEAD-JOIN SUSPENSION RULE (when head query's JOIN list non-empty AND bare unqualified column ref at positions 1–6 absent from schema_columns(table, OrgId) → E-QUERY-038 MUST NOT fire, fail-open; applies to Ast::Sql and Ast::SqlPipe head SQL positions 1–6; distinct from STAGE-JOIN — HEAD-JOIN operates on SQL-head positions directly; EC-11-074/075; FP-001 fail-open for head-join); (2) §Token Budget table BC-2.11.016 row. BC-2.11.017 v1.7→v1.8 (pin-only): two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell; (2) §Token Budget table BC-2.11.017 row. Key Clauses extension notes with origin anchors (BC-2.11.016 v1.19) for STAR-WITH-JOIN and STAGE-JOIN SUSPENSION RULE extensions are UNCHANGED per task instructions (origin anchors). BC-2.11.020, BC-2.11.004, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.14→2.15; updated 2026-07-09 (POL-23).** |
| 2.14 | FIX-IEQ-ERRPATH-001-pass-14-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync BC-2.11.016 v1.19 / BC-2.11.017 v1.7 / BC-2.11.020 v1.12 / BC-2.11.004 v1.24 / error-taxonomy v2.32 (STAGE-JOIN SUSPENSION RULE, ADV-FIX-P14-OBS-001) + semantic-cell currency (FIX-IEQ-ERRPATH-001 pass-14 story pin round). BC-2.11.016 v1.18→v1.19: two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell + Key Clauses cell extended with STAGE-JOIN SUSPENSION RULE (when pipe-stage walk encounters PipeStage::Join → suspended:=true for remainder of walk, EC-11-072/073, FP-001 fail-open for stage-join); (2) §Token Budget table BC-2.11.016 row. BC-2.11.017 v1.6→v1.7: two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell + Key Clauses cell: (BC-2.11.016 v1.18) reference updated to (BC-2.11.016 v1.19) + STAGE-JOIN SUSPENSION RULE extension added (when PipeStage::Join encountered during pipe-stage walk, suspended:=true engages — E-QUERY-002 also suspended for all remaining stages, parallel to STAR-WITH-JOIN extension); (2) §Token Budget table BC-2.11.017 row. BC-2.11.004, BC-2.11.020, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.13→2.14; updated 2026-07-09 (POL-23).** |
| 2.13 | FIX-IEQ-ERRPATH-001-pass-12-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync BC-2.11.016 v1.18 / BC-2.11.017 v1.6 / BC-2.11.020 v1.11 / BC-2.11.004 v1.23 / error-taxonomy v2.31 (STAR-WITH-JOIN SUSPENSION RULE, ADV-FIX-P12-OBS-002) + semantic-cell currency (FIX-IEQ-ERRPATH-001 pass-12 story pin round). BC-2.11.016 v1.17→v1.18: two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell + Key Clauses cell extended with STAR-WITH-JOIN SUSPENSION RULE (in branches (a) and (c), when head JOIN list non-empty AND ≥1 Star/TableStar present → suspended:=true for pipe-stage walk, overriding partial seeding, EC-11-070/071, FP-001 fail-open); (2) §Token Budget table BC-2.11.016 row. BC-2.11.017 v1.5→v1.6: two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell + Key Clauses cell extended with STAR-WITH-JOIN SUSPENSION RULE extension (BC-2.11.016 v1.18: when initial binding context SUSPENDED via STAR-WITH-JOIN SUSPENSION RULE, E-QUERY-002 also suspended for ALL subsequent pipe stages); (2) §Token Budget table BC-2.11.017 row. BC-2.11.004, BC-2.11.020, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.12→2.13; updated 2026-07-09 (POL-23).** |
| 2.12 | FIX-IEQ-ERRPATH-001-pass-10-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync BC-2.11.016 v1.17 / BC-2.11.017 v1.5 / BC-2.11.020 v1.10 / BC-2.11.004 v1.22 / error-taxonomy v2.30 (LAST-SEGMENT rule, ADV-FIX-P10-OBS-001) + semantic-cell currency (FIX-IEQ-ERRPATH-001 pass-10 story pin round). BC-2.11.016 v1.16→v1.17: two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell + Key Clauses cell extended with LAST-SEGMENT OUTPUT-NAME RULE (join-qualified head SELECT items (t.col) use last segment ('col') as output name in binding context, EC-11-069); (2) §Token Budget table BC-2.11.016 row. BC-2.11.017 v1.4→v1.5: two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell + Key Clauses cell extended with LAST-SEGMENT / SIBLING-GATE CONSISTENCY extension (join-qualified head SELECT cols (t.col) — last segment ('col') treated as DERIVED output name, E-QUERY-002 skips per SIBLING-GATE CONSISTENCY, EC-11-069); (2) §Token Budget table BC-2.11.017 row. BC-2.11.004, BC-2.11.020, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.11→2.12; updated 2026-07-09 (POL-23).** |
| 2.11 | FIX-IEQ-ERRPATH-001-pass-8-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync BC-2.11.016 v1.16 (narrative-only E-QUERY-039 note fix, ADV-FIX-P8-OBS-002). BC-2.11.016 v1.15→v1.16: two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell; (2) §Token Budget table BC-2.11.016 row. Semantic-cell currency: Key Clauses cell verified CURRENT — v1.16 is narrative-only (Enrich bullet E-QUERY-039 note corrected to actual gate ordering); no rule/EC/behavior change affects the fourteen gate positions, DERIVED-COLUMN BINDING RULE, SQLPIPE HEAD-PROJECTION BINDING RULE, SIBLING-GATE CONSISTENCY, FROM-ALIAS RESOLUTION, or FIELDS TRANSITION content. "unreachable-by-design" grep across story: zero hits — no semantic-cell edits required. AC semantics UNCHANGED. Frontmatter version 2.10→2.11; updated 2026-07-09 (POL-23).** |
| 2.10 | FIX-IEQ-ERRPATH-001-pass-7-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync to BC-2.11.016 v1.15 / BC-2.11.017 v1.4 / BC-2.11.020 v1.9 / BC-2.11.004 v1.21 / error-taxonomy v2.29 + semantic-cell currency (FIX-IEQ-ERRPATH-001 pass-7 fix round). BC-2.11.016 v1.14→v1.15: two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell + Key Clauses cell extended with v1.15 additions (SIBLING-GATE CONSISTENCY: E-QUERY-002 skips DERIVED-provenance names fail-open FP-001; FROM-ALIAS RESOLUTION: alias-qualified pipe-stage refs stripped and checked EC-11-065; FIELDS TRANSITION: include→REPLACE, exclude→subtract EC-11-066..068); (2) §Token Budget table BC-2.11.016 row. BC-2.11.017 v1.3→v1.4: two live sites updated — (1) §Behavioral Contracts table BC-2.11.017 row version cell + Key Clauses cell extended with v1.4 SIBLING-GATE CONSISTENCY invariant (E-QUERY-002 MUST skip DERIVED-provenance names, fail-open per BC-2.11.016 FP-001); (2) §Token Budget table BC-2.11.017 row. BC-2.11.004, BC-2.11.020, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.9→2.10; updated 2026-07-09 (POL-23).** |
| 2.9 | ADV-FIX-P6-MED-001-OBS-001-pin-sync-2026-07-09 | 2026-07-09 | story-writer | **pin-sync to BC-2.11.016 v1.14 / BC-2.11.004 v1.20 / BC-2.11.020 v1.8 / error-taxonomy v2.28 + Key Clauses semantic refresh (closes ADV-FIX-P6-MED-001/OBS-001). BC-2.11.016 v1.13→v1.14: two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row version cell + Key Clauses cell rewritten to v1.14 semantics (FOURTEEN positions, DERIVED-COLUMN BINDING RULE, SQLPIPE HEAD-PROJECTION BINDING RULE three branches (a)/(b)/(c) including new MIXED-STAR branch (c)); (2) §Token Budget table BC-2.11.016 row. BC-2.11.004, BC-2.11.020, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.8→2.9; updated 2026-07-09 (POL-23).** |
| 2.8 | pin-sync-FIX-IEQ-ERRPATH-001-D-1615-2026-07-09 | 2026-07-09 | story-writer | **pin-sync to BC-2.11.016 v1.13 / BC-2.11.004 v1.19 / BC-2.11.020 v1.7 / error-taxonomy v2.27 (FIX-IEQ-ERRPATH-001 D-1615 frozen pin round). BC-2.11.016 v1.12→v1.13: two live sites updated — (1) §Behavioral Contracts table BC-2.11.016 row; (2) §Token Budget table BC-2.11.016 row. BC-2.11.004, BC-2.11.020, and error-taxonomy not cited in this story. AC semantics UNCHANGED. Frontmatter version 2.7→2.8; updated 2026-07-09 (POL-23).** |
...
| 2.3 | BC-2.11.016-v1.8-pin-propagation-2026-07-08 | 2026-07-08 | story-writer | **BC-2.11.016 v1.7→v1.8 version-pin propagation (pass-2 CRIT closure burst, POL-29/POL-23).** PO bumped BC-2.11.016 v1.7→v1.8 (14-position gate + derived-column binding rule). Two live version-pin cites updated: (1) §Behavioral Contracts table BC-2.11.016 row version cell v1.7→v1.8; (2) §Token Budget table BC-2.11.016 row v1.7→v1.8. Historical changelog rows left unchanged per POL-29. AC semantics UNCHANGED. Frontmatter version 2.2→2.3; updated 2026-07-08 (POL-23). |
