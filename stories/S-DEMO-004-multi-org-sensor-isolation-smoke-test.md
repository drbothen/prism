...
| `crates/prism-bin/tests/e2e_multi_org.rs` (regression guard) | NOTE | A regression test for MCP-stdout cleanliness is being added to the prism-bin test suite by the test-writer (LOCAL adversary pass-2 finding M2-02: `tracing must not pollute MCP stdout`). This is a GREEN regression guard, NOT a Red Gate AC test — it validates the boot.rs `step1_init_tracing` stderr-only invariant post-fix. It does not change `acceptance_criteria_count` (10) or `red_gate_tests` (4). |
