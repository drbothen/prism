...
| 22 | `test_ec006_empty_input_yields_null` | prism-query | BC-2.19.001 v2.2 E-INFUSE-014; general empty-string-coercion guard: `coerce_to_typed("", Int64)` → None (empty string input to integer coercion fails via `i64::from_str("".trim())` → `warn_coercion_failed` → NULL; complement to EC-004; NOT the empty-`iocs`-array trigger, which produces SQL null via `extract_at_path` Err → null-input short-circuit in `invoke_async_with_args` without calling `coerce_to_typed` at all) | unit |
...
| EC-004 | Empty string projected into typed field | `i64::from_str("".trim())` fails → NULL + E-INFUSE-014; `f64::from_str("".trim())` fails → NULL + E-INFUSE-014 (both coercion branches call `warn_coercion_failed` on parse failure) | AC-004 |
...
| EC-006 | `iocs` array is empty (e.g., cyberint alert with no IOCs) | `$.iocs[0].value` extraction: `extract_at_path` returns `Err` (index 0 out of bounds in empty array) → `extract_raw` returns `None` → `iocs_value_first` cell is SQL null (BC-2.16.002 `column_source_path_extraction_failed` event emitted at adapter layer; NOT E-INFUSE-014). In `invoke_async_with_args`, null input row short-circuits (`None => { enriched.push(None); continue; }`) → NULL output; `coerce_to_typed` is NOT called → NO E-INFUSE-014 emitted (the coercion path is unreachable for a null input). Observable output: NULL in the enrichment column (benign: no IOC to enrich). | AC-011 |
...
| error-taxonomy v2.26 | E-INFUSE-013 sub-conditions 7/8 added; E-INFUSE-014 TypeCoercionFailed allocated; SEC-001 Rendering Note (CWE-117 control-char sanitization) and SEC-002 Assessment (CWE-770 boolean size cap) added in v2.17; current canonical version v2.26 |
