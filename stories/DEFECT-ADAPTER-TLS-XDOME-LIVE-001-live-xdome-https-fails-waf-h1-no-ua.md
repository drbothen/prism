...
#   22 Red Gate tests (RG-001..RG-022) + live verification support: 2.2 pt
#   Total: ~6.0 points
estimated_days: 1.5
risk: HIGH
# Risk justification:
...
retry") without parsing error strings for primary triage. Sibling fix: BC-2.08.001 v1.6
EC-08-001 also corrected `reachable:false → reachable:true` for HTTP 503 (POL-29
...
| RG-020 | `test_BC_2_08_002_degraded_envelope_summary_matches_overall_status` | AC-WIRE-003 (F-P42-HIGH-001; genuine RED — current `server.rs` summary predicate missing `&& s.error.is_none()`, miscounts Degraded sensor as fully healthy) | `crates/prism-bin/tests/defect_adapter_tls_xdome_live_001.rs` (in-process integration test; wire-level serialized JSON assertions on full `SensorHealthStructuredContent` envelope) |
...
**Red Gate test count:** 22 (RG-001..RG-022)
**Acceptance criteria count:** 20 (AC-H2-001, AC-UA-001, AC-UA-002, AC-CARGO-001, AC-ERR-001..007, AC-WIRE-001, AC-WIRE-002, AC-WIRE-003, AC-SAP1-001, AC-QERR-001, AC-QERR-002, AC-LIVE-001..003)
**Density:** 22 / 20 = 1.1000 — PASSES (≥0.5 required by BC-5.38.001)
...
**Ordering rule (SAC-1 rule 3):** All Red Gate test authoring tasks (Phase 1 below) MUST be dispatched and completed BEFORE implementation tasks (Phase 2). The test-writer agent works from this story's RG-001..RG-022 list and the BC texts; the implementer receives the failing test suite before touching production code.
...
- [ ] **RG-020** `test_BC_2_08_002_degraded_envelope_summary_matches_overall_status`: in-process integration test in `crates/prism-bin/tests/defect_adapter_tls_xdome_live_001.rs`; use wiremock 503 (same setup as RG-019); drive the end-to-end path through `SpecDrivenSensorAdapter::probe_connectivity` → `SensorHealthChecker::check_one` → `check_sensor_health` handler → MCP wire serialization of the FULL `SensorHealthStructuredContent` envelope; assert on the FULL serialized JSON envelope byte output (SID-2 wire-assertion discipline): (1) envelope contains literal substring `"overall_status":"partial"`; (2) envelope does NOT contain `"N of N sensor(s) healthy"` where N=1 (i.e., no `"1 of 1"` healthy count when the sensor is Degraded); (3) envelope summary count reflects 0 fully-healthy sensors (e.g., contains `"0 of 1"` or similar exclusion wording). This test FAILS on current code where `server.rs` `fully_healthy_count` predicate counts Degraded as healthy (missing `&& s.error.is_none()`). Must be RED before T-SERVER-1. Distinct from RG-019: RG-019 asserts the per-sensor `SensorHealthResult` struct; RG-020 asserts the ENVELOPE (`SensorHealthStructuredContent.summary` + `SensorHealthStructuredContent.overall_status`). SAP-3 end-to-end from MCP tool public surface. Traces to AC-WIRE-003; BC-2.08.002 §Postconditions EC-08-009 aggregate contract.
