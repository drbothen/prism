...
# deferred to it by BC-2.06.019 v1.8 §Interim State and §Per-Sensor IOC-Surface Matrix.
...
# coexist in the same route handler (BC-2.06.019 v1.8 §Interim State). SAP-2 must be
...
risk_mitigations:
  - "_ioc_value removal (BC-2.06.019 v1.8 §Interim State): the synthetic _ioc_value /
     ...
     BC-2.06.019 v1.8 §Interim State clause is explicit: 'The synthetic filter and the
     real-schema filter MUST NOT coexist.' Any implementer attempt to ship the synthetic
     filter as interim is a BC-2.06.019 v1.8 violation and a BPRL-P4-01 recurrence."
  - "CrowdStrike IOC scope (BC-2.06.019 v1.8 Per-Sensor IOC-Surface Matrix): behaviors[]
  ...
  - "Armis and Claroty: NO IOC stamping (permanent exclusion per BC-2.06.019 v1.8 matrix).
  ...
  - "Route Coverage Table update (BC-2.06.019 v1.8 §Route Coverage Table standing rule):
     ...
     MANDATORY same-commit change per the standing rule in BC-2.06.019 v1.8."
...
**BC-2.06.019 v1.8 scope ownership:** This story implements the deferred IOC-stamping
...
BC-2.06.019 v1.8 applies corrected CrowdStrike ioc_type enum and Cyberint dual-alias
requirement relative to v1.7.
...
(traces to BC-2.06.019 v1.8 postcondition 4 — Per-Sensor IOC-Surface Matrix, Cyberint row; inner-key dual-alias requirement)
...
- `alert_data: Option<AlertData>` where `AlertData { ip: Option<String>, domain: Option<String>, url: Option<String> }`
  (`url` CONFIRMED; `ip`/`domain` plausible-unconfirmed per BC-2.06.019 v1.8)
- `ioc: Option<Ioc>` — RETAINED for now but flagged for removal: no public-documentation
  ...
  (BC-2.06.019 v1.8 Cyberint row — "UNCONFIRMED, flagged for likely removal"). Confirm via
...
(inner-key forms are INCONCLUSIVE-pending-live-tenant-validation per BC-2.06.019 v1.8):
```rust
pub struct Ioc {
...
(BC-2.06.019 v1.8 research: uncertainty-pivot003-s504-2026-06-19.md §Item 2).
...
(traces to BC-2.06.019 v1.8 postcondition 4 — IOC-surface fields populated from catalog)
...
(traces to BC-2.06.019 v1.8 postcondition 4 §Interim State — synthetic filter must be replaced atomically)
...
(traces to BC-2.06.019 v1.8 postcondition 4 — CrowdStrike detections row in Per-Sensor IOC-Surface Matrix; corrected ioc_type enum)
...
- `"ioc_type": "hash_sha256"` — algorithm-qualified token per real CrowdStrike API
  (BC-2.06.019 v1.8 correction — bare `"hash"` is WRONG; real API uses algorithm-qualified
...
**CrowdStrike ioc_type enum (CORRECTED in v1.8):**
...
**Red Gate test expectation update (v1.8 correction):** The Red Gate test
...
(traces to BC-2.06.019 v1.8 postcondition 4 — TOML spec alignment with real-schema fields)
...
to emit (INCONCLUSIVE-pending-live-validation per BC-2.06.019 v1.8 — dual-alias toleration
...
(traces to BC-2.06.019 v1.8 postcondition 4 + BC-2.06.020 INV-THREATINTEL-IOC-CORRELATION-001)
...
(traces to BC-2.06.019 v1.8 postcondition 4 + BC-2.06.020 INV-NVD-CVE-CORRELATION-001)
...
(traces to BC-2.06.019 v1.8 §Route Coverage Table standing rule)
...
- Guard Mechanism: `Real-schema filter on ioc.value / iocs[].value / alert_data.ip/domain`
  (dual-alias: Ioc.value deserialized from either "value" or "ioc_value" wire key)
- Status: `ACTIVE` (NOT `INTERIM`)

This update is required in the SAME commit as AC-003 per the Route Coverage Table
standing rule (BC-2.06.019 v1.8: "any future story adding or modifying a StageMask-relevant
...
|---|-----------|-------|-----------|------|
| 1 | `test_BC_2_06_019_cyberint_alert_struct_has_real_ioc_fields` | prism-dtu-cyberint | BC-2.06.019 v1.8 PC-4 matrix | unit |
| 2 | `test_BC_2_06_019_cyberint_ioc_struct_dual_alias_deserializes_both_key_forms` | prism-dtu-cyberint | BC-2.06.019 v1.8 PC-4 Cyberint row — INCONCLUSIVE inner-key dual-alias | unit |
| 3 | `test_BC_2_06_019_cyberint_fixture_generator_stamps_scenario_iocs` | prism-dtu-cyberint | BC-2.06.019 v1.8 PC-4 / BC-2.06.020 PC-1 | unit |
| 4 | `test_BC_2_06_019_cyberint_alerts_real_schema_ioc_filter_no_synthetic` | prism-dtu-cyberint | BC-2.06.019 v1.8 PC-4 §Interim State | unit |
| 5 | `test_BC_2_06_019_crowdstrike_detection_behaviors_ioc_hash_stamped` | prism-dtu-crowdstrike | BC-2.06.019 v1.8 PC-4 matrix — asserts `ioc_type == "hash_sha256"` (corrected) | unit |
| 6 | `test_BC_2_06_019_crowdstrike_detection_toml_spec_has_ioc_columns` | prism-dtu-crowdstrike or sensor spec tests | BC-2.06.019 v1.8 PC-4 + SAP-2 | unit/parity |
| 7 | `test_BC_2_06_019_cyberint_alert_toml_spec_has_ioc_columns` | prism-dtu-cyberint or sensor spec tests | BC-2.06.019 v1.8 PC-4 + SAP-2 | unit/parity |
| 8 | `test_BC_2_06_019_canonical_threatintel_pivot_query_returns_malicious_at_stage_3` | prism-query or prism-bin integration | BC-2.06.019 v1.8 PC-4 + BC-2.06.020 INV-THREATINTEL-IOC-CORRELATION-001 | integration (demo server) |
| 9 | `test_BC_2_06_019_canonical_nvd_pivot_query_returns_high_cvss_at_containment_stage` | prism-query or prism-bin integration | BC-2.06.019 v1.8 PC-4 + BC-2.06.020 INV-NVD-CVE-CORRELATION-001 | integration (demo server) |
| 10 | `test_BC_2_06_019_ioc_hashes_false_withholds_cyberint_alert_with_matching_hash` | prism-dtu-cyberint | BC-2.06.019 v1.8 PC-4 ioc_hashes=false filtering | unit |

NOTE: Red Gate count bumped from 9 to 10 (added test #2 for dual-alias deserialization).
Update `red_gate_tests: 9` frontmatter to `10` on this story's next frontmatter edit.

---

## Route Coverage Table (for StageMask IOC fields — per BC-2.06.019 v1.8 §Question 4)
...
| BC-2.06.019 v1.8 (full — authoritative for IOC matrix, Interim State, Route Coverage Table, corrected CrowdStrike ioc_type + Cyberint dual-alias) | ~7,500 |
...
- [ ] Read BC-2.06.019 v1.8 §Per-Sensor IOC-Surface Matrix and §Route Coverage Table — both
  are authoritative for this story's scope (v1.8 has corrected CrowdStrike ioc_type enum
  and Cyberint dual-alias requirement)
...
- [ ] Add `Ioc` struct to `types.rs` (or reuse from ThreatItem if already has dual-alias) with
  MANDATORY dual-alias serde annotations (BC-2.06.019 v1.8 INCONCLUSIVE inner-key requirement):
  ```rust
  pub struct Ioc {
      #[serde(rename = "type", alias = "ioc_type")]
      pub ioc_type: String,
...
- [ ] Modify `make_detection()` in `src/generator.rs` (U19: serde_json::Value JSON record, NOT a struct):
  ...
  using the CORRECTED algorithm-qualified ioc_type token (BC-2.06.019 v1.8 correction):
...
|------|--------|-------------|
| `_ioc_value` and real-schema filter MUST NOT coexist in the same route handler | BC-2.06.019 v1.8 §Interim State | AC-003 + grep verification |
| CrowdStrike `behaviors[].ioc_type` restricted to algorithm-qualified tokens `{hash_sha256, hash_md5, domain, filename, registry_key}` — bare `hash` WRONG, bare `registry` WRONG, `cmdline` is a SEPARATE sibling field (NOT an ioc_type), NOT `ipv4`/`ipv6` | BC-2.06.019 v1.8 Per-Sensor IOC-Surface Matrix (CrowdStrike row; corrected 2026-06-19 per ThreatQ + XSOAR research) | AC-004 + adversary probe; Red Gate asserts `ioc_type == "hash_sha256"` |
| CrowdStrike ioc_type parser MUST be tolerant of unknown tokens — log + preserve raw string, non-fatal | BC-2.06.019 v1.8 CrowdStrike row — tolerant-unknown-type policy | Adversary: verify no hard-reject branch on unrecognized ioc_type in generator.rs or route handler |
| Cyberint `Ioc` struct MUST use serde dual-alias: `#[serde(rename = "type", alias = "ioc_type")]` on `ioc_type` field; `#[serde(alias = "ioc_value")]` on `value` field | BC-2.06.019 v1.8 Cyberint row — INCONCLUSIVE inner-key, dual-alias required | AC-001 + adversary reads types.rs after implementation |
| Cyberint singleton `ioc: Option<Ioc>` on `Alert` struct — retain but flag: confirm via live-tenant validation whether this field ever appears; remove if absent | BC-2.06.019 v1.8 Cyberint row — no public-documentation basis for singleton field | Adversary: note as PENDING-LIVE-VALIDATION; do not add code that hard-depends on it |
| Armis and Claroty: NO IOC fields added (permanent exclusion) | ADR-031 DTU=True-DTU fidelity + BC-2.06.019 v1.8 matrix (Armis/Claroty: NO permanent) | Adversary: grep for ioc in prism-dtu-armis/prism-dtu-claroty types.rs |
| Route Coverage Table in BC-2.06.019 MUST be updated in same commit as route change | BC-2.06.019 v1.8 §Route Coverage Table standing rule | Noted in PR description; state-manager burst |
...
| Pivot queries MUST return non-empty results at stage >= 3 (ThreatIntel) / stage 4 (NVD with device_cves) | BC-2.06.019 v1.8 PC-2 StageMask table + BC-2.06.020 correlation invariants | Tests 8, 9 |
...
- `ioc_type = "hash"` (bare, not algorithm-qualified) in CrowdStrike behaviors[] stamping — use `"hash_sha256"` or `"hash_md5"` (BC-2.06.019 v1.8 correction)
- `ioc_type = "registry"` (bare) in CrowdStrike behaviors[] stamping — use `"registry_key"` (BC-2.06.019 v1.8 correction)
- `ioc_type = "cmdline"` in CrowdStrike behaviors[] stamping — `cmdline` is a SEPARATE sibling field, NOT an ioc_type value (BC-2.06.019 v1.8 correction)
...
|----|--------|-------------|-------------------|
| EC-001 | BC-2.06.019 v1.8 §Interim State | `_ioc_value` filter present alongside real-schema filter | FORBIDDEN — not a valid edge case; this is a MUST NOT state per BC-2.06.019 v1.8 |
| EC-002 | BC-2.06.019 v1.8 Per-Sensor IOC-Surface Matrix | IOC stamping attempted on Armis or Claroty records | FORBIDDEN — permanent exclusion; must not occur |
...
| EC-006 | BC-2.06.019 v1.8 Per-Sensor IOC-Surface Matrix | CrowdStrike host/device record (not detection) — no IOC fields | Device records have NO IOC stamping; IOCs live on detection records only |
...
1. Read `crates/prism-dtu-cyberint/src/types.rs` post-implementation — verify `Alert.ioc`,
   ...
   on `ioc_type` field, `#[serde(alias = "ioc_value")]` on `value` field (BC-2.06.019 v1.8
...
2. Read `crates/prism-dtu-crowdstrike/src/generator.rs` `make_detection()` — verify `"behaviors"`
   ...
   catalog hash IOCs** (BC-2.06.019 v1.8 correction; bare `"hash"` = P1 CRITICAL); also read
