...
# deferred to it by BC-2.06.019 v1.5 §Interim State and §Per-Sensor IOC-Surface Matrix.
...
# coexist in the same route handler (BC-2.06.019 v1.5 §Interim State). SAP-2 must be
...
risk_mitigations:
  - "_ioc_value removal (BC-2.06.019 v1.5 §Interim State): the synthetic _ioc_value /
     ...
     BC-2.06.019 v1.5 §Interim State clause is explicit: 'The synthetic filter and the
     real-schema filter MUST NOT coexist.' Any implementer attempt to ship the synthetic
     filter as interim is a BC-2.06.019 v1.5 violation and a BPRL-P4-01 recurrence."
  - "CrowdStrike IOC scope (BC-2.06.019 v1.5 Per-Sensor IOC-Surface Matrix): behaviors[]
  ...
  - "Armis and Claroty: NO IOC stamping (permanent exclusion per BC-2.06.019 v1.5 matrix).
  ...
  - "Route Coverage Table update (BC-2.06.019 v1.5 §Route Coverage Table standing rule):
     ...
     MANDATORY same-commit change per the standing rule in BC-2.06.019 v1.5."
...
**BC-2.06.019 v1.5 scope ownership:** This story implements the deferred IOC-stamping
work that BC-2.06.019 v1.5 assigned to `S-DEMO-ENRICHMENT-PIVOT-003` via the
...
|----|-------|-------------|
| BC-2.06.019 v1.5 | Demo-Server Scenario Progression — Per-Sensor IOC-Surface Masking | PC-4 Per-Sensor IOC-Surface Matrix: Cyberint alerts and CrowdStrike detections carry real-schema IOC fields; Armis/Claroty permanently excluded; `_ioc_value` synthetic filter removed atomically; Route Coverage Table updated |
...
(traces to BC-2.06.019 v1.5 postcondition 4 — Per-Sensor IOC-Surface Matrix, Cyberint row)
...
(research-agent 2026-06-12 findings recorded in BC-2.06.019 v1.5).
...
(traces to BC-2.06.019 v1.5 postcondition 4 — IOC-surface fields populated from catalog)
...
(traces to BC-2.06.019 v1.5 postcondition 4 §Interim State — synthetic filter must be replaced atomically)
...
(traces to BC-2.06.019 v1.5 postcondition 4 — CrowdStrike detections row in Per-Sensor IOC-Surface Matrix)
...
FalconPy SDK / CrowdStrike Detect API (BC-2.06.019 v1.5 matrix, CrowdStrike row).
...
(traces to BC-2.06.019 v1.5 postcondition 4 — TOML spec alignment with real-schema fields)
...
(traces to BC-2.06.019 v1.5 postcondition 4 + BC-2.06.020 INV-THREATINTEL-IOC-CORRELATION-001)
...
(traces to BC-2.06.019 v1.5 postcondition 4 + BC-2.06.020 INV-NVD-CVE-CORRELATION-001)
...
(traces to BC-2.06.019 v1.5 §Route Coverage Table standing rule)
...
standing rule (BC-2.06.019 v1.5: "any future story adding or modifying a StageMask-relevant
...
## Route Coverage Table (for StageMask IOC fields — per BC-2.06.019 v1.5 §Question 4)
...
| BC-2.06.019 v1.5 (full — authoritative for IOC matrix, Interim State, Route Coverage Table) | ~7,500 |
...
- [ ] Read BC-2.06.019 v1.5 §Per-Sensor IOC-Surface Matrix and §Route Coverage Table — both
...
|------|--------|-------------|
| `_ioc_value` and real-schema filter MUST NOT coexist in the same route handler | BC-2.06.019 v1.5 §Interim State | AC-003 + grep verification |
| CrowdStrike `behaviors[].ioc_type` restricted to `{hash, domain, filename, registry, cmdline}` — NOT `ipv4`/`ipv6` | BC-2.06.019 v1.5 Per-Sensor IOC-Surface Matrix (CrowdStrike row) | AC-004 + adversary probe |
| Armis and Claroty: NO IOC fields added (permanent exclusion) | ADR-031 DTU=True-DTU fidelity + BC-2.06.019 v1.5 matrix (Armis/Claroty: NO permanent) | Adversary: grep for ioc in prism-dtu-armis/prism-dtu-claroty types.rs |
| Route Coverage Table in BC-2.06.019 MUST be updated in same commit as route change | BC-2.06.019 v1.5 §Route Coverage Table standing rule | Noted in PR description; state-manager burst |
...
|----|--------|-------------|-------------------|
| EC-001 | BC-2.06.019 v1.5 §Interim State | `_ioc_value` filter present alongside real-schema filter | FORBIDDEN — not a valid edge case; this is a MUST NOT state per BC-2.06.019 v1.5 |
| EC-002 | BC-2.06.019 v1.5 Per-Sensor IOC-Surface Matrix | IOC stamping attempted on Armis or Claroty records | FORBIDDEN — permanent exclusion; must not occur |
...
| EC-006 | BC-2.06.019 v1.5 Per-Sensor IOC-Surface Matrix | CrowdStrike host/device record (not detection) — no IOC fields | Device records have NO IOC stamping; IOCs live on detection records only |
