...
| `class_selector.rs` `select()` Cyberint arms must map to the same class_uid as the TOML `ocsf_class` | ADR-058 §K2/§I5 | Grep `select_by_class_name` and `select()` arms; verify mapping |
