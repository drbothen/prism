...
document_type: story
story_id: S-REL-002
title: "devops: prism-bin version alignment to 1.0.0-rc.1 + ADR-062 product version policy"
wave: F-A
epic_id: E-REL
priority: P0
status: draft
...
risk_mitigations:
  ...
  - "ADR-062 must be registered in ARCH-INDEX.md before the story can close. Same commit."
...
# S-REL-002 — devops: prism-bin version alignment to 1.0.0-rc.1 + ADR-062
...
with the product tag. ADR-062 documents the policy: product version = git tag; prism-bin
...
6. **Create ADR-062** at `.factory/specs/architecture/decisions/ADR-062-product-version-alignment.md`:
   ...
   # ADR-062: Product Version Alignment
...
7. **Register ADR-062 in ARCH-INDEX.md:**
   Add a row to the ADR Registry table:
   ```
   | ADR-062 | Product Version Alignment — git tag as canonical version; prism-bin aligned; crates independent | ACCEPTED v1.0 | 2026-07-19 | decisions/ADR-062-product-version-alignment.md |
   ```
...
### AC-005: ADR-062 file created with required content
Given: The implementation is complete.
When: `ls .factory/specs/architecture/decisions/ADR-062*.md` is run.
...
### AC-006: ADR-062 registered in ARCH-INDEX.md
Given: `.factory/specs/architecture/ARCH-INDEX.md` is read.
When: `grep 'ADR-062' .factory/specs/architecture/ARCH-INDEX.md` is run.
Then: Exactly one row found with ADR-062, status ACCEPTED, correct path.
(traces to delta-analysis.md §2.1: "ADR-062 proper registration")
...
| ADR must be ACCEPTED before story closes | VSDD ADR lifecycle | ADR-062 status: ACCEPTED in frontmatter |
...
| `.factory/specs/architecture/decisions/ADR-062-product-version-alignment.md` | Create | See Tasks §6 |
| `.factory/specs/architecture/ARCH-INDEX.md` | Modify | Add ADR-062 row |
...
| ADR-062 | `.factory/specs/architecture/decisions/` | N/A (spec artifact) |
...
| ADR-062 | N/A | Spec artifact — no Rust purity boundary applies |
...
| EC-003 | Future bump to 1.0.0 | Only prism-bin changes again; policy codified in ADR-062 |
...
|---------|------|---------|
| 0.3 | 2026-09-03 | D-2440: ADR renumber 053→062 collision fix — every ADR-053 ref updated to ADR-062 (title, H1, Origin, risk_mitigations, Task §6/§7 ADR file content, AC-005/AC-006, File Structure, Architecture Mapping); file renamed S-REL-002-version-alignment-adr-062.md |
| 0.2 | 2026-07-19 | Fix-burst: U6 semver-checks framing corrected — 0.1.0→1.0.0-rc.1 is MAJOR bump (runs, not skipped); semver-checks is in check-ci/pre-tag NOT just check; risk_mitigations + AC-004 + EC-001 + ADR body updated accordingly; research file added to inputs |
