...
# crowdstrike.sensor.toml, a different sensor TOML. No shared file; no ordering constraint.
blocks: []
...
> **Execute:** `/vsdd-factory:deliver-story S-OCSF-FIDELITY-CROWDSTRIKE-001`

# S-OCSF-FIDELITY-CROWDSTRIKE-001: CrowdStrike OCSF Schema Validation and Correction
...
2004; (b) invalid `ocsf_class` names (`device` object used as class); (c) reserved base-event
metadata overwrite (`class_name`, `type_name`, `category_name`, `count`) → envelope corruption;
...
- **As a** MSSP analyst consuming OCSF-normalized CrowdStrike data via Prism
- **I want** every `ocsf_class` and `ocsf_field` declaration in `crowdstrike.sensor.toml`
...
Apply the ADR-058 §K dual-validation methodology to `crates/prism-sensors/specs/crowdstrike.sensor.toml`
and the corresponding `class_selector.rs` routing arms.

**Known pre-validation finding:** `ocsf_class = "device"` on the `devices` table is
structurally WRONG — `device` is an OCSF v1.7.0 object, not a class (same defect as
Claroty KF-02). Correct class: `inventory_info` (class_uid 5001). `ocsf_class =
"incident_finding"` validity requires confirmation against the 83-class enumeration.

**Validation surface (current TOML):**

| Table | `ocsf_class` (current) | Pre-validation status |
|-------|------------------------|-----------------------|
| `detections` | `detection_finding` | ocsf_class likely VALID; ocsf_field paths TBD |
| `devices` | `device` | ocsf_class WRONG — object not class |
| `incidents` | `incident_finding` | validity TBD at materialization |

All 20 `ocsf_field` declarations require per-column path-validation at materialization
per §K3 pattern from ADR-058.
...
|-----------|--------|---------------|
| CrowdStrike sensor spec | `crates/prism-sensors/specs/crowdstrike.sensor.toml` | Effectful (sensor config) |
...
|--------|---------------|---------------|
| `crowdstrike.sensor.toml` | effectful-config | Parsed at sensor-boot; drives live API calls |
...
- [ ] T-PREP-01: Run §K1 methodology against `crowdstrike.sensor.toml`; produce §K2/§K3/§K4 finding tables
...
- [ ] T-IMPL-02: Correct `crowdstrike.sensor.toml` TOML entries per §K findings
- [ ] T-IMPL-03: Correct `class_selector.rs` CrowdStrike routing arms per §K findings
...
|-------|--------------|---------------------|-------------------|
| S-ADR058-OCSF-ROUTING-001 | ADR-058 §K dual-validation methodology for Claroty; found 12/35 defects | §K1-§K4 finding table format; `inventory_info` for device tables; `finding_info.*` not `finding.*` | `device` is an OCSF object not a class; reserved fields (`class_name`, `type_name`, `count`, `category_name`) corrupt OCSF envelope when overwritten; `class_selector.rs` code defect accompanies TOML `ocsf_class` defect |
...
| File | Action | Purpose |
|------|--------|---------|
| `crates/prism-sensors/specs/crowdstrike.sensor.toml` | modify | Correct `ocsf_class` and `ocsf_field` defects per §K findings |
| `crates/prism-ocsf/src/class_selector.rs` | modify | Correct CrowdStrike routing arms to match corrected `ocsf_class` values |
