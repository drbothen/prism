...
After `S-WAVE-A-ENGINE-001` merges, `validate_sensor_spec()` is wired into the production
spec-loading pipeline with Rule 9 live. Rule 9 fires on `auth_type = "cookie_roundtrip"` +
absent `header_scheme` → E-SPEC-027(c). The current `cyberint.sensor.toml` has exactly
this shape: `auth_type = "cookie_roundtrip"` with no `header_scheme` field. Consequence:
...
The reciprocal edge (ENGINE-001 `blocks:` frontmatter listing this story) is managed by
the state-manager in the same burst that registers this story.
...
via `validate_sensor_spec()` when S-ADR055-WAVE-A-001 is also merged) and confirms the
patch is sufficient.
