...
Note: the `body_template` block in BC §Postconditions §1 is illustrative only and not
...
implicit iso8601 default applies to non-null present values (`effective_formats` returns
`["iso8601"]`). When `published_date` is null, it is stored in `raw_extensions` as a Tier-2
field; no standalone Datetime Arrow column is materialized and no E-SPEC-018 is raised
(BC-2.16.015 EC-016-015-006).
...
The `name` column carries `options = ["REQUIRED"]` in the TOML. `ColumnOptions::Required`
marks push-down eligibility in `pushdown.rs` — it is NOT a presence-enforcement gate.
When the API response contains a vulnerability row where `name` is absent or null,
the `finding_info_title` Arrow column receives a null cell (default nullable-column behavior);
no hard error is raised. Subsequent rows in the page continue to be materialized normally.
...
### AC-008: `id` key absent from `raw_extensions` when API response lacks the `id` key; when present, extracted into `raw_extensions["id"]`; does not block pagination (traces to BC-2.16.015 postcondition 2 — optional secondary id identifier; invariant; edge case EC-016-015-002)
...
the `id` key is absent from `raw_extensions` (no standalone `id` column materialized). This does
NOT cause a parse error, does NOT raise E-SPEC-018, and does NOT halt pagination. When the
`id` key IS present in the API response, its value is extracted into `raw_extensions["id"]`
as a string (CJYASHKR-format opaque Claroty identifier).

**Test:** `test_BC_2_16_015_claroty_vulnerabilities_source_path_id_absent_when_missing`
...
| RG-008 | `test_BC_2_16_015_claroty_vulnerabilities_source_path_id_absent_when_missing` | Unit (mock response) | AC-008 absent case: id key absent from envelope → `id` key absent from `raw_extensions`; no error; pagination unaffected |
| RG-009 | `test_BC_2_16_015_claroty_vulnerabilities_source_path_id_present_when_supplied` | Unit (mock response) — `crates/prism-sensors/tests/bc_2_16_015_claroty_vulnerabilities.rs §source_path_id` | AC-008 positive case: when a record carries `"id": "<val>"`, `source_path = "$.id"` extraction lands value into `raw_extensions["id"]`; complements RG-008 absent-case |

**BC-5.38.001 density check:** 11 Red Gate tests (RG-001, RG-002, RG-003a, RG-003b, RG-004, RG-004b, RG-005, RG-006, RG-007, RG-008, RG-009) / 8 acceptance criteria = 1.375 ≥ 0.5 threshold. PASS.
...
|----|-------------|-------------------|
| EC-001 | Row where `name` is absent (REQUIRED column) | `finding_info_title` null (default nullable-column behavior); `ColumnOptions::Required` is push-down eligibility, not presence enforcement; no hard error; pagination continues (EC-016-015-001) |
...
| EC-005 | `cve_ids` field is an empty array `[]` | Serialized as native JSON array `[]` (not the stringified `"[]"`) in `raw_extensions`; not null; `column_type="json"` emits native-array wire shape per BC-2.16.015 §EC-016-015-005 / BC-2.16.016 cross-story standardization |
...
| Test files (11 RGTs; unit + live integration + mock wire-shape) | ~7,000 |
...
- [ ] **Task 3 (Red Gate — test first):** Write RG-006, RG-007, RG-008, RG-009 — unit tests using mock HTTP responses (no live sensor required). These test: REQUIRED name absent → null row; nullable count → empty-page halt; source_path id absent → `id` key absent from `raw_extensions`; source_path id present → value extracted into `raw_extensions["id"]` (AC-008 positive case). Place in `crates/prism-sensors/tests/bc_2_16_015_claroty_vulnerabilities.rs` or `crates/prism-spec-engine/src/pipeline.rs #[cfg(test)]`. All MUST fail before Tasks 6–7.
...
|---------|------|--------|-------|
| 2.2 | 2026-08-30 | story-writer | F-RECON-P1-HIGH-001: removed re-introduced POL-39 volatile version pin from EC-005 prose (cite by ID+§anchor). |
