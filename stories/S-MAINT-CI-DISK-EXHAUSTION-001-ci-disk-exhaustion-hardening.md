...
count=$(grep -cE '^\s+- name: Report initial disk space\s*$' .github/workflows/ci.yml || true)
[ "$count" -ge 2 ] || {
...
The two reclaimer steps differ in step name AND the `if: runner.os == 'Linux'` guard — the
test-matrix leg carries the guard (mixed-OS matrix includes windows/macOS legs), the
test-no-default-features leg is unconditional (ubuntu-only job); matches AC-007's two-variant
precedent. The `with:` block inputs are identical in both cases
...
# the ^ anchor cannot self-match.
count=$(grep -cE '^\s+uses: insightsengineering/disk-space-reclaimer' .github/workflows/ci.yml || true)
[ "$count" -ge 2 ] || {
...
count=$(grep -cE '^\s+- name: Neutralize apt-spy2 sources\.list rewrite' .github/workflows/ci.yml || true)
[ "$count" -ge 2 ] || {
...
(libdbus-1-dev, gnome-keyring, dbus-x11)` (step name, now anchored by step-name). This step is subject to the same EC-010 mirror-flake class.
...
count=$(grep -cE '^\s+if ! \( sudo apt-get update && sudo apt-get install' .github/workflows/e2e.yml || true)
[ "$count" -ge 1 ] || {
...
count=$(grep -cE '^\s+sudo tee /etc/apt/apt-mirrors\.txt' .github/workflows/e2e.yml || true)
[ "$count" -ge 1 ] || {
...
The redesigned fallback has five ordered operations:

**Step 1 — Diagnostic dump on fallback entry (F-MAINT-P10-PG-009 + F-MAINT-P11-CRIT-001):** Three diagnostic commands: `echo "=== [fallback] apt-mirrors.txt on entry ===" && cat /etc/apt/apt-mirrors.txt 2>/dev/null || true`, `echo "=== [fallback] sources.list on entry ===" && cat /etc/apt/sources.list 2>/dev/null || true`, and `echo "=== [fallback] sources.list.d on entry ===" && ls /etc/apt/sources.list.d/ 2>/dev/null || true` — captures mirror config, sources.list content (for apt-spy2 corruption forensics), and source-file listing at fallback entry for future flake forensics without log scraping. `2>/dev/null || true` guards prevent abort on absent file.
...
count=$(grep -cE '^\s+if ! \( sudo apt-get update && sudo apt-get install' .github/workflows/ci.yml || true)
[ "$count" -ge 12 ] || {
...
# Self-match-proof: assertion lines start with whitespace+count=$(grep..., not whitespace+sudo.
count=$(grep -cE '^\s+sudo tee /etc/apt/apt-mirrors\.txt' .github/workflows/ci.yml || true)
[ "$count" -ge 12 ] || {
...
count=$(grep -cE '^\s+sudo apt-get install -y build-essential libc6-dev clang libclang-dev\s*$' .github/workflows/ci.yml || true)
[ "$count" -ge 2 ] || {
...
**Pre-existing apt install steps — two-attempt pattern (AC-006):** The ten `sudo apt-get update && sudo apt-get install ...` steps across the CI pipeline (Install musl-tools in test matrix; Install libdbus-1-dev in test matrix; Install libdbus-1-dev in test-no-default-features; Install libdbus-1-dev in clippy; Install libdbus-1-dev in semver-checks; Install libdbus-1-dev in fuzz-smoke-vp021; Install libdbus-1-dev in perimeter-compile-fail; Install libdbus-1-dev in non-exhaustive-violation-compile-fail; Install libdbus-1-dev in no-hardcoded-sensors-compile-fail; Install shellcheck in shellcheck-demo-scripts) must each be expanded from the single-attempt inline form into the two-attempt pattern specified in AC-006. The redesigned fallback (F-MAINT-P10-CRIT-001 + F-MAINT-P11-CRIT-001) emits three diagnostic dumps of `/etc/apt/apt-mirrors.txt`, `/etc/apt/sources.list`, and `ls /etc/apt/sources.list.d/` on entry (F-MAINT-P10-PG-009), removes third-party source files (4 defensive variants), truncates `/etc/apt/sources.list` (apt-spy2 defense; F-MAINT-P11-CRIT-001), overwrites `/etc/apt/apt-mirrors.txt` with HTTPS-only 2-mirror list: `https://archive.ubuntu.com/ubuntu/` priority:1, `https://security.ubuntu.com/ubuntu/` priority:2 (SEC-002/CWE-319: HTTP mirror removed; the ubuntu package source uses `mirror+file:` scheme — sed URL-rewriting was always a structural no-op; F-MAINT-P10-CRIT-001), calls `sudo dpkg --configure -a 2>/dev/null || true`, then retries. The `2>/dev/null || true` guards silence absent-file errors on the rm and diagnostic commands. The fallback `sudo apt-get update` is NOT `|| true` — canonical archive failures fail loud. No `continue-on-error: true` on these steps; they are not best-effort (unlike the disk-space-reclaimer). See AC-006 for byte-exact snippets and the Red Gate assertions (RG-5 count ≥ 12 outer wrapper; RG-5b count ≥ 12 apt-mirrors.txt tee).
...
| Item | Tokens (approx.) |
|------|-----------------|
| This story spec (1295 lines, 7 ACs, 10 Red Gate tests, 13 fallback YAML blocks) | ~42 k |
| `.github/workflows/ci.yml` (2055 lines; full read required to audit all apt sites and verify-workflow-structure) | ~32 k |
| `.github/workflows/e2e.yml` (AC-006 e2e scope extension) | ~4 k |
| `insightsengineering/disk-space-reclaimer` README / SHA (pre-validated, no lookup needed) | ~1 k |
| `.cargo/config.toml` (AC-003 invariant verification) | ~1 k |
| Implementation scratch + comments | ~2 k |
| **Total** | **~82 k** |

Workable within a single agent context window (200k+ context); the story spec alone (~42 k) is the dominant cost. The implementer should read ci.yml selectively (target jobs + verify-workflow-structure) rather than loading the full 32 k at once where possible.
...
- [ ] Linux Test job legs: add `insightsengineering/disk-space-reclaimer@dae9fabcb8febe09f6585471948acf9dc9a57489 # v1.1.2` step after checkout and before rust-cache; include `if: runner.os == 'Linux'` guard (mixed-OS matrix includes macOS/Windows legs where the reclaimer is not applicable); configure `android: true, dotnet: true, haskell: true, docker-images: true, large-packages: true, swap-storage: false`; include `continue-on-error: true` (reclaim is best-effort — apt-mirror flake class motivated this; the ≥25 GB gate is the sole authoritative check; see EC-009); include `timeout-minutes: 10` (step-level hang guard; F-MAINT-P14-LOW-003 origin; F-MAINT-P20-HIGH-001) (AC-002; swap=false per EC-008)
- [ ] `test-no-default-features` job: add the same reclaimer step with identical inputs (`swap-storage: false`), `continue-on-error: true`, and `timeout-minutes: 10` after checkout and before rust-cache (AC-002 + F-CIDISK-P4-MED-002 + EC-009; step-level hang guard per F-MAINT-P14-LOW-003)
...
- [ ] Verify each fallback block includes three diagnostic dump commands on fallback entry (F-MAINT-P10-PG-009 + F-MAINT-P11-CRIT-001): `echo "=== [fallback] apt-mirrors.txt on entry ===" && cat /etc/apt/apt-mirrors.txt 2>/dev/null || true`, `echo "=== [fallback] sources.list on entry ===" && cat /etc/apt/sources.list 2>/dev/null || true`, and `echo "=== [fallback] sources.list.d on entry ===" && ls /etc/apt/sources.list.d/ 2>/dev/null || true` — these must appear in all 13 fallback blocks (12 ci.yml + 1 e2e.yml); they self-evidence future flake runs (including apt-spy2 sources.list corruption) without requiring log scraping
...
- The `test-no-default-features` job MAY be modified only to add: the four v0.6-ratified protective
  ...
  its libdbus install step, the AC-007 C toolchain baseline install step, the SEC-001 job-level
  `permissions: contents: read` block (CWE-272, human-approved 2026-07-17, security-review
  fix-burst @9c315608), the `timeout-minutes: 45` job-level timeout, and the reclaimer step-level
  `timeout-minutes: 10` (F-MAINT-P14-LOW-003 origin; F-MAINT-P20-HIGH-001 resize, human-approved
  2026-07-17) — the job's existing `PROPTEST_CASES`, `RUSTFLAGS`, test-invocation lines,
  and cache configuration must NOT be changed
  (carve-out ratified v0.6 + F-MAINT-P8-MED-001; neutralization step added v0.20; SEC-001 added v0.25; timeout-minutes keys added v0.26)
...
| Modifying `test-no-default-features` job beyond the four v0.6 protective steps + AC-006 wrapper + AC-007 C toolchain install + SEC-001 permissions block + `timeout-minutes: 45` job-level + reclaimer `timeout-minutes: 10` | The five explicitly-ratified modification types are the only permitted changes; existing `PROPTEST_CASES`, `RUSTFLAGS`, test-invocation lines, and cache configuration must not be changed (F-MAINT-P8-MED-001 carve-out; SEC-001 `permissions: contents: read` carve-out added v0.25, CWE-272, human-approved 2026-07-17, security-review fix-burst @9c315608; timeout-minutes keys carve-out added v0.26, F-MAINT-P14-LOW-003 origin, F-MAINT-P20-HIGH-001 resize, human-approved 2026-07-17) |
...
| EC-010 | Persistent apt-mirror outage affecting pre-existing apt install steps: the runner image selects its apt mirror via `/etc/apt/apt-mirrors.txt` (image 20260714.240.1); ubuntu.sources uses `URIs: mirror+file:/etc/apt/apt-mirrors.txt` — NOT any http:// URL. A flaky priority:1 mirror (`azure.archive.ubuntu.com` at image-build time) causes `apt-get update` to fail with exit code 100. `/etc/apt/sources.list` is effectively empty (comment-only). Evidence: run 29438854846 rerun (2026-07-15) — enzu.com 404; runs 29531645116/29531648104 (0939973f, 2026-07-16) — v0.18 fallback was a structural no-op (sed cannot match `mirror+file:` scheme; probe 29540085270, image 20260714.240.1, confirmed the source-file layout). Runner-image mirror selection is entirely outside our control. | Redesigned two-attempt pattern per AC-006 (F-MAINT-P10-CRIT-001 + F-MAINT-P11-CRIT-001): on first-attempt failure, (1) emit three diagnostic dumps of `/etc/apt/apt-mirrors.txt`, `/etc/apt/sources.list`, and `ls /etc/apt/sources.list.d/` for forensics (F-MAINT-P10-PG-009); (2) remove third-party source files (4 defensive variants: microsoft-prod.list, azure-cli.sources, microsoft-prod.sources, azure-cli.list); (3) `sudo truncate -s 0 /etc/apt/sources.list` — apt-spy2 defense; (4) overwrite `/etc/apt/apt-mirrors.txt` with HTTPS-only 2-mirror list: `https://archive.ubuntu.com/ubuntu/` priority:1, `https://security.ubuntu.com/ubuntu/` priority:2 (SEC-002/CWE-319: HTTP mirror removed) — the `mirror+file:` method re-reads this file on next `apt-get update`; (5) `sudo dpkg --configure -a 2>/dev/null || true` + retry `sudo apt-get update && sudo apt-get install -y <pkgs>`. `archive.ubuntu.com` is authoritative (HTTPS only; APT GPG verification unchanged). No `continue-on-error` on these steps — if the canonical archive also fails, the job fails loud. |
...
- v0.27 (2026-07-17): PR-LEVEL pass-22 fix-burst closing F-MAINT-P22-MED-001. **F-MAINT-P22-MED-001** [EC-001 resize-commit SHA inaccuracy — in-place correction]: the v0.26 changelog row's phrase "implementer resized job-level timeouts at HEAD 9c315608" is inaccurate — 9c315608 is the observation HEAD (run 29615360280; worst green durations 35m39s / 22m33s) and the SEC-001/SEC-002 fix HEAD; the actual resize commit (test 45→75 min, tndef 25→45 min) is d412defe. The v0.26 changelog row is immutable (POL-32). EC-001 in-place correction (mutable body prose): final sentence "job-level values resized at HEAD 9c315608 (F-MAINT-P20-HIGH-001)" changed to "job-level values resized at HEAD d412defe (F-MAINT-P20-HIGH-001, human-approved 2026-07-17)". The two observation-HEAD citations in EC-001's duration clauses ("at HEAD 9c315608") are correct and unchanged. POL-29 sweep: two other current-state 9c315608 references are observation/fix-HEAD citations (§Architecture Compliance Rules "security-review fix-burst @9c315608" SEC-001 carve-out; §Forbidden Patterns "security-review fix-burst @9c315608" SEC-001 carve-out) — both correct, unchanged. Changelog rows v0.24–v0.26 citing 9c315608 are immutable per POL-32, unchanged.

- v0.26 (2026-07-17): PR-LEVEL pass-20 spec-side findings closed (F-MAINT-P20-MED-001 + F-MAINT-P20-HIGH-001 spec-sync). **F-MAINT-P20-MED-001** [scope-contract enumeration of timeout-minutes keys — two exhaustive-allowlist sites]: the pass-14 implementation (HEAD c5e559d3) added `timeout-minutes: 10` step-level to both reclaimer steps and job-level timeouts (`timeout-minutes: 45` for `test`, `timeout-minutes: 25` for `test-no-default-features`); the v0.25 tndef exhaustive allowlists omitted these keys, making the shipped timeout configuration appear out-of-scope. Fixed at two sites (parallel to v0.25 SEC-001 carve-out style): (1) §Architecture Compliance Rules: `test-no-default-features` MAY clause extended with `timeout-minutes: 45` job-level and reclaimer step-level `timeout-minutes: 10` (F-MAINT-P14-LOW-003 origin; F-MAINT-P20-HIGH-001 resize, human-approved 2026-07-17; version note updated to "timeout-minutes keys added v0.26"); (2) §Forbidden Patterns: `test-no-default-features` row Pattern column extended with "+ `timeout-minutes: 45` job-level + reclaimer `timeout-minutes: 10`" and Reason column updated from "four" to "five" ratified modification types with corresponding v0.26 citation. **F-MAINT-P20-HIGH-001 spec-sync** [job-level timeout resize to ~2× observed cold-cache worst, human-approved 2026-07-17]: implementer resized job-level timeouts at HEAD 9c315608 — `test-no-default-features` 25→45 min (observed worst green 22m33s), `test` matrix 45→75 min (observed worst green 35m39s on x86_64-apple-darwin). Spec updated at three sites: (a) EC-001 Expected Behavior: `timeout-minutes: 45` (`test`) → `timeout-minutes: 75`, `timeout-minutes: 25` (`test-no-default-features`) → `timeout-minutes: 45`, evidence and ~2× margin rationale added (F-MAINT-P20-HIGH-001 resize, human-approved 2026-07-17); (b) reclaimer step `timeout-minutes: 10` added to both AC-002 YAML snippets (test-matrix and tndef) after `continue-on-error: true` — step-level key was shipped in code but absent from spec; (c) §Tasks reclaimer bullets and §File Structure Requirements ci.yml MODIFY notes updated to enumerate `timeout-minutes: 10`. POL-29 sweep evidence: `timeout-minutes: 25` (old tndef job-level value): 1 current-state site (EC-001, fixed); 1 historical changelog row (v0.23, immutable per POL-32, unchanged). `timeout-minutes: 45` as test job's OLD value: 1 current-state site (EC-001, updated to 75); 1 historical changelog row (v0.23, immutable, unchanged). "2.5×": 0 occurrences in file. "10.2 min": 0 occurrences. "22 min"/"22.0 min" worst-case claims: 0 occurrences. All current-state citations of old values corrected; historical changelog rows immutable per POL-32.
...
- v0.21 (2026-07-16): PR-LEVEL pass-11 spec-layer fix-burst (2 LOW findings). **F-MAINT-P11-LOW-003** [four-step enumeration contradicts v0.20 snippets]: §AC-006 body prose said "four ordered steps" but v0.20 fallback has 5 operations (3 diagnostics + rm-f + truncate + tee + dpkg). Fixed: "four ordered steps" → "five ordered operations"; Step 1 updated to list all 3 diagnostic dumps (added sources.list); new Step 3 added for `truncate sources.list`; old Steps 3+4 renumbered to 4+5. POL-29 sweep: §Tasks diagnostic dump bullet updated from 2 commands to 3; §Tasks Implementation Notes paragraph updated to include sources.list dump and truncate; EC-010 fallback description updated to 6-step enumeration matching snippets. **F-MAINT-P11-LOW-004** [8 grep-cE snippet forms missing `|| true`]: under `bash -e`, `grep -c` exits 1 when count=0, which would abort the script before the `[ "$count" -ge N ]` check fires and the `::error::` diagnostic would never emit. Added `|| true` to all 8 count-based assertions: AC-001, RG-2 (AC-002 reclaimer), RG-8 (neutralization), RG-7 (e2e), RG-7b (e2e mirrors), AC-006, RG-5b (ci mirrors), AC-007. F-MAINT-P10-LOW-005 precedent. red_gate_tests unchanged (10). Echo arithmetic unchanged.
