...
behavioral_contracts:
  - BC-2.16.009  # Spec File Validation — v1.9 (FB-PR2 spec fix-burst 2026-06-04):
                 ...
                 # Full-match skip-guard (F-PR1-OBS-001): skip fires only when entire method
                 # equals exactly one well-formed env token (not substring containment).
                 # PO FLAG E-SPEC-NNN code assignment: CLOSED — code is E-SPEC-025 (BC-2.16.009
                 # v1.9 §Error Conditions confirms assignment).
# BC status: BC-2.16.009 is active (lifecycle_status: active — auto-promoted at
# PLUGIN-MIGRATION-001-D merge D-776). v1.9 adds SEC-001 echo-cap + OBS-001 full-match guard.
...
#   - Error return using E-SPEC-025 (confirmed by PO in BC-2.16.009 v1.9): 0.5 pts
...
risk_mitigations:
  - "E-SPEC-025 code: CONFIRMED by PO in BC-2.16.009 v1.9 (Wave-5 Phase-A PO burst 2026-06-03; v1.9 FB-PR2 2026-06-04).
...
1. `crates/prism-spec-engine/src/validation.rs` contains a `validate_step_methods()` function
   ...
   against the whitelist per BC-2.16.009 v1.9 §Validation Rules 7.
2. The whitelist `const ALLOWED_HTTP_METHODS: &[&str] = &["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"]`
   ...
   `E-SPEC-025` error (confirmed assigned by PO in BC-2.16.009 v1.9 §Error Conditions).
3. The `_=>GET` fallback in `PipelineExecutor` remains in place as a belt-and-suspenders
   safety net (BC-2.16.009 v1.9 §Validation Rules 7 "belt-and-suspenders" clause).
4. BC-2.16.009 is ALREADY amended to v1.9 (Wave-5 Phase-A PO burst 2026-06-03; v1.9 FB-PR2
...
5. The `method_value` echoed in `E-SPEC-025` is truncated to 32 codepoints via
   ...
   per BC-2.16.009 v1.9 §Validation Rules 7 32-codepoint echo cap). For inputs ≤32 codepoints
...
6. The Rule 7 env-token skip-guard uses a FULL-MATCH check: it skips a `step.method` value only
   ...
   check (F-PR1-OBS-001, per BC-2.16.009 v1.9 §Validation Rules 7 full-match guard clause).
...
|-------|---------|-------|-------------------|
| BC-2.16.009 | v1.9 | Spec File Validation | §Validation Rules 7 HTTP Method Whitelist Validation (AC-7). E-SPEC-025 assigned. Whitelist: `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, `OPTIONS`. Rule 7 runs AFTER Rule 6 env-var resolution; absent `step.method` is NOT an error; wrong-case/unsupported methods produce E-SPEC-025; multi-error collection per INV-ERR-003. 32-codepoint echo cap (SEC-001/CWE-400): `method_value` in E-SPEC-025 truncated via `truncate_at_char_boundary`. Full-match skip-guard (F-PR1-OBS-001): skip fires only when entire method equals one well-formed env token. This story implements the full §Validation Rules 7 specification from BC-2.16.009 v1.9. |

**E-SPEC-025 — CONFIRMED ASSIGNED (BC-2.16.009 v1.9 §Error Conditions):**
E-SPEC-025 is the canonical code for "invalid or unsupported HTTP method in sensor spec step".
The PO flag is CLOSED. Error message template (per BC-2.16.009 v1.9):
...
(traces to BC-2.16.009 v1.9 §Validation Rules 7 — valid method passes; BC-2.16.009 v1.9
...
and the invalid method value in the message, matching the BC-2.16.009 v1.9 message template:
`"Step '<step_name>' in '<sensor_id>.<table_name>' declares method '<method_value>' which is not a supported HTTP method. Supported: GET, POST, PUT, PATCH, DELETE, HEAD, OPTIONS"`.
Spec is rejected at load time. The pipeline `_=>GET` fallback is NEVER reached for this spec.
(traces to BC-2.16.009 v1.9 §Validation Rules 7 — invalid method produces E-SPEC-025;
...
that step (double-reporting prevention per BC-2.16.009 v1.9 §Validation Rules 7 ordering).
(traces to BC-2.16.009 v1.9 §Validation Rules 7 — Rule 7 ordering: runs AFTER Rule 6;
...
(traces to BC-2.16.009 v1.9 §Validation Rules 7 32-codepoint echo cap; BC-2.16.009 v1.9
...
(traces to BC-2.16.009 v1.9 §Validation Rules 7 full-match skip-guard clause; F-PR1-OBS-001)
...
| method_value echoed in E-SPEC-025 MUST be truncated to ≤32 codepoints | BC-2.16.009 v1.9 §VR7 SEC-001 / CWE-400 mitigation | Use `truncate_at_char_boundary(&step.method, 32)` before constructing `InvalidHttpMethod { method_value }` |
| Rule 7 skip-guard MUST be a full-match, not substring | BC-2.16.009 v1.9 §VR7 F-PR1-OBS-001 full-match clause | Use `ENV_TOKEN_FULL_RE.is_match(method)` or equivalent that matches the entire string, not `is_match` on a substring |
...
| `.factory/specs/behavioral-contracts/BC-2.16.009-spec-file-validation.md` | READ ONLY | Already amended to v1.9 (Wave-5 Phase-A PO burst 2026-06-03; v1.9 FB-PR2 2026-06-04). Implementer reads to confirm §Validation Rules 7 + E-SPEC-025. Do NOT amend in this story's PR. |
...
4. **Confirm** E-SPEC-025 is registered in `error-taxonomy.md` and in BC-2.16.009 v1.9
   §Error Conditions. This was done by PO in the Wave-5 Phase-A burst (2026-06-03; v1.9 updated
   2026-06-04 in FB-PR2). The implementer reads both to confirm before writing error-emitting
...
11. **Verify PO artifacts** (READ ONLY — already done by PO): confirm `error-taxonomy.md`
    contains E-SPEC-025 row and BC-2.16.009 v1.9 §Error Conditions contains E-SPEC-025 with
...
- **BC-2.16.009 v1.9** (current, Wave-5 Phase-A PO burst 2026-06-03; FB-PR2 2026-06-04):
  Added §Validation Rules 7 HTTP method whitelist + E-SPEC-025 (v1.8), then 32-codepoint
  echo cap (SEC-001/CWE-400) + full-match skip-guard (F-PR1-OBS-001) (v1.9). The BC is
  already at v1.9. No further PO amendment needed. Implementer reads v1.9 as the authoritative
  spec for this story.
...
| EC-002 | `step.method = "get"` (lowercase) | Invalid per whitelist (case-sensitive); E-SPEC-025 error. BC-2.16.009 v1.9 §Validation Rules 7: case-sensitive match; `"get"` is NOT equivalent to `"GET"`. |
...
| EC-007 | Multiple invalid method steps in same spec | All invalid steps collected; multiple E-SPEC-025 errors in same pass (INV-ERR-003; BC-2.16.009 v1.9 multi-error collection clause) |
