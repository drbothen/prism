...
Running `rg '\[profile\.prepush\]' .config/nextest.toml` returns exactly one hit (the
section header). The broader pattern `rg 'profile.prepush' .config/nextest.toml` returns
two hits: the `[profile.prepush]` section header and the `[[profile.prepush.overrides]]`
signal_handlers entry added by AC-011.
...
following is the verbatim shipped content of those entries (the `[test-groups]` table
and the two `[[profile.prepush.overrides]]`/`[[profile.ci.overrides]]` entries in
...
| 1.7 | 2026-06-27 | story-writer | F-P2R7-MED-001 close + codeblock-verbatim-excerpt durable fix. (1) AC-008 codeblock: removed the editorially-inserted `— NOTE: the shipped stanza supersedes/corrects ...` content from the `# Measured speedup` line; replaced with byte-for-byte verbatim excerpt of the shipped `.cargo/config.toml` sccache stanza (including the `# To enable: uncomment...` paragraph that was previously absent); moved the supersession commentary entirely into the prose paragraph below the codeblock. (2) AC-004 first codeblock: corrected `.map_err` format from single-line inline to the shipped multi-line `|e| { format!(...) }` block (the `.map_err` closure in `build_http_client_with_custom_timeout` in `spec_driven_adapter.rs`). (3) AC-011 codeblock: re-framed from implicitly illustrative to explicitly verbatim — added prose note that the 5-line comment preamble that precedes `[test-groups]` in the file is omitted from the codeblock since the codeblock shows the `[test-groups]` table and `[[profile.prepush.overrides]]`/`[[profile.ci.overrides]]` entries in `nextest.toml`. (4) All other shipped-excerpt codeblocks (AC-001, AC-003) confirmed MATCHES verbatim against shipped files. Durable principle applied: ALL commentary, supersession notes, rationale, and meta-explanation moved to prose; codeblocks contain ONLY byte-for-byte content from the shipped file. |
| 1.8 | 2026-06-27 | story-writer | F-P2R8-LOW-001 close + exhaustive TD-VSDD-091 line-pin elimination. Replaced all 4 remaining volatile line-number/byte-range citations with behavioral anchors: (1) AC-011 prose "lines 90-98 of `.config/nextest.toml`" → "the `[test-groups]` table and the two `[[profile.prepush.overrides]]`/`[[profile.ci.overrides]]` entries in `.config/nextest.toml`"; (2) §Changelog v1.2 "Justfile line 26" → "the `check` recipe's `cargo nextest run` invocation in `Justfile`"; (3) §Changelog v1.7 "bytes 1071-1083 of spec_driven_adapter.rs" → "the `.map_err` closure in `build_http_client_with_custom_timeout` in `spec_driven_adapter.rs`"; (4) §Changelog v1.7 "lines 90-98 of nextest.toml" → "`[test-groups]` table and `[[profile.prepush.overrides]]`/`[[profile.ci.overrides]]` entries in `nextest.toml`". Grep-verified: zero volatile line-number citations remain in narrative prose. Completes the TD-VSDD-091 sweep started in v1.4. |
| 1.9 | 2026-06-27 | story-writer | F-P2R9-LOW-001 close — §Changelog ordering/date inversion fix. Corrected non-monotonic row order (v1.8 was listed before v1.7, violating ascending sequence convention) and reconciled date inversion (v1.8 row had date 2026-06-26 while v1.7 row had 2026-06-27, making v1.8 appear earlier than v1.7). Fixed: v1.7 row now precedes v1.8 row; v1.8 date corrected to 2026-06-27 (monotonic with v1.7). Frontmatter `version:` and `spec_version:` bumped 1.8→1.9. Full changelog 1.0 through 1.9 verified strictly ascending with no duplicate versions and dates monotonically non-decreasing with version order. |
