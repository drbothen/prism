...
| EC-008 | Invalid characters in `clone_name` argument — CWE-117 log-injection surface (SEC-001, 2026-07-17 one-time accelerated-convergence exception, HEAD 5c9458d6) | `validate_clone_name()` runs as the FIRST line of `cmd_configure`, BEFORE `resolve_configure_url`, any sidecar I/O, token lookup, or tracing. Rejects clone names containing characters outside `[a-zA-Z0-9_-]` with error: `"configure: invalid clone name '<sanitized>': clone names may contain only alphanumerics, '-' and '_'"` where `<sanitized>` replaces each disallowed character with `'?'` via `sanitize_clone_name()`. Exits code 1. This rejection happens BEFORE `resolve_configure_url` is called — it precedes all sidecar resolution, HTTP I/O, and tracing. The validation error is NOT an E-DEMO-NNN code and is intentionally excluded from error-taxonomy registration: it is an argument-validation gate, not a runtime state-resolution failure; AC-003's E-DEMO-007 taxonomy scope (unresolvable-token path) is unaffected. Contract-locked by two unit tests in `crates/prism-dtu-demo-server/src/main.rs` `#[cfg(test)] mod tests`: `test_validate_clone_name_rejects_invalid` (inputs: embedded newline `\n`, null byte `\0`, ANSI escape sequence `\x1b[31m`, forward slash `/`, space ` `) and `test_validate_clone_name_accepts_valid` (inputs: bare sensor names `crowdstrike` / `armis`, org-slug composite `acme-crowdstrike`, names with underscore `my_sensor` and hyphen `my-sensor`). |
