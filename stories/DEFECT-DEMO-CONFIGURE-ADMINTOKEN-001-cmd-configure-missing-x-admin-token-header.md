...
| `bc_2_06_019_scenario_progression.rs` demo-server test | `crates/prism-dtu-demo-server/tests/bc_2_06_019_scenario_progression.rs` | ~533-537 | YES | Correct (uses `admin_token()`) |
...
| Per-DTU-crate rate-limit, error-injection, reset, and auth-mode `ac_*` tests | `crates/prism-dtu-{armis,claroty,cyberint,nvd,threatintel}/tests/ac_{6,7,8}_*.rs`, `crates/prism-dtu-{cyberint,nvd}/tests/bc_2_01_017_access_token_auth.rs` etc. | varies | YES | Correct — 22 POST calls; configure state before testing rate-limit / error / auth AC scenarios |
...
| Misc test POST calls — `multi_tenant.rs` (cyberint), `reset_state_invariants.rs` (armis), `dtu_reset_mount.rs` (threatintel), `ac_4/6/7_*.rs` (threatintel), `ac_tests.rs` (slack), `org_tagging.rs` (jira) | Various DTU crates | varies | YES | Correct — ~15 POST calls total; all configure clone state for behavioral tests using `clone.admin_token()` |
...
> **Sweep stats (v0.8 reconciliation, 2026-07-16):** `rg -n 'dtu/configure' crates/ --type rust` returned **451 total hits** across 100+ files. Breakdown: server route registrations 21 · comment/doc lines ~238 (some contain `.post(` text as doc examples — false positives in naive POST filter) · client POST call lines: **116 same-line** (URL inline in `.post(format!(...))`) **+ 7 dynamic-URL** (URL stored in `let url = format!(...); client.post(&url)`) = **123 total POST client calls**. Per-class tally: production CLI POST (defect, now FIXED) 1 · harness library production POST with header 1 · test POST with correct X-Admin-Token 99 · test POST intentionally headerless / no token (negative test) 11 · test POST with wrong token (negative test) 10 · synthetic hung-socket timeout N/A 1. Total: 123. Future passes: re-run the sweep and verify the tally increases only for intentional new tests; any unexpected new headerless POST is a CRITICAL defect.

**Result:** `cmd_configure()` in `main.rs` was the ONLY affected client-side call site (the defect, now FIXED). The complete sweep (123 total POST client calls across all files) confirms no other caller is missing the required header: 99 are correct-token test POSTs, 11 are intentional no-token negative tests (verifying 401 behavior), 10 are wrong-token negative tests (verifying 401 on bad credentials), and 1 is a synthetic hung-socket timeout unit test with no DTU handler (header N/A). The harness library production method `Harness::inject_failure()` (harness.rs:~287) and the harness test callers that invoke it (`bc_3_6_001_ops_clone_failure_modes.rs`, `review_2026_06_10_deny_unknown.rs`) all correctly attach the `x-admin-token` header via `Harness::admin_token_for()`. No production binary or library path outside the demo server and test harness infrastructure calls `/dtu/configure` directly.
