...
#   SS-17 (Plugin Runtime, prism-spec-engine) owns all sandbox BCs (BC-2.17.001..007) and
...
|----|-------|-----------------|
| BC-2.16.002 | Multi-Step Fetch Pipeline — Structured Event Catalog | AC-16 (MAX_REQUESTS_PER_PIPELINE cap; traces to BC-2.16.002 preconditions); Structured Event Catalog Additions §intro (9 new event_type rows) |
...
### AC-9 — Single shared reqwest::Client constructed once at boot with 30-second timeout; injected into PluginRuntime (traces to ADR-023 §C4 plugin HTTP defaults + BC-2.17.002 v1.5 §Error Conditions E-PLUGIN-005; closes TD-S-PLUGIN-PREREQ-B-005)
...
`HostState { allowed_urls: Some(vec![...]), plugin_id: "...".to_string(), ..HostState::test_default() }`
(or override only fields that differ per test). Both the constructor introduction and all 6 site
...
impl HostState {
    ...
    pub fn test_default() -> Self {
        HostState {
            ...
            allowed_urls: None, // default-deny (None = permit-all pre-AC-17); tests that
                                // need allowlist enforcement must override this field.
...
14. **[prism-spec-engine] Verify Structured Event Catalog wiring** — emit each event from the function-name anchor recorded in BC-2.16.002 v1.12 catalog row; if implementation discovers ANY new event_type site beyond the 9 already cataloged, amend BC-2.16.002 in the same commit per PG-LP11-001; see §Structured Event Catalog Additions for the canonical 9-row list and BC source-of-truth for Level/Emitter/Fields/Trigger
...
This is approximately 15.9% of a 256k-token context window — within the 20-30% limit.
...
| `HostState { ... }` struct literal construction — test site 1 | `crates/prism-spec-engine/tests/plugin_tests.rs` | line 287 (`state_open` — full struct literal, 6 fields) | AC-17: migrate to functional-update syntax `HostState { http_client: Arc::new(reqwest::Client::new()), allowed_urls: None, plugin_id: "ac5-test-plugin".to_string(), ..HostState::test_default() }` (or equivalent per-test fields); requires `HostState::test_default()` constructor introduced in same commit |
| `HostState { ... }` struct literal construction — test site 2 | `crates/prism-spec-engine/tests/plugin_tests.rs` | line 305 (`state_restricted` — full struct literal, 6 fields) | AC-17: migrate to functional-update syntax `HostState { allowed_urls: Some(vec!["allowed-sensor.internal".to_string()]), plugin_id: "ac5-test-plugin".to_string(), ..HostState::test_default() }` |
| `HostState { ... }` struct literal construction — test site 3 | `crates/prism-spec-engine/tests/plugin_tests.rs` | line 912 (`state` — full struct literal, 6 fields, `allowed_urls: None`) | AC-17: migrate to functional-update syntax `HostState { plugin_id: "test-plugin".to_string(), ..HostState::test_default() }` (allowed_urls: None is the default_default) |
| `HostState { ... }` struct literal construction — test site 4 | `crates/prism-spec-engine/tests/plugin_tests.rs` | line 946 (`state` — full struct literal, 6 fields, `allowed_urls: Some(vec!["example.com"...])`) | AC-17: migrate to functional-update syntax `HostState { plugin_id: "test-plugin".to_string(), allowed_urls: Some(vec!["example.com".to_string()]), ..HostState::test_default() }` |
...
| `HostState { ... }` struct literal construction — test site 6 | `crates/prism-spec-engine/tests/plugin_tests.rs` | line 1018 (`host_state` — full struct literal, 6 fields, `allowed_urls: None`, `plugin_id: "timeout-test"`) | AC-17: migrate to functional-update syntax `HostState { plugin_id: "timeout-test".to_string(), ..HostState::test_default() }` |
...
Per BC-2.16.002 v1.12 (Canonical Structured Event Catalog) and PG-LP11-001: every new
`tracing::*!(event_type=…)` site introduced by this story is enumerated as a row in the
BC-2.16.002 v1.12 Canonical Structured Event Catalog. The 9 events below have already been
added to BC-2.16.002 (7 in fix-burst-8 commit 4ed96e06; 2 additional in fix-burst-17 BC-2.16.002 v1.12 amendment); the implementer's responsibility is
...
The BC-2.16.002 v1.12 catalog row is the authoritative source for each event's field schema,
...
All four `PluginError` variants must be `#[non_exhaustive]` if `PluginError` is a non-exhaustive enum.
...
1. **PG-LP11-001: New structured event type sites MUST amend BC-2.16.002 in the same burst.**
   This story introduces 9 new event types (see §Structured Event Catalog Additions). All 9 rows
   already exist in BC-2.16.002 v1.12 (Path B Canonical Structured Event Catalog: 7 rows added in
   fix-burst-8 commit 4ed96e06; 2 additional rows for E-PLUGIN-015/016 added in fix-burst-17
   BC-2.16.002 v1.12 amendment). The implementer's responsibility is to wire each emission site to
   match the BC row metadata (Level/Emitter/Fields/Trigger). If implementation discovers ANY new
   event_type site beyond the 9 cataloged, amend BC-2.16.002 in the same commit per PG-LP11-001 invariant.
