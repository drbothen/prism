...
audit-channel routing are orthogonal via `event_type` field per BC-2.16.002 v1.11 catalog
...
- Extract the host from the requested URL using `url::Url::parse`; compare against each entry in
  ...
  to plugin AND a single `tracing::warn!(event_type = "plugin_http_request_blocked", plugin_id, url, reason = "allowlist_mismatch")` emission (single structured emission per BC-2.16.002 v1.11 catalog discipline — WARN-level log and audit-channel routing are orthogonal via `event_type` field)
