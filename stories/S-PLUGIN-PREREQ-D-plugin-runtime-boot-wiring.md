...
audit-channel routing are orthogonal via `event_type` field per BC-2.16.002 v1.12 §Postconditions (Canonical Structured Event Catalog bullet, v1.12) (row plugin_load_unsigned Trigger cell). The MCP server bind proceeds normally (zero plugins registered).
...
### AC-16 — MAX_REQUESTS_PER_PIPELINE cumulative cap enforced in executor loop (traces to TD-S-PLUGIN-PREREQ-B-004; BC-2.16.002 §Postconditions (Canonical Structured Event Catalog bullet, v1.12) row pipeline_max_requests_exceeded)
