...
| BC ID | Version | Title |
|-------|---------|-------|
| BC-2.11.020 | v1.12 | SQL→Pipe Composition — `SqlPipe` AST Variant and FORBID-BOTH Dual-Limit Rule |
...
| EC-002b | `SELECT * FROM t LIMIT 5 | enrich fn(x) | tail 3` — FORBID-BOTH with `| tail` pipe stage (BC-2.11.020 v1.12 EC-11-020-008) | E-QUERY-040 with sql_limit=5, pipe_limit=3; `| tail M` is a row-capping stage in the same FAMILY as `| limit M` |
...
5. Add FORBID-BOTH plan-time check: if `sq.head.limit.is_some()` AND any stage matches `PipeStage::Limit(_) | PipeStage::Tail(_)` (i.e., any row-capping pipe stage per BC-2.11.020 v1.12 FAMILY rule), return `Err(PrismError::RedundantRowLimit { sql_limit, pipe_limit })`.
