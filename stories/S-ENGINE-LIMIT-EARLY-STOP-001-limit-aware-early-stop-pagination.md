...
input-hash: "109f9fa"
# input-hash: updated 2026-08-28 (v1.25); BC-2.16.002 v2.46 (DI-019 cache-completeness ADR-060 §D8.10; pass-18 finding closures)
...
- When DI-019 fires → `FetchOutput.pipeline_truncated` → `FanOutResult.any_pipeline_truncated` → `MaterializationOutput.any_pipeline_truncated` → (a) cache gate `errors.is_empty() && !any_early_stopped && !any_pipeline_truncated` = false → NOT cached; (b) Step 6 `is_truncated = (total_rows>limit) || any_early_stopped || any_pipeline_truncated` = true. Tests: RG-PSG-035, RG-PSG-036.
...
- [ ] **Task 16 (Implementation — full `any_early_stopped` chain wiring):** Wire the
  ...
  **Step G — `crates/prism-query/src/engine.rs` `execute_scheduled_inner` scheduled path:**
  ...
  `just iter prism-bin` — MCP integration tests including RG-PSG-028 wire assertion must pass.
