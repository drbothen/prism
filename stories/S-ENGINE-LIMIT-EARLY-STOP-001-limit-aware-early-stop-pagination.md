...
input-hash: "5fb9e6c"
# input-hash: updated 2026-08-28 (v1.24); BC-2.16.002 v2.45 (EC-01-039 early-stopped response not cached as complete; pass-17 finding closures)
...
- [ ] **Task 16 (Implementation — full `any_early_stopped` chain wiring):** Wire the
  ...
  **Step F — cache-completeness gate (`BC-2.07.003 §Postconditions`):** Locate the query-result
  ...
  ```rust
  if response_errors.is_empty() && !materialization_output.any_early_stopped {
      cache.insert(cache_key, result.clone());
  }
  ```
  An early-stopped result (`any_early_stopped=true`) MUST NOT be written to the cache as a
  complete response. This prevents a subsequent identical query from receiving a partial result
  as authoritative. (Anchored: AC-009; BC-2.16.002 EC-01-039; BC-2.07.003 §Postconditions
  cacheability; RG-PSG-034)

  After steps A–F: run `just iter prism-query` — RG-PSG-027, RG-PSG-028, and RG-PSG-034 MUST
  turn GREEN. Run `just iter prism-sensors` — fan-out tests must pass. Run `just iter prism-bin`
  — MCP integration tests including RG-PSG-028 wire assertion must pass.
