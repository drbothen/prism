...
input-hash: "4d4c809"
# input-hash: refreshed 2026-08-29 (v1.35); BC-2.16.002 bumped v2.49→v2.50 (CursorToken mode-scope §D8.4; EC-01-041/EC-11-094 anchors discharged to GREEN); BC-2.11.001 bumped v1.28→v1.29; hash recomputed (stored 6ba0af1 → 4d4c809)
...
### AC-014: Partial-final-page discriminator is mode-general (OffsetLimit + CursorToken with declared `page_size`): `early_stopped=false` / `is_truncated=false` when source exhausted; CursorToken `None` → conservative fallback (traces to BC-2.11.001 EC-11-094, BC-2.16.002 EC-01-041, ADR-060 §D8.2/§D8.3/§D8.4)
...
**Mode scope (ADR-060 §D8.4):** The discriminator applies across early-stop-eligible pagination modes using `active_page_size`:
- `OffsetLimit`: `active_page_size = page_size` from the sensor TOML `offset_limit` block — discriminator uses this declared page size.
- `CursorToken Some(ps)`: `active_page_size = ps` — discriminator `page_record_count >= ps`; PARTIAL final page yields `early_stopped = false`.
- `CursorToken None` / `PageNumber` / no pagination config: `active_page_size = 0` → discriminator `page_record_count >= 0` is always true → conservative fallback `early_stopped = true` (prevents incorrect `is_truncated=false` for modes where page size is not statically known).
...
**Test:** `test_cursor_token_partial_final_page_not_early_stopped` (RG-PSG-041, pipeline-level unit, CursorToken PARTIAL arm — driving RED test for CursorToken extension)
...
| RG-PSG-041 | `test_cursor_token_partial_final_page_not_early_stopped` | Unit — prism-spec-engine, pipeline internal (`crates/prism-spec-engine/tests/bc_2_16_002_early_stop_tests.rs`); CursorToken pagination with declared page_size=1000, `early_stop_limit=Some(5)`, mock returns exactly 5 records on the first page — PARTIAL final page (5 < 1000) | AC-014 CursorToken PARTIAL arm (ADR-060 §D8.4): `active_page_size = 1000` resolved from `CursorToken Some(1000)`; `page_record_count=5 < 1000` → `PipelineResult.early_stopped = false`; `is_truncated = false`. This is the DRIVING RED test for the `active_page_size` CursorToken branch extension. MUST FAIL before the CursorToken arm is added to `active_page_size` match in `execute_impl` — without the extension, CursorToken falls through to `active_page_size = 0` → discriminator `page_record_count >= 0` is always true → `early_stopped = true` unconditionally → test fails on `== false` assertion. GREEN after CursorToken extension delivered (feature HEAD e2c8d0426). |
| RG-PSG-042 | `test_cursor_token_full_final_page_is_early_stopped` | Unit — prism-spec-engine, pipeline internal (`crates/prism-spec-engine/tests/bc_2_16_002_early_stop_tests.rs`); CursorToken pagination with declared page_size=1000, `early_stop_limit=Some(1000)`, mock returns exactly 1000 records on the first page — FULL final page (1000 >= 1000) | AC-014 CursorToken FULL arm: `active_page_size = 1000`; `page_record_count=1000 >= 1000` → `early_stopped = true`; `is_truncated = true`. Regression guard — confirms the FULL arm fires correctly for CursorToken mode. PASSES both before and after the CursorToken extension (conservative path already yields `early_stopped = true` for the FULL-page case). |
| RG-PSG-043 | `test_cursor_token_no_page_size_conservative` | Unit — prism-spec-engine, pipeline internal (`crates/prism-spec-engine/tests/bc_2_16_002_early_stop_tests.rs`); CursorToken with no declared page_size (`cursor_page_size=None`), `early_stop_limit=Some(5)`, mock returns 5 records | AC-014 CursorToken `None` conservative fallback (ADR-060 §D8.4): `active_page_size = 0` → discriminator `page_record_count >= 0` always true → conservative `early_stopped = true`. Regression guard — confirms CursorToken without a declared page_size does NOT yield `early_stopped = false` (prevents incorrect `is_truncated = false` when page size is unknown). PASSES both before and after extension. |
...
- [ ] **Task 7 (Implementation — execute_impl early-stop check + PipelineResult.early_stopped field):**
  ...
  **After the initial discriminator is GREEN — write RG-PSG-041/042/043 RED BEFORE extending `active_page_size` to CursorToken (SAC-1 red-then-green ordering, ADR-060 §D8.4):**

  Write RG-PSG-041: `test_cursor_token_partial_final_page_not_early_stopped`
  ...
  mock returns exactly 5 records on the first page (PARTIAL page, source exhausted).
  Assert `PipelineResult.early_stopped == false`. MUST FAIL before the CursorToken arm is added
  to `active_page_size` match in `execute_impl` — without the extension, `active_page_size = 0`
  for CursorToken, so discriminator `page_record_count >= 0` is always true → `early_stopped = true`
  unconditionally → test fails on `== false` assertion. This is the DRIVING RED test.
  ...
  Regression guard — PASSES both before and after the extension.
  ...
  `active_page_size = 0` → `5 >= 0` → true). Regression guard — PASSES both before and after.

  Run `just iter prism-spec-engine` — RG-PSG-041 MUST be RED; RG-PSG-042 and RG-PSG-043 may be GREEN.

  **Then extend `active_page_size` match in `execute_impl` to resolve CursorToken page size:**
  In the `active_page_size` derivation within `execute_impl`, extend the `match` arm for `CursorToken`:
  - `CursorToken Some(ps)` → `active_page_size = ps` (uses the declared cursor-token page size)
  - `CursorToken None` → `active_page_size = 0` (conservative fallback, discriminator always true)
  The `OffsetLimit` arm and all other arms are unchanged.

  After the extension: run `just iter prism-spec-engine` — RG-PSG-041 MUST turn GREEN;
  RG-PSG-042 and RG-PSG-043 MUST remain GREEN; RG-002, RG-003, RG-PSG-039 MUST remain GREEN.
