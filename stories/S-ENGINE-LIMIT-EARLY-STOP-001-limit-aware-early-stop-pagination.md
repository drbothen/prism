...
input-hash: "8ae4906"
# input-hash: verified 2026-08-29 (v1.40); inputs at ADR-060 §v1.16, BC-2.16.002 §v2.52, BC-2.16.015 §v1.8, ADR-061 §v1.2; hash changed 019d29c→8ae4906 (BC-2.16.002 v2.51→v2.52 input drift; F-P3-LENSC2-001 propagation)
...
behavioral_contracts:
  ...
  # OffsetLimit (precise discriminator) + CursorToken/PaginationConfig::None (conservative `_ => 0` bucket per ADR-060 §D8.4); D8.5 ORDER BY limitation documented.
...
|----|-------|---------|------|
| BC-2.16.002 | Multi-Step Fetch Pipeline Execution — Sequential Steps with Variable Interpolation | v2.52 | §Postconditions "LIMIT-Aware Early-Stop Pagination (ADR-060 §D8)": FetchContext field, execute_impl check placement, truncated=false semantics, applicable pagination modes, D8.5 ORDER BY limitation. Plan-Shape Gate (ADR-060 §D8.7 / §D8.9): Conditions A–K + conservative default suppress early-stop; where_filters NOT forwarded to gate; `fetch_limit=0` sentinel flow through `QueryParams.limit=0` → `FetchContext::early_stop_limit=None`; EC-016-002-001..018, EC-01-030..041 edge cases. Temporal-exemption soundness (§D8.9): `Ast::Filter` unconditionally suppressed; `PipeStage::Where` unconditionally suppressed; `is_pushed_temporal_predicate` (range-op + INDEX datetime column + `Literal::Timestamp` RHS) replaces `is_purely_temporal_predicate` for SQL/SqlPipe-head WHERE. `PipelineResult.early_stopped: bool` + `FetchOutput { batches, any_early_stopped, pipeline_truncated }` return-type contract; `any_early_stopped` propagation chain: FetchOutput → FanOutResult → MaterializationOutput → engine Step 6. Atomicity-reconciliation scope clause. **EC-01-041 (partial-final-page PARTIAL arm, ADR-060 §D8.2/§D8.4):** when early-stop fires at a PARTIAL final page (`page_record_count < page_size`, source exhausted), `early_stopped = false`; `is_truncated = false` (complete dataset). Discriminator: `early_stopped = page_record_count >= page_size`. Mode-scope (ADR-060 §D8.4): partial-final-page precise discriminator applies to **OffsetLimit only** (`active_page_size = page_size` from TOML); **CursorToken (all sub-cases, incl `Some(ps)` and `None`) → `active_page_size = 0` → conservative `early_stopped = true`** (page-fill is not a valid cursor exhaustion signal per ADR-060 §D8.4; precise detection deferred to S-ENGINE-CURSOR-EXHAUSTION-PRECISE-001). Worked example (a): LIMIT 5 on 5-row tenant, page_size=1000 → is_truncated=false. Anchor: AC-014 + RG-PSG-039/040/041/042/043, GREEN. |
...
*BC-2.11.001 (trace-only — not in behavioral_contracts): EC-11-092 (`any_early_stopped` feeds `is_truncated`; exact-limit boundary: `is_truncated = (total_rows > limit) OR any_early_stopped`; `total_available` is a LOWER BOUND when `any_early_stopped = true`), EC-11-093 (Step 6 as SOLE owner of tool-level cap; materialization returns full set without pre-cap; F-R13-CRIT-001 prohibited), EC-11-094 (partial-final-page PARTIAL arm: when early-stop fires at `page_record_count < page_size`, `early_stopped = false` and `is_truncated = false`; complete dataset; discriminator formula: `early_stopped = page_record_count >= page_size`; mode-scope per ADR-060 §D8.4: OffsetLimit only uses declared page_size; CursorToken (all sub-cases, incl `Some(ps)`) → conservative `early_stopped = true` (S-ENGINE-CURSOR-EXHAUSTION-PRECISE-001); anchor: AC-014 + RG-PSG-039/040/041/042/043, GREEN). Governing contract for the MCP tool response layer. See §References.*
...
| MODIFY | `crates/prism-spec-engine/src/pipeline.rs` | (a) Add `early_stop_limit` field to `FetchContext`; (b) expand `FetchContext::new` signature; (c) add `pub early_stopped: bool` field to `PipelineResult`; (d) add early-stop check after DI-019 in `execute_impl`; (e) **SET `early_stopped = (page_record_count >= active_page_size)` BEFORE `break 'steps`** in the §D8.2/§D8.4 early-stop block (ADR-060 §D8.2/§D8.4 partial-final-page discriminator: `active_page_size` match: `OffsetLimit { page_size } => page_size as usize`; `CursorToken { .. } | _ => 0` (conservative; `_` also captures `PaginationConfig::None` per ADR-060 §D8.4). FULL page (`page_record_count >= active_page_size`) → `early_stopped=true`; PARTIAL page (`page_record_count < active_page_size`, source exhausted) → `early_stopped=false` → `is_truncated=false`; track `page_record_count` per loop iteration); (f) update ~15 in-file test sites (including `default_context()` helper) to pass `None` |
