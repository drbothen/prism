...
input-hash: "6a22a8d"
# input-hash: verified 2026-08-30 (v1.42); inputs at ADR-060 §v1.18, BC-2.16.002 §v2.54, BC-2.16.015 §v1.8, ADR-061 §v1.2; hash changed b8cad49→6a22a8d (ADR-060 v1.17→v1.18 + BC-2.16.002 v2.53→v2.54 input drift; F-B1V-002 pin currency)
...
#       (e) SET `early_stopped = (page_record_count >= active_page_size) || !is_last_batch` BEFORE `break 'steps` in the §D8.2/§D8.4 early-stop block
#           (ADR-060 §D8.2/§D8.3 full discriminator: FULL page-fill term OR non-final fan-out batch term. `active_page_size` match: OffsetLimit { page_size } => page_size as usize; CursorToken { .. } | _ => 0 (conservative; `_` also captures PaginationConfig::None per ADR-060 §D8.4). `is_last_batch` = whether current fan_out_batch is the last in fan_out_batches; non-final batch → early_stopped=true regardless of page fill (remaining batches abandoned). FULL final page → early_stopped=true; PARTIAL final page, source exhausted → early_stopped=false → is_truncated=false)
...
#       `FetchOutput { batches: result.batches, any_early_stopped: pipeline_result.early_stopped, pipeline_truncated: pipeline_result.truncated }`
...
#       crates/prism-bin/src/spec_driven_adapter.rs — `FetchOutput { batches, any_early_stopped: pipeline_result.early_stopped, pipeline_truncated: pipeline_result.truncated }`
#     TEST STUBS (mechanical wrap — `any_early_stopped: false, pipeline_truncated: false`):
...
behavioral_contracts:
  ...
  # OffsetLimit (precise discriminator) + CursorToken/PaginationConfig::None (conservative `_ => 0` bucket per ADR-060 §D8.4); D8.5 ORDER BY limitation documented. BC-2.16.002 v2.54 (EC-01-042 non-final fan-out batch; worked-example-(d) numbers corrected to reachable scenario per ADR-060 §D8.3).
...
DI-019 in `PipelineExecutor::execute_impl`; `truncated` NOT set on early-stop; all pagination
modes covered — `OffsetLimit` (precise discriminator), `CursorToken` and `PaginationConfig::None`
(conservative `_ => 0` bucket per ADR-060 §D8.4); `early_stop_limit = None` = unchanged full
pagination; D8.5 ORDER BY limitation text.
...
**Read §D8.7 (Plan-Shape Gate) and §D8.9 (temporal-exemption soundness):**
`ast_is_reducing_plan(&ast)` function in `materialization.rs`; Conditions A–K + conservative default;
`where_filters` computed for push-down and cache key derivation but NOT forwarded to gate (gate performs
own AST inspection via `has_client_side_where(ast, datetime_index_cols)`); **temporal-exemption
...
gate application in `run_materialization_pipeline`. **Read §D8.2/§D8.3 (early-stop discriminator + `any_early_stopped` propagation):**
Full discriminator: `early_stopped = (page_record_count >= active_page_size) || !is_last_batch`;
`is_last_batch` = whether the current `fan_out_batch` is the last entry in `fan_out_batches` (INTRA-PIPELINE
step fan-out, distinct from multi-sensor `FanOutResult`); non-final batch → `early_stopped=true` regardless
of page fill (EC-01-042, worked example (d)); final/single batch → reduces to page-fill discriminator
(EC-01-041, worked example (a)).
`PipelineResult.early_stopped` field; `FetchOutput { batches, any_early_stopped, pipeline_truncated }` return type;
...
**ADR-061 §D1/D2/D4/D5/D7/D9** is the governing decision for ACs 010–013 (OrgSlug cache-key
isolation / fail-closed slug resolution). Read ADR-061 in full before implementing AC-010
through AC-013. Key decisions: §D1 (cache-key identity invariant — only authoritative
`OrgRegistry::slug_for` or explicit caller `OrgSlug` are valid; 8-hex synthesis and
`"synthetic-unmapped"` sentinel forbidden when `org_registry: Some(_)`); §D2 (fail-closed
policy — skip-with-`tracing::warn!` when registry present but slug missing); §D4 (bare-filter
Step 3b fix — three-arm `match` dispatch replacing unconditional 8-hex synthesis); §D5
(`resolve_source_refs` ALL-scope fix — split unified `else` branch into D1/D2/D3 arms);
§D7 (delete `crates/prism-core/tests/org_slug_from_uuid_prefix.rs` — test legitimizes defect);
§D9 (Red Gate obligations RG-SLUG-001..006, anchored to this story). Security classification:
CWE-284 (Improper Access Control), CWE-340 (Predictable Identifiers), CWE-200 (Information
Exposure), OWASP A01 (Broken Access Control).
...
|----|-------|---------|------|
| BC-2.16.002 | Multi-Step Fetch Pipeline Execution — Sequential Steps with Variable Interpolation | v2.54 | §Postconditions "LIMIT-Aware Early-Stop Pagination (ADR-060 §D8)": FetchContext field, execute_impl check placement, truncated=false semantics, applicable pagination modes, D8.5 ORDER BY limitation. Plan-Shape Gate (ADR-060 §D8.7 / §D8.9): Conditions A–K + conservative default suppress early-stop; where_filters NOT forwarded to gate; `fetch_limit=0` sentinel flow through `QueryParams.limit=0` → `FetchContext::early_stop_limit=None`; EC-016-002-001..018, EC-01-030..042 edge cases. Temporal-exemption soundness (§D8.9): `Ast::Filter` unconditionally suppressed; `PipeStage::Where` unconditionally suppressed; `is_pushed_temporal_predicate` (range-op + INDEX datetime column + `Literal::Timestamp` RHS) replaces `is_purely_temporal_predicate` for SQL/SqlPipe-head WHERE. `PipelineResult.early_stopped: bool` + `FetchOutput { batches, any_early_stopped, pipeline_truncated }` return-type contract; `any_early_stopped` propagation chain: FetchOutput → FanOutResult → MaterializationOutput → engine Step 6. Atomicity-reconciliation scope clause. **EC-01-041 (FINAL-batch partial-page case, ADR-060 §D8.2/§D8.4):** when early-stop fires on the FINAL (or only) fan-out batch at a PARTIAL page (`page_record_count < page_size`, source exhausted), `early_stopped = false`; `is_truncated = false` (complete dataset). Full discriminator: `early_stopped = (page_record_count >= active_page_size) || !is_last_batch`; for the final batch `is_last_batch=true` → reduces to page-fill term only. Mode-scope (ADR-060 §D8.4): partial-final-page precise discriminator applies to **OffsetLimit only** (`active_page_size = page_size` from TOML); **CursorToken (all sub-cases, incl `Some(ps)` and `None`) → `active_page_size = 0` → conservative `early_stopped = true`** (page-fill is not a valid cursor exhaustion signal per ADR-060 §D8.4; precise detection deferred to S-ENGINE-CURSOR-EXHAUSTION-PRECISE-001). Worked example (a): LIMIT 5 on 5-row tenant, page_size=1000 → is_truncated=false. **EC-01-042 (non-final fan-out batch case, ADR-060 §D8.2/§D8.3):** when early-stop fires on a NON-FINAL fan-out batch (`is_last_batch=false`), `early_stopped = true` regardless of page fill (`!is_last_batch` term dominates); remaining batches abandoned by `break 'steps` → dataset incomplete → `is_truncated = true`. Worked example (d): non-final fan-out batch, partial page → `is_last_batch=false` → `early_stopped=true` → `is_truncated=true`. Anchor: AC-014 + RG-PSG-039/040/041/042/043, GREEN; AC-015 + RG-PSG-044, GREEN. |
...
*BC-2.11.001 (trace-only — not in behavioral_contracts): EC-11-092 (`any_early_stopped` feeds `is_truncated`; exact-limit boundary: `is_truncated = (total_rows > limit) OR any_early_stopped`; `total_available` is a LOWER BOUND when `any_early_stopped = true`), EC-11-093 (Step 6 as SOLE owner of tool-level cap; materialization returns full set without pre-cap; F-R13-CRIT-001 prohibited), EC-11-094 (FINAL-batch partial-page PARTIAL arm: when early-stop fires on the final/single fan-out batch at `page_record_count < page_size`, `early_stopped = false` and `is_truncated = false`; complete dataset; full discriminator formula: `early_stopped = (page_record_count >= active_page_size) || !is_last_batch`; for the final batch `is_last_batch=true` → reduces to page-fill term; mode-scope per ADR-060 §D8.4: OffsetLimit only uses declared page_size as `active_page_size`; CursorToken (all sub-cases, incl `Some(ps)`) → conservative `early_stopped = true` (S-ENGINE-CURSOR-EXHAUSTION-PRECISE-001); EC-01-042 covers non-final fan-out batch case (`is_last_batch=false` → `early_stopped=true` regardless of page fill); anchor: AC-014 + RG-PSG-039/040/041/042/043, GREEN; AC-015 + RG-PSG-044, GREEN). Governing contract for the MCP tool response layer. See §References.*
...
**Test:** `test_BC_2_16_002_early_stop_multi_page_stops_after_second_page` (k>1 proof: early_stop_limit=Some(15), page_size=10, 3 pages available; 2 pages fetched, stops after second page accumulates 20 ≥ 15 records; truncated=false)

### AC-004: DI-019 10K truncation check fires BEFORE early-stop; `truncated = true` when DI-019 fires (traces to BC-2.16.002 postcondition — LIMIT-Aware Early-Stop §D8 ordering; DI-019 check position unchanged; ADR-060 §D8.10: PipelineResult.truncated propagates to cache-completeness gate and is_truncated formula via any_pipeline_truncated chain)
...
- When DI-019 fires → `FetchOutput.pipeline_truncated` → `FanOutResult.any_pipeline_truncated` → `MaterializationOutput.any_pipeline_truncated` → (a) cache gate `errors.is_empty() && !any_early_stopped && !any_pipeline_truncated` = false → NOT cached; (b) Step 6 `is_truncated = (total_rows>limit) || any_early_stopped || any_pipeline_truncated` = true. Tests: RG-PSG-035, RG-PSG-036, RG-PSG-037 (real-adapter DI-019 propagation end-to-end via `SpecDrivenSensorAdapter::fetch` + wiremock).
...
`where_filters: &FilterMap` parameter is REMOVED — the gate performs its own
...
- **Condition K** (new; multi-INDEX-datetime conservative suppression) — `collect_datetime_index_cols`
...
**Tests:** RG-PSG-001 through RG-PSG-019, RG-PSG-030c, RG-PSG-030d, RG-PSG-032, RG-PSG-033 (see §Red Gate Tests).

### AC-008: `has_client_side_where` temporal-exemption soundness — Filter-mode and Pipe-`|where` are unconditionally suppressed; SQL temporal PERMIT requires range-op + INDEX-datetime column + `Literal::Timestamp` RHS (traces to BC-2.16.002 postcondition — LIMIT-Aware Early-Stop Condition G revised, ADR-060 §D8.7/§D8.9)

`has_client_side_where(ast, datetime_index_cols)` implements temporal-exemption soundness per ADR-060 §D8.9:

(a) **Filter-mode unconditional suppression** (`Ast::Filter`): returns `true` UNCONDITIONALLY for all filter-mode predicates, including purely temporal ones. `extract_time_bounds_from_predicate` (ADR-033 T1) does NOT process `Ast::Filter` mode — temporal predicates in filter-mode queries are DataFusion client-side filters. The `!is_purely_temporal_predicate` check for this arm was UNSOUND and is removed.
...
**Tests:** RG-PSG-021, RG-PSG-022, RG-PSG-023, RG-PSG-024, RG-PSG-029, RG-PSG-030, RG-PSG-030b, RG-PSG-031
...
**Test:** RG-PSG-025 (Rust-struct layer), RG-PSG-026 (MCP wire layer), RG-PSG-038 (real-adapter early-stop propagation via `SpecDrivenSensorAdapter::fetch` + wiremock)

### AC-010: `resolve_source_refs` ALL-scope — `org_registry: Some(reg)` + slug missing → fan-out target SKIPPED + `tracing::warn!(event_type = "query.org_slug_resolution_failure")`; `org_registry: None` → D3 synthetic slug used, target IS included (traces to BC-2.16.002 §`query.org_slug_resolution_failure` catalog row; ADR-061 D2, D3, D5)
...
### AC-011: Bare-filter fan-out Step 3b (`Ast::Filter` adapter loop) — same three-arm dispatch: `org_registry: Some(reg)` + slug missing → target NOT pushed, warn fired; `org_registry: None` → synthetic slug, target IS pushed (traces to BC-2.16.002 §`query.org_slug_resolution_failure` catalog row; ADR-061 D2, D3, D4)
...
### AC-014: Partial-final-page discriminator applies to **OffsetLimit only**; CursorToken (all sub-cases) → conservative `early_stopped=true`; OffsetLimit PARTIAL final page: `early_stopped=false` / `is_truncated=false` when source exhausted (traces to BC-2.11.001 EC-11-094, BC-2.16.002 EC-01-041, ADR-060 §D8.2/§D8.3/§D8.4)

When the early-stop check fires (`all_records.len() >= limit`) on the FINAL (or only) fan-out batch
(`is_last_batch = true`) but that page is a PARTIAL page (`page_record_count < page_size`, meaning the
sensor API has been exhausted), the correct behavior is `early_stopped = false`. The full discriminator
formula is:

```rust
// ADR-060 §D8.2/§D8.3 full discriminator:
early_stopped = (page_record_count >= active_page_size) || !is_last_batch;
// For the final/single batch: is_last_batch=true → !is_last_batch=false → reduces to page-fill term.
// Non-final fan-out batch (is_last_batch=false): early_stopped=true regardless of page fill (AC-015, EC-01-042).
```
...
**Mode scope (ADR-060 §D8.4):** The partial-final-page precise discriminator applies to **OffsetLimit only**:
- `OffsetLimit`: `active_page_size = page_size` from the sensor TOML `offset_limit` block — discriminator `page_record_count >= page_size` distinguishes FULL pages (more pages may exist) from PARTIAL pages (source exhausted).
- **`CursorToken` (all sub-cases, incl `Some(ps)` and `None`)**: `active_page_size = 0` → conservative `early_stopped = true`. Page-fill is not a valid cursor exhaustion signal per ADR-060 §D8.4 — cursor pagination requires next-cursor presence/absence as the exhaustion indicator. Precise detection deferred to S-ENGINE-CURSOR-EXHAUSTION-PRECISE-001.
...
**Test:** `test_BC_2_16_002_early_stop_partial_final_page_not_early_stopped` (RG-PSG-039, pipeline-level unit, OffsetLimit PARTIAL arm)
**Test:** `test_psg_rg040_partial_final_page_is_truncated_false_wire` (RG-PSG-040, MCP wire-level serialized JSON assertion)
**Test:** `test_cursor_token_partial_page_conservative_early_stopped` (RG-PSG-041, pipeline-level unit, CursorToken partial page → conservative `early_stopped=true` — regression sentinel, F-FP1-LENSA-001)
**Test:** `test_cursor_token_full_final_page_is_early_stopped` (RG-PSG-042, pipeline-level unit, CursorToken FULL arm — regression guard)
**Test:** `test_cursor_token_no_page_size_conservative` (RG-PSG-043, pipeline-level unit, CursorToken None → conservative fallback — regression guard)

### AC-015: Intra-pipeline fan-out multi-batch early-stop — non-final fan-out batch yields `early_stopped=true` regardless of page fill; final/single batch reduces to page-fill discriminator (traces to BC-2.16.002 EC-01-042, ADR-060 §D8.2/§D8.3)

When the early-stop check fires (`all_records.len() >= limit`) during a NON-FINAL fan-out batch
(`is_last_batch = false` — the current `fan_out_batch` is not the last entry in `fan_out_batches`),
`early_stopped = true` regardless of whether the current page is FULL or PARTIAL. The remaining
fan-out batches are abandoned (the outer `break 'steps` exits the loop before they execute), meaning
the dataset is necessarily incomplete. The full discriminator formula is:
...
- **Non-final fan-out batch** (`is_last_batch = false`): `!is_last_batch = true` → `early_stopped = true`
  regardless of page fill. Remaining batches abandoned → truncation certain → `is_truncated = true`.
- **Final or single fan-out batch** (`is_last_batch = true`): `!is_last_batch = false` → reduces to the
  page-fill discriminator `early_stopped = page_record_count >= active_page_size` (AC-014, EC-01-041).

**ADR-060 §D8.3 worked example (d):** Non-final fan-out batch, partial page — `is_last_batch=false` →
`!is_last_batch=true` → `early_stopped = true` → `is_truncated = true` (remaining batches not fetched,
dataset incomplete even though the current page was not full).

**Test:** `test_early_stop_multi_batch_partial_page_is_truncated` (RG-PSG-044, wiremock integration,
`crates/prism-spec-engine/tests/bc_2_16_002_early_stop_tests.rs` — already GREEN)
...
| RG-PSG-030 | `test_psg_rg030_redundant_lower_bound_suppresses_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, `PlanShapeGateMockAdapter` | AC-008 EC-01-034: SQL temporal AND-predicate with two same-direction bounds (`WHERE ts > X AND ts > Y LIMIT 1`) → `count_temporal_bound_directions` yields lower=2 → Step-2 fails → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS). First-wins soundness: second lower bound dropped server-side; permitting early-stop risks silent under-return. MUST FAIL before the ADR-060 §D8.7 fix. Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-034, ADR-060 §D8.7. |
| RG-PSG-031 | `test_psg_rg031_ocsf_arrow_name_permits_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, OCSF-named fixture | AC-008 EC-01-035: SQL temporal range on OCSF-flattened Arrow name (`WHERE time > '...' LIMIT 5`) on an `ocsf_column_naming=true` sensor → `"time"` now in `datetime_index_cols` → `ast_is_reducing_plan = false` (PERMIT), early-stop fires after page 1. MUST FAIL before the ADR-060 §D8.9 fix (prior implementation had only `["timestamp"]` → wrongly SUPPRESSED). Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-035, ADR-060 §D8.9. |
| RG-PSG-030b | `test_psg_rg030b_redundant_upper_bound_suppresses_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, `PlanShapeGateMockAdapter` | AC-008 EC-01-034: SQL temporal AND-predicate with two same-direction UPPER bounds (`WHERE ts < X AND ts < Y LIMIT 1`) → `count_temporal_bound_directions` yields upper=2 → Step-2 fails → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS). Companion to RG-PSG-030 (two-lower). First-wins soundness: second upper bound dropped server-side; permitting early-stop risks silent under-return. MUST FAIL before the ADR-060 §D8.7 fix. Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-034, ADR-060 §D8.7. |
| RG-PSG-030c | `test_psg_rg030c_reversed_operand_suppresses` | IN-CRATE UNIT (defense-in-depth, SAP-3 rule-3 annotated; `materialization.rs` `plan_shape_gate_unit_tests`) | AC-007/EC-01-037: reversed-operand temporal predicate (`WHERE 'X' > ts` — literal on LHS, column on RHS) → `is_pushed_temporal_predicate` returns `false` (requires column on LHS) → `has_client_side_where = true` → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS). SAP-3 rule-3 annotated: parser may reject reversed operands in some surface paths; unit test provides isolated gate verification (defense-in-depth). MUST FAIL before the ADR-060 §D8.7 `is_pushed_temporal_predicate` reversed-operand fix. Anchor BC-2.16.002 EC-01-037, ADR-060 §D8.7. |
| RG-PSG-030d | `test_psg_rg030d_multi_index_datetime_suppresses_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, `PlanShapeGateMockAdapter` | AC-007/EC-01-038 Condition K (ADR-060 §D8.7): the QUERIED SOURCE TABLE schema has ≥2 Datetime+INDEX columns — `collect_datetime_index_cols` counts table-schema INDEX datetime columns; `dt_idx_count >= 2` triggers Condition K → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS), regardless of how many columns the predicate references. MUST FAIL before the ADR-060 §D8.7 Condition K implementation. Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-038, ADR-060 §D8.7. |
| RG-PSG-032 | `test_psg_rg032_cross_sensor_source_scope_suppresses_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, multi-sensor fixture | AC-007/EC-01-036: `collect_datetime_index_cols` shared function (ADR-060 §D8.7) scopes to the current sensor's schema — INDEX datetime columns from sensor-A's schema do NOT bleed into sensor-B's `datetime_index_cols` set during a cross-sensor query. Verifies schema-scoped isolation of the shared extractor across sensor boundaries. MUST FAIL if `datetime_index_cols` bleeds across sensor boundaries (early-stop incorrectly permitted/suppressed for the wrong sensor's temporal predicates). Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-036, ADR-060 §D8.7. |
| RG-PSG-033 | `test_psg_rg033_armis_source_scope_permits_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, `PlanShapeGateMockAdapter` | POSITIVE CONTROL for Condition K: single INDEX datetime column in temporal predicate (`WHERE ts > X LIMIT N`) → `collect_datetime_index_cols` yields exactly 1 INDEX datetime column → Condition K does NOT fire (requires ≥2) → `is_pushed_temporal_predicate` returns `true` (valid range op + INDEX col + concrete timestamp RHS) → `has_client_side_where = false` → `ast_is_reducing_plan = false` → early-stop fires. Confirms Condition K does NOT over-suppress the normal single-INDEX-datetime-column case. MUST PASS before AND after ADR-060 §D8.7 Condition K implementation. Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-038, ADR-060 §D8.7. |
| RG-PSG-034 | `test_psg_rg034_early_stopped_response_not_cached_as_complete` | END-TO-END / Integration via `QueryEngine::execute` or `run_materialization_pipeline` (in `crates/prism-query/tests/execute_integration_tests.rs`) | AC-009 cacheability gate: an early-stopped fetch result (`any_early_stopped=true`) MUST NOT be cached as a complete response; cache-completeness gate requires `errors.is_empty() && !any_early_stopped`; asserts that a subsequent query with the same parameters does NOT receive the partial early-stopped result as if it were authoritative (the cache MISS forces a fresh fetch). MUST FAIL before the `any_early_stopped` signal is plumbed into the cache-completeness gate (Task 16 chain wiring). Anchor BC-2.16.002 EC-01-039, BC-2.07.003 §Postconditions cacheability. |
| RG-PSG-035 | `test_psg_rg035_di019_truncated_response_not_cached_as_complete` | END-TO-END via `QueryEngine::execute` (in `crates/prism-query/tests/execute_integration_tests.rs`) | AC-004 DI-019 cache-completeness: DI-019-truncated fetch (`any_pipeline_truncated=true`, `any_early_stopped=false`) MUST NOT be cached as complete; 2nd same-key query issues a fresh adapter call; gate `errors.is_empty() && !any_early_stopped && !any_pipeline_truncated`. MUST FAIL before Task 16 Step F is extended for `any_pipeline_truncated`. Anchor BC-2.16.002 EC-01-040, BC-2.07.003 §Postconditions cacheability. |
...
| RG-PSG-044 | `test_early_stop_multi_batch_partial_page_is_truncated` | Wiremock integration — prism-spec-engine (`crates/prism-spec-engine/tests/bc_2_16_002_early_stop_tests.rs`); multi-batch fan-out scenario where early-stop fires on a non-final fan-out batch with a partial page (`is_last_batch=false`) | AC-015 non-final fan-out batch case (BC-2.16.002 EC-01-042, ADR-060 §D8.2/§D8.3): `is_last_batch=false` when early-stop check fires → `!is_last_batch=true` → `early_stopped = true` regardless of page fill; remaining batches abandoned → `is_truncated = true`. PASSES in correct implementation (already GREEN). Regression sentinel: prevents reintroduction of a page-fill-only discriminator that would incorrectly set `early_stopped=false` on a partial page of a non-final fan-out batch. |
...
**BC-5.38.001 density check:** 59 Red Gate tests (RG-001 through RG-006 + RG-PSG-001 through RG-PSG-044 incl. 030b/030c/030d + RG-SLUG-001 through RG-SLUG-006; RG-003, RG-PSG-007, RG-PSG-008, RG-PSG-033, RG-SLUG-002, RG-SLUG-004, RG-PSG-041, RG-PSG-042, RG-PSG-043, RG-PSG-044 are regression/positive-control sentinels that pass in correct implementation; RG-PSG-037 and RG-PSG-038 are adapter-boundary guards — mutation-killing coverage of the `SpecDrivenSensorAdapter::fetch` DI-019 and early-stop propagation paths, pass in both states once real-adapter wiring is built; RG-PSG-039 and RG-PSG-040 gate the ADR-060 §D8.2 partial-final-page discriminator (OffsetLimit); RG-PSG-041 is a regression sentinel preventing reintroduction of the unsound cursor `early_stopped=false` arm (F-FP1-LENSA-001; ADR-060 §D8.4 conservative cursor policy); RG-PSG-044 is a regression sentinel for the non-final fan-out batch early-stop case (ADR-060 §D8.2/§D8.3, BC-2.16.002 EC-01-042)) / 15 acceptance criteria ≈ 3.93 ≥ 0.5 threshold. PASS.
...
- ADR-060 §D8.7 — Plan-Shape Gate: `ast_is_reducing_plan(&ast)` Conditions A–K + conservative default; `where_filters` NOT forwarded; enforcement in `materialization.rs §run_materialization_pipeline` before fan-out construction; temporal WHERE safety; ORDER BY non-suppression
...
| EC-005 | `PaginationConfig::None` (single-page fetch) with `early_stop_limit = Some(1)` | `PaginationConfig::None` falls into the conservative `_ => 0` catch-all per ADR-060 §D8.4 — same bucket as `CursorToken`. When the single page has >= `limit` records, the early-stop check fires (`all_records.len() >= 1`): `active_page_size = 0` → `early_stopped = page_record_count >= 0 = true` (conservative over-report) → `is_truncated = true`. Loop MECHANICS: the `PaginationConfig::None` break (`Some(PaginationConfig::None) \| None => break`) fires after the early-stop check, so the early-stop check runs first and sets `early_stopped = true` before the natural exit. This IS behaviorally distinct from `early_stop_limit = None` (which produces `early_stopped = false` and `is_truncated = false`). **Coverage note: intentionally non-behavioral-tested.** The `_ => 0` code path is the SAME match arm as `CursorToken`; RG-PSG-041/042/043 already cover conservative `early_stopped = true` for the `_ => 0` bucket via CursorToken tests. A separate `PaginationConfig::None` behavioral test adds no additional mutation-killing coverage and is not warranted. |
...
| BC-2.16.002 §Postconditions LIMIT-Aware Early-Stop section + §D8.7/§D8.9 plan-shape gate + EC-016-002-001..018 + EC-01-030..042 + §Atomicity Reconciliation clause (1 BC in behavioral_contracts) | ~3,000 |
...
- [ ] **Task 7 (Implementation — execute_impl early-stop check + PipelineResult.early_stopped field):**
  ...
  ```rust
  // ADR-060 §D8.2/§D8.3: LIMIT-aware early-stop. Fires at COMPLETE page boundary, after DI-019.
  ...
  // ADR-060 §D8.2/§D8.3 full discriminator: `(page_record_count >= active_page_size) || !is_last_batch`
  ...
  //   is_last_batch: true when current fan_out_batch is the final entry in fan_out_batches (intra-pipeline).
  //   Non-final batch (is_last_batch=false): early_stopped=true regardless of page fill (EC-01-042,
  //   worked example (d)); remaining batches abandoned → truncation certain.
  //   FULL final page (page_record_count >= active_page_size, is_last_batch=true) → early_stopped=true.
  //   PARTIAL final page (page_record_count < active_page_size, is_last_batch=true, source exhausted)
  //   → early_stopped=false (complete dataset; is_truncated=false). Worked example (a): LIMIT 5 on
  ...
  if let Some(limit) = context.early_stop_limit {
      if all_records.len() >= limit {
          early_stopped = (page_record_count >= active_page_size) || !is_last_batch;  // ADR-060 §D8.2/§D8.3 full discriminator
          break 'steps;
      }
  }
  ```
  ...
  Applies to all pagination modes inside the `'steps:` loop — `OffsetLimit` (precise discriminator),
  `CursorToken` and `PaginationConfig::None` (both use the conservative `_ => 0` bucket per ADR-060 §D8.4:
  `active_page_size = 0` → `early_stopped = true` conservatively when early-stop fires; loop MECHANICS:
  `PaginationConfig::None` breaks naturally after one iteration, but the early-stop check fires BEFORE
  the natural exit). After editing: run
  ...
  **After the initial discriminator is GREEN — write RG-PSG-041/042/043 (ADR-060 §D8.4 conservative cursor policy; SAC-1 red-then-green ordering applies to the OffsetLimit RG-PSG-039/040 tasks above):**

  Write RG-PSG-041: `test_cursor_token_partial_page_conservative_early_stopped`
  in `crates/prism-spec-engine/tests/bc_2_16_002_early_stop_tests.rs`. Construct a
  CursorToken pipeline with `cursor_page_size = Some(1000)`, `early_stop_limit = Some(5)`,
  mock returns exactly 5 records (partial page, source exhausted).
  Assert `PipelineResult.early_stopped == true`. This is a regression sentinel (F-FP1-LENSA-001):
  CursorToken (all sub-cases, incl `Some(ps)`) → `active_page_size = 0` → `page_record_count >= 0`
  always true → conservative `early_stopped = true`. The `_ => 0` catch-all in the `active_page_size`
  match MUST NOT be narrowed to add a `CursorToken Some(ps) => ps` arm — page-fill is not a valid
  ...
  **Verify `active_page_size` uses `_ => 0` for all non-OffsetLimit modes (ADR-060 §D8.4):**
  ...
  with a `_ => 0` catch-all covering CursorToken and all other pagination modes. Do NOT add a
  `CursorToken Some(ps) => ps` arm — this is the unsound cursor arm that was reverted per ADR-060 §D8.4.

  After writing all three tests: run `just iter prism-spec-engine` — RG-PSG-041, RG-PSG-042, and
  RG-PSG-043 MUST all be GREEN; RG-002, RG-003, RG-PSG-039 MUST remain GREEN.
...
- [ ] **Task 9 (Implementation — spec_driven_adapter wiring + FetchOutput return):** In
  ...
  ```rust
  Ok(FetchOutput { batches: pipeline_result.batches, any_early_stopped: pipeline_result.early_stopped, pipeline_truncated: pipeline_result.truncated })
  ```
...
- [ ] **Task 10 (Red Gate — test first):** Write RG-PSG-001 through RG-PSG-019 in
  ...
  **Suppression tests (gate MUST fire; early-stop MUST be suppressed) — END-TO-END / Integration:**
  ...
  - RG-PSG-013 (`test_BC_2_16_002_plan_shape_gate_filter_mode_where_suppresses_early_stop`):
    ...
    `ast_is_reducing_plan = true`. MUST FAIL before Task 12 (prior gate's `where_filters` empty for Filter mode).
  - RG-PSG-014 (`test_BC_2_16_002_plan_shape_gate_pipe_where_suppresses_early_stop`):
    ...
    `ast_is_reducing_plan = true`. MUST FAIL before Task 12 (prior gate's `where_filters` empty for Pipe stages).
  - RG-PSG-015 (`test_BC_2_16_002_plan_shape_gate_non_equality_sql_where_suppresses_early_stop`):
    ...
    `ast_is_reducing_plan = true`. MUST FAIL before Task 12 (prior gate's `where_filters` equality-only).
  ...
  - RG-PSG-030d (`test_psg_rg030d_multi_index_datetime_suppresses_early_stop`): temporal
    predicate referencing ≥2 distinct INDEX datetime columns → `collect_datetime_index_cols` counts
    ≥2 → Condition K fires → `ast_is_reducing_plan = true` → `fetch_limit = 0`. MUST FAIL before
    Task 12 (ADR-060 §D8.7 Condition K not yet implemented). (Anchored: AC-007; EC-01-038; ADR-060 §D8.7)
  - RG-PSG-032 (`test_psg_rg032_cross_sensor_source_scope_suppresses_early_stop`): multi-sensor
    fixture; sensor-A's INDEX datetime cols do NOT bleed into sensor-B's `datetime_index_cols` set.
    Assert schema-scoped isolation. MUST FAIL if `datetime_index_cols` bleeds across sensor boundaries.
    (Anchored: AC-007; EC-01-036; ADR-060 §D8.7)
  ...
  **Suppression tests — IN-CRATE UNIT (defense-in-depth; SAP-3 rule-3; in `materialization.rs` plan_shape_gate_unit_tests):**
  ...
  - RG-PSG-030c (`test_psg_rg030c_reversed_operand_suppresses`): call
    `is_pushed_temporal_predicate` (or call `ast_is_reducing_plan`) directly with an AST containing a
    reversed-operand temporal predicate (`WHERE 'X' > ts` — literal on LHS, column on RHS). Assert
    `is_pushed_temporal_predicate = false` → `has_client_side_where = true` → `ast_is_reducing_plan = true`.
    SAP-3 rule-3: defense-in-depth for paths the parser may reject at the surface; unit test provides
    isolated gate verification. MUST FAIL before the ADR-060 §D8.7 reversed-operand fix.
    (Anchored: AC-007; EC-01-037; ADR-060 §D8.7)
  ...
  **Positive controls (gate MUST NOT fire; early-stop MUST proceed):**
  ...
  - RG-PSG-033 (`test_psg_rg033_armis_source_scope_permits_early_stop`): single
    INDEX datetime column in temporal predicate (`WHERE ts > X LIMIT N`) → `collect_datetime_index_cols`
    yields 1 INDEX datetime col → Condition K does NOT fire (requires ≥2) → `is_pushed_temporal_predicate`
    returns `true` → `ast_is_reducing_plan = false` → early-stop fires. MUST PASS before AND after Task 12
    (confirms Condition K does NOT over-suppress the normal single-INDEX-col case). (Anchored: AC-007;
    EC-01-038; ADR-060 §D8.7)
  ...
  **Temporal-exemption soundness (END-TO-END / Integration via `run_materialization_pipeline`, in-process `PlanShapeGateMockAdapter`):**
  - RG-PSG-021 (`test_psg_filter_mode_temporal_suppresses_early_stop`) in
    ...
    MUST FAIL before Task 12 (prior unsound behavior: temporal filter-mode → PERMIT → under-returned rows).
  - RG-PSG-022 (`test_psg_pipe_where_temporal_suppresses_early_stop`) in the same file.
    ...
    `ast_is_reducing_plan = true`; `fetch_limit = 0`. MUST FAIL before Task 12 (prior unsound behavior:
  ...
  - RG-PSG-030 (`test_psg_rg030_redundant_lower_bound_suppresses_early_stop`) in
    `crates/prism-query/tests/plan_shape_gate_tests.rs`. SQL temporal AND-predicate with two
    same-direction bounds (`WHERE ts > X AND ts > Y LIMIT 1`); `count_temporal_bound_directions`
    yields lower=2 → Step-2 fails → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS).
    MUST FAIL before the ADR-060 §D8.7 fix. Write RED; Task 12 makes it GREEN.
    (Anchored: AC-008 EC-01-034; ADR-060 §D8.7)
  - RG-PSG-031 (`test_psg_rg031_ocsf_arrow_name_permits_early_stop`) in
    `crates/prism-query/tests/plan_shape_gate_tests.rs`. SQL temporal range on OCSF-flattened
    Arrow column name (`WHERE time > '...' LIMIT 5`) on `ocsf_column_naming=true` sensor →
    `"time"` in `datetime_index_cols` → `ast_is_reducing_plan = false` (PERMIT), early-stop fires.
    MUST FAIL before the ADR-060 §D8.9 fix (prior code had only `["timestamp"]` → wrongly SUPPRESSED).
    Write RED; Task 12 makes it GREEN. (Anchored: AC-008 EC-01-035; ADR-060 §D8.9)
  - RG-PSG-030b (`test_psg_rg030b_redundant_upper_bound_suppresses_early_stop`) in
    `crates/prism-query/tests/plan_shape_gate_tests.rs`. SQL temporal AND-predicate with two
    same-direction UPPER bounds (`WHERE ts < X AND ts < Y LIMIT 1`) → `count_temporal_bound_directions`
    yields upper=2 → Step-2 fails → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS).
    Companion to RG-PSG-030 (two-lower). MUST FAIL before the ADR-060 §D8.7 fix.
    Write RED; Task 12 makes it GREEN. (Anchored: AC-008 EC-01-034; ADR-060 §D8.7)
  ...
  **Multi-sensor fan-out / heuristic-rejection (in `crates/prism-query/tests/execute_integration_tests.rs` and `crates/prism-bin/tests/mcp_integration_tests.rs`):**
  - RG-PSG-034 (`test_psg_rg034_early_stopped_response_not_cached_as_complete`) in
    `crates/prism-query/tests/execute_integration_tests.rs`. Issue a query that triggers
    early-stop (`any_early_stopped=true`); assert the response is NOT returned from cache on a
    re-issue with identical parameters (cache-completeness gate `errors.is_empty() && !any_early_stopped`
    prevents caching partial results). MUST FAIL before Task 16 (cache gate not yet plumbed).
    Write RED; Task 16 (chain wiring) makes it GREEN.
    (Anchored: AC-009; BC-2.16.002 EC-01-039; BC-2.07.003 §Postconditions cacheability)
  - RG-PSG-027 (`test_psg_multi_sensor_fanout_exact_limit_one_early_stopped_is_truncated_true`):
...
- [ ] **Task 12 (Implementation — `ast_is_reducing_plan` + `run_materialization_pipeline` gate):**
  ...
  ```rust
  // ADR-060 §D8.7 Condition A (revised): detects FuncCall::Aggregate, FuncCall::Window,
  ...
  fn ast_is_reducing_plan(ast: &Ast) -> bool {
      ...
      // ... full implementation per ADR-060 §D8.7 Conditions A–K + conservative default
  }
  ```
  ...
  RG-PSG-019, RG-PSG-021 through RG-PSG-024, RG-PSG-029 through RG-PSG-033 incl. 030b/030c/030d
  MUST turn GREEN; RG-PSG-007, RG-PSG-008, and RG-PSG-033 (positive controls) MUST remain GREEN.
  (RG-PSG-021..024 test temporal-exemption soundness via the updated `has_client_side_where` logic:
  Filter-mode unconditional suppress, Pipe-WHERE unconditional suppress, Eq-operator temporal suppress,
  non-INDEX column suppress. RG-PSG-029 tests the folded relative-temporal path: `inject_now`
  folds `now() - interval '7d'` to `Literal::Timestamp` BEFORE the gate; suppression is then
  via non-INDEX column check — same mechanism as RG-PSG-024, also exercises the `inject_now`
  fold path. RG-PSG-030 gates redundant-same-direction-bound suppression (EC-01-034).
  RG-PSG-030b gates the two-upper-bound variant of EC-01-034. RG-PSG-030c gates reversed-operand
  suppression (EC-01-037, IN-CRATE UNIT). RG-PSG-030d gates Condition K ≥2 INDEX datetime cols
  suppress (EC-01-038, ADR-060 §D8.7). RG-PSG-031 gates OCSF Arrow-name PERMIT path (EC-01-035).
  RG-PSG-032 gates cross-sensor `collect_datetime_index_cols` schema isolation (EC-01-036).
  RG-PSG-033 (positive control): Condition K does NOT over-suppress single-INDEX-col case.)
...
- [ ] **Task 15 (Final gate — ADR-060 paths):** Run `just check` (full workspace). Confirm all
  ...
  RG-PSG-001 through RG-PSG-033 incl. 030b/030c/030d. Confirm `EXPECTED_SYMBOLS` in
  ...
  `is_pushed_temporal_predicate`). `FetchOutput` is a new `pub` struct in
  `crates/prism-sensors/src/adapter.rs` — it is NOT subject to the `#[non_exhaustive]` gate
  (prism-sensors is out-of-scope; CLAUDE.md §`#[non_exhaustive]` discipline scopes the gate to
  prism-core, prism-spec-engine, and prism-query only; cross-crate by-name construction of
  `FetchOutput` by 21 test stubs and the production adapter requires NOT adding `#[non_exhaustive]`).
  No `EXPECTED_SYMBOLS` update needed for `FetchOutput`.
  ...
  gate range (RG-PSG-001 through RG-PSG-033 incl. 030b/030c/030d). RG-PSG-034 (cache-completeness
  gate — `any_early_stopped` prevents caching partial results) is authored in Task 10 and made
  GREEN in Task 16 (Step F); verify RG-PSG-034 after Task 16 completes. See Task 19 for the
  ADR-061 final gate.
...
- [ ] **Task 16 (Implementation — full `any_early_stopped` chain wiring):** Wire the
  ...
  **Step D — `crates/prism-query/src/engine.rs` Step 6:** Update `is_truncated` formula:
  ```rust
  let is_truncated = total_rows > limit || materialization_output.any_early_stopped || materialization_output.any_pipeline_truncated;
  ```

  **Step E — test-stub sweep (21 files):** Update all test stubs that implement `SensorAdapter`
  to return `Ok(FetchOutput { batches, any_early_stopped: false, pipeline_truncated: false })`. See `crates_touched`
  frontmatter comment for the complete list. This is a mechanical change.

  **Step F — cache-completeness gate (`BC-2.07.003 §Postconditions`):** Locate the query-result
  cache write site in `crates/prism-query/src/engine.rs` (or wherever the result is stored after
  Step 6). Guard the cache write with:
  ```rust
  if response_errors.is_empty() && !materialization_output.any_early_stopped && !materialization_output.any_pipeline_truncated {
      cache.insert(cache_key, result.clone());
  }
  ```
  Neither an early-stopped result (`any_early_stopped=true`) NOR a DI-019-truncated result
  (`any_pipeline_truncated=true`) MUST be written to the cache as a complete response. This
  prevents a subsequent identical query from receiving a partial result as authoritative.
  (Anchored: AC-009; AC-004; BC-2.16.002 EC-01-039; BC-2.16.002 EC-01-040; BC-2.07.003 §Postconditions
  cacheability; RG-PSG-034; RG-PSG-035)

  **Step G — `crates/prism-query/src/engine.rs` `execute_scheduled_inner` scheduled path:**
  Locate the `is_truncated: false` hardcoded value in `execute_scheduled_inner` and replace with:
  ```rust
  is_truncated: output.any_early_stopped || output.any_pipeline_truncated,
  ```
  Rationale: DI-019 can fire on large scheduled scans, setting `any_pipeline_truncated=true`;
  the scheduled path was silently discarding this signal and always reporting `is_truncated: false`
  regardless of whether DI-019 fired (F-R16-P18-LENSA-OBS-001 sibling sweep — ADR-060 §D8.10).
  (Anchored: AC-004; ADR-060 §D8.10)

  After steps A–G: run `just iter prism-query` — RG-PSG-027, RG-PSG-028, RG-PSG-034, RG-PSG-035,
  and RG-PSG-036 MUST turn GREEN. Run `just iter prism-sensors` — fan-out tests must pass. Run
  `just iter prism-bin` — MCP integration tests including RG-PSG-028 wire assertion, RG-PSG-037
  (real-adapter DI-019 propagation), and RG-PSG-038 (real-adapter early-stop propagation) must pass.
...
- [ ] **Task 19 (Implementation — ADR-061 Site 3 sentinel removal + D7 test deletion):**
  ...
  **Site 3 (ADR-061 D3 sentinel removal):** Remove `OrgSlug::new("synthetic-unmapped")` from
  ...
  unconditionally superseded by the D3 deterministic-prefix form from Task 18
  (`format!("org-{}", &org_id.to_string()[..8])` — valid by construction; the `"org-"`
  literal prefix guarantees ORG_SLUG_PATTERN compliance regardless of the hex characters;
  no digit-prefix special case, no fallback). Verify by
...
From ADR-060 §D8.4:
- `PaginationConfig::None` is NOT excluded from the early-stop check. It falls into the
  conservative `_ => 0` catch-all (same bucket as `CursorToken`): `active_page_size = 0` →
  when `all_records.len() >= limit`, `early_stopped = page_record_count >= 0 = true`
  (conservative over-report → `is_truncated = true`). This is distinct from excluding `None`
  entirely: a `PaginationConfig::None` sensor page with >= limit records reports
  `is_truncated = true` (conservative safe over-report), not `is_truncated = false`.
  Loop MECHANICS (still true): the `PaginationConfig::None` loop body naturally exits after
  one page — but the early-stop check fires BEFORE the natural exit and sets
  `early_stopped = true` conservatively. See also §ACR `any_early_stopped` propagation chain
  entry (lines below) which correctly states `CursorToken { .. } | _ => 0` captures `None`.
...
- `ast_is_reducing_plan` MUST be evaluated BEFORE fan-out targets are constructed in
  ...
  and was the root cause of under-detection for Filter-mode, Pipe-stage WHERE, and
  non-equality SQL predicates (ADR-060 §D8.7 Condition G revised).
...
From ADR-060 §D8.7/§D8.9 (temporal-exemption soundness):
- `has_client_side_where` MUST treat `Ast::Filter` mode as UNCONDITIONALLY SUPPRESSED regardless
  ...
  The `!is_purely_temporal_predicate` check for this arm was UNSOUND and is removed.
...
From ADR-060 §D8.2/§D8.3/§D8.9 (`any_early_stopped` propagation chain + full early-stop discriminator):
- `PipelineResult.early_stopped: bool` MUST be set via the full discriminator on the §D8.2 `break 'steps`
  exit (DISTINCT from `truncated`: `truncated` = DI-019 capacity exceeded; `early_stopped` = query-driven early exit).
  **ADR-060 §D8.2/§D8.3 full discriminator:** `early_stopped = (page_record_count >= active_page_size) || !is_last_batch`.
  `is_last_batch` = whether the current `fan_out_batch` is the last entry in `fan_out_batches` (INTRA-PIPELINE
  step fan-out, distinct from multi-sensor `FanOutResult`). Non-final batch (`is_last_batch=false`): `!is_last_batch=true`
  → `early_stopped=true` regardless of page fill; remaining batches abandoned → truncation certain (EC-01-042,
  worked example (d)).
  ...
  For CursorToken and None, `active_page_size = 0` → page-fill term `page_record_count >= 0` always true → conservative over-report.
  FULL final page (`page_record_count >= active_page_size`, `is_last_batch=true`) → `early_stopped = true`.
  PARTIAL final page (`page_record_count < active_page_size`, `is_last_batch=true`, source exhausted) → `early_stopped = false`
  (complete dataset; `is_truncated = false`). Worked example (a): LIMIT 5 on 5-row OffsetLimit tenant,
  page_size=1000 → `active_page_size=1000` → `page_record_count=5 < 1000` → `is_last_batch=true` → `early_stopped=false` → `is_truncated=false`.
  **CRITICAL ORDER:** the discriminator assignment MUST be executed BEFORE `break 'steps`. Assigning
  ...
  (Anchored: RG-PSG-025 `test_psg_exact_limit_is_truncated_true`; RG-PSG-039 `test_BC_2_16_002_early_stop_partial_final_page_not_early_stopped`; RG-PSG-040 `test_psg_rg040_partial_final_page_is_truncated_false_wire`; RG-PSG-044 `test_early_stop_multi_batch_partial_page_is_truncated`; BC-2.11.001 EC-11-094; BC-2.16.002 EC-01-041; BC-2.16.002 EC-01-042; AC-014; AC-015)
- `SensorAdapter::fetch` return type MUST change from `Result<Vec<RecordBatch>, SensorError>` to
  ...
  `Ok(FetchOutput { batches, any_early_stopped: false, pipeline_truncated: false })`. The production adapter MUST return
  `Ok(FetchOutput { batches: result.batches, any_early_stopped: pipeline_result.early_stopped, pipeline_truncated: pipeline_result.truncated })`.
...
| MODIFY | `crates/prism-spec-engine/src/pipeline.rs` | (a) Add `early_stop_limit` field to `FetchContext`; (b) expand `FetchContext::new` signature; (c) add `pub early_stopped: bool` field to `PipelineResult`; (d) add early-stop check after DI-019 in `execute_impl`; (e) **SET `early_stopped = (page_record_count >= active_page_size) || !is_last_batch` BEFORE `break 'steps`** in the §D8.2/§D8.3 early-stop block (ADR-060 §D8.2/§D8.3 full discriminator: page-fill term OR non-final fan-out batch term. `active_page_size` match: `OffsetLimit { page_size } => page_size as usize`; `CursorToken { .. } | _ => 0` (conservative; `_` also captures `PaginationConfig::None` per ADR-060 §D8.4). `is_last_batch` = current fan_out_batch is last in fan_out_batches. Non-final batch: `early_stopped=true` regardless of page fill (EC-01-042). FULL final page (`page_record_count >= active_page_size`) → `early_stopped=true`; PARTIAL final page (`page_record_count < active_page_size`, source exhausted) → `early_stopped=false` → `is_truncated=false`; track `page_record_count` per loop iteration); (f) update ~15 in-file test sites (including `default_context()` helper) to pass `None` |
...
| MODIFY (×21) | Test stub sweep — all `impl SensorAdapter` in test files (see `crates_touched` frontmatter comment) | Mechanical wrap: `Ok(Vec<RecordBatch>)` → `Ok(FetchOutput { batches, any_early_stopped: false, pipeline_truncated: false })` for all test stubs |
...
| CREATE or EXTEND | `crates/prism-bin/tests/bc_2_16_002_early_stop_adapter_tests.rs` OR extend existing | RG-005, RG-006; RG-PSG-037 (`test_psg_rg037_di019_truncation_propagates_through_real_adapter` — real-adapter DI-019 propagation guard via `SpecDrivenSensorAdapter::fetch` + wiremock); RG-PSG-038 (`test_psg_rg038_early_stop_propagates_through_real_adapter` — real-adapter early-stop propagation guard via `SpecDrivenSensorAdapter::fetch` + wiremock) |
| CREATE or EXTEND | `crates/prism-query/tests/plan_shape_gate_tests.rs` OR extend `materialization_tests.rs` | RG-PSG-001 through RG-PSG-019, RG-PSG-021 through RG-PSG-024, RG-PSG-029, RG-PSG-030, RG-PSG-030b, RG-PSG-030d, RG-PSG-031, RG-PSG-032, RG-PSG-033; RG-PSG-009/012/019 are in-crate unit tests in `materialization.rs` `#[cfg(test)] mod plan_shape_gate_unit_tests`; RG-PSG-021..024 are temporal-exemption soundness E2E tests (Filter-mode/Pipe-WHERE/SQL-Eq/non-INDEX); RG-PSG-029..031 are temporal-exemption regression guards (relative-temporal fold path, two-lower-bounds, OCSF Arrow name); RG-PSG-030b is the two-upper-bound companion to RG-PSG-030; RG-PSG-030d is Condition K E2E (ADR-060 §D8.7 ≥2 INDEX datetime cols); RG-PSG-032 is cross-sensor schema isolation for `collect_datetime_index_cols`; RG-PSG-033 is the Condition K positive control (single INDEX datetime col → PERMIT) |
...
- `scripts/check-non-exhaustive-per-symbol.py` — `FetchContext` gets a new field only (no new type for gate purposes); `FetchOutput` IS a new type but lives in `crates/prism-sensors/` (out-of-scope crate; CLAUDE.md §`#[non_exhaustive]` discipline scopes the gate to prism-core, prism-spec-engine, and prism-query; cross-crate by-name construction forbids adding `#[non_exhaustive]`). No EXPECTED_SYMBOLS update needed.
...
- BC-2.16.002 EC-01-041 — FINAL-batch partial-page PARTIAL arm: when early-stop fires on the final/single fan-out batch at `page_record_count < page_size` (source exhausted), `early_stopped=false`; `is_last_batch=true` → reduces to page-fill discriminator; governed by ADR-060 §D8.2/§D8.3
- BC-2.16.002 EC-01-042 (NEW) — non-final fan-out batch early-stop: when `is_last_batch=false`, `early_stopped=true` regardless of page fill; remaining batches abandoned → `is_truncated=true`; governed by ADR-060 §D8.2/§D8.3 full discriminator; anchor: AC-015 + RG-PSG-044
...
- ADR-060 §D8.7 / §D8.9 — Plan-Shape Gate: `ast_is_reducing_plan(&ast)` Conditions A–K + conservative default; `where_filters` NOT forwarded to gate; enforcement in `run_materialization_pipeline` before fan-out construction; temporal-exemption soundness (`has_client_side_where` Filter-mode/Pipe-WHERE unconditional suppress; `is_pushed_temporal_predicate` for SQL/SqlPipe-head); `FetchOutput`/`any_early_stopped` propagation chain; ORDER BY non-suppression
...
- ADR-061 §D1 — Cache-Key Identity Invariant: `derive_response_cache_key` MUST use authoritative `OrgSlug` only; 8-hex synthesis forbidden when `org_registry: Some(_)` (governing AC-010..013)
- ADR-061 §D2 — Fail-Closed Policy: skip-with-`tracing::warn!(event_type = "query.org_slug_resolution_failure")` when registry present but slug missing (CWE-284/340, OWASP A01)
- ADR-061 §D4 — Bare-Filter Step 3b Fix: replace unconditional 8-hex `OrgSlug::new(format!("org-{}",…))` with three-arm registry-first `match` dispatch (governing AC-011)
- ADR-061 §D5 — `resolve_source_refs` ALL-scope Fix: split unified `else` branch into D1 authoritative / D2 skip-with-warn / D3 synthetic arms (governing AC-010)
- ADR-061 §D7 — Test Removal: delete `crates/prism-core/tests/org_slug_from_uuid_prefix.rs` (legitimizes defect; governing AC-012)
- ADR-061 §D9 — Red Gate Obligations: RG-SLUG-001..006 anchored to this story
...
|---------|------|--------|-------|
| 1.42 | 2026-08-30 | story-writer | **F-B1V-002 story-side closure — BC-2.16.002 v2.53→v2.54 + ADR-060 v1.17→v1.18 pin currency + input-hash refresh + TD-VSDD-091 volatile-SHA removal.** (1) **AC-015 number-fix check (DO item 1):** OLD unreachable numbers (page_size=1000, partial page=3) are NOT present in AC-015, RG-PSG-044, §ACR, Task-7, or §References — all loci describe worked example (d) in general terms only; no number correction needed. AC-015 and RG-PSG-044 are behaviorally consistent with BC-2.16.002 EC-01-042 and the corrected ADR-060 §D8.3 worked example (d) (page_size=10, LIMIT 5, 2 batches, batch-0 partial page 5 records, discriminator (5>=10)\|\|true=true). (2) **BC-2.16.002 structured pin: v2.53→v2.54** — BC table Version cell updated; frontmatter BC comment updated; reflects F-B1V-001 EC-01-042 anchor concretization + F-B1V-002 worked-example-(d) number correction. (3) **RG-PSG-044 volatile SHA removed** — `code HEAD `704aac24a`` removed per TD-VSDD-091 anti-volatile-pin (mirror of BC-2.16.002 v2.54 F-B1V-001 `@704aac24a` removal); RG-PSG-044 behavioral description unchanged. (4) **input-hash: b8cad49→6a22a8d** (ADR-060 v1.17→v1.18 + BC-2.16.002 v2.53→v2.54 input drift; hook-computed). §Authority and narrative cites remain §-anchor only per POL-39 — no version pins in narrative text; no narrative changes. TD-VSDD-097: Dim-1 — no named story-twin; BC table version pin, frontmatter BC comment, RG-PSG-044 row are three affected loci; all swept; CLEAR. Dim-2 — BC table version pin is not a verbatim copy-source in any downstream artifact; CLEAR. Dim-3 — no new MUSTs introduced; CLEAR. |
| 1.41 | 2026-08-30 | story-writer | **F-B1-001 closure — AC-015 (non-final fan-out batch early-stop) + RG-PSG-044 + full discriminator formula reconciliation + BC-2.16.002 v2.53 sync + input-hash refresh.** (1) **AC-015 added** (acceptance_criteria_count 14→15): non-final fan-out batch early-stop — when `is_last_batch=false` and early-stop check fires, `early_stopped=true` regardless of page fill; remaining batches abandoned → truncation. Traces to BC-2.16.002 EC-01-042 + ADR-060 §D8.2/§D8.3. (2) **RG-PSG-044 added** (red_gate_tests 58→59): records existing GREEN test `test_early_stop_multi_batch_partial_page_is_truncated` (`crates/prism-spec-engine/tests/bc_2_16_002_early_stop_tests.rs`, code HEAD `704aac24a`); role: AC-015 non-final-batch case (EC-01-042, ADR-060 §D8.2/§D8.3); regression sentinel preventing reintroduction of page-fill-only discriminator; PASSES in correct implementation. BC-5.38.001 density check: 59/15≈3.93 PASS. (3) **Full discriminator formula reconciled at seven loci** to `(page_record_count >= active_page_size) || !is_last_batch` with intra-pipeline `is_last_batch` note: (a) frontmatter `crates_touched` item (e); (b) Task-7 code snippet; (c) §Architecture Compliance Rules discriminator entry; (d) §File Structure Requirements item (e); (e) AC-014 code block (final-batch reduction note added); (f) BC-2.16.002 table row EC-01-041 Discriminator — was `page_record_count >= page_size`; (g) BC-2.11.001 trace note EC-11-094 Discriminator — was `page_record_count >= page_size`. (4) **BC-2.16.002 table row v2.52→v2.53**: EC range `EC-01-030..041` → `EC-01-030..042`; EC-01-042 description added; EC-01-041 noted as FINAL-batch case; Anchor extended with AC-015 + RG-PSG-044. (5) **BC-2.11.001 trace note EC-11-094 updated**: formula extended; EC-01-042 added; anchor extended with AC-015 + RG-PSG-044. (6) **§Authority §D8.2/§D8.3 note** extended with `is_last_batch` description and EC-01-042 reference. (7) **§References**: EC-01-041 updated (FINAL-batch qualifier); EC-01-042 bullet added; EC range updated to `EC-01-030..042` in §Token Budget and BC-2.16.002 §Edge Cases reference. (8) **BC-2.16.002 frontmatter comment** updated v2.52→v2.53. (9) **input-hash** recomputed (ADR-060 §v1.17, BC-2.16.002 §v2.53 input drift); previous 8ae4906. TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; seven-locus sweep documented below; CLEAR. Dim-2 — BC-2.16.002 table row, BC-2.11.001 trace note, §ACR, §FSR item (e), crates_touched item (e), Task-7 code snippet, AC-014 code block, §Authority, §References all consistently reflect the full discriminator formula; `acceptance_criteria_count: 15` and `red_gate_tests: 59` consistent in frontmatter, BC-5.38.001 density paragraph (59/15≈3.93), §Red Gate Tests table (59 rows); FULL. Dim-3 — AC-015 MUST anchored to RG-PSG-044 + BC-2.16.002 EC-01-042 + ADR-060 §D8.2/§D8.3; §ACR discriminator MUST anchored extended with AC-015/RG-PSG-044/EC-01-042; no unanchored MUSTs introduced; CLEAR. |
| 1.40 | 2026-08-29 | story-writer | **F-P3-LENSC2-001 propagation — §ACR + EC-005 None-mode reconciliation + BC-2.16.002 v2.52 sync + input-hash refresh.** (1) **§Architecture Compliance Rules ADR-060 §D8.4 entry (primary stale locus):** "Early-stop applies ONLY to OffsetLimit and CursorToken" rewritten to the §D8.4 conservative-bucket model — `PaginationConfig::None` is NOT excluded; falls into `_ => 0` catch-all (same as CursorToken) → `active_page_size = 0` → `early_stopped = true` (conservative over-report) when early-stop fires. Loop MECHANICS note preserved ("None breaks naturally after one page — but early-stop fires first"). (2) **EC-005 None-mode reconciliation:** "documented no-op" and "net behavior is identical to the `None` case" descriptions replaced with §D8.4 model — `active_page_size = 0` → `early_stopped = true` → `is_truncated = true` (conservative over-report). Coverage note "intentionally non-behavioral-tested" PRESERVED with corrected rationale: `_ => 0` arm already covered by RG-PSG-041/042/043 (CursorToken conservative tests, same match arm). (3) **§Authority text:** "OffsetLimit and CursorToken only" updated to "all pagination modes covered — OffsetLimit (precise discriminator), CursorToken and PaginationConfig::None (conservative `_ => 0` bucket)". (4) **Frontmatter BC comment (line ~140):** "OffsetLimit and CursorToken only" updated to reflect conservative-bucket model. (5) **Task 7 applicability statement:** "Applies to OffsetLimit and CursorToken pagination" updated to "Applies to all pagination modes inside the `'steps:` loop". (6) **BC-2.16.002 table row version:** v2.51 → v2.52 (downstream copy target; BC corrected to conservative-bucket model). (7) **input-hash:** 019d29c → 8ae4906 (BC-2.16.002 v2.51→v2.52 input drift). No AC count (14), RG count (58), density (4.14), or behavioral changes. TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; six-locus sweep documented; CLEAR. Dim-2 — §Authority text + frontmatter BC comment + BC table row are downstream representations of the §D8.4 applicability claim; all three swept and updated in this burst; CLEAR. Dim-3 — no new MUSTs introduced; existing MUST anchors unchanged; CLEAR. |
| 1.39 | 2026-08-29 | story-writer | **F-P1-LENSC2-001 (MED) + F-P1-LENSC2-002 (LOW) + OBS-lensC1 — five-locus discriminator sweep + input-hash provenance update.** (1) **F-P1-LENSC2-001 (MED) — §File Structure Requirements prism-spec-engine MODIFY row item (e):** Three defects fixed: (a) `early_stopped = (page_record_count >= page_size)` → `early_stopped = (page_record_count >= active_page_size)`; (b) CursorToken/None conservative arm added (`CursorToken { .. } | _ => 0` → conservative `early_stopped=true`; `_` also captures `PaginationConfig::None` per ADR-060 §D8.4); (c) cite corrected `§D8.2/§D8.3` → `§D8.2/§D8.4`. (2) **F-P1-LENSC2-002 (LOW) — AC-014 discriminator code block:** `early_stopped = page_record_count >= page_size` → `early_stopped = page_record_count >= active_page_size` for whole-story consistency (OffsetLimit: `active_page_size = page_size`; CursorToken: `active_page_size = 0`; per §Mode scope ADR-060 §D8.4). (3) **OBS-lensC1 — input-hash provenance comment updated:** prior comment cited only v1.37 recompute and ADR-060 §v1.14→v1.15; updated to reflect v1.39 verification pass at ADR-060 §v1.16, BC-2.16.002 §v2.51, BC-2.16.015 §v1.8, ADR-061 §v1.2; hash 019d29c confirmed current (validate-input-hash hook). Five-locus post-fix enumeration: (1) crates_touched item (e) — `active_page_size`, three-arm, §D8.2/§D8.4 — CORRECT pre-existing; (2) Task-7 code snippet — `active_page_size`, §D8.2/§D8.4 — CORRECT pre-existing; (3) §Architecture Compliance Rules — `active_page_size`, three-arm, §D8.2/§D8.4 — CORRECT pre-existing; (4) §File Structure Requirements item (e) — FIXED this burst (was: `page_size`, two-arm, §D8.2/§D8.3); (5) AC-014 code block — FIXED this burst (was: `page_size`). No AC count (14), RG count (58), density (4.14), or behavioral changes. TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; §File Structure Requirements item (e) and AC-014 code block are not transcribed verbatim into any downstream artifact. Dim-2 — no downstream copy-target for either fixed locus. Dim-3 — no new MUSTs introduced; existing MUST anchors unchanged. |
...
| 1.27 | 2026-08-28 | story-writer | **F-P20-LENSB-LOW-001 AC-003 constant alignment + ADR-060 §D8.9 input-hash refresh.** (1) **AC-003 constant alignment (F-P20-LENSB-LOW-001):** k>1 multi-page proof test citation in AC-003 corrected from `early_stop_limit=Some(11), 20 ≥ 11` to `early_stop_limit=Some(15), 20 ≥ 15` to match the on-disk green test `test_BC_2_16_002_early_stop_multi_page_stops_after_second_page` which uses `FetchContext::new(..., Some(15))` and asserts `20 >= 15`. Both constants prove the same `ceil(N/10)=2` k>1 boundary; coverage is genuinely present — this is pure story-text drift corrected by aligning the story to the test. Within-file sweep (TD-VSDD-097 Dim-1): single occurrence in the AC-003 body; no duplicate cites in §Tasks, §Red Gate Tests, or AC table — confirmed by whole-file grep for `Some(11)` / `≥ 11`. (2) **ADR-060 §D8.9 input-hash refresh:** ADR-060 updated §D8.9 (doc-sync reconciling the struct block to the canonical 3-field form already present in this story since v1.25; no semantic change). Input-hash refreshed `d9912a6` → `7d05bd2` (computed by validate-input-hash hook). Story semantic consistency with ADR-060 §D8.9 confirmed: `FetchOutput { pub batches, pub any_early_stopped, pub pipeline_truncated }` 3-field form present throughout story body (§Authority, §crates_touched, §Behavioral Contracts table, AC-009, §Architecture Compliance Rules, §File Structure Requirements, §Previous Story Intelligence). No structural counts changed (AC count 13, RG count 53, density 4.08 all unchanged). TD-VSDD-097: Dim-1 — no named story-twin; within-file sweep confirmed single AC-003 occurrence changed; CLEAR. Dim-2 — no downstream copy-target sections for the AC-003 test citation line; input-hash updated in frontmatter only (no body-level hash duplication); CLEAR. Dim-3 — no new MUSTs introduced; CLEAR. |
| 1.26 | 2026-08-28 | story-writer | **Pass-19 adapter-boundary coverage: RG-PSG-037/038 table rows, density 51→53, AC-009 RG-PSG-038 citation, §File Structure update, Task 16 prism-bin gate extended.** (1) **RG-PSG-037 and RG-PSG-038 added** to §Red Gate Tests table after RG-PSG-036: RG-PSG-037 (`test_psg_rg037_di019_truncation_propagates_through_real_adapter`, `crates/prism-bin/tests/bc_2_16_002_early_stop_adapter_tests.rs`, AC-004 DI-019 adapter propagation, asserts `FetchOutput.pipeline_truncated == true` AND `any_early_stopped == false`, anchor BC-2.16.002 EC-01-040 + ADR-060 §D8.10); RG-PSG-038 (`test_psg_rg038_early_stop_propagates_through_real_adapter`, same file, AC-009 early-stop adapter propagation, asserts `FetchOutput.any_early_stopped == true` AND `pipeline_truncated == false`, anchor ADR-060 §D8.9). (2) **BC-5.38.001 density paragraph updated**: 51→53 Red Gate tests; range `RG-PSG-001 through RG-PSG-038 incl. 030b/030c/030d`; ratio 3.92→4.08; RG-PSG-037/038 noted as adapter-boundary guards (mutation-killing coverage, pass in both states once real-adapter wiring is built). (3) **AC-009 Tests line extended**: added `RG-PSG-038 (real-adapter early-stop propagation via SpecDrivenSensorAdapter::fetch + wiremock)`. (4) **BC-2.11.001 pin check**: BC-2.11.001 is a trace reference in this story; it carries NO numeric version pin anywhere in the story body (not in §Behavioral Contracts table, not in §References, not in Token Budget) — no v1.26→v1.27 update needed. (5) **§File Structure Requirements updated**: `bc_2_16_002_early_stop_adapter_tests.rs` row extended with RG-PSG-037/038 test names and descriptions. (6) **Task 16 prism-bin gate extended**: `just iter prism-bin` gate now names RG-PSG-037 and RG-PSG-038 alongside RG-PSG-028. STORY-INDEX row version-pin update deferred to state-manager (target pin v1.26). TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; ADR-060 §D8.9/§D8.10 have no sibling ADR at the same decision level; CLEAR. Dim-2 — `red_gate_tests: 53` consistent in frontmatter, BC-5.38.001 density paragraph (53/13≈4.08), §Red Gate Tests table (53 rows: RG-001..006 + RG-PSG-001..038 incl. 030b/030c/030d + RG-SLUG-001..006), Task 16 prism-bin gate (RG-PSG-037/038 named), §File Structure `bc_2_16_002_early_stop_adapter_tests.rs` row; AC-009 Tests line extended; FULL. Dim-3 — RG-PSG-037 MUST anchored to AC-004 + BC-2.16.002 EC-01-040 + ADR-060 §D8.10 in §Red Gate Tests table; RG-PSG-038 MUST anchored to AC-009 + ADR-060 §D8.9 in §Red Gate Tests table and AC-009 Tests line; no unanchored MUSTs introduced; CLEAR. |
| 1.25 | 2026-08-28 | story-writer | **ADR-060 §D8.10 DI-019 cache-completeness propagation; pass-18 records closure (F-R16-P18-LENSC-LOW-001).** (1) **DI-019 cache-completeness chain (ADR-060 §D8.10):** `FetchOutput.pipeline_truncated: bool` field added to carry DI-019 truncation signal; propagates through `FanOutResult.any_pipeline_truncated` → `MaterializationOutput.any_pipeline_truncated`; (a) cache gate amended to `errors.is_empty() && !any_early_stopped && !any_pipeline_truncated`; (b) engine Step 6 formula amended to `total_rows > limit \|\| any_early_stopped \|\| any_pipeline_truncated`. (2) **AC-004 amended**: traces line updated to cite `ADR-060 §D8.10` and DI-019 check position; DI-019 propagation sub-bullet added naming the chain and anchoring RG-PSG-035, RG-PSG-036. (3) **RG-PSG-035 and RG-PSG-036 added** to §Red Gate Tests table: RG-PSG-035 (`test_psg_rg035_di019_truncated_response_not_cached_as_complete`, execute_integration_tests.rs, cache-completeness gate); RG-PSG-036 (`test_psg_rg036_di019_truncated_step6_is_truncated_true`, execute_integration_tests.rs, Step-6 formula). (4) **Task 16 updated**: Step D formula extended for `any_pipeline_truncated`; Step F cache guard extended for `any_pipeline_truncated`; Step G added (`execute_scheduled_inner` scheduled path — replace hardcoded `is_truncated: false` with `output.any_early_stopped \|\| output.any_pipeline_truncated`; rationale F-R16-P18-LENSA-OBS-001 sibling sweep, ADR-060 §D8.10); "After steps A–F" updated to "After steps A–G". (5) **§File Structure Requirements updated**: `adapter.rs` `FetchOutput` gains `pipeline_truncated`; `fanout.rs` `FanOutResult` gains `any_pipeline_truncated`; `materialization.rs` gains `any_pipeline_truncated` in `MaterializationOutput`; `engine.rs` Step 6 formula + `execute_scheduled_inner` noted; `execute_integration_tests.rs` RG-PSG-034/035/036 listed. (6) **BC-2.16.002 §Behavioral Contracts table Version cell** incremented (ADR-060 §D8.10 integration). (7) **`red_gate_tests` frontmatter** 49→51; **BC-5.38.001 density** paragraph updated: 51 RGTs (RG-PSG-001 through RG-PSG-036 incl. 030b/030c/030d), 51/13≈3.92. (8) **F-R16-P18-LENSC-LOW-001 (records fix)**: v1.24 changelog row description (4) reproduced a volatile numeric line-cite in its closure text; rewritten to reference "a volatile numeric line-cite in the v1.22 changelog row" using only a symbol anchor. `input-hash` updated (BC-2.16.002 §D8.10 input drift). STORY-INDEX row version-pin update deferred to state-manager (target pin v1.25). TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; ADR-060 §D8.10 has no sibling at same decision level; CLEAR. Dim-2 — `red_gate_tests: 51` consistent in frontmatter, BC-5.38.001 density paragraph (51/13≈3.92), §Red Gate Tests table (51 rows), Task 16 (Steps D/F/G updated, "After steps A–G"); BC §Behavioral Contracts table Version cell incremented; AC-004 traces + sub-bullet; §File Structure rows updated; FULL. Dim-3 — RG-PSG-035 MUST anchored to AC-004 + BC-2.16.002 EC-01-040 + BC-2.07.003 §Postconditions in §Red Gate Tests; RG-PSG-036 MUST anchored to AC-004 + BC-2.16.002 §Postconditions LIMIT-Aware Early-Stop + ADR-060 §D8.10; Task 16 Step G MUST anchored to F-R16-P18-LENSA-OBS-001 + ADR-060 §D8.10; no unanchored MUSTs introduced; CLEAR. (9) **FetchOutput struct-shape reconciliation (in-place v1.25 consistency fix):** All 2-field `FetchOutput` struct literals and definitions in implementer-facing body content reconciled to the 3-field form `{ batches, any_early_stopped, pipeline_truncated }` per ADR-060 §D8.10. Sites updated: `crates_touched` `adapter.rs` define clause; `crates_touched` `spec_driven_adapter.rs` production return (both the `prism-bin:` note and the PRODUCTION sweep entry); PRODUCTION/TEST-STUBS wrap comment; §Authority `FetchOutput` return-type clause; §Behavioral Contracts table `FetchOutput` return-type contract; AC-009(a) `FetchOutput` return-type notation; Task 9 production return literal; Task 16 Step E test-stub literal; §Previous Story Intelligence struct definition and both stub/production return literals; §File Structure `spec_driven_adapter.rs` row return literal and MODIFY(×21) stub literal. Stub stubs: `pipeline_truncated: false`; production adapter: `pipeline_truncated: pipeline_result.truncated` (maps `PipelineResult.truncated` per ADR-060 §D8.10). Historical changelog entries describing pre-v1.25 design state are unchanged. |
| 1.24 | 2026-08-28 | story-writer | **Pass-17 story-owned finding closures: phantom RG symbols, RG-PSG-030d trigger description, narrative version pins, changelog volatile cite, FetchOutput `#[non_exhaustive]` contradiction, RG-PSG-034 cache-completeness propagation.** (1) **F-R16-P17-LENSB-MED-001 — 5 phantom RG test symbols corrected** in §Red Gate Tests table, Task 10 authoring bullets, and v1.23 changelog row. On-disk symbols from `plan_shape_gate_tests.rs` and `materialization.rs` substituted: `test_psg_rg030b_redundant_upper_bound_suppresses_early_stop`, `test_psg_rg030c_reversed_operand_suppresses`, `test_psg_rg030d_multi_index_datetime_suppresses_early_stop`, `test_psg_rg032_cross_sensor_source_scope_suppresses_early_stop`, `test_psg_rg033_armis_source_scope_permits_early_stop`. (2) **F-R16-P17-LENSB-MED-002 — RG-PSG-030d trigger description corrected**: rewritten to state the trigger is "QUERIED SOURCE TABLE schema has ≥2 Datetime+INDEX columns" (Condition K fires on schema column count, not predicate column count); anchored to `ADR-060 §D8.7`. (3) **F-R16-P17-LENSC-HIGH-001 — 8 narrative `ADR-060 §D8.7` version pins removed from body prose**: each occurrence stripped from §Red Gate Tests rows (RG-PSG-030c, RG-PSG-032, RG-PSG-033), Task 10 authoring bullets (rg030d, rg030c), Task 12 note, §File Structure `materialization.rs` item (j), §File Structure `plan_shape_gate_tests.rs` row; replaced with `ADR-060 §D8.7` section anchors throughout. Body prose now ZERO narrative version pins (verified by script sweep). (4) **F-R16-P17-LENSC-LOW-001 — v1.23 changelog TASK 4 volatile-cite repaired**: removed a volatile numeric line-cite in the v1.22 changelog row; rewrote to reference the `Plan-Shape Gate` section anchor instead; false zero-hits claim from v1.23 repaired (8 narrative ADR-060 version pins existed in body prose at v1.23 write time, closed by v1.24). (5) **F-R16-P17-LENSC-MED-001 — FetchOutput `#[non_exhaustive]` contradiction resolved**: Task 15 ADD instruction removed (was: "ADD FetchOutput to EXPECTED_SYMBOLS with `#[non_exhaustive]`"); replaced with explanation that `FetchOutput` is a new `pub` struct in `crates/prism-sensors/src/adapter.rs` — out-of-scope crate (gate scopes to prism-core, prism-spec-engine, prism-query only); cross-crate by-name construction by 21 test stubs forbids `#[non_exhaustive]`; EXPECTED_SYMBOLS update NOT needed. §File Structure MUST NOT modify bullet for `scripts/check-non-exhaustive-per-symbol.py` updated with FetchOutput exemption rationale. (6) **RG-PSG-034 cache-completeness gate propagation** (`test_psg_rg034_early_stopped_response_not_cached_as_complete`, `crates/prism-query/tests/execute_integration_tests.rs`): row added to §Red Gate Tests table (END-TO-END, AC-009, BC-2.16.002 EC-01-039, BC-2.07.003 §Postconditions cacheability); Task 10 authoring bullet added (cache-completeness sub-group); Task 15 note added (RG-PSG-034 covered by Task 16 Step F, not Task 15 ADR-060 range); Task 16 Step F added (cache write guard: `errors.is_empty() && !any_early_stopped`). `red_gate_tests` frontmatter 48→49. BC-5.38.001 density paragraph: 48→49 RGTs, range `RG-PSG-001 through RG-PSG-034 incl. 030b/030c/030d`, ratio 3.69→3.77. BC-2.16.002 §Behavioral Contracts table Version cell v2.44→v2.45. `input-hash` updated to reflect v1.24 content. STORY-INDEX row version-pin update deferred to state-manager (target pin v1.24). TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; ADR-060 has no sibling ADR at the same decision level; CLEAR. Dim-2 — `red_gate_tests: 49` consistent in frontmatter, density paragraph (49/13≈3.77), §Red Gate Tests table (49 rows), Task 10 (RG-PSG-034 authoring bullet added), Task 15 (RG-PSG-034 covered by Task 16 noted), Task 16 (Step F added); BC Version cell v2.45; FULL. Dim-3 — RG-PSG-034 MUST anchored to `test_psg_rg034_early_stopped_response_not_cached_as_complete` + AC-009 + BC-2.16.002 EC-01-039 + BC-2.07.003 §Postconditions cacheability in §Red Gate Tests and Task 10; Task 16 Step F MUST anchored to RG-PSG-034 + AC-009; no unanchored MUSTs introduced; CLEAR. |
| 1.23 | 2026-08-28 | story-writer | **ADR-060 v1.8 / BC-2.16.002 v2.44 propagation + pass-16 story-owned finding closures.** (1) **TASK 1 — 5 new Red Gates:** RG-PSG-030b (`test_psg_rg030b_redundant_upper_bound_suppresses_early_stop`), RG-PSG-030c (`test_psg_rg030c_reversed_operand_suppresses`, IN-CRATE UNIT), RG-PSG-030d (`test_psg_rg030d_multi_index_datetime_suppresses_early_stop`), RG-PSG-032 (`test_psg_rg032_cross_sensor_source_scope_suppresses_early_stop`), RG-PSG-033 (`test_psg_rg033_armis_source_scope_permits_early_stop`, positive control) added to §Red Gate Tests table with full type/description/AC/EC/ADR anchors. `red_gate_tests` frontmatter 43→48. BC-5.38.001 density paragraph: 43→48 RGTs, range `RG-PSG-001 through RG-PSG-033 incl. 030b/030c/030d`, ratio 3.31→3.69. AC-007 §Tests extended with 030c/030d/032/033; AC-008 §Tests extended with 030b. Task 10 gains authoring bullets for new tests in appropriate sub-sections (Suppression END-TO-END: 030d/032; IN-CRATE UNIT: 030c; Positive controls: 033; Temporal-exemption soundness: 030b). Task 12 green-gate range extended to `RG-PSG-033 incl. 030b/030c/030d`. Task 15 final gate range extended to `RG-PSG-033 incl. 030b/030c/030d`. (2) **TASK 2 — §File Structure fix (F-R16-P16-LENSC-MED-002):** `plan_shape_gate_tests.rs` row corrected to enumerate ALL gated tests: RG-PSG-001 through RG-PSG-033 incl. 029/030/030b/030d/031/032/033 (was omitting 029/030/031 and all new tests). `materialization.rs` row extended with item (j): `collect_datetime_index_cols` shared function (ADR-060 v1.8 Condition K) + RG-PSG-030c in-crate placement. (3) **TASK 3 — RG-SLUG-005 AC↔test inversion fix (F-R16-P16-LENSB-MED-002):** RG-SLUG-005 row rewritten: `test_rg_slug_005_wire_cross_tenant_isolation_collision_resistant_cache_keys` in `crates/prism-bin/tests/mcp_integration_tests.rs` named as AUTHORITATIVE wire-shape gate (MCP `CallToolResult` JSON assertion); `test_rg_slug_005_cross_tenant_wire_isolation_collision_resistant_cache_keys` in `crates/prism-query/tests/execute_integration_tests.rs` (Arrow RecordBatch structs) named as engine-layer pre-serialization defense-in-depth. AC-013 §Test updated accordingly. No new RG row added; density count unchanged at 48. (4) **TASK 4 — Changelog volatile line-cite (F-R16-P16-LENSC-LOW-001, TD-VSDD-091):** a volatile numeric line-cite in the v1.22 changelog row was replaced with the `Plan-Shape Gate` section anchor (symbol/section anchor). NOTE: the v1.23 zero-hits claim for narrative version pins was inaccurate — 8 narrative `ADR-060` version pins existed in body prose at v1.23 write time; these are closed by v1.24. (5) **TASK 5 — BC pin + version:** BC-2.16.002 §Behavioral Contracts table Version cell v2.43→v2.44. `input-hash` updated d127c30→050a13f (computed from ADR-060 v1.8 + BC-2.16.002 v2.44 input files). TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; ADR-060 has no sibling ADR at the same decision level; CLEAR. Dim-2 — `red_gate_tests: 48` consistent in frontmatter, density paragraph (48/13≈3.69), Task 10 (new bullets in all sub-sections), Task 12 (range extended), Task 15 (range extended); AC-007 §Tests and AC-008 §Tests updated; §File Structure Requirements plan_shape_gate_tests.rs and materialization.rs rows updated; BC Version cell v2.44; FULL. Dim-3 — RG-PSG-030b/030c/030d/032/033 MUSTs anchored to named tests + AC/EC/ADR cites in §Red Gate Tests; RG-SLUG-005 MUST anchored to authoritative wire test + defense-in-depth engine test; no unanchored MUSTs introduced; CLEAR. POL-39 whole-artifact sweep: version pins in prose — ZERO non-exempt matches (BC-2.16.002 in §Behavioral Contracts table Version column is POL-39-exempt; ADR-060 v1.8 references are in `input-hash` comment inside `---` block, exempt). Volatile-cite grep (`line \d+`/`:\d+` in prose): ZERO hits in body prose. STORY-INDEX row version-pin update deferred to state-manager (target pin v1.23). |
| 1.22 | 2026-08-28 | story-writer | **EC-01-034/035 coverage + BC v2.43 + POL-39 sweep.** (1) RG-PSG-030 (`test_psg_rg030_redundant_lower_bound_suppresses_early_stop`) + RG-PSG-031 (`test_psg_rg031_ocsf_arrow_name_permits_early_stop`) added to §Red Gate Tests table; each row includes test type, description, AC anchor, and BC/ADR section cite. (2) BC-5.38.001 density paragraph updated: 41→43 RGTs, range `RG-PSG-001 through RG-PSG-031`, ratio 3.15→3.31. (3) AC-008 Tests citation extended to include RG-PSG-030/031; Task 10 temporal-exemption soundness bullet list extended with RG-PSG-030/031 authoring bullets (red-then-green ordering preserved); Task 12 green-gate range extended `...RG-PSG-029` → `...RG-PSG-031`; Task 15 final gate range extended to RG-PSG-031. (4) BC-2.16.002 Version column cell v2.42 → v2.43 (POL-39-exempt pin location). (5) POL-39 narrative-version-pin sweep: stripped `v1.2`/`v1.3`/`v1.5` labels from: BC table Role cell (`§D8.7 v1.3/§D8.9 v1.5` → `§D8.7/§D8.9`; `§D8.9 v1.5` → `§D8.9`); BC addendum note (`BC-2.16.002 v2.42 addendum` → `BC-2.16.002 addendum`; catalog label `(v1.70)→(v1.71)` → removed); AC-007 Conservative Default `(new in v1.3)` parenthetical; AC-008 heading `Condition G revised v1.5` → `Condition G revised`; AC-008 body `The v1.3 \`!is_purely_temporal_predicate\`` → `The \`!is_purely_temporal_predicate\``; AC-010/011/013 heading `catalog row v2.42` → `catalog row`; Architecture Mapping `§D8.7 v1.3 —` → `§D8.7 —`; Task 12 code comment `§D8.7 v1.3:` → `§D8.7:`; Architecture Compliance Rules: `v1.2 under-detection` → `under-detection`, `Condition G revised, v1.3` → `Condition G revised`, `soundness — v1.5` → `soundness`, `The v1.3 \`!is_purely_temporal_predicate\`` → `The \`!is_purely_temporal_predicate\``; References bullet `§D8.7 v1.3 / §D8.9 v1.5 —` → `§D8.7 / §D8.9 —`. Frontmatter `v1.3` in the `Plan-Shape Gate` comment inside the frontmatter `behavioral_contracts:` block is EXEMPT (inside `---` block). TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; ADR-060 has no sibling ADR at the same decision level; CLEAR. Dim-2 — `red_gate_tests: 43` consistent in frontmatter and density paragraph (43/13≈3.31) and Task 10 (RG-PSG-030/031 bullets) and Task 12 (range extended) and Task 15 (range extended); AC-008 Tests citation updated; BC Version cell v2.43; POL-39 sweep comprehensive (all 15 identified non-exempt `v[0-9]+\.[0-9]+` occurrences in body prose stripped); FULL. Dim-3 — RG-PSG-030 MUST anchored to AC-008 EC-01-034 + `test_psg_rg030_redundant_lower_bound_suppresses_early_stop` + ADR-060 §D8.7; RG-PSG-031 MUST anchored to AC-008 EC-01-035 + `test_psg_rg031_ocsf_arrow_name_permits_early_stop` + ADR-060 §D8.9; no unanchored MUSTs introduced; CLEAR. STORY-INDEX row version-pin update deferred to state-manager (target pin v1.22). |
| 1.21 | 2026-08-28 | story-writer | **F-R16-P6-MED-001 — Full-document sweep: Task 19 + §File Structure Requirements item (i) false `x`-prefix form corrected.** v1.20 corrected AC-012 + Task 18 but left two live false references. (1) Task 19 §"Site 3 — sentinel removal": replaced "the deterministic `x`-prefix form from Task 18" with "the D3 deterministic-prefix form from Task 18 (`format!("org-{}", &org_id.to_string()[..8])` — valid by construction; no digit-prefix special case, no fallback)". (2) §File Structure Requirements, materialization.rs row item (i): replaced "replace with deterministic `x`-prefix form" with "replace with the D3 deterministic-prefix form (`format!("org-{}", &org_id.to_string()[..8])` — valid by construction; no `x`-prefix, no digit special case, no fallback)". Input-hash updated 248f3c0→95fd5e3 (ADR-061 v1.1 input drift). No AC, RG, task count, or behavior changes; spec-content-only corrections. TD-VSDD-097: Dim-1 — no named story-twin for S-ENGINE-LIMIT-EARLY-STOP-001; ADR-061 has no sibling ADR at the same decision level; CLEAR. Dim-2 — grepped ENTIRE document (all sections: frontmatter, ACs 1–13, all Tasks 1–19, §File Structure Requirements, §Architecture, §Red Gate Tests, §Changelog, all prose); only 2 live false references found at Task 19 and §File Structure item (i); §AC-012 body and §Task 18 D3-helper paragraph already use correct "D3 deterministic-prefix form" and `format!("org-{}", ...)` language from v1.20; both updated consistently; frontmatter `version: "1.20"→"1.21"` and `modified: "2026-08-28"` (unchanged); `acceptance_criteria_count: 13` and `red_gate_tests: 41` UNCHANGED; STORY-INDEX row updated; FULL. Dim-3 — no new MUSTs introduced; CLEAR. |
