...
input-hash: "050a13f"
# input-hash: updated 2026-08-28 (v1.23); ADR-060 v1.8 (shared collect_datetime_index_cols, Condition K ≥2 INDEX datetime cols suppress, reversed-operand suppress); BC-2.16.002 v2.44 (EC-01-036 cross-sensor schema isolation, EC-01-037 reversed-operand, EC-01-038 Condition K)
...
| RG-PSG-030b | `test_psg_rg030b_two_upper_bounds_suppresses_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, `PlanShapeGateMockAdapter` | AC-008 EC-01-034: SQL temporal AND-predicate with two same-direction UPPER bounds (`WHERE ts < X AND ts < Y LIMIT 1`) → `count_temporal_bound_directions` yields upper=2 → Step-2 fails → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS). Companion to RG-PSG-030 (two-lower). First-wins soundness: second upper bound dropped server-side; permitting early-stop risks silent under-return. MUST FAIL before the ADR-060 §D8.7 fix. Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-034, ADR-060 §D8.7. |
| RG-PSG-030c | `test_psg_rg030c_reversed_operand_suppresses_early_stop` | IN-CRATE UNIT (defense-in-depth, SAP-3 rule-3 annotated; `materialization.rs` `plan_shape_gate_unit_tests`) | AC-007/EC-01-037: reversed-operand temporal predicate (`WHERE 'X' > ts` — literal on LHS, column on RHS) → `is_pushed_temporal_predicate` returns `false` (requires column on LHS) → `has_client_side_where = true` → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS). SAP-3 rule-3 annotated: parser may reject reversed operands in some surface paths; unit test provides isolated gate verification (defense-in-depth). MUST FAIL before the ADR-060 v1.8 `is_pushed_temporal_predicate` reversed-operand fix. Anchor BC-2.16.002 EC-01-037, ADR-060 §D8.7. |
| RG-PSG-030d | `test_psg_rg030d_condition_k_two_index_datetime_cols_suppress_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, `PlanShapeGateMockAdapter` | AC-007/EC-01-038 Condition K (ADR-060 v1.8): temporal predicate referencing ≥2 distinct INDEX datetime columns → `collect_datetime_index_cols` counts ≥2 INDEX datetime columns in predicate → Condition K fires → `ast_is_reducing_plan = true` → `fetch_limit = 0` (SUPPRESS). Cross-column temporal predicates require full pagination for correct boundary semantics. MUST FAIL before the ADR-060 v1.8 Condition K implementation. Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-038, ADR-060 §D8.7. |
| RG-PSG-032 | `test_psg_rg032_collect_datetime_index_cols_cross_sensor_schema_isolation` | END-TO-END / Integration via `run_materialization_pipeline`, multi-sensor fixture | AC-007/EC-01-036: `collect_datetime_index_cols` shared function (ADR-060 v1.8) scopes to the current sensor's schema — INDEX datetime columns from sensor-A's schema do NOT bleed into sensor-B's `datetime_index_cols` set during a cross-sensor query. Verifies schema-scoped isolation of the shared extractor across sensor boundaries. MUST FAIL if `datetime_index_cols` bleeds across sensor boundaries (early-stop incorrectly permitted/suppressed for the wrong sensor's temporal predicates). Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-036, ADR-060 §D8.7. |
| RG-PSG-033 | `test_psg_rg033_condition_k_single_index_datetime_col_permits_early_stop` | END-TO-END / Integration via `run_materialization_pipeline`, `PlanShapeGateMockAdapter` | POSITIVE CONTROL for Condition K: single INDEX datetime column in temporal predicate (`WHERE ts > X LIMIT N`) → `collect_datetime_index_cols` yields exactly 1 INDEX datetime column → Condition K does NOT fire (requires ≥2) → `is_pushed_temporal_predicate` returns `true` (valid range op + INDEX col + concrete timestamp RHS) → `has_client_side_where = false` → `ast_is_reducing_plan = false` → early-stop fires. Confirms Condition K does NOT over-suppress the normal single-INDEX-datetime-column case. MUST PASS before AND after ADR-060 v1.8 Condition K implementation. Located in `plan_shape_gate_tests.rs`. Anchor BC-2.16.002 EC-01-038, ADR-060 §D8.7. |
...
- [ ] **Task 10 (Red Gate — test first):** Write RG-PSG-001 through RG-PSG-019 in
  ...
  **Suppression tests (gate MUST fire; early-stop MUST be suppressed) — END-TO-END / Integration:**
  ...
  - RG-PSG-030d (`test_psg_rg030d_condition_k_two_index_datetime_cols_suppress_early_stop`): temporal
    ...
    Task 12 (ADR-060 v1.8 Condition K not yet implemented). (Anchored: AC-007; EC-01-038; ADR-060 §D8.7)
  - RG-PSG-032 (`test_psg_rg032_collect_datetime_index_cols_cross_sensor_schema_isolation`): multi-sensor
  ...
  **Suppression tests — IN-CRATE UNIT (defense-in-depth; SAP-3 rule-3; in `materialization.rs` plan_shape_gate_unit_tests):**
  ...
  - RG-PSG-030c (`test_psg_rg030c_reversed_operand_suppresses_early_stop`): call
    ...
    isolated gate verification. MUST FAIL before the ADR-060 v1.8 reversed-operand fix.
  ...
  **Positive controls (gate MUST NOT fire; early-stop MUST proceed):**
  ...
  - RG-PSG-033 (`test_psg_rg033_condition_k_single_index_datetime_col_permits_early_stop`): single
  ...
  **Temporal-exemption soundness (END-TO-END / Integration via `run_materialization_pipeline`, in-process `PlanShapeGateMockAdapter`):**
  ...
  - RG-PSG-030b (`test_psg_rg030b_two_upper_bounds_suppresses_early_stop`) in
    `crates/prism-query/tests/plan_shape_gate_tests.rs`. SQL temporal AND-predicate with two
...
- [ ] **Task 12 (Implementation — `ast_is_reducing_plan` + `run_materialization_pipeline` gate):**
  ...
  suppress (EC-01-038, ADR-060 v1.8). RG-PSG-031 gates OCSF Arrow-name PERMIT path (EC-01-035).
...
| CREATE or EXTEND | `crates/prism-query/tests/plan_shape_gate_tests.rs` OR extend `materialization_tests.rs` | RG-PSG-001 through RG-PSG-019, RG-PSG-021 through RG-PSG-024, RG-PSG-029, RG-PSG-030, RG-PSG-030b, RG-PSG-030d, RG-PSG-031, RG-PSG-032, RG-PSG-033; RG-PSG-009/012/019 are in-crate unit tests in `materialization.rs` `#[cfg(test)] mod plan_shape_gate_unit_tests`; RG-PSG-021..024 are temporal-exemption soundness E2E tests (Filter-mode/Pipe-WHERE/SQL-Eq/non-INDEX); RG-PSG-029..031 are temporal-exemption regression guards (relative-temporal fold path, two-lower-bounds, OCSF Arrow name); RG-PSG-030b is the two-upper-bound companion to RG-PSG-030; RG-PSG-030d is Condition K E2E (ADR-060 v1.8 ≥2 INDEX datetime cols); RG-PSG-032 is cross-sensor schema isolation for `collect_datetime_index_cols`; RG-PSG-033 is the Condition K positive control (single INDEX datetime col → PERMIT) |
