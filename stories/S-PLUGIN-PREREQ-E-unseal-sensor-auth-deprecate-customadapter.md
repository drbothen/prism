...
architectural_decisions:
  ...
  - ADR-023  # Plugin-only sensor architecture — §C5 rules authoritative for this story's scope
holdout_scenarios:
...
assumption_validations:
  ...
  - "spec_parser.rs contains zero CustomAdapter/CustomAdapterRegistry references (ADR-023 §C5 F-CRIT-NEW-001-PASS2-RESIDUAL verified by grep)"
  - "S-WAVE5-PREP-01 already removed custom_adapter_registry references from boot.rs — no boot.rs changes required in PREREQ-E"
  - "TD-A-003 (WriteToolInvalidationMap extensibility) is routed to PREREQ-E per S-PLUGIN-PREREQ-A fix-burst-2 decision"
risk_mitigations:
  ...
  - "AC-9: TD-A-003 closure via RwLock<Vec<WriteToolInvalidationMap>> — write lock held only during registration; read-side is zero-copy"
...
| BC-2.16.012 | PluginRegistry Dispatch in spec_parser.rs — Hardcoded Sensor Names Replaced with Registry Lookup | SS-16 | Primary delivery — open dispatch migration; behavioral equivalence test; TD-A-003 WriteToolInvalidationMap closure |
...
7. **Migrate `WriteToolInvalidationMap` to runtime-extensible container (TD-A-003)**
...
(traces to BC-2.16.011 postconditions; ADR-023 §C5 confirmed three sites)
...
**AC-9 (WriteToolInvalidationMap Runtime Extensibility — TD-A-003 Closed):**
...
(traces to BC-2.16.012 postcondition — TD-A-003 WriteToolInvalidationMap; INV-INVALIDATION-EXT-001)
...
2. **`test_BC_2_01_016_002_auth_composition_runtime_rejection`** (prism-spec-engine) — constructs a `SensorSpec` with `auth_type = ["oauth2_client_credentials", "bearer_static"]` and attempts to load it; fails RED because the E-SPEC-012 rejection path must be verified to exist (passes once the path is confirmed active; fails if the validator does not enforce Rule 2, Rule A).
...
|------|--------|-------------|
| `SensorAuth` MUST NOT have a `Sealed` supertrait after PREREQ-E | ADR-023 §C5 Rule 2; BC-2.01.016 | `grep -rn "private::Sealed\|: Sealed\|impl Sealed" crates/prism-sensors/src/auth/` = 0 hits |
| `CustomAdapter`, `CustomAdapterRegistry`, `CustomAuth` MUST NOT exist in `src/` after PREREQ-E | ADR-023 §C5 Rule 5; BC-2.16.011 | `grep -rn "CustomAdapter\|CustomAdapterRegistry\|CustomAuth" crates/` = 0 hits in `src/` |
| Hardcoded sensor name dispatch in `spec_parser.rs` MUST be replaced with open path | ADR-023 §C5; BC-2.16.012 | `grep -rn '"crowdstrike"\|"cyberint"\|"claroty"\|"armis"' crates/prism-spec-engine/src/spec_parser.rs` = 0 hits in dispatch contexts |
| `boot.rs` MUST NOT be modified in this story | ADR-023 §C5 scope + F-MED-NEW-005 | Code review; `git diff develop...HEAD -- crates/prism-bin/src/boot.rs` = empty |
...
| `E-SPEC-008` | RETIRED (annotate only, do NOT delete) | Annotate with `retired:` note: "Retired in S-PLUGIN-PREREQ-E. No live code path triggers this code post-CustomAdapter removal. Plugin execution panics surface via E-PLUGIN-001." |
...
**TD-A-003 (routed to this story):** `WriteToolInvalidationMap` was converted from `&[...]` static slice to `LazyLock<Vec<...>>` in S-PLUGIN-PREREQ-A fix-burst-2. The `LazyLock<Vec<T>>` provides only `Deref<Target=Vec<T>>` (read-only). True runtime extensibility requires `RwLock<Vec<...>>` + a `register_write_tool` API. This story closes TD-A-003 by delivering both.
...
8. TD-A-003 is closed in `tech-debt-register.md` with a pointer to this story's PR
...
- [ADR-023](../specs/architecture/decisions/ADR-023-plugin-only-sensor-architecture.md) §C5 — SensorAuth un-sealing + CustomAdapter removal + spec_parser migration
...
Tech debt closed:
- [TD-A-003](../tech-debt-register.md) (line citing TD-S-PLUGIN-PREREQ-A-003) — WriteToolInvalidationMap extensibility
...
| 1.0 | S-PLUGIN-PREREQ-E-authoring | 2026-05-15 | product-owner | Initial draft. Authored from ADR-023 §C5 scope + PREREQ-A/D context. Three new BCs (BC-2.01.016, BC-2.16.011, BC-2.16.012), three holdout scenarios (HS-PREREQ-E-001/002/003), 10 ACs, 8 Red Gate tests, TD-A-003 closure in scope. |
