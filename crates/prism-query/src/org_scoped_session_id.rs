//! Org-scoped UUID v7 session ID generation for sensor pagination cursors (S-3.2.08 / D-048).
//!
//! # D-048 Resolution
//!
//! When `prism-query` generates a sensor pagination session ID (a UUID used as
...
//! probabilistic. CrowdStrike was the first consumer; the mechanism is sensor-agnostic.
//!
...
//! consulting an in-process registry populated by [`generate_org_scoped_session_id`]
...
/// of concurrent sensor pagination queries active in a single prism-query process).
...
///
/// Populated by [`generate_org_scoped_session_id`]; queried by
...
/// Generate an org-scoped UUID v7 session ID with the calling [`OrgId`] embedded
...
/// suitable for use as the `X-DTU-Session-Id` HTTP header value sent to sensor DTU
/// clones (e.g., `prism-dtu-crowdstrike`).
///
...
pub fn generate_org_scoped_session_id(org_id: OrgId) -> String {
...
/// Extract the embedded [`OrgId`] from a session ID previously generated
/// by [`generate_org_scoped_session_id`] in this process.
///
...
/// 3. The session ID was generated by [`generate_org_scoped_session_id`] in this
...
pub fn extract_org_id_from_session_id(session_id: &str) -> Option<OrgId> {
    ...
    // generate_org_scoped_session_id, and returning None early avoids a spurious
...
///
/// Separated from [`generate_org_scoped_session_id`] to allow unit testing of the
...
///   may change as a result of XOR. The caller ([`generate_org_scoped_session_id`])
