...
/// serve IOC-stamped detections (AC-004 / BC-2.06.019 v1.11, F-PIVOT003-R2-001).
///
...
async fn test_BC_2_06_019_scenario_clone_detection_0_carries_ioc_value_from_catalog() {
    ...
    assert!(
        ...
        "build_scenario_entity_catalog must produce a non-empty ioc_hashes slice; \
         got empty. BC-2.06.019 v1.11 F-PIVOT003-R2-001."
    );
    ...
    // Also verify ioc_type is "hash_sha256" (BC-2.06.019 v1.11 algorithm-qualified token).
    let ioc_type = behaviors[0].get("ioc_type").and_then(|v| v.as_str());

    assert_eq!(
        ioc_type,
        Some("hash_sha256"),
        "AC-004 / BC-2.06.019 v1.11: behaviors[0].ioc_type MUST be 'hash_sha256' \
...
///
/// BC-2.06.019 v1.11 PC-4: "ioc_hashes=false: detection records where
...
async fn test_BC_2_06_019_crowdstrike_ioc_bearing_detection_stagemask_served_route() {
    ...
    if !det_ids_stage1.is_empty() {
        ...
        // BC-2.06.019 v1.11 PC-4 / F-PIVOT003-R7A-002.
        ...
        assert!(
            ioc_bearing.is_none(),
            "F-PIVOT003-R7A-002 BC-2.06.019 v1.11 PC-4: at stage 1 (Recon, \
    ...
    assert!(
        ...
        "F-PIVOT003-R7A-002 BC-2.06.019 v1.11 PC-4: at stage 2 (LateralMovement, \
