//! W3-FIX-SEC-002: `POST /dtu/reset` admin token auth — prism-dtu-crowdstrike
//!
//! Acceptance criteria:
...
async fn test_AC_001_dtu_reset_without_admin_token_returns_401() {
    ...
    clone
        .start()
        .await
        .expect("W3-FIX-SEC-002 crowdstrike: start() must succeed");
...
async fn test_AC_003_cross_clone_admin_token_returns_401() {
    let mut clone_a = CrowdstrikeClone::new();
    clone_a
        .start()
        .await
        .expect("W3-FIX-SEC-002 crowdstrike clone-A: start() must succeed");

    let mut clone_b = CrowdstrikeClone::new();
    clone_b
        .start()
        .await
        .expect("W3-FIX-SEC-002 crowdstrike clone-B: start() must succeed");
