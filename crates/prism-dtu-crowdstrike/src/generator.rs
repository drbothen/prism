...
/// BC-2.06.019 v1.8 (bare `"hash"` is incorrect; `"cmdline"` is a SEPARATE
...
pub(crate) fn make_detection_with_ioc(
    ...
    let behaviors = if let Some(ioc_hash) = scenario_ioc_hash {
        // AC-004: ioc_type MUST be "hash_sha256" (algorithm-qualified per BC-2.06.019 v1.8).
...
mod tests {
    ...
    /// Test 5 — BC-2.06.019 v1.8 PC-4 CrowdStrike IOC stamp correction:
    /// `make_detection_with_ioc()` must stamp `behaviors[0]["ioc_type"] == "hash_sha256"`
    /// (algorithm-qualified token), NOT bare `"hash"` (BC-2.06.019 v1.8 correction).
    ///
    /// Also asserts:
    ...
    ///             behaviors[0]["ioc_description"] = "scenario IOC"
    ///
    /// BC-2.06.019 v1.8 PC-4 (CrowdStrike detections IOC stamp).
    ...
    fn test_BC_2_06_019_crowdstrike_detection_behaviors_ioc_hash_stamped() {
        ...
        assert!(
            !behaviors.is_empty(),
            "BC-2.06.019 v1.8 PC-4: behaviors[] array must be non-empty after IOC stamping"
        );

        let b0 = &behaviors[0];

        // CRITICAL assertion: ioc_type MUST be "hash_sha256" (NOT bare "hash").
        // BC-2.06.019 v1.8 correction: algorithm-qualified tokens only.
        let ioc_type = b0
            .get("ioc_type")
            .and_then(|v| v.as_str())
        ...
        assert_eq!(
            ...
            "BC-2.06.019 v1.8 correction: behaviors[0]['ioc_type'] must be 'hash_sha256' \
    ...
    ///
    /// BC-2.06.019 v1.8: tolerant-unknown-type policy applies to READING; writing "hash_sha256"
