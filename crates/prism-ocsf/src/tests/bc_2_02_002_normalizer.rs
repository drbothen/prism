...
//! # Green State
//!
//! ocsf-proto-gen is provisioned and tests pass. The `class_uid` field population
...
fn test_BC_2_02_002_empty_json_produces_dynamic_message() {
    ...
    assert!(
        result.is_ok(),
        "normalize() with empty JSON must return Ok(DynamicMessage); actual: {:?}",
        result.unwrap_err()
    );
}
...
fn test_BC_2_02_002_vp016_dynamic_message_round_trips() {
    ...
    let message = normalizer
        ...
        .expect("normalize() must succeed for VP-016 round-trip");
