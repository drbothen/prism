...
// =============================================================================
// OCSF-CLASS-MIGRATION-001 Red Gate tests (BC-2.02.012 v1.4)
//
...
mod ocsf_migration_red_gate {
    ...
    /// BC-2.02.012 v1.4 / AC-001 / TV-BC-2.02.012-009 / INV-PRODUCTION-TOML-NO-SECURITY-FINDING:
    ...
    /// BC-2.02.012 v1.4 / AC-002 / TV-BC-2.02.012-007:
    ...
    fn test_BC_2_02_012_select_by_class_name_detection_finding_returns_2004_no_warn() {
        ...
        // Assert Ok(2004) — OCSF Detection Finding class_uid (BC-2.02.012 v1.4 TV-007).
        assert!(
            result.is_ok(),
        ...
        assert_eq!(
            ...
            "select_by_class_name(\"detection_finding\") must return Ok(2004), \
             OCSF Detection Finding class_uid (BC-2.02.012 v1.4 AC-002 / TV-BC-2.02.012-007)"
        );
    ...
    /// BC-2.02.012 v1.4 / AC-003 / TV-BC-2.02.012-008 / TV-BC-2.01.013-005:
    ...
    fn test_BC_2_02_012_select_by_class_name_security_finding_returns_2004_with_warn() {
        ...
        // NOT the deprecated Security Finding (2001). (BC-2.02.012 v1.4 Option A)
        assert!(
            result.is_ok(),
            "select_by_class_name(\"security_finding\") must return Ok \
             (BC-2.02.012 v1.4 AC-003 / TV-BC-2.02.012-008)"
        );
        assert_eq!(
            result.unwrap(),
            2004,
        ...
        assert!(
            ...
            "ocsf.deprecated_class_alias WARN must include class_name = \"security_finding\" field \
             (BC-2.02.012 v1.4 AC-003); captured log: {captured}"
        );
        assert!(
            captured.contains("2004"),
            "ocsf.deprecated_class_alias WARN must include resolved_class_uid = 2004 field \
             (BC-2.02.012 v1.4 AC-003 / TV-BC-2.02.012-008); captured log: {captured}"
        );
    }
    ...
    /// BC-2.02.012 v1.4 / AC-004 / INV-NO-2001-SELECT-PATH:
    ...
    fn test_BC_2_02_012_select_path_no_token_returns_2001() {
        ...
        assert!(
            ...
            "AC-004 / INV-NO-2001-SELECT-PATH VIOLATED:\n\
             ...
             current OCSF class (BC-2.02.012 v1.4).\n\
             Violations ({} total):\n{}",
            violations.len(),
            violations.join("\n")
        );
    ...
    fn test_BC_2_02_012_no_stale_2001_assertions_in_workspace() {
        ...
        // All class names in the BC-2.02.012 v1.4 mapping table.
                ...
                if uid == DEPRECATED_SECURITY_FINDING_UID {
                    stale_2001_returns.push(format!(
                        "select_by_class_name({class_name:?}) returned Ok(2001) — \
                         ...
                         another current OCSF class (BC-2.02.012 v1.4 AC-005)"
                    ));
                }
            }
        }
