...
fn test_BC_2_06_020_cyberint_alert_cve_resolves_in_nvd() {
    ...
    assert!(
        !catalog.device_cves.is_empty(),
        "catalog.device_cves must be non-empty for pivot-chain test"
    );

    let catalog_set: std::collections::BTreeSet<String> =
        catalog.device_cves.iter().cloned().collect();

    // Build a scenario Cyberint clone with catalog CVE injection.
    let scenario_start: i64 = chrono::Utc::now().timestamp() - 1_000;
    let timeline = Arc::new(build_default_incident_timeline(
        catalog.clone(),
        scenario_start,
    ...
    // Collect all CVE-surface records.
    ...
    // For each CVE record: assert cve_id ∈ catalog.device_cves.
    // This guarantees that NvdClone::new_with_scenario(&catalog) will contain
    // each cve_id (NVD injects exactly catalog.device_cves at construction time;
    // see BC-2.06.020 PC-3 / INV-NVD-CVE-CORRELATION-001).
    for (i, record) in cve_records.iter().enumerate() {
        let cve_id = record
            .get("cve_id")
            .and_then(|v| v.as_str())
        ...
        assert!(
            catalog_set.contains(cve_id),
            "CVE record[{i}] cve_id '{cve_id}' is NOT in catalog.device_cves {:?}; \
             scenario CVEs must be catalog members so that NvdClone::new_with_scenario(&catalog) \
             resolves them with base_score >= 7.0. \
             PC-8 + INV-CYBERINT-ALERT-CVE-CORRELATION-001 + INV-NVD-CVE-CORRELATION-001. \
             TV-020-013.",
            catalog.device_cves
        );

        // Collision-safety: even catalog CVEs must use CVE-9999- namespace.
        assert!(
            !matches_real_year(cve_id),
            "CVE record[{i}] cve_id '{cve_id}' matches a real calendar-year prefix; \
             catalog CVEs are generated by gen_device_cves (CVE-9999- sentinel, SEC-001). \
             INV-CYBERINT-ALERT-CVE-CORRELATION-001 universal collision-safety."
        );
    }
}
