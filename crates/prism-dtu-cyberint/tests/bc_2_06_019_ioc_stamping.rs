...
async fn test_BC_2_06_019_cyberint_alerts_real_schema_ioc_filter_no_synthetic() {
    ...
    // Stage 0 server: elapsed ≈ D-30s < 60s → ioc_hashes=false (90s budget).
...
async fn test_BC_2_06_019_ip_domain_ioc_stage_gating_served_route() {
    ...
    // -------------------------------------------------------------------------
    // Pre-Exfil server: stage 0 (elapsed ≈ D-30s < 60s → ioc_ips=false, ioc_domains=false; 90s budget)
    // -------------------------------------------------------------------------
    let now = chrono::Utc::now().timestamp();
    let start_pre_exfil: i64 = now + 30; // elapsed ≈ D-30s (stage 0 budget 90s: +30s CPU contention margin)
