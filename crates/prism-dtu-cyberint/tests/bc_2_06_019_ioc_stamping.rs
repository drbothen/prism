...
//! Traces to:
//!   BC-2.06.019 v1.12 PC-4 — Cyberint alerts carry real IOC fields; StageMask filter on
//!     real-schema accessors (ioc.value / iocs[].value / alert_data.ip / alert_data.domain)
//!   BC-2.06.019 v1.12 §Interim State — _ioc_value synthetic filter MUST NOT coexist with
...
/// Test 1 — BC-2.06.019 v1.12 PC-4: Cyberint `Alert` type carries real IOC fields.
///
/// Asserts:
...
///
/// BC-2.06.019 v1.12 PC-4 (Per-Sensor IOC-Surface Matrix — Cyberint: YES).
...
/// Test 2 — BC-2.06.019 v1.12 §Cyberint INCONCLUSIVE inner-key: `Ioc` struct must
...
///
/// This test uses exact canonical test vectors from BC-2.06.019 v1.12 §Cyberint IOC section.
///
...
///
/// BC-2.06.019 v1.12 §Cyberint INCONCLUSIVE inner-key.
...
fn test_BC_2_06_019_cyberint_ioc_struct_dual_alias_deserializes_both_key_forms() {
    ...
    // Cross-check exact values (canonical test vector from BC-2.06.019 v1.12).
...
/// Test 3 — BC-2.06.020 PC-1 + BC-2.06.019 v1.12 PC-4: `generate_with_scenario_iocs()`
...
fn test_BC_2_06_019_cyberint_fixture_generator_stamps_scenario_iocs() {
    ...
    assert!(
        has_stamped_ioc,
        "BC-2.06.019 v1.12 PC-4 / AC-002: at least one alert record must carry an IOC value \
...
/// Test 4 — BC-2.06.019 v1.12 PC-4 §Interim State: the Cyberint alerts route
...
///
/// BC-2.06.019 v1.12 PC-4 §Interim State.
...
async fn test_BC_2_06_019_cyberint_alerts_real_schema_ioc_filter_no_synthetic() {
    ...
    assert!(
        ...
        "BC-2.06.019 v1.12 PC-4 / AC-003: at stage 0 (ioc_hashes=false), alert \
    ...
    assert!(
        ...
        "BC-2.06.019 v1.12 PC-4: non-IOC alert 'non-ioc-alert-003' must always be \
...
/// Test 7 — BC-2.06.019 v1.12 PC-4 + SAP-2: the Cyberint sensor TOML spec must
    ...
    for toml_path in &toml_paths {
        ...
        for col in &required_columns {
            assert!(
                ...
                "BC-2.06.019 v1.12 PC-4 / AC-006: cyberint.sensor.toml at {:?} must \
...
/// Test 10 — BC-2.06.019 v1.12 PC-4 ioc_hashes StageMask field:
...
///
/// Test vectors (canonical from BC-2.06.019 v1.12 §Cyberint route coverage):
...
///
/// BC-2.06.019 v1.12 PC-4, ioc_hashes StageMask field.
...
async fn test_BC_2_06_019_ioc_hashes_false_withholds_cyberint_alert_with_matching_hash() {
    ...
    assert!(
        ...
        "BC-2.06.019 v1.12 PC-4 / test #10: at stage 0 (ioc_hashes=false), alert \
    ...
    assert!(
        ...
        "BC-2.06.019 v1.12 PC-4 / test #10: at stage 3 (ioc_hashes=true), alert \
...
// ---------------------------------------------------------------------------
// Test 11 — F-PIVOT003-R2-005: fail-closed projection integrity (BC-2.06.019 v1.12 PC-4 §6)
// ---------------------------------------------------------------------------

/// Test 11 — BC-2.06.019 v1.12 PC-4 step 6: fail-closed projection integrity.
///
...
/// (F-PIVOT003-R2-005, BC-2.06.019 v1.12 PC-4 §6).
///
...
///
/// BC-2.06.019 v1.12 PC-4 step 6. F-PIVOT003-R2-005.
#[tokio::test]
async fn test_BC_2_06_019_fail_closed_malformed_alert_is_withheld() {
    ...
    assert!(
        ...
        "BC-2.06.019 v1.12 PC-4 §6 / F-PIVOT003-R2-005: fail-closed — malformed record \
    ...
    assert!(
        ...
        "BC-2.06.019 v1.12 PC-4 §6: valid alert 'valid-alert-fail-closed-test11' must be PRESENT; \
         not found in: {:?}",
        valid_ids
    );
}
