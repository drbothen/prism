...
///    BC-2.06.019 v1.12 PC-4 alert-surface semantics: `ioc_ips/ioc_domains/ioc_hashes=false`
///    → alert records referencing those catalog IOCs are excluded from the response.
///    NOTE: the synthetic-ioc filter has been REMOVED (BC-2.06.019 v1.12 §Interim State);
...
pub async fn get_alerts(
    ...
    if state.fixture_gen_seeded {
        ...
        if let Some(ref timeline) = state.timeline {
            ...
            // AC-003 / BC-2.06.019 v1.12 PC-4: real-schema IOC filter.
            //
            // Filter logic (per BC-2.06.019 v1.12 PC-4):
            ...
            let data: Vec<serde_json::Value> = alert_records
                ...
                .filter(|rec| {
                    ...
                    // BC-2.06.019 v1.12 PC-4 step 6: fail-closed.
