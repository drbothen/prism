...
pub struct Alert {
    ...
    /// No public-documentation basis found (BC-2.06.019 v1.10 Cyberint row).
    ...
    /// `url` CONFIRMED; `ip`/`domain` UNCONFIRMED-plausible (BC-2.06.019 v1.10).
    #[serde(skip_serializing_if = "Option::is_none")]
...
///
/// AC-001 (S-DEMO-ENRICHMENT-PIVOT-003): serde dual-alias required per BC-2.06.019 v1.10
...
pub struct Ioc {
    ...
    /// `"ioc_type"` (feed convention, serde alias). BC-2.06.019 v1.10 dual-alias requirement.
...
///
/// AC-001 (S-DEMO-ENRICHMENT-PIVOT-003): added per BC-2.06.019 v1.10 Per-Sensor IOC-Surface
