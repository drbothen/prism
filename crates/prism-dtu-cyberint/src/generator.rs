...
/// filter in `routes/alerts.rs` can project it against the StageMask (BC-2.06.019 v1.10 PC-4).
...
pub fn generate_with_scenario_iocs(
    ...
    // BC-2.06.019 v1.10 PC-4: iocs[0].value carries the catalog IOC hash value so the
    ...
    if archetype == Archetype::CompromisedEndpoint && !catalog_ioc_hashes.is_empty() {
        ...
        for record in fixture_set.records.iter_mut() {
            ...
            // Primary wire key "type" per Ioc serde rename (BC-2.06.019 v1.10 INCONCLUSIVE
