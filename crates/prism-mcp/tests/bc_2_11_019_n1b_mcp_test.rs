...
// ─── MED-5: suggestion string byte-for-byte conformance to BC-2.11.019 v1.3 §MCP surface ─────

/// MED-5 BC-2.11.019 v1.3 §MCP surface — non-empty suggestion form.
///
...
fn test_med5_enrich_udf_not_found_suggestion_non_empty_no_brackets() {
    ...
    // BC-2.11.019 v1.3 §MCP surface canonical text (no brackets around list):
    ...
    assert_eq!(
        suggestion, expected,
        "MED-5 BC-2.11.019 v1.3 §MCP surface: suggestion must not wrap the list in brackets \
...
/// MED-5 BC-2.11.019 v1.3 §MCP surface — empty infusions form.
///
/// BC specifies:
...
fn test_med5_enrich_udf_not_found_suggestion_empty_infusions() {
    ...
    assert_eq!(
        suggestion, expected,
        "MED-5 BC-2.11.019 v1.3 §MCP surface: empty-infusions suggestion must match BC text.\n\
         Expected: {expected:?}\n\
         Got:      {suggestion:?}"
    );
}
