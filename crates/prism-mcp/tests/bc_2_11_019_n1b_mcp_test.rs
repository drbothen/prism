//! Red Gate test for S-DEMO-FIDELITY-REMEDIATION-001 AC-N1B MCP mapping — BC-2.11.019 v1.4.
//!
...
//! | Test | AC | BC |
//! |------|----|----|
//! | test_bc_2_11_019_n1b_mcp_maps_to_32602 | AC-N1B | BC-2.11.019 v1.4 |
...
/// BC-2.11.019 v1.4 AC-N1B — `map_prism_error` for E-QUERY-039 Red Gate test.
///
...
// ─── MED-5: suggestion string byte-for-byte conformance to BC-2.11.019 v1.4 §MCP surface ─────

/// MED-5 BC-2.11.019 v1.4 §MCP surface — non-empty suggestion form.
///
...
fn test_med5_enrich_udf_not_found_suggestion_non_empty_no_brackets() {
    ...
    // BC-2.11.019 v1.4 §MCP surface canonical text (no brackets around list):
    ...
    assert_eq!(
        suggestion, expected,
        "MED-5 BC-2.11.019 v1.4 §MCP surface: suggestion must not wrap the list in brackets \
...
/// MED-5 BC-2.11.019 v1.4 §MCP surface — empty infusions form.
///
/// BC specifies:
...
fn test_med5_enrich_udf_not_found_suggestion_empty_infusions() {
    ...
    assert_eq!(
        suggestion, expected,
        "MED-5 BC-2.11.019 v1.4 §MCP surface: empty-infusions suggestion must match BC text.\n\
         Expected: {expected:?}\n\
         Got:      {suggestion:?}"
    );
}
