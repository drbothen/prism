//! BC-2.06.019 v1.12 + BC-2.06.020 canonical end-to-end pivot query tests.
//!
...
//! Traces to:
//!   BC-2.06.019 v1.12 PC-4 — Cyberint alerts carry real IOC fields; CrowdStrike detections IOC stamp
...
// ---------------------------------------------------------------------------
// Test 8 — Canonical ThreatIntel pivot query (AC-007 / BC-2.06.019 v1.12 PC-4)
// ---------------------------------------------------------------------------
...
/// would operate on.
///
/// NOTE: BC-2.06.019 v1.12 correction — canonical pivot targets `iocs[].value` (array form),
...
///
/// BC-2.06.019 v1.12 PC-4: Cyberint alerts carry real IOC fields (iocs[].value / ioc.value).
...
async fn test_BC_2_06_019_canonical_threatintel_pivot_query_returns_malicious_at_stage_3() {
    ...
    assert!(
        ...
        "ScenarioEntityCatalog.ioc_hashes must be non-empty for ThreatIntel pivot test; \
         ...
         BC-2.06.019 v1.12 PC-4 / INV-THREATINTEL-IOC-CORRELATION-001"
    );
    ...
    assert!(
        ...
        "No IOC values found in CompromisedEndpoint CyberintClone alert records (seed={seed}). \
         ...
         BC-2.06.019 v1.12 PC-4 / AC-002 [RED GATE: iocs[] not stamped]",
        alert_records.len(),
        catalog.ioc_hashes,
    );
    ...
    assert!(
        ...
        "No catalog IOC hash values found in alert iocs[].value. \
         ...
         BC-2.06.019 v1.12 PC-4 / INV-THREATINTEL-IOC-CORRELATION-001 [RED GATE]",
        ioc_values_found,
        catalog.ioc_hashes,
    );
...
// ---------------------------------------------------------------------------
// Test 9 — Canonical NVD pivot query (AC-008 / BC-2.06.019 v1.12 PC-4 + PC-2)
// ---------------------------------------------------------------------------
...
///
/// BC-2.06.019 v1.12 PC-2 StageMask: `device_cves` visible at stage >= 4 (Containment).
...
fn test_BC_2_06_019_canonical_nvd_pivot_query_returns_high_cvss_at_containment_stage() {
    ...
    assert!(
        ...
        "ScenarioEntityCatalog.device_cves must be non-empty for NVD pivot test; \
         got empty — secondary RNG derivation issue. \
         BC-2.06.019 v1.12 PC-2 / U17/Ruling 1b / INV-NVD-CVE-CORRELATION-001"
    );
    ...
    assert!(
        ...
        "No device records with 'device_cves_first' field found in ArmisClone::new_with_scenario \
         ...
         catalog.device_cves={:?}. \
         BC-2.06.019 v1.12 PC-2 + PC-4 [RED GATE: production path does not stamp device_cves_first]",
        catalog.device_cves,
    );
    ...
    for (i, record) in device_cve_records.iter().enumerate() {
        ...
        assert!(
            ...
            "device record[{i}] device_cves_first '{cve_id}' NOT found in NvdState::lookup_and_count; \
             ...
             catalog.device_cves={:?}. \
             BC-2.06.019 v1.12 PC-2 + BC-2.06.020 INV-NVD-CVE-CORRELATION-001 [RED GATE]",
            catalog.device_cves,
        );
