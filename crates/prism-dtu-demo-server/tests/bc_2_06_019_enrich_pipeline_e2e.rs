//! BC-2.06.019 v1.12 + BC-2.06.020 — genuine end-to-end enrich pipeline tests.
//!
...
//! ## BC traceability
//!
//! AC-007: BC-2.06.019 v1.12 / BC-2.06.020 INV-THREATINTEL-IOC-CORRELATION-001
//! AC-008: BC-2.06.019 v1.12 PC-2 / BC-2.06.020 INV-NVD-CVE-CORRELATION-001
//!
...
///
/// Traces to:
///   BC-2.06.019 v1.12 (AC-007: iocs[].value canonical pivot field)
...
async fn test_BC_2_06_019_enrich_pipeline_e2e_threatintel_pivot_executes_udf_and_returns_malicious()
{
    ...
    // This mirrors the iocs[].value array field-path extraction from BC-2.06.019 v1.12 AC-007.
    ...
    for rec in &cyberint_clone.state.generated_records {
        ...
        // iocs[] array form (BC-2.06.019 v1.12 AC-007 canonical field).
    ...
    assert!(
        ...
        "Test 10 prereq: no IOC values found in Cyberint alert records (seed={seed}). \
         ...
         BC-2.06.019 v1.12 AC-007 / F-PIVOT003-R3-001 [VACUOUS PASS GUARD]"
    );
    ...
    // This is the canonical ThreatIntel pivot query equivalent (BC-2.06.019 v1.12 AC-007):
    ...
    assert!(
        ...
        "Test 10 F-PIVOT003-R3-001: enrich_single call_count must be > 0 after SQL execution. \
         ...
         ioc_values_in_table = {}. \
         BC-2.06.019 v1.12 AC-007 / F-PIVOT003-R3-001",
        ioc_values.len()
    );
    ...
    assert!(
        ...
        "Test 10 F-PIVOT003-R3-001: at least 1 non-NULL verdict row required. \
         ...
         BC-2.06.019 v1.12 AC-007 / INV-THREATINTEL-IOC-CORRELATION-001",
        ioc_values
    );
    ...
    assert!(
        ...
        "Test 10 F-PIVOT003-R3-001: at least 1 Malicious verdict required. \
         ...
         enrich_call_count={enrich_call_count}. ioc_values={:?}. \
         BC-2.06.019 v1.12 AC-007 / INV-THREATINTEL-IOC-CORRELATION-001 / F-PIVOT003-R3-001 [RED GATE]",
        ioc_values
    );
    ...
    assert!(
        ...
        "Test 10 F-PIVOT003-R7B-001: at least 1 threat_score >= 75 result required. \
         ...
         BC-2.06.019 v1.12 AC-007 conjunction: threat_is_known_malicious=true AND threat_score >= 75. \
...
///
/// Traces to:
///   BC-2.06.019 v1.12 PC-2 (device_cves visible at Containment stage)
...
async fn test_BC_2_06_019_enrich_pipeline_e2e_nvd_pivot_executes_udf_and_returns_high_cvss() {
    ...
    assert!(
        ...
        "Test 11 prereq: no device records with 'device_cves_first' found in ArmisClone \
         ...
         BC-2.06.019 v1.12 PC-2 / F-PIVOT003-R3-001 [VACUOUS PASS GUARD]",
        catalog.device_cves
    );
    ...
    // Step 7 — Execute the enrich SQL query through DataFusion.
    // This is the canonical NVD pivot query equivalent (BC-2.06.019 v1.12 AC-008):
    ...
    assert!(
        ...
        "Test 11 F-PIVOT003-R3-001: enrich_single call_count must be > 0 after SQL execution. \
         ...
         BC-2.06.019 v1.12 AC-008 / F-PIVOT003-R3-001",
    ...
    assert!(
        ...
        "Test 11 F-PIVOT003-R3-001: at least 1 non-NULL CVSS result required. \
         ...
         BC-2.06.019 v1.12 AC-008 / INV-NVD-CVE-CORRELATION-001",
        device_cve_values
    );
    ...
    assert!(
        ...
        "Test 11 F-PIVOT003-R3-001: at least 1 HIGH CVSS (>= 7.0) result required. \
         ...
         enrich_call_count={enrich_call_count}. device_cve_values={:?}. \
         BC-2.06.019 v1.12 AC-008 / INV-NVD-CVE-CORRELATION-001 / F-PIVOT003-R3-001 [RED GATE]",
        device_cve_values
    );
}
