...
pub struct IncidentRecord {
    ...
    ///
    /// MSSP-scoped: generated by Prism (UUID). MUST NOT embed `org_id` per ADR-008 §1.2.
    ...
    pub summary: String,
    /// OrgId UUID string of the originating organization (S-3.2.06 / BC-3.2.004).
    ///
    /// Embedded in `custom_details` / record metadata at ingress.
    /// MUST appear here — never in `dedup_key`, URL path, or HTTP headers.
    /// Only present when populated via `capture_incident_tagged` (shared-mode path).
...
impl PagerDutyState {
    ...
    /// Capture an incident record tagged with the originating `OrgId` (S-3.2.06).
    ///
    /// The `OrgId` UUID string is embedded in `IncidentRecord.org_id` at ingress per
    /// BC-3.2.004 postcondition 1 and ADR-007 §2.6 Step 3.
    ///
    /// # Constraints (BC-3.2.004)
    /// - `OrgId` MUST appear in the record metadata — never in `dedup_key`, URL, or headers.
    /// - `dedup_key` MUST remain MSSP-scoped (not org-scoped) per ADR-008 §1.2.
