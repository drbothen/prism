...
/// AC-007 (S-3.4.05) / BC-3.2.005 EC-002: `CustomerSpec` with `mode = "client"` for PagerDuty
...
async fn ac_client_mode_override_does_not_produce_startup_error() {
    ...
    assert!(
        ...
        "BC-3.2.005 EC-002: HarnessBuilder with DtuType::PagerDuty must NOT produce a \
         startup error; got: {:?}",
        harness_result.err()
    );
    ...
    assert_eq!(
        resp.status().as_u16(),
        200,
        "BC-3.2.005 EC-002: PagerDuty clone health check must return 200 after clean startup"
    );
}
