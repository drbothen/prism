//! W3-FIX-SEC-002: `POST /dtu/reset` admin token auth — prism-dtu-armis
//!
//! Acceptance criteria:
...
async fn test_AC_001_dtu_reset_without_admin_token_returns_401() {
    let mut clone = ArmisClone::new().expect("W3-FIX-SEC-002 armis: new must succeed");
    clone
        .start()
        .await
        .expect("W3-FIX-SEC-002 armis: start() must succeed");
...
/// must be rejected.
///
/// Spin up two independent `ArmisClone` instances.  Token from clone B
...
async fn test_AC_003_cross_clone_admin_token_returns_401() {
    let mut clone_a = ArmisClone::new().expect("W3-FIX-SEC-002 armis clone-A: new must succeed");
    clone_a
        .start()
        .await
        .expect("W3-FIX-SEC-002 armis clone-A: start() must succeed");

    let mut clone_b = ArmisClone::new().expect("W3-FIX-SEC-002 armis clone-B: new must succeed");
    clone_b
        .start()
        .await
        .expect("W3-FIX-SEC-002 armis clone-B: start() must succeed");
