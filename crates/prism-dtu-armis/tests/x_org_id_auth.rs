...
async fn start_clone_with_org(_org_id: OrgId) -> (ArmisClone, String) {
    todo!("AC-001: start_clone_with_org helper — create ArmisClone whose instance_org_id == org_id")
}
...
async fn test_AC_001_x_org_id_validated_against_bearer_token() {
    todo!("AC-001: same-org X-Org-Id header returns HTTP 200 from Armis /api/v1/devices")
}
...
async fn test_AC_002_cross_org_credential_returns_401() {
    todo!("AC-002: cross-org X-Org-Id header returns HTTP 401 from Armis /api/v1/devices")
}
...
async fn test_AC_002_cross_org_401_body_is_json_error_object() {
    todo!("AC-002: verify 401 response body is JSON object with 'error' key on cross-org Armis request")
}
...
async fn test_AC_003_missing_x_org_id_header_returns_401() {
    todo!("AC-003: absent X-Org-Id header returns HTTP 401 from Armis /api/v1/devices")
}
...
async fn test_cross_org_header_rejected() {
    todo!("AC-005: cross-org credential mismatch returns 401, not 200, not silent empty (Armis)")
}
...
async fn test_EC_001_non_uuid_x_org_id_returns_401() {
    todo!("EC-001: non-UUID X-Org-Id header returns HTTP 401 from Armis /api/v1/devices")
}
...
async fn test_EC_003_sentinel_uuid_as_x_org_id_returns_401() {
    todo!("EC-003: sentinel UUID 00000000-0000-7000-8000-000000000000 in X-Org-Id returns HTTP 401 (Armis)")
}
