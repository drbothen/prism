...
impl JsonLookupSource {
    ...
    pub fn load(json_path: &str) -> Result<Self, prism_core::InfusionError> {
        unimplemented!(
            "JsonLookupSource::load — implement in S-1.14 (BC-2.19.001)"
        )
    }
}
...
impl InfusionSource for JsonLookupSource {
    fn enrich_single(&self, _input: &str, _input_type: &str) -> Option<serde_json::Value> {
        unimplemented!(
            "JsonLookupSource::enrich_single — implement in S-1.14 (BC-2.19.001)"
        )
    }

    fn enrich_batch(&self, _inputs: &[String], _input_type: &str) -> Vec<Option<serde_json::Value>> {
        unimplemented!(
            "JsonLookupSource::enrich_batch — implement in S-1.14 (BC-2.19.001)"
        )
    }
}
