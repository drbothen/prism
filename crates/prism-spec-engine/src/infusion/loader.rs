...
impl InfusionLoader {
    ...
    pub fn parse(_toml_input: &str, _source_path: &str) -> Result<InfusionSpec, InfusionError> {
        unimplemented!("InfusionLoader::parse — implement in S-1.14 (BC-2.19.001)")
    }
    ...
    pub fn load_all(&self) -> (Vec<InfusionSpec>, Vec<InfusionError>) {
        unimplemented!("InfusionLoader::load_all — implement in S-1.14 (BC-2.19.001)")
    }

    /// Validate that `pipe_stage.adds_columns` matches the `[[infusion.fields]]` names.
    ///
    /// Returns `Ok(())` or a list of mismatched names.
    pub fn validate_pipe_stage_columns(_spec: &InfusionSpec) -> Result<(), InfusionError> {
        unimplemented!(
    ...
    pub fn validate_credentials(_spec: &InfusionSpec) -> Result<(), InfusionError> {
        unimplemented!("InfusionLoader::validate_credentials — implement in S-1.14 (BC-2.19.005)")
    }
}
